...
function zGroundFire(fleet){
    ...
    global.space.shipyard.ships.forEach(function(ship){
        if (!ship.inTransit && ship.location.name === 'spc_home') {
        ...
        vBind({
            ...
            methods: {
                ...
                fleetAllowed(id){
                    ...
                    return global.space.shipyard.ships.some(o => o !== s && !o.inTransit && o.location === s.location);
                },
                fleetText(id){
                    let s = global.space.shipyard.ships[id];
...
export function shipFleet(ship){
    ...
    let key = fleetKey(ship);
    return global.space.shipyard.ships.filter(s => s.fleet && fleetKey(s) === key);
}

// Identifier by which fleets are distinguished, `location` for stationary ships, 
// and `destination | time` for ships in transit
function fleetKey(ship){
    return ship.inTransit ? `${ship.location.name}` : `${ship.destination.name} | ${ship.timeToNextStep}`;
}
...
export function fleetCmdDay(){
    ...
    global.space.shipyard.ships.forEach(function(ship){
        ...
        if (ship['ret'] && !ship.inTransit && ship.location === ship.ret){
            delete ship.ret;
            moved = true;
        }
    });

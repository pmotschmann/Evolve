...
const outerTruth = {
  spc_titan: {
    info: {
      ...
      desc() {
        return loc("space_titan_info_desc", [
          planetName().titan,
    ...
    titan_mission: {
      ...
      cost: {
        Helium_3(o, wiki) {
          return +fuel_adjust(250000, false, wiki).toFixed(0);
        },
        Elerium() {
          return 100;
        },
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_titan_mission_action", [
              planetName().titan,
              races[global.race.species].home,
            ]),
            "info",
    ...
    titan_spaceport: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("titan_spaceport", offset, 2500000, 1.32);
        },
        Lumber(offset) {
          return spaceCostMultiplier("titan_spaceport", offset, 750000, 1.32);
        },
        Cement(offset) {
          return spaceCostMultiplier("titan_spaceport", offset, 350000, 1.32);
        },
        Mythril(offset) {
          return spaceCostMultiplier("titan_spaceport", offset, 10000, 1.32);
        },
      },
      effect() {
        let water = global.resource.Water.display
          ? `<div>${loc("plus_max_resource", [sizeApproximation(spatialReasoning(250)), global.resource.Water.name])}</div>`
          : ``;
        let support =
          global.tech["enceladus"] && global.tech.enceladus >= 2
            ? `<div>+${loc(`galaxy_alien2_support`, [$(this)[0].support(), planetName().enceladus])}</div>`
            : ``;
        let storage =
          global.tech["titan"] && global.tech.titan >= 5
            ? `<div>${loc(`space_titan_spaceport_storage`, [25])}</div>`
            : ``;
    ...
    electrolysis: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("electrolysis", offset, 1000000, 1.25);
        },
        Copper(offset) {
          return spaceCostMultiplier("electrolysis", offset, 185000, 1.25);
        },
        Steel(offset) {
          return spaceCostMultiplier("electrolysis", offset, 220000, 1.25);
        },
        Polymer(offset) {
          return spaceCostMultiplier("electrolysis", offset, 380000, 1.25);
        },
      },
      effect(wiki) {
      ...
      support(wiki) {
        return global.tech["titan_ai_core"] &&
          global.tech.titan_ai_core >= 2 &&
          (wiki ? global.space.ai_core2.on : p_on["ai_core2"])
          ? 3
          : 2;
      },
      support_fuel() {
    ...
    hydrogen_plant: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("hydrogen_plant", offset, 1500000, 1.28);
        },
        Iridium(offset) {
          return spaceCostMultiplier("hydrogen_plant", offset, 292000, 1.28);
        },
        Stanene(offset) {
          return spaceCostMultiplier("hydrogen_plant", offset, 599000, 1.28);
        },
        Cement(offset) {
          return spaceCostMultiplier("hydrogen_plant", offset, 180000, 1.28);
        },
      },
      effect() {
    ...
    titan_quarters: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("titan_quarters", offset, 1200000, 1.28);
        },
        Furs(offset) {
          return spaceCostMultiplier("titan_quarters", offset, 85000, 1.28);
        },
        Plywood(offset) {
          return spaceCostMultiplier("titan_quarters", offset, 100000, 1.28);
        },
        Horseshoe() {
    ...
    titan_mine: {
      ...
      reqs: { titan: 4 },
      condition() {
        return global.space["titan_quarters"] &&
          global.space.titan_quarters.count > 0
          ? true
          : false;
      },
      path: ["truepath"],
      cost: {
        Money(offset) {
          return spaceCostMultiplier("titan_mine", offset, 475000, 1.28);
        },
        Lumber(offset) {
          return spaceCostMultiplier("titan_mine", offset, 568000, 1.28);
        },
        Wrought_Iron(offset) {
          return spaceCostMultiplier("titan_mine", offset, 250000, 1.28);
        },
      },
      effect() {
    ...
    storehouse: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("storehouse", offset, 175000, 1.28);
        },
        Lumber(offset) {
          return spaceCostMultiplier("storehouse", offset, 100000, 1.28);
        },
        Aluminium(offset) {
          return spaceCostMultiplier("storehouse", offset, 120000, 1.28);
        },
        Cement(offset) {
          return spaceCostMultiplier("storehouse", offset, 45000, 1.28);
        },
      },
      wide: true,
      res() {
        ...
        for (const res of $(this)[0].res()) {
          if (global.resource[res].display) {
            ...
            let val = sizeApproximation(
              +(
                spatialReasoning($(this)[0].val(res)) *
    ...
    titan_bank: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier(
            "titan_bank",
            offset,
            traitCostMod("untrustworthy", 2500000),
            1.32,
          );
        },
        Titanium(offset) {
          return spaceCostMultiplier(
            "titan_bank",
            offset,
            traitCostMod("untrustworthy", 380000),
            1.32,
          );
        },
        Neutronium(offset) {
          return spaceCostMultiplier(
            "titan_bank",
            offset,
            traitCostMod("untrustworthy", 5000),
            1.32,
          );
        },
      },
    ...
    g_factory: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("g_factory", offset, 950000, 1.28);
        },
        Copper(offset) {
          return spaceCostMultiplier("g_factory", offset, 165000, 1.28);
        },
        Stone(offset) {
          return spaceCostMultiplier("g_factory", offset, 220000, 1.28);
        },
        Adamantite(offset) {
    ...
    sam: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("sam", offset, 2500000, 1.28);
        },
        Steel(offset) {
          return spaceCostMultiplier("sam", offset, 450000, 1.28);
        },
        Elerium(offset) {
          return spaceCostMultiplier("sam", offset, 120, 1.28);
        },
        Brick(offset) {
          return spaceCostMultiplier("sam", offset, 160000, 1.28);
        },
      },
      effect() {
    ...
    decoder: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("decoder", offset, 12500000, 1.275);
        },
        Elerium(offset) {
          return spaceCostMultiplier("decoder", offset, 750, 1.275);
        },
        Orichalcum(offset) {
          return spaceCostMultiplier("decoder", offset, 330000, 1.275);
        },
        Quantium(offset) {
          return spaceCostMultiplier("decoder", offset, 180000, 1.275);
        },
      },
      effect(wiki) {
        ...
        return (
          desc +
          `<div class="has-text-caution">${loc("spend", [cipher, global.resource[$(this)[0].support_fuel().r].name])}</div>`
        );
      },
      s_type: "titan",
      support() {
    ...
    ai_core: {
      ...
      desc(wiki) {
        if (
          !global.space.hasOwnProperty("ai_core") ||
          global.space.ai_core.count < 100 ||
          wiki
        ) {
          return (
            `<div>${loc("space_ai_core")}</div><div class="has-text-special">${loc("requires_segments", [100])}</div>` +
            (global.space.hasOwnProperty("ai_core") &&
            global.space.ai_core.count >= 100
              ? `<div class="has-text-special">${loc("requires_power")}</div>`
              : ``)
          );
        } else {
    ...
    ai_core2: {
      ...
      path: ["truepath"],
      condition() {
        return global.space.hasOwnProperty("ai_core") &&
          global.space.ai_core.count >= 100
          ? true
          : false;
      },
      ...
      flair() {
        return global.space.hasOwnProperty("ai_core2") &&
          global.space.ai_core2.on >= 1
          ? loc(`space_ai_core_flair`)
          : loc(`space_ai_core_flair2`);
      },
      struct() {
        return {
          d: { count: 0, on: 0 },
    ...
    ai_colonist: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("ai_colonist", offset, 112000000, 1.35);
        },
        Alloy(offset) {
          return spaceCostMultiplier("ai_colonist", offset, 750000, 1.35);
        },
        Elerium(offset) {
          return spaceCostMultiplier("ai_colonist", offset, 500, 1.35);
        },
        Nano_Tube(offset) {
          return spaceCostMultiplier("ai_colonist", offset, 525000, 1.35);
        },
        Quantium(offset) {
          return spaceCostMultiplier("ai_colonist", offset, 150000, 1.35);
        },
        Cipher(offset) {
          return spaceCostMultiplier("ai_colonist", offset, 10000, 1.35);
        },
      },
      effect() {
  ...
  spc_enceladus: {
    info: {
      ...
      desc() {
        return loc("space_enceladus_info_desc", [
    ...
    enceladus_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_enceladus_mission_action", [planetName().enceladus]),
            "info",
            false,
            ["progress"],
          );
    ...
    water_freighter: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("water_freighter", offset, 450000, 1.25);
        },
        Iron(offset) {
          return spaceCostMultiplier("water_freighter", offset, 362000, 1.25);
        },
        Nano_Tube(offset) {
          return spaceCostMultiplier("water_freighter", offset, 125000, 1.25);
        },
        Sheet_Metal(offset) {
          return spaceCostMultiplier("water_freighter", offset, 75000, 1.25);
        },
      },
      effect(wiki) {
    ...
    zero_g_lab: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("zero_g_lab", offset, 5000000, 1.25);
        },
        Alloy(offset) {
          return spaceCostMultiplier("zero_g_lab", offset, 125000, 1.25);
        },
        Graphene(offset) {
          return spaceCostMultiplier("zero_g_lab", offset, 225000, 1.25);
        },
        Stanene(offset) {
          return spaceCostMultiplier("zero_g_lab", offset, 600000, 1.25);
        },
      },
      effect() {
        ...
        if (global.resource.Quantium.display) {
          desc =
            desc +
            `<div>${loc("space_zero_g_lab_effect", [jobScale(1)])}</div>`;
        }
        if (global.resource.Cipher.display) {
          desc =
            desc +
            `<div>${loc("plus_max_resource", [10000, global.resource.Cipher.name])}</div>`;
        }
        return (
          desc +
          `<div class="has-text-caution">${loc("minus_power", [$(this)[0].powered()])}</div>`
    ...
    operating_base: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("operating_base", offset, 7500000, 1.3);
        },
        Furs(offset) {
          return spaceCostMultiplier("operating_base", offset, 500000, 1.3);
        },
        Adamantite(offset) {
          return spaceCostMultiplier("operating_base", offset, 375000, 1.3);
        },
        Stanene(offset) {
          return spaceCostMultiplier("operating_base", offset, 750000, 1.3);
        },
        Mythril(offset) {
          return spaceCostMultiplier("operating_base", offset, 225000, 1.3);
        },
        Horseshoe() {
    ...
    munitions_depot: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("munitions_depot", offset, 5000000, 1.22);
        },
        Iron(offset) {
          return spaceCostMultiplier("munitions_depot", offset, 185000, 1.22);
        },
        Sheet_Metal(offset) {
          return spaceCostMultiplier("munitions_depot", offset, 100000, 1.22);
        },
      },
      effect() {
        let containers = 25;
  ...
  spc_triton: {
    info: {
      ...
      desc() {
        return loc("space_triton_info_desc", [
          planetName().triton,
          races[global.race.species].home,
        ]);
      },
      ...
      syndicate_cap() {
        return global.tech["outer"] && global.tech.outer >= 4 ? 5000 : 3000;
      },
      extra(region) {
        if (global.tech["triton"] && global.tech.triton >= 3) {
          $(`#${region}`).append(
            `<div id="${region}resist" v-show="${region}" class="syndThreat has-text-caution">${loc("space_ground_resist")} <span class="has-text-danger" v-html="threat(enemy,troops)"></span></div>`,
          );
          vBind({
            el: `#${region}resist`,
    ...
    triton_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +fuel_adjust(600000, false, wiki).toFixed(0);
        },
        Elerium() {
          return 2500;
        },
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(
            loc("space_triton_mission_action", [planetName().triton]),
            "info",
            false,
            ["progress"],
          );
    ...
    fob: {
      ...
      cost: {
        Money(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("fob") ? global.space.fob.count : 0) >=
            1
            ? 0
            : spaceCostMultiplier("fob", offset, 250000000, 1.1);
        },
        Copper(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("fob") ? global.space.fob.count : 0) >=
            1
            ? 0
            : spaceCostMultiplier("fob", offset, 8000000, 1.1);
        },
        Uranium(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("fob") ? global.space.fob.count : 0) >=
            1
            ? 0
            : spaceCostMultiplier("fob", offset, 50000, 1.1);
        },
        Nano_Tube(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("fob") ? global.space.fob.count : 0) >=
            1
            ? 0
            : spaceCostMultiplier("fob", offset, 2500000, 1.1);
        },
        Graphene(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("fob") ? global.space.fob.count : 0) >=
            1
            ? 0
            : spaceCostMultiplier("fob", offset, 3000000, 1.1);
        },
        Sheet_Metal(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("fob") ? global.space.fob.count : 0) >=
            1
            ? 0
            : spaceCostMultiplier("fob", offset, 7500000, 1.1);
        },
        Quantium(offset) {
          return (offset || 0) +
            (global.space.hasOwnProperty("fob") ? global.space.fob.count : 0) >=
            1
            ? 0
            : spaceCostMultiplier("fob", offset, 500000, 1.1);
        },
        Horseshoe(offset) {
          return global.race["hooved"] &&
            (offset || 0) +
              (global.space.hasOwnProperty("fob")
      ...
      effect(wiki) {
        ...
        return (
          desc +
          `<div class="has-text-caution">${loc("requires_power_combo_effect", [$(this)[0].powered(), helium, global.resource.Helium_3.name])}</div>`
        );
      },
      powered() {
    ...
    lander: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("lander", offset, 2400000, 1.15);
        },
        Aluminium(offset) {
          return spaceCostMultiplier("lander", offset, 185000, 1.15);
        },
        Neutronium(offset) {
          return spaceCostMultiplier("lander", offset, 10000, 1.15);
        },
        Nano_Tube(offset) {
          return spaceCostMultiplier("lander", offset, 158000, 1.15);
        },
      },
      powered() {
        return 0;
    ...
    crashed_ship: {
      ...
      cost: {},
      effect() {
        let control = global.space["crashed_ship"]
  ...
  spc_kuiper: {
    ...
    kuiper_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +fuel_adjust(1000000, false, wiki).toFixed(0);
        },
        Elerium() {
          return 1000;
        },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(loc("space_kuiper_mission_action"), "info", false, [
            "progress",
          ]);
          return true;
        }
    ...
    orichalcum_mine: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("orichalcum_mine", offset, 25000000, 1.25);
        },
        Graphene(offset) {
          return spaceCostMultiplier("orichalcum_mine", offset, 900000, 1.25);
        },
        Elerium(offset) {
          return spaceCostMultiplier("orichalcum_mine", offset, 200, 1.25);
        },
        Mythril(offset) {
          return spaceCostMultiplier("orichalcum_mine", offset, 450000, 1.25);
        },
        Quantium(offset) {
          return spaceCostMultiplier("orichalcum_mine", offset, 150000, 1.25);
        },
      },
      effect(wiki) {
        ...
        return (
          desc +
          `<div class="has-text-caution">${loc("requires_power_combo_effect", [$(this)[0].powered(), fuel, global.resource[$(this)[0].p_fuel().r].name])}</div>`
        );
      },
      powered() {
        return powerCostMod(10);
    ...
    uranium_mine: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("uranium_mine", offset, 5000000, 1.25);
        },
        Iridium(offset) {
          return spaceCostMultiplier("uranium_mine", offset, 250000, 1.25);
        },
        Steel(offset) {
          return spaceCostMultiplier("uranium_mine", offset, 620000, 1.25);
        },
      },
      effect(wiki) {
    ...
    neutronium_mine: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("neutronium_mine", offset, 8000000, 1.25);
        },
        Adamantite(offset) {
          return spaceCostMultiplier("neutronium_mine", offset, 650000, 1.25);
        },
        Stanene(offset) {
          return spaceCostMultiplier("neutronium_mine", offset, 1250000, 1.25);
        },
      },
      effect(wiki) {
    ...
    elerium_mine: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("elerium_mine", offset, 20000000, 1.25);
        },
        Titanium(offset) {
          return spaceCostMultiplier("elerium_mine", offset, 250000, 1.25);
        },
        Neutronium(offset) {
          return spaceCostMultiplier("elerium_mine", offset, 120000, 1.25);
        },
        Orichalcum(offset) {
          return spaceCostMultiplier("elerium_mine", offset, 175000, 1.25);
        },
      },
      effect(wiki) {
  ...
  spc_eris: {
    info: {
      ...
      extra(region) {
        if (global.tech["eris"] && global.tech["eris"] === 1) {
          $(`#${region}`).append(
            `<div id="${region}scanned" v-show="${region}" class="syndThreat has-text-caution">${loc("space_scanned")} <span class="has-text-info">{{ eris_scan }}%</span></div>`,
          );
          vBind({
            el: `#${region}scanned`,
            data: global.tech,
    ...
    eris_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +fuel_adjust(1250000, false, wiki).toFixed(0);
        },
        Elerium() {
          return 1250;
        },
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("space_eris_mission_action", [planetName().eris]),
            "info",
            false,
            ["progress"],
          );
    ...
    drone_control: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("drone_control", offset, 75000000, 1.3);
        },
        Neutronium(offset) {
          return spaceCostMultiplier("drone_control", offset, 100000, 1.3);
        },
        Stanene(offset) {
          return spaceCostMultiplier("drone_control", offset, 450000, 1.3);
        },
        Quantium(offset) {
          return spaceCostMultiplier("drone_control", offset, 300000, 1.3);
        },
      },
      effect() {
    ...
    shock_trooper: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("shock_trooper", offset, 4250000, 1.225);
        },
        Polymer(offset) {
          return spaceCostMultiplier("shock_trooper", offset, 375000, 1.225);
        },
        Adamantite(offset) {
          return spaceCostMultiplier("shock_trooper", offset, 500000, 1.225);
        },
        Graphene(offset) {
          return spaceCostMultiplier("shock_trooper", offset, 220000, 1.225);
        },
        Elerium(offset) {
      ...
      effect() {
        let rating = Math.round(
          armyRating(1, "army", 0) * syndicate("spc_eris"),
        );
    ...
    tank: {
      ...
      cost: {
        Money(offset) {
          return spaceCostMultiplier("tank", offset, 100000000, 1.25);
        },
        Alloy(offset) {
          return spaceCostMultiplier("tank", offset, 1250000, 1.25);
        },
        Orichalcum(offset) {
          return spaceCostMultiplier("tank", offset, 600000, 1.25);
        },
        Mythril(offset) {
          return spaceCostMultiplier("tank", offset, 500000, 1.25);
        },
        Uranium(offset) {
          return spaceCostMultiplier("tank", offset, 25000, 1.25);
        },
      },
      effect() {
...
const tauCetiModules = {
  tau_star: {
    ...
    ringworld: {
      ...
      desc(wiki) {
        if (
          !global.tauceti.hasOwnProperty("ringworld") ||
          global.tauceti.ringworld.count < 1000 ||
          wiki
        ) {
      ...
      queue_size: 50,
      queue_complete() {
        return 1000 - global.tauceti.ringworld.count;
      },
      cost: {
        Money(offset) {
          return (offset || 0) +
        ...
        Quantium(offset) {
          return (offset || 0) +
            (global.tauceti.hasOwnProperty("ringworld")
              ...
              : 0) <
            1000
            ? wom_recycle(global.race["lone_survivor"] ? 10100 : 101000)
            : 0;
        },
      },
      effect(wiki) {
    ...
    matrix: {
      ...
      effect() {
        ...
        let power_label =
          power > 0
            ? `<div class="has-text-caution">${loc("minus_power", [power])}</div>`
            : "";
  ...
  tau_home: {
    info: {
      ...
      extra(region) {
        if (
          global.tech["tau_home"] &&
          global.tech.tau_home >= 2 &&
          !tauEnabled()
        ) {
          $(`#${region}`).append(
            `<div id="${region}Mats" v-show="tauShow()" class="syndThreat has-text-warning">${loc("resource_Materials_name")} <span class="has-text-info">{{ amount | round | locale }}</span> / <span class="has-text-info">{{ max | locale }}</span></div>`,
          );
          vBind({
            el: `#${region}Mats`,
    ...
    home_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("tau_home_mission_result", [races[global.race.species].home]),
            "info",
            false,
            ["progress"],
          );
    ...
    dismantle: {
      ...
      effect() {
        ...
        if (
          ...
          let shipId = global.space.shipyard.ships.findIndex(
            (x) => x.location === "tauceti" && x.class === "explorer",
          );
          if (shipId !== -1) {
      ...
      action() {
        ...
        if (
          ...
          shipId = global.space.shipyard.ships.findIndex(
            (x) => x.location === "tauceti" && x.class === "explorer",
          );
        }
    ...
    colony: {
      ...
      cost: {
        ...
        Materials(offset) {
          return tauEnabled()
            ? 0
            : spaceCostMultiplier("colony", offset, 650000, 1.225, "tauceti");
        },
        Furs(offset) {
          return tauEnabled()
            ? spaceCostMultiplier("colony", offset, 720000, 1.225, "tauceti")
            : 0;
        },
        Graphene(offset) {
          return tauEnabled()
            ? spaceCostMultiplier("colony", offset, 485000, 1.225, "tauceti")
            : 0;
        },
        Brick(offset) {
          return tauEnabled()
      ...
      cost: {
        ...
        Chrysotile(offset) {
          return global.race["smoldering"]
            ? spaceCostMultiplier("tau_housing", offset, 50000, 1.25, "tauceti")
            : 0;
        },
        Horseshoe() {
          return global.race["hooved"] ? 1 : 0;
    ...
    pylon: {
      ...
      reqs: { magic: 2 },
      condition() {
        return global.tech["isolation"] &&
          global.tauceti.hasOwnProperty("pylon")
          ? true
          : false;
      },
      cost: {
        ...
        Money(offset) {
          return global["resource"][global.race.species].amount
            ? spaceCostMultiplier(
                ...
                offset,
                Math.round(
                  (global.race["high_pop"] ? 100000 : 125000) / jobScale(1),
                ),
                global.race["high_pop"] ? 1.01 : 1.02,
                "tauceti",
                global.race["high_pop"] ? 1.003 : 1.005,
              )
            : 0;
        },
        Copper(offset) {
          return !global.race["artifical"] || global.race["deconstructor"]
            ? 0
            : global["resource"][global.race.species].amount >= 5
              ? spaceCostMultiplier(
                  "citizen",
                  offset,
                  Math.round(50 / jobScale(1)),
                  1.01,
                  "tauceti",
                  global.race["high_pop"] ? 1.0032 : 1.005,
                )
              : 0;
        },
        Aluminium(offset) {
    ...
    tau_farm: {
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("produce", [+production("tau_farm", "food").toFixed(2), global.resource.Food.name])}</div>`;
        if (!global.race["kindling_kindred"] && !global.race["smoldering"]) {
          desc =
            desc +
            `<div>${loc("produce", [+production("tau_farm", "lumber").toFixed(2), global.resource.Lumber.name])}</div>`;
        }
        if (global.tech["isolation"]) {
          let water = +production("tau_farm", "water").toFixed(2);
          desc =
            desc +
            `<div>${loc("produce", [water, global.resource.Water.name])}</div>`;
          if (global.race["artifical"]) {
            let sig_cap = spatialReasoning(350);
            desc =
              desc + `<div>${loc("city_transmitter_effect", [sig_cap])}</div>`;
          }
        }
        desc =
          desc +
    ...
    mining_pit: {
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("plus_max_resource", [$(this)[0].workers(), loc("job_pit_miner")])}</div>`;
        if (!tauEnabled()) {
          desc =
            desc +
            `<div>${loc("plus_max_resource", [1000000, loc("resource_Materials_name")])}</div>`;
          desc =
            desc +
            `<div>${loc("tau_home_mining_pit_effect", [global.resource.Materials.name])}</div>`;
        } else {
          if (global.tech["isolation"]) {
            if (global.race["lone_survivor"]) {
              let res_list = [
                global.resource.Bolognium.name,
                global.resource.Adamantite.name,
                global.resource.Stone.name,
                global.resource.Copper.name,
                global.resource.Iron.name,
                global.resource.Aluminium.name,
                global.resource.Coal.name,
              ];
              if (global.race["smoldering"]) {
                res_list.push(global.resource.Chrysotile.name);
                desc =
                  desc +
                  `<div>${loc("tau_home_mining_pit_effect2cs", res_list)}</div>`;
              } else {
                desc =
                  desc +
                  `<div>${loc("tau_home_mining_pit_effect2c", res_list)}</div>`;
              }
            } else {
              let res_list = [
                global.resource.Bolognium.name,
              ...
              if (global.race["smoldering"]) {
                ...
                desc =
                  desc +
                  `<div>${loc("tau_home_mining_pit_effect2bs", res_list)}</div>`;
              } else {
                desc =
                  desc +
                  `<div>${loc("tau_home_mining_pit_effect2b", res_list)}</div>`;
              }
            }
            desc =
              desc +
              `<div>${loc("production", [8, global.resource.Cement.name])}</div>`;
          } else {
            let res_list = [
              global.resource.Bolognium.name,
              global.resource.Adamantite.name,
            ...
            if (global.race["smoldering"]) {
              ...
              desc =
                desc +
                `<div>${loc("tau_home_mining_pit_effect2s", res_list)}</div>`;
            } else {
              desc =
                desc +
                `<div>${loc("tau_home_mining_pit_effect2", res_list)}</div>`;
            }
          }
        }
        return desc;
    ...
    excavate: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("tau_home_excavate_msg"), "info", false, [
            "progress",
          ]);
          return true;
        }
    ...
    alien_outpost: {
      ...
      effect() {
        ...
        if (global.tech["isolation"]) {
          desc =
            desc +
            `<div>${loc("plus_max_resource", [(global.race["lone_survivor"] ? 3500000 : 6500000).toLocaleString(), global.resource.Knowledge.name])}</div>`;
          desc =
            desc +
            `<div>${loc("plus_max_resource", [(200000).toLocaleString(), global.resource.Cipher.name])}</div>`;
          desc =
            desc +
            `<div>${loc(`space_lander_effect3`, [production("alien_outpost"), global.resource.Cipher.name])}</div>`;
        }
        ...
        if (global.race["lone_survivor"]) {
          desc =
            desc +
            `<div>${loc("city_wardenclyffe_effect1", [jobScale(1), global.civic.professor.name])}</div>`;
        }
        desc =
          desc +
          `<div class="has-text-caution">${loc("minus_power", [$(this)[0].powered()])}</div>`;
    ...
    fusion_generator: {
      ...
      effect() {
        ...
        if (global.race["lone_survivor"]) {
          desc =
            desc +
            `<div>${loc("gain", [-fuel, global.resource[$(this)[0].p_fuel().r].name])}</div>`;
        } else {
          desc =
            desc +
            `<div class="has-text-caution">${loc("spend", [fuel, global.resource[$(this)[0].p_fuel().r].name])}</div>`;
        }
        return desc;
      },
      p_fuel() {
    ...
    repository: {
      ...
      effect(wiki) {
        ...
        if (global.tech["isolation"]) {
          storage =
            storage +
            `<span>${loc("plus_max_resource", [containers, global.resource.Crates.name])}</span><span>${loc("plus_max_resource", [containers, global.resource.Containers.name])}</span>`;
        }
        storage = storage + "</div>";
        return storage;
      },
      action() {
        if (payCosts($(this)[0])) {
          ...
          for (const res of $(this)[0].res()) {
            if (global.resource[res].display) {
              global.resource[res].max +=
                spatialReasoning($(this)[0].val(res)) * multiplier;
            }
          }
          return true;
        }
    ...
    tau_factory: {
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("tau_home_tau_factory_effect", [$(this)[0].manufacturing()])}</div>`;
        if (global.tech["isolation"]) {
          if (!global.race["flier"]) {
            desc =
              desc +
              `<div>${loc("plus_max_resource", [jobScale(2), loc(`job_cement_worker`)])}</div>`;
          }
          desc =
            desc +
            `<div>${loc("space_red_fabrication_effect1", [jobScale(5)])}</div>`;
        }
        desc =
          desc +
          `<div>${loc("city_crafted_mats", [global.tech["isolation"] ? 275 : 90])}</div>`;
        desc =
          desc +
          `<div class="has-text-caution">${loc("minus_power", [$(this)[0].powered()])}</div>`;
        return desc;
    ...
    infectious_disease_lab: {
      ...
      title() {
        return global.tech["isolation"]
          ? loc("tech_infectious_disease_lab_alt")
          : loc(
              global.race["artifical"]
                ? "tech_infectious_disease_lab_s"
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("city_max_knowledge", [Math.round(sci).toLocaleString()])}</div>`;
        if (global.tech["isolation"]) {
          let elerium = spatialReasoning(375);
          desc =
            desc +
            `<div>${loc("plus_max_resource", [elerium, global.resource.Elerium.name])}</div>`;
          desc =
            desc +
            `<div>${loc("city_wardenclyffe_effect1", [jobScale(2), global.civic.professor.name])}</div>`;
          desc =
            desc +
            `<div>${loc("city_wardenclyffe_effect1", [jobScale(1), global.civic.scientist.name])}</div>`;
          desc =
            desc +
            `<div>${loc("space_zero_g_lab_effect", [jobScale(1)])}</div>`;
        ...
        if (global.tech["alien_crafting"]) {
          desc =
            desc +
            `<div>${loc("production", [65, global.resource.Quantium.name])}</div>`;
        }
        if (global.tech["focus_cure"]) {
          desc =
            desc +
            `<div>${loc("tau_home_disease_lab_cure", [+global.tauceti.infectious_disease_lab.cure.toFixed(1)])}</div>`;
      ...
      post() {
        if (global.tech.disease === 1) {
          ...
          messageQueue(
            loc("tau_plague4", [loc("tab_tauceti")]),
            "info",
            false,
            ["progress"],
          );
    ...
    tauceti_casino: {
      ...
      effect() {
        let pop = $(this)[0].citizens();
        let desc = global.race["lone_survivor"]
          ? ``
          : `<div>${loc("plus_max_resource", [pop, loc("citizen")])}</div>`;
  ...
  tau_red: {
    info: {
      ...
      extra(region) {
        if (global.tech["tau_red"] && global.tech.tau_red >= 5) {
          $(`#${region}`).append(
            `<div id="${region}Womlings" class="syndThreat has-text-warning">${loc("tau_red_womling_prod")} <span class="has-text-info">{{ prod }}%</span></div>`,
          );
          vBind({
            el: `#${region}Womlings`,
    ...
    red_mission: {
      ...
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("tau_red_mission_result", [planetName().red]),
            "info",
            false,
            ["progress"],
          );
    ...
    orbital_platform: {
        ...
        Oil(offset) {
          return tauEnabled()
            ? spaceCostMultiplier(
                "orbital_platform",
                offset,
                int_fuel_adjust(wom_repulse(275000)),
                1.3,
                "tauceti",
              )
            : 0;
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("plus_max_resource", [oil.toLocaleString(), global.resource.Oil.name])}</div>`;
        desc =
          desc +
          `<div class="has-text-caution">${loc("spend_power", [fuel, global.resource[$(this)[0].support_fuel().r].name, $(this)[0].powered()])}</div>`;
        return desc;
      },
      support_fuel() {
        return {
          r: global.race["lone_survivor"] ? "Helium_3" : "Oil",
          a: global.tech["isolation"]
            ? global.race["lone_survivor"]
              ? 8
              : 32
      ...
      action() {
        if (tauEnabled() && payCosts($(this)[0])) {
          ...
          if (global.tech["tau_red"] === 1) {
            ...
            messageQueue(
              loc("tau_red_orbital_platform_msg", [
                loc("tau_planet", [planetName().red]),
                loc("tau_planet", [races[global.race.species].home]),
              ]),
              "info",
              false,
              ["progress"],
    ...
    contact: {
      ...
      cost: {
        ...
        Food() {
          return global.race["lone_survivor"] && global.race["artifical"]
            ? 62000
            : 2500000;
        },
      },
    ...
    jeff: {
      ...
      cost: {},
      effect() {
        let injured = global.tauceti["overseer"]
          ? global.tauceti.overseer.injured
          : 0;
        if (
          global.tauceti["overseer"] &&
          global.tauceti.overseer.pop < injured
        ) {
        ...
        desc =
          desc +
          `<div>${loc("tau_red_jeff_effect2", [global.tauceti["overseer"] ? global.tauceti.overseer.working : 0])}</div>`;
        desc = desc + `<div>${loc("tau_red_jeff_effect3", [injured])}</div>`;
        desc =
          desc +
          `<div>${loc("tau_red_jeff_effect4", [global.tauceti["overseer"] ? global.tauceti.overseer.loyal : 0])}</div>`;
        desc =
          desc +
          `<div>${loc("tau_red_jeff_effect5", [global.tauceti["overseer"] ? global.tauceti.overseer.morale : 0])}</div>`;
        if (global.tech.hasOwnProperty("womling_tech")) {
          desc =
            desc +
            `<div>${loc("tau_red_jeff_effect6", [global.tech.womling_tech])}</div>`;
        }
        desc =
          desc +
          `<div class="divide-line">${loc("job_farmer")}: ${global.tauceti["womling_farm"] ? global.tauceti.womling_farm.farmers : 0}</div>`;
        desc =
          desc +
          `<div>${loc("job_miner")}: ${global.tauceti["womling_mine"] ? global.tauceti.womling_mine.miners : 0}</div>`;
        if (global.tauceti["womling_lab"]) {
          desc =
            desc +
            `<div>${loc("job_scientist")}: ${global.tauceti["womling_lab"] ? global.tauceti.womling_lab.scientist : 0}</div>`;
        }
        return desc;
      },
      action() {
    ...
    overseer: {
      ...
      cost: {
        ...
        Alloy(offset) {
          return global.race["womling_friend"]
            ? spaceCostMultiplier("overseer", offset, 1850000, 1.28, "tauceti")
            : 0;
        },
        Neutronium(offset) {
          return global.race["womling_lord"]
            ? spaceCostMultiplier("overseer", offset, 165000, 1.28, "tauceti")
            : 0;
        },
        Titanium(offset) {
          return global.race["womling_god"]
            ? spaceCostMultiplier("overseer", offset, 2250000, 1.28, "tauceti")
            : 0;
        },
      },
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("tau_red_overseer_effect", [$(this)[0].val()])}</div>`;
        return desc;
      },
      val() {
        let val = 0;
    ...
    womling_village: {
      ...
      effect() {
        ...
        let desc = `<div class="has-text-caution">${loc("tau_new_support", [$(this)[0].support(), planetName().red])}</div>`;
        desc =
          desc + `<div>${loc("tau_red_womling_village_effect", [pop])}</div>`;
        if (global.tech["tau_junksale"]) {
          desc =
            desc +
            `<div>${loc("tau_red_womling_village_effect2", [40, loc(`tau_gas2_alien_station_data4_r${global.race.tau_junk_item || 0}`)])}</div>`;
        }
        return desc;
      },
      s_type: "tau_red",
    ...
    womling_farm: {
      ...
      effect() {
        ...
        let farmers = global.tauceti.hasOwnProperty("womling_farm")
          ? global.tauceti.womling_farm.farmers
          : 0;
        let desc = `<div class="has-text-caution">${loc("tau_new_support", [$(this)[0].support(), planetName().red])}</div>`;
        desc =
          desc + `<div>${loc("tau_red_womling_farm_effect", [food])}</div>`;
        desc = desc + `<div>${loc("tau_red_womling_employ", [2])}</div>`;
        if (global.tech["isolation"]) {
          desc =
            desc +
            `<div>${loc("tau_red_womling_generate", [global.resource.Furs.name])}</div>`;
        }
        desc =
          desc +
          `<div>${loc("tau_red_womling_farm_effect2", [(food / 2) * farmers])}</div>`;
        return desc;
      },
      s_type: "tau_red",
      support() {
      ...
      effect() {
        ...
        if (global.tech["isolation"]) {
          if (global.race["lone_survivor"]) {
            desc =
              desc +
              `<div>${loc("tau_red_womling_mine_effect_c", [
            ...
            desc =
              desc +
              `<div>${loc("tau_red_womling_mine_effect_b", [global.resource.Unobtainium.name, global.resource.Uranium.name, global.resource.Titanium.name])}</div>`;
          }
        } else {
          desc =
            desc +
            `<div>${loc("tau_red_womling_mine_effect_a", [global.resource.Unobtainium.name])}</div>`;
        }
    ...
    womling_fun: {
      ...
      cost: {
        ...
        Furs(offset) {
          return global.race["womling_lord"] || global.race["womling_god"]
            ? spaceCostMultiplier(
                "womling_fun",
                offset,
                835000,
        ...
        Water(offset) {
          return global.race["womling_friend"]
            ? spaceCostMultiplier("womling_fun", offset, 3500, 1.28, "tauceti")
            : 0;
        },
      },
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("tau_red_womling_fun_effect", [$(this)[0].val()])}</div>`;
        return desc;
      },
      val() {
        let val = 0;
    ...
    womling_lab: {
      ...
      effect() {
        let overseer = global.tauceti.hasOwnProperty("overseer")
          ? global.tauceti.overseer.prod
          : 100;
  ...
  tau_gas: {
          ...
          switch (n || global.race[key]) {
            ...
            case 4: {
              switch (Math.round(global.stats.bioseed + ns) % 3) {
                case 1:
                  return loc("tau_gas_title3a", [
                    races[global.race.gods].solar.gas,
                  ]);
                case 2:
                  return loc("tau_gas_title3b", [
                    races[global.race.old_gods].name,
                  ]);
                default:
                  return loc("tau_gas_title3c", [
            ...
            case 5: {
              switch (Math.round(global.stats.portals + ns) % 3) {
                ...
                case 2:
                  return loc("tau_gas_title4b", [
                    flib("reverse", planetName().gas),
                  ]);
                default:
    ...
    gas_contest: {
      ...
      effect() {
        return global.race["lone_survivor"]
          ? loc("tau_gas_contest_effect_alt")
          : loc("tau_gas_contest_effect");
      },
      action() {
        if (payCosts($(this)[0])) {
          return true;
    ...
    refueling_station: {
      ...
      effect() {
        ...
        desc =
          desc +
          `<div>${loc("plus_max_resource", [helium_tank.toLocaleString(), global.resource.Helium_3.name])}</div>`;
        if (global.tech["tau_whale"] >= 2) {
          let oil_tank = spatialReasoning(6500);
          desc =
            desc +
            `<div>${loc("plus_max_resource", [oil_tank.toLocaleString(), global.resource.Oil.name])}</div>`;
        }
        if (global.tech["isolation"]) {
    ...
    ore_refinery: {
      ...
      effect() {
        let ore = global.tauceti.hasOwnProperty("ore_refinery")
          ? global.tauceti.ore_refinery.fill
          : 0;
        let max = global.tauceti.hasOwnProperty("ore_refinery")
        ...
        desc =
          desc + `<div>${loc("tau_gas_ore_refinery_effect2", [max])}</div>`;
        desc =
          desc + `<div>${loc("tau_gas_ore_refinery_effect3", [refine])}</div>`;
        desc =
          desc +
          `<div>${loc("interstellar_stellar_forge_effect3", [$(this)[0].smelting()])}</div>`;
        desc =
          desc +
          `<div class="has-text-caution">${loc("minus_power", [$(this)[0].powered()])}</div>`;
        return desc;
    ...
    whaling_station: {
      ...
      effect() {
        let blubber = global.tauceti.hasOwnProperty("whaling_station")
          ? global.tauceti.whaling_station.fill
          : 0;
        let max = global.tauceti.hasOwnProperty("whaling_station")
        ...
        desc =
          desc + `<div>${loc("tau_gas_whaling_station_effect2", [max])}</div>`;
        desc =
          desc +
          `<div>${loc("tau_gas_whaling_station_effect3", [refine])}</div>`;
        desc =
          desc +
          `<div class="has-text-caution">${loc("minus_power", [$(this)[0].powered()])}</div>`;
        return desc;
    ...
    womling_station: {
      ...
      effect() {
        ...
        if (!global.race["flier"]) {
          desc =
            desc +
            `<div>${loc("plus_max_resource", [jobScale(1), loc(`job_cement_worker`)])}</div>`;
        }
        desc =
          desc +
          `<div>${loc("space_red_fabrication_effect1", [jobScale(1)])}</div>`;
        desc =
          desc +
          `<div class="has-text-caution">${loc("minus_power", [$(this)[0].powered()])}</div>`;
        return desc;
  ...
  tau_roid: {
    ...
    roid_mission: {
      ...
      cost: {
        Helium_3(offset, wiki) {
          return +int_fuel_adjust(1250000, false, wiki).toFixed(0);
        },
      },
      effect() {
      ...
      action() {
        if (payCosts($(this)[0])) {
          messageQueue(loc("tau_roid_mission_result"), "info", false, [
            "progress",
          ]);
          return true;
        }
    ...
    patrol_ship: {
      ...
      effect() {
        ...
        desc =
          desc +
          `<div class="has-text-caution">${loc("spend", [fuel, global.resource[$(this)[0].support_fuel().r].name])}</div>`;
        return desc;
      },
      support_fuel() {
  ...
  tau_gas2: {
    ...
    alien_station_survey: {
      ...
      cost: {
        Money() {
          return global.race["lone_survivor"] ? 1500000000 : 3000000000;
        },
        Helium_3() {
          return 5000000;
        },
      ...
      effect() {
        return loc("tau_gas2_alien_station_repair_effect", [
          tauCetiModules.tau_gas2.info.name(),
        ]);
      },
      action() {
        if (payCosts($(this)[0])) {
          ...
          messageQueue(
            loc("tau_gas2_alien_station_msg", [
              tauCetiModules.tau_gas2.info.name(),
            ]),
            "info",
            false,
    ...
    alien_station: {
      ...
      desc(wiki) {
        if (
          !global.tauceti.hasOwnProperty("alien_station") ||
          global.tauceti.alien_station.count < 100 ||
          wiki
        ) {
          return (
            `<div>${loc("tau_gas2_alien_station")}</div>` +
            (global.tauceti.hasOwnProperty("alien_station") &&
            global.tauceti.alien_station.count >= 100
              ? `<div class="has-text-special">${loc("space_dwarf_reactor_desc_req")}</div>`
              : `<div class="has-text-special">${loc("tau_gas2_alien_station_repair")}</div>`)
          );
        } else {
    ...
    alien_space_station: {
      ...
      effect() {
        ...
        if (
          ...
          global.tech.tau_gas2 >= 6 &&
          global.tauceti.alien_space_station.hasOwnProperty("decrypted")
        ) {
          let devisor = global.race["lone_survivor"] ? 100000 : 25000000;
          let decrypted = +(
            global.tauceti.alien_space_station.decrypted / devisor
          ).toFixed(2);
          if (decrypted > 100) {
            decrypted = 100;
          }
          desc =
            desc +
            `<div>${loc("tau_gas2_alien_station_effect", [decrypted])}</div>`;
        }
        desc =
          desc +
          `<div class="has-text-caution">${loc("spend", [fuel, global.resource[$(this)[0].p_fuel().r].name])}</div>`;
    ...
    matrioshka_brain: {
      ...
      desc(wiki) {
        if (
          !global.tauceti.hasOwnProperty("matrioshka_brain") ||
          global.tauceti.matrioshka_brain.count < 1000 ||
          wiki
        ) {
      ...
      queue_size: 50,
      queue_complete() {
        return 1000 - global.tauceti.matrioshka_brain.count;
      },
      cost: {
        Money(offset) {
          return (offset || 0) +
    ...
    ignition_device: {
      ...
      desc(wiki) {
        if (
          !global.tauceti.hasOwnProperty("ignition_device") ||
          global.tauceti.ignition_device.count < 10 ||
          wiki
        ) {
      ...
      reqs: { tau_gas2: 8 },
      condition() {
        return global.tauceti["ignition_device"] &&
          global.tauceti.ignition_device.count < 10
          ? true
          : false;
      },
    ...
    ignite_gas_giant: {
      ...
      title() {
        return loc("tau_gas2_ignite_gas_giant", [
          tauCetiModules.tau_gas2.info.name(),
        ]);
      },
      desc() {
      ...
      effect() {
        ...
        if (!global.tech["m_brain"]) {
          desc =
            desc +
            `<div class="has-text-warning">${loc("tau_gas2_ignite_gas_giant_effect2", [loc("tech_matrioshka_brain")])}</div>`;
        } else {
          desc =
            desc + `<div>${loc("tau_gas2_ignite_gas_giant_effect3")}</div>`;
...
for (let i = 1; i < 9; i++) {
  tauCetiModules.tau_gas[`gas_contest-a${i}`] = {
    ...
    cost: {},
    effect() {
      return loc(i === 8 ? "tau_gas_contest_reject" : "tau_gas_contest_pick", [
        tauCetiModules.tau_gas.info.name(i),
      ]);
    },
    action() {
      if (payCosts($(this)[0])) {
  ...
  tauCetiModules.tau_gas2[`gas_contest-b${i}`] = {
    ...
    cost: {},
    effect() {
      return loc(i === 8 ? "tau_gas2_contest_reject" : "tau_gas_contest_pick", [
        tauCetiModules.tau_gas2.info.name(i),
      ]);
    },
    action() {
      if (payCosts($(this)[0])) {
...
function matrixProjection() {
  ...
  let skilled =
    global.stats.retire === global.stats.matrix + 1
      ? `<div class="has-text-advanced">${loc("tau_star_matrix_skilled", [1])}</div>`
      : ``;
...
function retireProjection() {
  ...
  let plasmidType =
    global.race.universe === "antimatter"
      ...
      : loc("resource_Plasmid_plural_name");
  let skilled =
    global.stats.retire + 1 === global.stats.matrix
      ? `<div class="has-text-advanced">${loc("tau_star_matrix_skilled", [1])}</div>`
      : ``;
  return `<div class="has-text-advanced">${loc("interstellar_ascension_trigger_effect2", [gains.plasmid, plasmidType])}</div><div class="has-text-advanced">${loc("interstellar_ascension_trigger_effect2", [gains.phage, loc("resource_Phage_name")])}</div><div class="has-text-advanced">${loc("tau_star_matrix_servants", [1])}</div>${skilled}`;
}
...
function wom_repulse(v) {
  if (
    ...
    global.tech["womling_logistics"] &&
    global.tech.womling_logistics >= 2
  ) {
...
function wom_recycle(v) {
  if (
    global.tech["womling_tech"] &&
    global.tech["womling_recycling"] &&
    global.tech.womling_recycling >= 1
  ) {
...
export function renderTauCeti() {
  if (
    ...
    (global.settings.civTabs !== 1 || global.settings.spaceTabs !== 6)
  ) {
    return;
  }
  let parent = $("#tauceti");
  ...
  Object.keys(tauCetiModules).forEach(function (region) {
    ...
    if (global.settings.tau[`${show}`]) {
      let name =
        typeof tauCetiModules[region].info.name === "string"
          ? tauCetiModules[region].info.name
          : tauCetiModules[region].info.name();
      ...
      if (tauCetiModules[region].info["support"]) {
        ...
        if (tauCetiModules[region].info["hide_support"]) {
          parent.append(
            `<div id="${region}" class="space"><div id="sr${region}"><h3 class="name has-text-warning">${name}</h3>${property}</div></div>`,
          );
        } else {
          parent.append(
            `<div id="${region}" class="space"><div id="sr${region}"><h3 class="name has-text-warning">${name}</h3> <span v-show="s_max">{{ support }}/{{ s_max }}</span>${property}</div></div>`,
          );
        }
        vBind({
          el: `#sr${region}`,
        ...
        parent.append(
          `<div id="${region}" class="space"><div><h3 class="name has-text-warning">${name}</h3>${property}</div></div>`,
        );
      }
      ...
      popover(
        region,
        function () {
          return typeof tauCetiModules[region].info.desc === "string"
            ? tauCetiModules[region].info.desc
            : tauCetiModules[region].info.desc();
        },
        {
          elm: `#${region} h3.name`,
      ...
      Object.keys(tauCetiModules[region]).forEach(function (tech) {
        if (
          tech !== "info" &&
          checkRequirements(tauCetiModules, region, tech)
        ) {
...
export function drawShipYard() {
  if (
    !global.settings.tabLoad &&
    (global.settings.civTabs !== 2 || global.settings.govTabs !== 5)
  ) {
    return;
  }
  setOrbits();
  ...
  if (global.space.hasOwnProperty("shipyard") && global.settings.showShipYard) {
    ...
    shipStats.append(
      `<div class="registry"><span class="has-text-caution">${loc(`outer_shipyard_registry`)}</span>: <b-input v-model="b.name" maxlength="25" class="nameplate"></b-input></div>`,
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(`crew`)}</span> <span v-html="crewText()"></span></div>`,
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(`power`)}</span> <span v-html="powerText()"></span></div>`,
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(`firepower`)}</span> <span v-html="fireText()"></span></div>`,
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(`outer_shipyard_sensors`)}</span> <span v-html="sensorText()"></span></div>`,
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(`speed`)}</span> <span v-html="speedText()"></span></div>`,
    );
    shipStats.append(
      `<div><span class="has-text-caution">${loc(`outer_shipyard_fuel`)}</span> <span v-html="fuelText()"></span></div>`,
    );
    ...
    assemble.append(
      `<button class="button is-info" slot="trigger" v-on:click="build()"><span>${loc("outer_shipyard_build")}</span></button>`,
    );
    assemble.append(
      `<span><b-checkbox class="patrol" v-model="s.expand" v-on:input="redraw()">${loc("outer_shipyard_fleet_details")}</b-checkbox></span>`,
    );
    assemble.append(
      `<span><b-checkbox class="patrol" v-model="s.sort" v-on:input="redraw()">${loc("outer_shipyard_fleet_sort")}</b-checkbox></span>`,
    );

    plans.append(assemble);
    assemble.append(
      `<div><span>${loc(`outer_shipyard_park`, [planetName().dwarf])}</span><a href="#" class="solarMap" @click="trigModal">${loc(`outer_shipyard_map`)}</span></a>`,
    );
      ...
      methods: {
        setVal(b, v) {
          if (b === "class" && v === "explorer") {
            ...
            b === "class" &&
            v !== "explorer" &&
            global.space.shipyard.blueprint.class === "explorer"
          ) {
        ...
        speedText() {
          let speed =
            (149597870.7 / 225 / 24 / 3600) *
            shipSpeed(global.space.shipyard.blueprint);
...
export function TPShipDesc(parent, obj) {
  ...
  var shipPattern = $(
    `<div class="divider">${loc(`outer_shipyard_class_${ship.class}`)} | ${loc(`outer_shipyard_engine_${ship.engine}`)} | ${loc(`outer_shipyard_weapon_${ship.weapon}`)} | ${loc(`outer_shipyard_power_${ship.power}`)} | ${loc(`outer_shipyard_sensor_${ship.sensor}`)}</div>`,
  );
  parent.append(desc);
  ...
  let tc = timeCheck(
    { id: ship.name, cost: costs, doNotAdjustCost: true },
    false,
    true,
  );
  Object.keys(costs).forEach(function (res) {
    if (costs[res]() > 0) {
      ...
      cost.append(
        $(
          `<div class="${color}" data-${res}="${costs[res]()}">${label}${sizeApproximation(costs[res](), 2)}</div>`,
        ),
      );
    }
  });
  ...
  if (tc && tc["t"]) {
    desc.append(
      $(
        `<div class="divider"></div><div id="popTimer" class="flair has-text-advanced">{{ t | timer }}</div>`,
      ),
    );
    vBind({
      el: "#popTimer",
...
function buildTPShip(ship, queue) {
  ...
  let name = ship.name;
  while (
    global.space.shipyard.ships.filter((s) => s.name === name).length > 0
  ) {
    num++;
    name = ship.name + ` ${num}`;
  }
...
function updateCosts() {
  ...
  Object.keys(costs).forEach(function (k) {
    if (k === "Money") {
      $(`#shipYardCosts`).append(
        `<span class="res-${k} has-text-success" data-${k}="${costs[k]}" data-ok="has-text-success">${global.resource[k].name}${sizeApproximation(costs[k])}</span>`,
      );
    } else {
      $(`#shipYardCosts`).append(
        `<span> | </span><span class="res-${k} has-text-success" data-${k}="${costs[k]}" data-ok="has-text-success">${global.resource[k].name} ${sizeApproximation(costs[k])}</span>`,
      );
    }
  });
}
...
export function shipPower(ship, wiki) {
  ...
  switch (ship.power) {
    ...
    case "fusion":
      watts = Math.round(
        (ship.class === "explorer" || wiki ? 174 : 175) * out_inflate,
      );
      break;
    case "elerium":
  ...
  switch (ship.engine) {
    ...
    case "emdrive":
      watts -= Math.round(
        (ship.class !== "explorer" && !wiki ? 1024 : 515) * use_inflate,
      );
      break;
  }
...
function shipyardShipCompare(a, b) {
  return (
    (shipyardRanks.location[a.location] ?? 0) -
      (shipyardRanks.location[b.location] ?? 0) ||
    a.transit - b.transit ||
    (shipyardRanks.class[a.class] ?? 0) - (shipyardRanks.class[b.class] ?? 0) ||
    (shipyardRanks.engine[a.engine] ?? 0) -
      (shipyardRanks.engine[b.engine] ?? 0) ||
...
function drawShips() {
  ...
  if (global.space.shipyard.sort) {
    global.space.shipyard.ships =
      global.space.shipyard.ships.sort(shipyardShipCompare);
  }
  ...
  for (let i = 0; i < global.space.shipyard.ships.length; i++) {
        ...
        if (ship.location !== region) {
          if (spaceRegions[region].info.syndicate() || region === "spc_dwarf") {
            if (
              ...
              (global.race["orbit_decayed"] && region !== "spc_moon")
            ) {
              let name =
                typeof spaceRegions[region].info.name === "string"
                  ? spaceRegions[region].info.name
                  : spaceRegions[region].info.name();
    ...
    let location =
      ship.location === "tauceti"
        ? loc("tech_era_tauceti")
        : typeof spaceRegions[ship.location].info.name === "string"
          ? spaceRegions[ship.location].info.name
          : spaceRegions[ship.location].info.name();
    ...
    if (global.space.shipyard.expand) {
      ...
      let row1 = $(
        `<div class="row1"><span class="name has-text-caution">${ship.name}</span> <span v-show="scrapAllowed(${i})">| </span><a class="scrap${i}" v-show="scrapAllowed(${i})" @click="scrap(${i})" role="button">${loc(`outer_shipyard_scrap`)}</a> | <span class="has-text-warning">${ship_class}</span> | <span class="has-text-danger">${loc(`outer_shipyard_weapon_${ship.weapon}`)}</span> | <span class="has-text-warning">${loc(`outer_shipyard_power_${ship.power}`)}</span> | <span class="has-text-warning">${loc(`outer_shipyard_armor_${ship.armor}`)}</span> | <span class="has-text-warning">${loc(`outer_shipyard_sensor_${ship.sensor}`)}</span></div>`,
      );
      ...
      row2.append(
        `<span class="has-text-warning">${loc(`crew`)}</span> <span class="pad" v-html="crewText(${i})"></span>`,
      );
      row2.append(
        `<span class="has-text-warning">${loc(`firepower`)}</span> <span class="pad" v-html="fireText(${i})"></span>`,
      );
      row2.append(
        `<span class="has-text-warning">${loc(`outer_shipyard_sensors`)}</span> <span class="pad" v-html="sensorText(${i})"></span>`,
      );
      row2.append(
        `<span class="has-text-warning">${loc(`speed`)}</span> <span class="pad" v-html="speedText(${i})"></span>`,
      );
      row2.append(
        `<span class="has-text-warning">${loc(`outer_shipyard_fuel`)}</span> <span class="pad" v-bind:class="{ 'has-text-danger': !fueled }" v-html="fuelText(${i})"></span>`,
      );
      row2.append(
        `<span class="has-text-warning">${loc(`outer_shipyard_hull`)}</span> <span class="pad" v-bind:class="hullDamage(${i})" v-html="hullText(${i})"></span>`,
      );

      row3.append(
        `<span v-show="show(${i})" class="has-text-caution" v-html="dest(${i})"></span>`,
      );
      ...
      let desc = $(
        `<div id="shipReg${i}" class="shipRow ship${i} compact"></div>`,
      );
    ...
    vBind({
      ...
      methods: {
        scrap(id) {
          if (
            global.space.shipyard.ships[id] &&
            global.space.shipyard.ships[id].location === "spc_dwarf"
          ) {
        ...
        setLoc(l, id) {
          ...
          if (l !== ship.location) {
            ...
            if (
              manned ||
              global.civic.garrison.workers - global.civic.garrison.crew >= crew
            ) {
        ...
        speedText(id) {
          let speed =
            (149597870.7 / 225 / 24 / 3600) *
            shipSpeed(global.space.shipyard.ships[id]);
          return Math.round(speed) + "km/s";
        },
        fuelText(id) {
        ...
        hullDamage(id) {
          if (global.space.shipyard.ships[id].damage <= 10) {
            ...
            global.space.shipyard.ships[id].damage >= 40 &&
            global.space.shipyard.ships[id].damage < 65
          ) {
        ...
        dest(id) {
          let name =
            ship.class === "explorer"
              ? loc("tech_era_tauceti")
              : typeof spaceRegions[global.space.shipyard.ships[id].location]
                    .info.name === "string"
                ? spaceRegions[global.space.shipyard.ships[id].location].info
                    .name
                : spaceRegions[
                    global.space.shipyard.ships[id].location
                  ].info.name();
    ...
    if (ship.class === "explorer") {
      if (ship.location !== "tauceti") {
        popover(
          `ship${i}loctauceti`,
          function () {
            return loc(`transit_time`, [
              Math.round(
                transferWindow(ship.xy, calcLandingPoint(ship, "tauceti")) /
                  shipSpeed(ship),
              ),
            ]);
          },
          ...
          if (ship.location !== region) {
            popover(
              ...
              function () {
                return loc(`transit_time`, [
                  Math.round(
                    transferWindow(ship.xy, calcLandingPoint(ship, region)) /
                      shipSpeed(ship),
                  ),
                ]);
              },
...
function calcLandingPoint(ship, planet) {
  ...
  let cross1w_dist = Math.abs(
    ship_dist - spacePlanetStats[planet].dist * width,
  );
  let cross2w_dist = Math.abs(
    ship_dist + spacePlanetStats[planet].dist * width,
  );
  let cross1_days = Math.floor(
    Math.min(cross1_dist, cross1w_dist, cross2_dist, cross2w_dist) / ship_speed,
  );
  let cross2_days = Math.ceil(
    Math.max(cross1_dist, cross1w_dist, cross2_dist, cross2w_dist) / ship_speed,
  );
  if (
    ship_dist >= spacePlanetStats[planet].dist &&
    ship_dist <= spacePlanetStats[planet].dist * width
  ) {
    cross1_days = 0;
  }
  ...
  let planet_degree =
    (global.space.position[planet] + cross1_days * planet_speed) % 360;
  let rads = Math.PI / 180;
  for (let i = cross1_days; i <= cross2_days; i++) {
    let planet_x = xPosition(
      Math.cos(planet_degree * rads) * spacePlanetStats[planet].dist,
      planet,
    );
    planet_x += xShift(planet);
    let planet_y =
      Math.sin(planet_degree * rads) * spacePlanetStats[planet].dist;
    let time =
      Math.sqrt((planet_x - ship.xy.x) ** 2 + (planet_y - ship.xy.y) ** 2) /
      ship_speed;
    if (time <= i) {
...
export function syndicate(region, extra) {
  if (
    ...
    global.space.syndicate.hasOwnProperty(region)
  ) {
    let divisor = 1000;
    ...
    if (
      ...
      global.race.ocularPowerConfig.f
    ) {
    ...
    if (
      ...
      if (
        region === "spc_enceladus" &&
        Math.min(support_on["operating_base"], p_on["operating_base"]) > 0
      ) {
        let active = Math.min(
          support_on["operating_base"],
          p_on["operating_base"],
        );
        patrol += active * 50;
...
export function tritonWar() {
  if (global.space["fob"]) {
    ...
    if (
      ...
      global.race.ocularPowerConfig.ds += Math.round(
        kills * traits.ocular_power.vars()[1],
      );
    }
...

    {
      ...
      let danger =
        global.space.fob.enemy -
        armyRating(global.space.fob.troops, "army", wounded);
...
export function erisWar() {
  if (global.space["digsite"]) {
    ...
    } else if (global.space.digsite.enemy > 10000) {
      global.space.digsite.enemy = 10000;
    }

    global.space.digsite.count = Math.floor(
      100 - global.space.digsite.enemy / 100,
    );
  }
}
...
export function genXYcoord(planet) {
  let cx = xPosition(
    +Math.cos(global.space.position[planet] * (Math.PI / 180)).toFixed(5) *
      spacePlanetStats[planet].dist,
    planet,
  );
  let cy =
    +Math.sin(global.space.position[planet] * (Math.PI / 180)).toFixed(5) *
    spacePlanetStats[planet].dist;
  cx += xShift(planet);
  return { x: cx, y: cy };
}
...
export function tpStorageMultiplier(type, heavy, wiki) {
  ...
  switch (type) {
    case "storehouse": {
      let titan_spaceport_on = wiki
        ? (global.space?.titan_spaceport?.on ?? 0)
        : p_on["titan_spaceport"];
      ...
      if (
        ...
        if (global.race["hooved"]) {
          if (actions.space[sector][k].cost?.hasOwnProperty("Horseshoe")) {
            global.race["shoecnt"] -=
              actions.space[sector][k].cost.Horseshoe() * global.space[k].count;
          }
        }
...
export function calcAIDrift(wiki) {
  ...
  let shock_trooper_on = wiki
    ? global.space.shock_trooper.on
    : support_on["shock_trooper"];
...
export function drawMap() {
  ...
  for (let [id, planet] of Object.entries(spacePlanetStats)) {
    ...
    if (
      actions.space[id] &&
      actions.space[id].info.syndicate() &&
      global.settings.space[id.substring(4)]
    ) {
      let shift = syndicate(id);
      color = (
        (Math.round(255 * (1 - shift)) << 16) +
        (Math.round(255 * shift) << 8)
      )
        .toString(16)
        .padStart(6, 0);
    }
    ...
    if (planet.moon) {
      switch (id) {
        case "spc_moon":
          ctx.arc(
            planetLocation[id].x + 0.05,
            planetLocation[id].y + 0.05,
            size,
            0,
            Math.PI * 2,
            true,
        ...
        case "spc_titan":
          ctx.arc(
            planetLocation[id].x - 0.2,
            planetLocation[id].y - 0.2,
            size,
            0,
            Math.PI * 2,
            true,
      ...
      if (planet.moon) {
        switch (id) {
          case "spc_moon":
            ctx.fillText(
              nameText,
              planetLocation[id].x + 0.1,
              planetLocation[id].y + 0.1,
            );
            break;
          case "spc_titan":
            ctx.fillText(
              nameText,
              planetLocation[id].x - 0.3,
              planetLocation[id].y - 0.3,
            );
            break;
          default:
            ctx.fillText(
              nameText,
              planetLocation[id].x + 0.25,
              planetLocation[id].y + 0.2,
            );
            break;
        }
        ...
        switch (id) {
          ...
          default:
            ctx.fillText(
              nameText,
              planetLocation[id].x,
              planetLocation[id].y - 0.2 * planet.size,
            );
            break;
        }
      }
...
function solarModal() {
  $("#modalBox").append(
    $(
      '<p id="modalBoxTitle" class="has-text-warning modalTitle">Solar System</p>',
    ),
  );

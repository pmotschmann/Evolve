  ...
  let desc = {
    ...
    democracy: loc(
      global.race.universe === "evil"
        ? "govern_managed_democracy_effect"
        : "govern_democracy_effect",
      govEffect.democracy(),
    ),
    oligarchy:
...
function government(govern) {
  ...
  var type = $(
    `<div>${loc("civics_government_type")} <span id="govLabel" class="has-text-warning">{{ type | govern }}</span></div>`,
  );
  gov.append(type);
  ...
  popover("govLabel", function () {
    ...
    return $(
      `<div>${govDescription(global.civic.govern.type)}</div><div class="has-text-advanced">${government_desc(effect_type)}</div>`,
    );
  });
...
function drawGovModal() {
  $("#modalBox").append(
    $(
      `<p id="modalBoxTitle" class="has-text-warning modalTitle">${loc("civics_government_type")}</p>`,
    ),
  );
  ...
  if (global.tech["govern"]) {
    if (global.civic.govern.type !== "autocracy") {
      body.append(
        $(
          `<button class="button gap" data-gov="autocracy" @click="setGov('autocracy')">${loc(`govern_autocracy`)}</button>`,
        ),
      );
    }
    if (global.civic.govern.type !== "democracy" && !global.race["warlord"]) {
      body.append(
        $(
          `<button class="button gap" data-gov="democracy" @click="setGov('democracy')">${global.race.universe === "evil" ? loc(`govern_managed_democracy`) : loc(`govern_democracy`)}</button>`,
        ),
      );
    }
    if (global.civic.govern.type !== "oligarchy" && !global.race["warlord"]) {
      body.append(
        $(
          `<button class="button gap" data-gov="oligarchy" @click="setGov('oligarchy')">${loc(`govern_oligarchy`)}</button>`,
        ),
      );
    }
    if (
      global.tech["gov_theo"] &&
      global.civic.govern.type !== "theocracy" &&
      !global.race["warlord"]
    ) {
      body.append(
        $(
          `<button class="button gap" data-gov="theocracy" @click="setGov('theocracy')">${loc(`govern_theocracy`)}</button>`,
        ),
      );
    }
    if (
      global.tech["govern"] >= 2 &&
      global.civic.govern.type !== "republic" &&
      !global.race["warlord"]
    ) {
      body.append(
        $(
          `<button class="button gap" data-gov="republic" @click="setGov('republic')">${loc(`govern_republic`)}</button>`,
        ),
      );
    }
    if (
      global.tech["gov_soc"] &&
      global.civic.govern.type !== "socialist" &&
      !global.race["warlord"]
    ) {
      body.append(
        $(
          `<button class="button gap" data-gov="socialist" @click="setGov('socialist')">${loc(`govern_socialist`)}</button>`,
        ),
      );
    }
    if (
      global.tech["gov_corp"] &&
      global.civic.govern.type !== "corpocracy" &&
      !global.race["warlord"]
    ) {
      body.append(
        $(
          `<button class="button gap" data-gov="corpocracy" @click="setGov('corpocracy')">${loc(`govern_corpocracy`)}</button>`,
        ),
      );
    }
    if (
      ...
      body.append(
        $(
          `<button class="button gap" data-gov="technocracy" @click="setGov('technocracy')">${loc(`govern_technocracy`)}</button>`,
        ),
      );
    }
    if (
      ...
      body.append(
        $(
          `<button class="button gap" data-gov="federation" @click="setGov('federation')">${loc(`govern_federation`)}</button>`,
        ),
      );
    }
    if (global.tech["gov_mage"] && global.civic.govern.type !== "magocracy") {
      body.append(
        $(
          `<button class="button gap" data-gov="magocracy" @click="setGov('magocracy')">${loc(`govern_magocracy`)}</button>`,
        ),
      );
    }
    if (
      ...
      body.append(
        $(
          `<button class="button gap" data-gov="dictator" @click="setGov('dictator')">${loc(`govern_dictator`)}</button>`,
        ),
      );
    }
  }
  ...
  popover(
    ...
    function (obj) {
      ...
      return $(
        `<div>${govDescription(govType)}</div><div class="has-text-advanced">${government_desc(effectType)}</div>`,
      );
    },
    {
      elm: `#govModal button`,
...
export function foreignGov() {
  if (
    ...
    foreign.append(
      $(
        `<div class="header"><h2 class="has-text-warning">${loc("civics_foreign")}</h2></div>`,
      ),
    );
    ...
    for (let i = 0; i < govEnd; i++) {
      let gov = $(
        `<div id="gov${i}" class="foreign" v-show="gvis(${i})"><span class="has-text-caution">{{ '${i}' | gov }}</span><span v-if="f${i}.occ" class="has-text-advanced"> - ${loc("civics_garrison_occupy")}</span><span v-else-if="f${i}.anx" class="has-text-advanced"> - ${loc("civics_garrison_annex")}</span></span><span v-else-if="f${i}.buy" class="has-text-advanced"> - ${loc("civics_garrison_purchase")}</span></div>`,
      );
      foreign.append(gov);
      ...
      actions.append(
        $(
          `<button :label="battleAssessment(${i})" class="button gaction attack" @click="campaign(${i})"><span v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy">${loc("civics_garrison_attack")}</span><span v-show="f${i}.occ || f${i}.anx || f${i}.buy">${loc("civics_garrison_unoccupy")}</span></button>`,
        ),
      );
      actions.append(
        $(
          `<span class="tspy inline"><button :label="spyDesc(${i})" v-show="t.spy >= 1 && !f${i}.occ && !f${i}.anx && !f${i}.buy" :disabled="spy_disabled(${i})" class="button gaction" @click="spy(${i})"><span v-show="f${i}.trn === 0">${loc("tech_spy")}: {{ f${i}.spy }}</span><span v-show="f${i}.trn > 0">${loc("civics_train")}: {{ f${i}.trn }}</span></button></span>`,
        ),
      );
      actions.append(
        $(
          `<span class="sspy inline"><button :label="espDesc()" v-show="t.spy >= 2 && !f${i}.occ && !f${i}.anx && !f${i}.buy && f${i}.spy >= 1" :disabled="f${i}.sab > 0" class="button gaction" @click="trigModal(${i})"><span v-show="f${i}.sab === 0">${loc("tech_espionage")}</span><span v-show="f${i}.sab > 0">{{ f${i}.act | sab }}: {{ f${i}.sab }}</span></button></span>`,
        ),
      );
      gov.append(actions);
      ...
      gov.append(
        $(
          `<div v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy"><span class="has-text-advanced glabel">${loc("civics_gov_mil_rate")}:</span> <span class="glevel">{{ f${i}.mil | military(${i}) }}<span class="has-text-warning" v-show="f${i}.spy >= 2"> ({{ f${i}.mil }})</span></span></div>`,
        ),
      );
      gov.append(
        $(
          `<div v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy"><span class="has-text-advanced glabel">${loc("civics_gov_relations")}:</span> <span class="glevel">{{ f${i}.hstl | relation }}<span class="has-text-warning" v-show="f${i}.spy >= 1"> ({{ f${i}.hstl | hate }})</span></span></div>`,
        ),
      );
      gov.append(
        $(
          `<div v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy"><span class="has-text-advanced glabel">${loc("civics_gov_eco_rate")}:</span> <span class="glevel">{{ f${i}.eco | eco(${i}) }}<span class="has-text-warning" v-show="f${i}.spy >= 3"> ({{ f${i}.eco }})</span></span></div>`,
        ),
      );
      gov.append(
        $(
          `<div v-show="f${i}.spy >= 2 && !f${i}.occ && !f${i}.anx && !f${i}.buy"><span class="has-text-advanced glabel">${loc("civics_gov_unrest")}:</span> <span class="glevel">{{ f${i}.unrest | discontent(${i}) }}<span class="has-text-warning" v-show="f${i}.spy >= 4"> ({{ f${i}.unrest | turmoil }})</span></span></div>`,
        ),
      );
    }
...
function drawEspModal(gov) {
  $("#modalBox").append(
    $(
      `<p id="modalBoxTitle" class="has-text-warning modalTitle">${loc("civics_espionage_actions")}</p>`,
    ),
  );
  ...
  if (
    ...
    body.append(
      $(
        `<button class="button gap" data-esp="influence" @click="influence('${gov}')">${loc(`civics_spy_influence`)}</button>`,
      ),
    );
    body.append(
      $(
        `<button class="button gap" data-esp="sabotage" @click="sabotage('${gov}')">${loc(`civics_spy_sabotage`)}</button>`,
      ),
    );
    if (gov < 3) {
      body.append(
        $(
          `<button class="button gap" data-esp="incite" @click="incite('${gov}')">${loc(`civics_spy_incite`)}</button>`,
        ),
      );
    }
    if (
      ...
      body.append(
        $(
          `<button class="button gap" data-esp="annex" @click="annex('${gov}')">${loc(`civics_spy_annex`)}</button>`,
        ),
      );
    }
    if (gov < 3 && global.civic.foreign[`gov${gov}`].spy >= 3) {
      body.append(
        $(
          `<button class="button gap" data-esp="purchase" @click="purchase('${gov}')">${loc(`civics_spy_purchase`)}</button>`,
        ),
      );
    }
  }
...
function taxRates(govern) {
  ...
  var sub = $(
    `<span tabindex="0" role="button" aria-label="decrease taxes" class="sub has-text-success" @click="sub">&laquo;</span>`,
  );
  var add = $(
    `<span tabindex="0" role="button" aria-label="increase taxes" class="add has-text-danger" @click="add">&raquo;</span>`,
  );
...
export function buildGarrison(garrison, full) {
  ...
  if (global.tech["world_control"] && !global.race["truepath"]) {
    garrison.append(
      $(
        `<div class="header"><h2 class="has-text-warning">${loc("civics_garrison")}</h2> - <span class="has-text-success"><span class="defenseRating">${loc("rating")} {{ g.workers | hell | rating }}</span> - <span class="soldierRating"><span class="has-text-warning">${loc(`civics_garrison_soldier_rating`)}</span> {{ g.workers | single | rating(true) }}</span></div>`,
      ),
    );
  } else {
    garrison.append(
      $(
        `<div class="header"><h2 class="has-text-warning">${loc("civics_garrison")}</h2> - <span class="has-text-success"><span class="defenseRating">${loc("rating")} {{ g.workers | hell | rating }}</span> / <span class="offenseRating">{{ g.raid | rating }}</span></span> - <span class="soldierRating"><span class="has-text-warning">${loc(`civics_garrison_soldier_rating`)}</span> {{ g.workers | single | rating }}</span></div>`,
      ),
    );
  }
  ...
  if (!global.tech["isolation"]) {
    ...
    bunks.append(
      $(
        `<div class="barracks" v-show="g.crew > 0"><span class="crew">${loc("civics_garrison_crew")}</span> <span>{{ g.crew }}</span></div>`,
      ),
    );
    bunks.append(
      $(
        `<div class="barracks"><span class="wounded">${loc("civics_garrison_wounded")}</span> <span v-html="$options.filters.wounded(g.wounded)"></span></div>`,
      ),
    );

    barracks.append(
      $(
        `<div class="hire"><button v-show="g.mercs" class="button first hmerc" @click="hire">${loc("civics_garrison_hire_mercenary")}</button><div>`,
      ),
    );
  }
  ...
  if (full) {
    ...
    garrison.append(
      $(
        `<div class="training"><span>${loc("civics_garrison_training")} - ${loc("arpa_to_complete")} {{ g.rate, g.progress | trainTime }}${egg8}</span> <progress class="progress" :value="g.progress" max="100">{{ g.progress }}%</progress></div>`,
      ),
    );
  }
  ...
  if (
    ...
    var tactics = $(
      `<div id="${full ? "tactics" : "c_tactics"}" v-show="g.display" class="tactics"><span>${loc("civics_garrison_campaign")}</span></div>`,
    );
    wrap.append(tactics);
    ...
    var last = $(
      '<span tabindex="0" role="button" aria-label="easier campaign" class="sub" @click="last">&laquo;</span>',
    );
    var next = $(
      '<span tabindex="0" role="button" aria-label="harder campaign" class="add" @click="next">&raquo;</span>',
    );
    tactics.append(last);
    ...
    var battalion = $(
      `<div id="${full ? "battalion" : "c_battalion"}" v-show="g.display" class="tactics"><span>${loc("civics_garrison_battalion")}</span></div>`,
    );
    wrap.append(battalion);

    var armysize = $('<span class="current bat">{{ g.raid }}</span>');
    var alast = $(
      '<span tabindex="0" role="button" aria-label="remove soldiers from campaign" class="sub" @click="aLast">&laquo;</span>',
    );
    var anext = $(
      '<span tabindex="0" role="button" aria-label="add soldiers to campaign" class="add" @click="aNext">&raquo;</span>',
    );
    ...
    if (full) {
      if (global.race["truepath"] && global.tech["rival"]) {
        campaign.append(
          $(
            `<div class="launch gov3" v-show="rvis()"><div class="has-text-caution">${govTitle(3)}</div><button class="button campaign" @click="campaign(3)"><span>${loc("civics_garrison_launch_campaign")}</span></button></div>`,
          ),
        );
      }
      if (!global.tech["world_control"]) {
        campaign.append(
          $(
            `<div class="launch gov0"><div class="has-text-caution">${govTitle(0)}</div><button class="button campaign" @click="campaign(0)"><span v-show="!g0.occ && !g0.anx && !g0.buy">${loc("civics_garrison_launch_campaign")}</span><span v-show="g0.occ || g0.anx || g0.buy">${loc("civics_garrison_deoccupy")}</span></button></div>`,
          ),
        );
        campaign.append(
          $(
            `<div class="launch gov1"><div class="has-text-caution">${govTitle(1)}</div><button class="button campaign" @click="campaign(1)"><span v-show="!g1.occ && !g1.anx && !g1.buy">${loc("civics_garrison_launch_campaign")}</span><span v-show="g1.occ || g1.anx || g1.buy">${loc("civics_garrison_deoccupy")}</span></button></div>`,
          ),
        );
        campaign.append(
          $(
            `<div class="launch gov2"><div class="has-text-caution">${govTitle(2)}</div><button class="button campaign" @click="campaign(2)"><span v-show="!g2.occ && !g2.anx && !g2.buy">${loc("civics_garrison_launch_campaign")}</span><span v-show="g2.occ || g2.anx || g2.buy">${loc("civics_garrison_deoccupy")}</span></button></div>`,
          ),
        );
      }
    }
  ...
  data.forEach(function (d) {
    if (d.v > 0 || d.v < 0) {
      if (d.k === "base") {
        ...
        let val =
          typeof d.v === "string" ? parseInt(d.v) : +(d.v * 100).toFixed(1);
...
export function armyRating(val, type, wound, analysis) {
  ...
  if (type === "army" || type === "hellArmy" || type === "Troops") {
    if (global.race["tactical"]) {
      let tactical =
        (traits.tactical.vars()[0] * global.race["tactical"]) / 100;
      army *= 1 + tactical;
  ...
  if (global.race.universe === "evil" && global.resource.Authority.display) {
    if (global.resource.Authority.amount > 100) {
      let boost =
        ((global.resource.Authority.amount - 100) /
  ...
  if (
    (type === "army" || type === "hellArmy" || type === "Troops") &&

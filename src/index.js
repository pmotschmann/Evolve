...
export function index() {
  ...
  // Accessibility: global keyboard handler for role="button" elements
  // Allows Enter and Space to activate any span/div with role="button"
  ...
  $("body").append(`<div id="topBar" class="topBar">
        ...
        <span class="calendar">
            ...
            <span v-show="city.calendar.day">
                <span class="is-sr-only" v-html="sign()"></span><span id="astroSign" aria-hidden="true" class="astro" v-html="getAstroSign()"></span>
                <b-tooltip :label="moon()" :aria-label="moon()" position="is-bottom" size="is-small" multilined animated><i id="moon" aria-hidden="true" class="moon wi"></i></b-tooltip>
                ...
                <b-tooltip :label="weather()" :aria-label="weather()" position="is-bottom" size="is-small" multilined animated><i id="weather" aria-hidden="true" class="weather wi"></i></b-tooltip>
                ...
                <span tabindex="0" role="button" class="atime" style="padding: 0 0.5rem; margin-left: 0.5rem; cursor: pointer" @click="pause" :aria-label="pausedesc()">
                    <span id="pausegame" aria-hidden="true"></span>
                </span>
            </span>
        </span>
  ...
  columns.append(`<div class="column is-one-quarter leftColumn">
        ...
        <div id="sideQueue">
            ...
            <div id="msgQueue" class="msgQueue vscroll has-text-info" role="region" aria-label="${loc("message_log")}" aria-live="polite">
                <div id="msgQueueHeader">
                    <h2 class="has-text-success">${loc("message_log")}</h2>
                    <span tabindex="0" class="special" role="button" title="message queue options" @click="trigModal">
                    ...
                    <span tabindex="0" role="button" class="zero has-text-advanced" @click="clearLog(m.view)">${loc("message_log_clear")}</span>
                    <span tabindex="0" role="button" class="zero has-text-advanced" @click="clearLog()">${loc("message_log_clear_all")}</span>
                </div>
                <h2 class="is-sr-only">${loc("message_filters")}</h2>
                <div id="msgQueueFilters" class="hscroll msgQueueFilters" role="region" aria-label="${loc("message_filters")}"></div>
                <h2 class="is-sr-only">${loc("messages")}</h2>
                <div id="msgQueueLog" role="log" aria-live="polite" aria-relevant="additions text"></div>
            </div>
        </div>
  ...
  message_filters.forEach(function (filter) {
    $(`#msgQueueFilters`).append(`
            <span tabindex="0" id="msgQueueFilter-${filter}" class="${filter === "all" ? "is-active" : ""}" aria-disabled="${filter === "all" ? "true" : "false"}" @click="swapFilter('${filter}')" v-show="s.${filter}.vis" role="button">${loc("message_log_" + filter)}</span>
        `);
  });
  vBind({
    el: `#msgQueue`,
    ...
    methods: {
      swapFilter(filter) {
        if (message_logs.view !== filter) {
          $(`#msgQueueFilter-${message_logs.view}`)
            .removeClass("is-active")
            .attr("aria-disabled", "false");
          $(`#msgQueueFilter-${filter}`)
            .addClass("is-active")
      ...
      trigModal() {
        ...
        let checkExist = setInterval(function () {
          if ($("#modalBox").length > 0) {
            ...
            $("#modalBox").append(
              $(
                `<p id="modalBoxTitle" class="has-text-warning modalTitle">${loc("message_log")}${egg16.length > 0 ? egg16 : ""}</p>`,
              ),
            );
            ...
            let saveInputs = {};
            message_filters.forEach(function (filter) {
              visSet += `<div class="msgInput" v-show="s.${filter}.unlocked"><span id="vis_label_${filter}">${loc("message_log_" + filter)}</span> <b-checkbox class="patrol" v-model="s.${filter}.vis" :disabled="checkDisabled('${filter}',s.${filter}.vis)" :input="check('${filter}')" aria-labelledby="vis_label_${filter}"></b-checkbox></div>`;
              maxSet += `<div class="msgInput" v-show="s.${filter}.unlocked"><span id="max_label_${filter}">${loc("message_log_" + filter)}</span> <b-numberinput :input="maxVal('${filter}')" min="1" v-model="mi.${filter}" :controls="false" aria-labelledby="max_label_${filter}"></b-numberinput></div>`;
              saveSet += `<div class="msgInput" v-show="s.${filter}.unlocked"><span id="save_label_${filter}">${loc("message_log_" + filter)}</span> <b-numberinput :input="saveVal('${filter}')" min="0" :max="s.${filter}.max" v-model="si.${filter}" :controls="false" aria-labelledby="save_label_${filter}"></b-numberinput></div>`;
              ...
              methods: {
                check(filter) {
                  if (
                    ...
                    let haveVis = false;
                    Object.keys(global.settings.msgFilters).forEach(
                      function (filt) {
  ...
  for (let i = 0; i < icons.length; i++) {
    if (
      ...
      iconlist =
        iconlist +
        `<b-dropdown-item v-on:click="icon('${icons[i].i}')">${drawIcon(icons[i].i, 16, irank)} {{ '${icons[i].i}' | label }}</b-dropdown-item>`;
  ...
  let settings = $(`<b-tab-item id="settings" class="settings sticky">
        ...
        <div>
            ...
            <div class="keyMap"><label for="x10Key">${loc("multiplier", [10])}</label> <b-input v-model="s.keyMap.x10" id="x10Key"></b-input></div>
            <div class="keyMap"><label for="x25Key">${loc("multiplier", [25])}</label> <b-input class="keyMap" v-model="s.keyMap.x25" id="x25Key"></b-input></div>
            <div class="keyMap"><label for="x100Key">${loc("multiplier", [100])}</label> <b-input class="keyMap" v-model="s.keyMap.x100" id="x100Key"></b-input></div>
            <div class="keyMap"><label for="queueKey">${loc("q_key")}</label> <b-input class="keyMap" v-model="s.keyMap.q" id="queueKey"></b-input></div>
        </div>
        <div class="importExport">
            <div>${loc("tab_mappings")}</div>
            <div class="keyMap"><label for="showCivKey">${loc("tab_civil")}</label> <b-input v-model="s.keyMap.showCiv" id="showCivKey"></b-input></div>
            <div class="keyMap"><label for="showCivicKey">${loc("tab_civics")}</label> <b-input v-model="s.keyMap.showCivic" id="showCivicKey"></b-input></div>
            <div class="keyMap"><label for="showResearchKey">${loc("tab_research")}</label> <b-input v-model="s.keyMap.showResearch" id="showResearchKey"></b-input></div>
            <div class="keyMap"><label for="showResourcesKey">${loc("tab_resources")}</label> <b-input v-model="s.keyMap.showResources" id="showResourcesKey"></b-input></div>
            <div class="keyMap"><label for="showGeneticsKey">${loc("tech_arpa")}</label> <b-input v-model="s.keyMap.showGenetics" id="showGeneticsKey"></b-input></div>
            <div class="keyMap"><label for="showAchieveKey">${loc("tab_stats")}</label> <b-input v-model="s.keyMap.showAchieve" id="showAchieveKey"></b-input></div>
            <div class="keyMap"><label for="settingshKey">${loc("tab_settings")}</label> <b-input v-model="s.keyMap.settings" id="settingshKey"></b-input></div>
        </div>

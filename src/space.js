  ...
  spc_home: {
    ...
    nav_beacon: {
      ...
      effect() {
        ...
        let effect3 =
          global.tech["luna"] >= 3
            ? `<div>${loc("space_red_spaceport_effect1", [planetName().red, 1])}</div>`
            : "";
    ...
    iridium_mine: {
      ...
      effect() {
        ...
        let cat_coal = global.race["cataclysm"]
          ? `<div>${loc("produce", [+production("iridium_mine", "coal").toFixed(2), global.resource.Coal.name])}</div>`
          : ``;
        let cat_uran = global.race["cataclysm"]
          ? `<div>${loc("produce", [+(production("iridium_mine", "coal") / 48).toFixed(3), global.resource.Uranium.name])}</div>`
          : ``;
  ...
  spc_red: {
    ...
    terraformer: {
      ...
      desc(wiki) {
        if (
          ...
          return (
            `<div>${loc("space_terraformer")}</div><div class="has-text-special">${loc("requires_segments", [100])}</div>` +
            (global.space.hasOwnProperty("terraformer") &&
            global.space.terraformer.count >= 100
    ...
    garage: {
      ...
      effect() {
        ...
        let crate =
          global.race["cataclysm"] || global.race["orbit_decayed"]
            ? `<span>${loc("plus_max_resource", [containers, global.resource.Crates.name])}</span>`
            : ``;

        let desc = '<div class="aTable">';
        desc =
          desc +
          `<span>${loc("plus_max_resource", [containers, global.resource.Containers.name])}</span>${crate}`;
    ...
    red_mine: {
      ...
      effect() {
        ...
        let decayed = global.race["orbit_decayed"]
          ? `<div>${loc("plus_max_resource", [jobScale(1), loc(`job_miner`)])}</div><div>${loc("plus_max_resource", [jobScale(1), loc(`job_coal_miner`)])}</div>`
          : "";
        let cat_stone =
          (global.race["cataclysm"] || global.race["orbit_decayed"]) &&
          !global.race["sappy"]
            ? `<div>${loc("space_red_mine_effect", [+production("red_mine", "stone").toFixed(2), global.resource.Stone.name])}</div>`
            : ``;
        let cat_asbestos =
          (global.race["cataclysm"] || global.race["orbit_decayed"]) &&
          global.race["smoldering"]
            ? `<div>${loc("space_red_mine_effect", [+production("red_mine", "asbestos").toFixed(2), global.resource.Chrysotile.name])}</div>`
            : ``;
        let cat_alum =
          global.race["cataclysm"] || global.race["orbit_decayed"]
            ? `<div>${loc("space_red_mine_effect", [+production("red_mine", "aluminium").toFixed(2), global.resource.Aluminium.name])}</div>`
            : ``;
    ...
    red_factory: {
      ...
      effect(wiki) {
        ...
        desc =
          desc +
          `<div class="has-text-caution">${loc("space_red_factory_effect3", [helium, $(this)[0].powered()])}</div>`;
        return desc;
      },
      powered() {
    ...
    biodome: {
      ...
      effect() {
        ...
        let cat_fd =
          global.race["cataclysm"] || global.race["orbit_decayed"]
            ? `<div>${loc("produce", [+production("biodome", "cat_food").toFixed(2), global.resource.Food.name])}</div>`
            : ``;
        let cat_wd =
          (global.race["cataclysm"] || global.race["orbit_decayed"]) &&
          !global.race["kindling_kindred"] &&
          !global.race["smoldering"]
            ? `<div>${loc("space_red_mine_effect", [+production("biodome", "lumber").toFixed(2), global.resource.Lumber.name])}</div>`
            : ``;
        let pop = global.tech.mars >= 6 ? 0.1 : 0.05;
    ...
    space_barracks: {
      ...
      effect(wiki) {
        ...
        let food = global.race["cataclysm"]
          ? ``
          : `<div class="has-text-caution">${loc("space_red_space_barracks_effect3", [global.resource.Food.name])}</div>`;
  ...
  spc_hell: {
    ...
    geothermal: {
      ...
      effect(wiki) {
        ...
        let smelter =
          num_smelters > 0
            ? `<div>${loc("interstellar_stellar_forge_effect3", [num_smelters])}</div>`
            : ``;
      ...
      effect(wiki) {
        ...
        let elerium =
          global.tech["asteroid"] >= 5
            ? `<div>${loc("plus_max_resource", [elerium_cap, global.resource.Elerium.name])}</div>`
            : "";
    ...
    world_collider: {
      ...
      desc(wiki) {
        if (
          ...
          return (
            `<div>${loc("space_dwarf_collider_desc")}</div><div class="has-text-special">${loc("space_dwarf_collider_desc_req")}</div>` +
            (global.space.hasOwnProperty("world_collider") &&
            global.space.world_collider.count >= 1859
              ? `<div class="has-text-special">${loc("requires_power")}</div>`
              : ``)
  ...
  int_proxima: {
    ...
    cargo_yard: {
      ...
      effect(wiki) {
        ...
        desc =
          desc +
          `<div>${loc("plus_max_resource", [neutronium, global.resource.Neutronium.name])}</div><div>${loc("plus_max_resource", [infernite, global.resource.Infernite.name])}</div>`;
    ...
    dyson: {
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          let power =
            count > 0
              ? `<div>${loc("space_dwarf_reactor_effect1", [powerModifier(count * 1.25)])}</div>`
              : ``;
          let remain = 100 - count;
      ...
      effect(wiki) {
        ...
        if (count < 100) {
          ...
          if (global.tech["blackhole"] >= 5) {
            ...
            let blackhole =
              exotic > 0
                ? loc("interstellar_stellar_engine_effect3", [
                    +r_mass.toFixed(10),
                    +exotic.toFixed(10),
                  ])
      ...
      powered() {
        ...
        let gWell =
          ...
          global.stats.achieve.escape_velocity["h"]
            ...
            : 0);
        let output = powerModifier(
          (20 + (r_mass - 8 + exotic * 10) * waves) * gWell,
        );
        if (output > 10000) {
    ...
    stargate: {
      ...
      desc(wiki) {
        if (
          ...
          return (
            `<div>${loc("interstellar_stargate")}</div><div class="has-text-special">${loc("requires_segments", [200])}</div>` +
            (global.interstellar.hasOwnProperty("stargate") &&
            global.interstellar.stargate.count >= 200
              ? `<div class="has-text-special">${loc("requires_power")}</div>`
              : ``)
    ...
    ascension_machine: {
      ...
      desc(wiki) {
        if (
          ...
          return (
            `<div>${loc("interstellar_ascension_machine")}</div><div class="has-text-special">${loc("requires_segments", [100])}</div>` +
            (global.interstellar.hasOwnProperty("ascension_machine") &&
            global.interstellar.ascension_machine.count >= 100
              ? `<div class="has-text-special">${loc("requires_power")}</div>`
              : ``)
        ...
        if (payCosts($(this)[0])) {
          if (global.interstellar.ascension_machine.count < 100) {
            ...
            if (global.interstellar.ascension_machine.count >= 100) {
              ...
              if (global.settings.alwaysPower) {
                powerOnNewStruct(
                  interstellarProjects.int_sirius.ascension_trigger,
                );
              }
              deepSpace();
              clearPopper();
  ...
  gxy_stargate: {
    ...
    gateway_depot: {
      ...
      effect(wiki) {
        ...
        desc =
          desc +
          `<span>${loc("plus_max_crates", [containers])}</span><span>${loc("plus_max_containers", [containers])}</span>`;
        desc =
          desc +
          `<span>${loc("plus_max_resource", [uranium, global.resource.Uranium.name])}</span>`;
        desc =
          desc +
          `<span>${loc("plus_max_resource", [nano, global.resource.Nano_Tube.name])}</span>`;
        desc =
          desc +
          `<span>${loc("plus_max_resource", [neutronium, global.resource.Neutronium.name])}</span>`;
        desc =
          desc +
          `<span>${loc("plus_max_resource", [infernite, global.resource.Infernite.name])}</span>`;
        desc = desc + "</div>";
      ...
      effect() {
        ...
        let joy =
          global.tech["theatre"] && !global.race["joyless"]
            ? `<div>${loc("plus_max_resource", [jobScale(2), loc(`job_entertainer`)])}</div>`
            : "";
  ...
  gxy_alien2: {
    ...
    alien2_mission: {
      ...
      cost: {
        Custom() {
          if (
            ...
            let total = 0;
            Object.keys(global.galaxy.defense.gxy_alien2).forEach(
              function (ship) {
                total +=
            ...
            return {
              label: loc(`galaxy_fleet_rating`, [
                `<span${total < 400 ? ` class="has-text-danger"` : ""}>400</span>`,
              ]),
      ...
      effect() {
        ...
        let odds =
          total >= 650
            ? `<span class="has-text-success">${loc(`galaxy_piracy_low`)}</span>`
            : `<span class="has-text-warning">${loc(`galaxy_piracy_avg`)}</span>`;
    ...
    scavenger: {
      ...
      effect(wiki) {
        ...
        let boost = global.race["cataclysm"]
          ? `<div>${loc("galaxy_scavenger_effect2_cata", [+(pirate * 100 * 0.75).toFixed(1)])}</div>`
          : `<div>${loc("galaxy_scavenger_effect2", [+((pirate * 100) / 4).toFixed(1)])}</div>`;
      ...
      cost: {
        Custom() {
          if (
            ...
            let total = 0;
            Object.keys(global.galaxy.defense.gxy_chthonian).forEach(
              function (ship) {
                total +=
                  galaxyProjects.gxy_gateway[ship].ship.rating() *
            ...
            return {
              label: loc(`galaxy_fleet_rating`, [
                `<span${total < 1250 ? ` class="has-text-danger"` : ``}>1250</span>`,
              ]),
      ...
      effect() {
        ...
        let odds =
          total >= 4500
            ...
            : total >= 2500
              ? `<span class="has-text-warning">${loc(`galaxy_piracy_avg`)}</span>`
              : `<span class="has-text-danger">${loc(`galaxy_piracy_high`)}</span>`;
...
export function piracy(region, rating, raw, wiki) {
  if (global.tech["piracy"] && !global.race["truepath"]) {
    ...
    if (region === "gxy_chthonian") {
      ...
      let num_raider_on = wiki
        ? (global.galaxy?.raider?.on ?? 0)
        : gal_on["raider"];
      if (num_raider_on) {
        armada +=
...
export function checkRequirements(action_set, region, action) {
  ...
  if (
    isMet &&
    action_set[region][action].hasOwnProperty("condition") &&
    !action_set[region][action].condition()
  ) {
    isMet = false;
  }
...
function space(zone) {
  ...
  parent.append(
    $(
      `<h2 class="is-sr-only">${loc(zone === "inner" ? "tab_space" : "tab_outer_space")}</h2>`,
    ),
  );
    ...
    if (global.settings.space[`${show}`]) {
      ...
      if ((noHome && region !== "spc_home") || !noHome) {
        ...
        if (
          ...
          $(`#${region}synd`).append(
            `<span class="syndThreat has-text-caution">${loc("space_syndicate")} <span class="has-text-danger" v-html="threat('${region}')"></span></span>`,
          );
          $(`#${region}synd`).append(
            `<span class="syndThreat has-text-caution">${loc("space_scan_effectiveness")} <span class="has-text-warning" v-html="scan('${region}')"></span></span>`,
          );
          $(`#${region}synd`).append(
            `<span v-show="overkill('${region}')" class="syndThreat has-text-caution">${loc("space_overkill")} <span class="has-text-warning" v-html="overkill('${region}')"></span></span>`,
          );
          vBind({
            el: `#${region}synd`,
    ...
    if (global.settings.space[`${show}`]) {
      ...
      if (global.tech["xeno"] && global.tech["xeno"] >= 3) {
        regionContent.append(
          `<span class="regionControl has-text-${galaxyProjects[region].info.control().color}">{{ r.control().name }}</span>`,
        );
      }
      ...
      if (global.tech["piracy"]) {
        regionContent.append(
          `<div><span class="has-text-caution pirate">${loc("galaxy_piracy_threat", [races[global.galaxy.alien2.id].name])}</span><span :class="threat('${region}')">{{ '${region}' | pirate }}</span><span class="sep">|</span><span class="has-text-warning">${loc("galaxy_armada")}</span>: <span class="has-text-success">{{ '${region}' | defense }}</span></div>`,
        );
      }
...
function armada(parent, id) {
  if (global.tech["piracy"] && !global.race["truepath"]) {
    let header = $(
      `<div id="h${id}" class="armHead"><h3 class="has-text-warning">${loc("galaxy_armada")}</h3></div>`,
    );
    ...
    header.append(
      $(
        `<span class="has-text-caution"><span class="crew1">${loc("job_crew_mil")}</span> <span>{{ g.crew }}</span></span>`,
      ),
    );
    header.append($(`<span>|</span>`));
    header.append(
      $(
        `<span class="has-text-success"><span class="crew2">${loc("job_crew_civ")}</span> <span>{{ c.workers }} / {{ c.max }}</span></span>`,
      ),
    );
    ...
    for (let i = 0; i < gatewayArmada.length; i++) {
      ...
      if (global.galaxy.hasOwnProperty(ship)) {
        cols[i + 1].append(
          $(
            `<span id="armada${ship}" class="ship has-text-advanced">${galaxyProjects.gxy_gateway[ship].title}</span>`,
          ),
        );
        cols[i + 1].append(
          $(`<span class="ship">{{ gateway.${ship} }}</span>`),
        );
      }
    }
    ...
    Object.keys(global.galaxy.defense).forEach(function (area) {
      ...
      if (global.settings.space[r] && r !== "gateway") {
        let region = $(
          `<span id="armada${r}" class="has-text-caution">${typeof galaxyProjects[area].info.name === "string" ? galaxyProjects[area].info.name : galaxyProjects[area].info.name()}</span>`,
        );
        ...
        for (let i = 0; i < gatewayArmada.length; i++) {
          ...
          if (global.galaxy.hasOwnProperty(ship)) {
            ...
            let sub = $(
              `<span tabindex="0" role="button" aria-label="remove ${ship}" class="sub has-text-danger" @click="sub('${area}','${ship}')"><span>&laquo;</span></span>`,
            );
            let count = $(`<span class="current">{{ ${r}.${ship} }}</span>`);
            let add = $(
              `<span tabindex="0" role="button" aria-label="add ${ship}" class="add has-text-success" @click="add('${area}','${ship}')"><span>&raquo;</span></span>`,
            );
    ...
    for (let i = 0; i < gatewayArmada.length; i++) {
      ...
      if (global.galaxy.hasOwnProperty(ship)) {
        popover(`armada${ship}`, function (obj) {
          actionDesc(
            obj.popper,
            galaxyProjects.gxy_gateway[ship],
            global.galaxy[ship],
          );
          return undefined;
        });
...
export function genPlanets() {
  ...
  if (global.race["geck"] && global.race.geck > 0) {
    let geck = $(
      `<div id="geck" class="geck"><span class="has-text-caution">${loc(`gecks_remaining`)}</span>: <span class="has-text-warning">{{ geck }}</span></div>`,
    );
...
export function setUniverse() {
  ...
  for (let i = 0; i < universes.length; i++) {
    ...
    let element = $(
      `<a class="button is-dark" v-on:click="action" role="link"><span class="aTitle">${universe_types[universe].name}</span></a>`,
    );
...
export function ascendLab(hybrid, wiki) {
  ...
  if (!isWiki && !global.race["noexport"]) {
    ...
    if (hybrid) {
      ...
      if (
        ...
        global.stats.achieve.what_is_best.e >= 5
      ) {
  ...
  Object.keys(races).forEach(function (race) {
    ...
    if (
      ...
      if (
        ...
        !["custom", "hybrid", "junker", "sludge", "ultra_sludge"].includes(race)
      ) {
  ...
  for (let i = genome.traitlist.length - 1; i >= 0; i--) {
    if (
      ...
      !unlockedTraits.hasOwnProperty(genome.traitlist[i]) ||
  ...
  vBind({
    ...
    methods: {
      ...
      geneEdit() {
        let newRanks = genome.traitlist.map((x) =>
          tRanks[x] ? { [x]: tRanks[x] } : { [x]: 1 },
        );
        let ranks = {};
      ...
      setRace() {
        ...
        if (
          calcGenomeScore(genome, false, tRanks) >= 0 &&
      ...
      allowed(t) {
        if (
          (genome.traitlist.includes("catnip") && t === "anise") ||
          (genome.traitlist.includes("anise") && t === "catnip")
        ) {
          return true;
        } else if (
          (!["synthetic", "hybrid"].includes(genome.genus) ||
            (genome.hasOwnProperty("hybrid") &&
              !genome.hybrid.includes("synthetic"))) &&
      ...
      fanatic() {
        ...
        var checkExist = setInterval(function () {
          if ($("#modalBox").length > 0) {
            ...
            $("#modalBox").append(
              $(
                `<p id="modalBoxTitle" class="has-text-warning modalTitle">${loc(`genelab_fanatic_set`)}</p>`,
              ),
            );
      ...
      genus(slot) {
        ...
        var checkExist = setInterval(function () {
          if ($("#modalBox").length > 0) {
            ...
            $("#modalBox").append(
              $(
                `<p id="modalBoxTitle" class="has-text-warning modalTitle">${loc(`genelab_genus`)}</p>`,
              ),
            );
            var body = $('<div id="specialModal" class="modalBody"></div>');
            $("#modalBox").append(body);
            ...
            Object.keys(genus_def).forEach(function (type) {
              if (type !== "hybrid") {
                if (
                  ...
                  if (
                    genome.genus === "hybrid" &&
                    ((slot === 0 && type !== genome.hybrid[1]) ||
                      (slot === 1 && type !== genome.hybrid[0]))
                  ) {
            ...
            Object.keys(genus_def).forEach(function (type) {
              if (
                ...
                if (
                  ...
                  (genome.genus === "hybrid" &&
                    slot === 0 &&
                    type !== genome.hybrid[1]) ||
                  (genome.genus === "hybrid" &&
                    slot === 1 &&
                    type !== genome.hybrid[0])
                ) {
                  popover(
                    `geneLabGenus${type}`,
                    function () {
        ...
        if (file) {
          ...
          reader.onload = function (evt) {
            ...
            if (importCustom.genus !== "hybrid" && hybrid) {
              genome["hybrid"] = [
                importCustom.genus,
                importCustom.genus === "humanoid" ? "small" : "humanoid",
              ];
              genome.genus = "hybrid";
            ...
            tRanks = importCustom.hasOwnProperty("ranks")
              ? importCustom.ranks
              : {};
            genome.ranks = {};
      ...
      customExport() {
        ...
        downloadToFile(
          JSON.stringify(exportGenome, null, 4),
          `evolve-${hybrid ? "hybrid" : "custom"}-${exportGenome.name}.txt`,
          "text/plain",
        );
      },
    },
    filters: {
      ...
      genus(g, i) {
        return typeof i === "undefined"
          ...
          : loc(`genelab_genus_${g[i]}`);
      },
      empower(e, t) {
        let valid_empower =
          traits[t].val >= traits.empowered.vars(tRanks["empowered"] || 1)[0] &&
          traits[t].val <= traits.empowered.vars(tRanks["empowered"] || 1)[1] &&
          !["empowered", "catnip", "anise"].includes(t) &&
          genome.traitlist.includes("empowered");
  ...
  let genus_trank =
    global.stats.achieve["pathfinder"] && global.stats.achieve.pathfinder.l >= 4
      ? 2
      : 1;
  if (hybrid) {
  ...
  function summaryTab(tab) {
    if (tab === 4 || tab == 5) {
      ...
      genome.traitlist.sort().forEach(function (trait) {
        if (traits.hasOwnProperty(trait) && traits[trait].type === "major") {
          if (traits[trait].val >= 0) {
            ...
            summary += `<span tabindex="0" role="button" aria-label="${loc(`genelab_rank_lower`, [loc(`trait_${trait}_name`)])}" class="sub has-text-danger" @click="reduce('${trait}')"><span>-</span></span>`;
            summary += `<span tabindex="0" role="button" aria-label="${loc(`genelab_rank_higher`, [loc(`trait_${trait}_name`)])}" class="add has-text-success" @click="increase('${trait}')"><span>+</span></span>`;
            summary += `</span></div>`;
          } else {
            ...
            negative_sum += `<span tabindex="0" role="button" aria-label="${loc(`genelab_rank_lower`, [loc(`trait_${trait}_name`)])}" class="sub has-text-danger" @click="reduce('${trait}')"><span>-</span></span>`;
            negative_sum += `<span tabindex="0" role="button" aria-label="${loc(`genelab_rank_higher`, [loc(`trait_${trait}_name`)])}" class="add has-text-success" @click="increase('${trait}')"><span>+</span></span>`;
      ...
      vBind({
        el:
          tab === 4
            ? "#traitSummary .trait_selection"
            : "#allSum .trait_selection",
        data: {
          g: genome,
          t: tRanks,
        },
        methods: {
          ...
          reduce(t) {
            let unlock =
              (global.stats.achieve[`extinct_${traits[t].origin}`] &&
                global.stats.achieve[`extinct_${traits[t].origin}`].l) ||
              0;
            switch (tRanks[t]) {
              case 0.25:
                if (unlock >= 5) {
        ...
        filters: {
          ...
          empower(e, t) {
            let valid_empower =
              traits[t].val >=
                traits.empowered.vars(tRanks["empowered"] || 1)[0] &&
              traits[t].val <=
                traits.empowered.vars(tRanks["empowered"] || 1)[1] &&
              !["empowered", "catnip", "anise"].includes(t) &&
              genome.traitlist.includes("empowered");
            return valid_empower
              ? `, <span class="has-text-caution">E</span>`
              : ``;
          },
        },
      });
      ...
      genome.traitlist.sort().forEach(function (trait) {
        if (traits.hasOwnProperty(trait) && traits[trait].type === "major") {
          popover(
            `celestialLabtraitSelection${trait}Sum`,
            function () {
...
function geneCost(genome, trait, tRanks) {
  ...
  let genus_origin =
    races[traits[trait].origin].type === "hybrid"
      ? races[traits[trait].origin].hybrid
      : [races[traits[trait].origin].type];
  if (active_genus.filter((x) => genus_origin.includes(x)).length > 0) {
    active_genus.filter((x) => genus_origin.includes(x)).length === 1
      ? gene_cost--
      : (gene_cost -= 2);
  }
  if (oppose_genus.filter((x) => genus_origin.includes(x)).length > 0) {
    oppose_genus.filter((x) => genus_origin.includes(x)).length === 1
      ? gene_cost++
      : (gene_cost += 2);
  }
...
export function terraformLab(wiki) {
  ...
  lab.append(
    `<div><h3 class="has-text-danger">${loc("planetlab_title")}</h3> - <span class="has-text-warning">${loc("planetlab_points")} {{ p.pts }}</span></div>`,
  );
  ...
  geoList.forEach(function (res) {
    ...
    geo_list += `<span tabindex="0" role="button" aria-label="export ${res}" class="sub has-text-danger" @click="less('${res}')"><span>-</span></span>`;
    geo_list += `<span class="current" v-html="$options.filters.res('${res}')"></span>`;
    geo_list += `<span tabindex="0" role="button" aria-label="import ${res}" class="add has-text-success" @click="more('${res}')"><span>+</span></span>`;
    geo_list += `</div></div>`;
  });

...
$(document).keydown(function (e) {
  ...
  if (!$(`input`).is(":focus") && !$(`textarea`).is(":focus")) {
    Object.keys(quickMap).forEach(function (k) {
      if (
        key === global.settings.keyMap[k] &&
        global.settings.civTabs !== 0 &&
        (k === "settings" || global.settings[k])
      ) {
        ...
        // Accessibility: move focus into the newly active tab panel
        setTimeout(function () {
          let panel = document.querySelector(
            '#mainTabs > .tab-content > .tab-item:not([style*="display: none"])',
          );
          if (panel) {
            let target = panel.querySelector(
              'h2, h3, [tabindex="0"], button, a, input, select, textarea',
            );
            if (target) {
              target.focus();
            }
...
if (global["beta"]) {
  $("#topBar .version > a").html(
    `v${global.version} Beta ${global.beta}${revision}`,
  );
} else {
...
global.settings.sPackMsg = save.getItem("string_pack_name")
  ? loc(`string_pack_using`, [save.getItem("string_pack_name")])
  : loc(`string_pack_none`);
...
if (
  global.city["replicator"] &&
  global.race?.replicator?.pow &&
  global.race?.governor?.config?.replicate?.pow?.on
) {
...
vBind({
  ...
  filters: {
    replicate(kw) {
      if (
        global.race.hasOwnProperty("governor") &&
        global.race.governor.hasOwnProperty("tasks") &&
        global.race.hasOwnProperty("replicator") &&
        Object.values(global.race.governor.tasks).includes("replicate") &&
        global.race.governor.config.replicate.pow.on &&
        global.race.replicator.pow > 0
      ) {
...
popover(
  "race",
  function () {
    return typeof races[global.race.species].desc === "string"
      ? races[global.race.species].desc
      : races[global.race.species].desc();
  },
  {
    elm: "#race > .name",
...
popover(
  ...
  function (obj) {
    if (global.city.morale.unemployed !== 0) {
      ...
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(global.race["playful"] ? "morale_hunter" : "morale_unemployed")}</span> <span class="has-text-${type}"> ${+global.city.morale.unemployed.toFixed(1)}%</span></p>`,
      );
    }
    if (global.city.morale.stress !== 0) {
      let type = global.city.morale.stress > 0 ? "success" : "danger";
      obj.popper.append(
        `<p class="modal_bd"><span>${loc("morale_stress")}</span> <span class="has-text-${type}"> ${+global.city.morale.stress.toFixed(1)}%</span></p>`,
      );
    }
    ...
    Object.keys(global.city.morale).forEach(function (morale) {
      if (
        ...
        if (
          morale === "entertain" &&
          global.civic.govern.type === "democracy"
        ) {
        ...
        obj.popper.append(
          `<p class="modal_bd"><span>${loc(`morale_${morale}`)}</span> <span class="has-text-${type}"> ${+value.toFixed(1)}%</span></p>`,
        );
        ...
        if (
          ...
          obj.popper.append(
            `<p class="modal_bd"><span>ᄂ${loc("govern_democracy")}</span> <span class="has-text-success"> +${democracy}%</span></p>`,
          );
        }
      }
    });
    ...
    if (global.city.morale.season !== 0) {
      ...
      obj.popper.append(
        `<p class="modal_bd"><span>${season}</span> <span class="has-text-${type}"> ${+global.city.morale.season.toFixed(1)}%</span></p>`,
      );
    }
    ...
    if (global.civic.govern.type === "corpocracy") {
      ...
      total -= penalty;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc("govern_corpocracy")}</span> <span class="has-text-danger"> -${penalty}%</span></p>`,
      );
    }
    if (global.civic.govern.type === "republic") {
      ...
      total += repub;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc("govern_republic")}</span> <span class="has-text-success"> ${repub}%</span></p>`,
      );
    }
    if (global.civic.govern.type === "federation") {
      ...
      total += fed;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc("govern_federation")}</span> <span class="has-text-success"> ${fed}%</span></p>`,
      );
    }
    ...
    if (milVal) {
      total -= milVal;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc("gov_trait_militant")}</span> <span class="has-text-danger"> -${milVal}%</span></p>`,
      );
    }

    if (global.race["cheese"]) {
      let raw_cheese = global.stats.hasOwnProperty("reset")
        ? global.stats.reset + 1
        : 1;
      let cheese = +((raw_cheese / (raw_cheese + 10)) * 11).toFixed(2);
      total += cheese;
      obj.popper.append(
        `<p class="modal_bd"><span>${swissKnife(true, false)}</span> <span class="has-text-success"> ${cheese}%</span></p>`,
      );
    }
    ...
    if (global.race["motivated"]) {
      ...
      total += boost;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(`event_motivation_bd`)}</span> <span class="has-text-success"> ${boost}%</span></p>`,
      );
    }

    if (global.race["artisan"] && global.civic.craftsman.workers > 0) {
      let boost = +(
        traits.artisan.vars()[2] * global.civic.craftsman.workers
      ).toFixed(2);
      total += boost;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(`trait_artisan_name`)}</span> <span class="has-text-success"> ${boost}%</span></p>`,
      );
    }
    ...
    if (global.race["pet"]) {
      ...
      if (global.race["pet"]) {
        ...
        if (global.race.pet.pet > 0) {
          change +=
            global.race.pet.type === "cat"
              ? global.race["catnip"]
                ? traits.catnip.vars()[1]
                : 2
              : global.race["anise"]
                ? traits.anise.vars()[1]
                : 1;
      ...
      if (change !== 0) {
        ...
        obj.popper.append(
          `<p class="modal_bd"><span>${loc(`event_pet_${global.race.pet.type}_owner`)}</span> <span class="has-text-${style}"> ${change}%</span></p>`,
        );
      }
    }
    ...
    if (global.race["wishStats"] && global.race.wishStats.fame !== 0) {
      ...
      if (global.race.wishStats.fame > 0) {
        obj.popper.append(
          `<p class="modal_bd"><span>${loc(`wish_reputable`)}</span> <span class="has-text-success"> ${global.race.wishStats.fame}%</span></p>`,
        );
      } else {
        obj.popper.append(
          `<p class="modal_bd"><span>${loc(`wish_notorious`)}</span> <span class="has-text-danger"> ${global.race.wishStats.fame}%</span></p>`,
        );
      }
    }
    ...
    if (global.civic["homeless"]) {
      ...
      total -= homeless;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(`homeless`)}</span> <span class="has-text-danger"> -${homeless}%</span></p>`,
      );
    }
    ...
    if (global.tech["vax_c"] || global.tech["vax_f"]) {
      ...
      total -= drop;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(global.tech["vax_c"] ? `tech_vax_strat4_bd` : `tech_vax_strat2_bd`)}</span> <span class="has-text-danger"> -${drop}%</span></p>`,
      );
    } else if (global.tech["vax_s"]) {
      let gain = 20;
      total += gain;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(`tech_vax_strat3_bd`)}</span> <span class="has-text-success"> ${gain}%</span></p>`,
      );
    }
    ...
    if (global.city["tormented"]) {
      ...
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(`trait_tormented_name`)}</span> <span class="has-text-danger"> -${global.city.tormented}%</span></p>`,
      );
    }
    ...
    if (
      ...
      global.race.wishStats.bad > 0
    ) {
      let badPress = Math.floor(global.race.wishStats.bad / 75) + 1;
      total -= badPress * 5;
      obj.popper.append(
        `<p class="modal_bd"><span>${loc(`wish_bad`)}</span> <span class="has-text-danger"> -${badPress * 5}%</span></p>`,
      );
    }
    ...
    container.append(
      `<div class="modal_bd sum"><span>${loc("morale_total")}</span> <span class="has-text-warning"> ${+total.toFixed(1)}%</span></div>`,
    );
    container.append(
      `<div class="modal_bd"><span>${loc("morale_max")}</span> <span class="has-text-${total > moraleCap ? "caution" : "warning"}"> ${+moraleCap.toFixed(1)}%</span></div>`,
    );
    container.append(
      `<div class="modal_bd"><span>${loc("morale_current")}</span> <span class="has-text-warning"> ${+global.city.morale.current.toFixed(1)}%</span></div>`,
    );
  ...
  function (obj) {
    ...
    Object.keys(power_generated).forEach(function (k) {
      if (power_generated[k]) {
        ...
        obj.popper.append(
          `<p class="modal_bd"><span>${k}</span> <span class="has-text-success">+${+gen.toFixed(2)}</span></p>`,
        );
      }
    });
    obj.popper.append(
      `<p class="modal_bd"><span>${loc("power_consumed")}</span> <span class="has-text-danger"> -${drain}</span></p>`,
    );
    let avail = +global.city.power.toFixed(2);
    if (global.city.power > 0) {
      obj.popper.append(
        `<p class="modal_bd sum"><span>${loc("power_available")}</span> <span class="has-text-success">${avail}</span></p>`,
      );
    } else {
      obj.popper.append(
        `<p class="modal_bd sum"><span>${loc("power_available")}</span> <span class="has-text-danger">${avail}</span></p>`,
      );
    }
  },
  {
  ...
  function (obj) {
    if (global.race.species === "protoplasm") {
      ...
      if (global.race["truepath"]) {
        challenges =
          challenges + `<div>${loc("evo_challenge_truepath_recap")}</div>`;
      }
      if (global.race["junker"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_junker_desc")} ${loc("evo_challenge_junker_conditions")}</div>`;
      }
      if (global.race["joyless"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_joyless_desc")} ${loc("evo_challenge_joyless_conditions")}</div>`;
      }
      if (global.race["steelen"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_steelen_desc")} ${loc("evo_challenge_steelen_conditions")}</div>`;
      }
      if (global.race["decay"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_decay_desc")} ${loc("evo_challenge_decay_conditions")}</div>`;
      }
      if (global.race["emfield"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_emfield_desc")} ${loc("evo_challenge_emfield_conditions")}</div>`;
      }
      if (global.race["inflation"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_inflation_desc")} ${loc("evo_challenge_inflation_conditions")}</div>`;
      }
      if (global.race["banana"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_banana_desc")} ${loc("wiki_achieve_banana1")}. ${loc("wiki_achieve_banana2")}. ${loc("wiki_achieve_banana3")}. ${loc("wiki_achieve_banana4", [500])}. ${loc("wiki_achieve_banana5", [50])}.</div>`;
      }
      if (global.race["witch_hunter"]) {
        challenges =
          challenges + `<div>${loc("evo_challenge_witch_hunter_desc")}</div>`;
      }
      if (global.race["nonstandard"]) {
        challenges =
          challenges + `<div>${loc("evo_challenge_nonstandard_desc")}</div>`;
      }
      if (global.race["gravity_well"]) {
        challenges =
          challenges + `<div>${loc("evo_challenge_gravity_well_desc")}</div>`;
      }
      if (global.race["warlord"]) {
        challenges =
          challenges + `<div>${loc("evo_challenge_warlord_desc")}</div>`;
      }
      if (global.race["fasting"]) {
        challenges =
          challenges + `<div>${loc("evo_challenge_fasting_desc")}</div>`;
      }
      if (global.race["lone_survivor"]) {
        challenges =
          challenges + `<div>${loc("evo_challenge_lone_survivor_desc")}</div>`;
      }
      if (global.race["sludge"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_sludge_desc")} ${loc("evo_challenge_sludge_conditions")}</div>`;
      }
      if (global.race["ultra_sludge"]) {
        challenges =
          challenges +
          `<div>${loc("evo_challenge_ultra_sludge_desc")} ${loc("evo_challenge_ultra_sludge_conditions")}</div>`;
      }
      if (global.race["orbit_decay"]) {
        let impact = global.race["orbit_decayed"]
          ? ""
          : loc("evo_challenge_orbit_decay_impact", [
              global.race["orbit_decay"] - global.stats.days,
            ]);
        let state = global.race["orbit_decayed"]
          ? global.race["tidal_decay"]
            ? loc(`planet_kamikaze_msg`)
            : loc("evo_challenge_orbit_decay_impacted", [
                races[global.race.species].home,
              ])
          : loc("evo_challenge_orbit_decay_desc");
        challenges =
          challenges +
          `<div>${state} ${loc("evo_challenge_orbit_decay_conditions")} ${impact}</div>`;
        if (calc_mastery() >= 100 && global.race.universe !== "antimatter") {
          challenges =
            challenges +
            `<div class="has-text-caution">${loc("evo_challenge_cataclysm_warn")}</div>`;
        } else {
          challenges =
            challenges +
            `<div class="has-text-danger">${loc("evo_challenge_scenario_warn")}</div>`;
        }
      }
      ...
      if (global.race["cataclysm"]) {
        if (calc_mastery() >= 50 && global.race.universe !== "antimatter") {
          challenges =
            challenges +
            `<div>${loc("evo_challenge_cataclysm_desc")}</div><div class="has-text-caution">${loc("evo_challenge_cataclysm_warn")}</div>`;
        } else {
          challenges =
            challenges +
            `<div>${loc("evo_challenge_cataclysm_desc")}</div><div class="has-text-danger">${loc("evo_challenge_scenario_warn")}</div>`;
        }
      }
      obj.popper.append(
        $(
          `<div>${loc(global.race["cataclysm"] ? "no_home" : "home", [planet, race, planet_label, orbit])}</div>${geo_traits}${challenges}`,
        ),
      );
    }
    return undefined;
...
popover(
  ...
  function (obj) {
    obj.popper.append(
      $(`<div>${universe_types[global.race.universe].desc}</div>`),
    );
    obj.popper.append(
      $(`<div>${universe_types[global.race.universe].effect}</div>`),
    );
    return undefined;
  },
  {
...
popover(
  ...
  function (obj) {
    obj.popper.append(
      $(`<div>${loc(`evo_challenge_simulation_topbar`)}</div>`),
    );
    return undefined;
  },
  {
...
if (global.race["orbit_decay"] && !global.race["orbit_decayed"]) {
  popover(
    `infoTimer`,
    function () {
      return global.race["orbit_decayed"]
        ? ""
        : loc("evo_challenge_orbit_decay_impact", [
            global.race["orbit_decay"] - global.stats.days,
          ]);
...
if (global.race.species === "protoplasm") {
  ...
  let perk_rank =
    global.stats.feat["master"] &&
    global.stats.achieve["ascended"] &&
    global.stats.achieve.ascended.l > 0
      ? Math.min(global.stats.achieve.ascended.l, global.stats.feat["master"])
      : 0;
  if (global["sim"]) {
    perk_rank = 5;
  }
  ...
  let grand_rank =
    global.stats.feat["grandmaster"] &&
    global.stats.achieve["corrupted"] &&
    global.stats.achieve.corrupted.l > 0
      ? Math.min(
          global.stats.achieve.corrupted.l,
          global.stats.feat["grandmaster"],
        )
      : 0;
  if (global["sim"]) {
  ...
  if (
    global.portal.hasOwnProperty("soul_forge") &&
...
var firstRun = true;
var gene_sequence =
  global.arpa["sequence"] && global.arpa["sequence"]["on"]
    ? global.arpa.sequence.on
    : 0;
function fastLoop() {
  if (!global.race["no_craft"]) {
    $(".craft").each(function (e) {
      if (typeof $(this).data("val") === "number") {
        $(this).html(
          sizeApproximation($(this).data("val") * keyMultiplier(), 1),
        );
      }
    });
  }
  ...
  if (
    global.prestige.Plasmid.count > 0 &&
    (global.race.universe !== "antimatter" ||
      (global.genes["bleed"] && global.race.universe === "antimatter"))
  ) {
    breakdown.p["Global"][loc("resource_Plasmid_name")] = pBonus[1] * 100 + "%";
    applyPlasmid = true;
  }
  if (
    global.prestige.AntiPlasmid.count > 0 &&
    (global.race.universe === "antimatter" ||
      (global.genes["bleed"] &&
        global.genes["bleed"] >= 2 &&
        global.race.universe !== "antimatter"))
  ) {
    breakdown.p["Global"][loc("resource_AntiPlasmid_name")] =
      pBonus[2] * 100 + "%";
    applyPlasmid = true;
  }
  if (applyPlasmid) {
  ...
  if (global.prestige.Supercoiled.count > 0) {
    let bonus =
      global.prestige.Supercoiled.count /
      (global.prestige.Supercoiled.count + 5000);
    breakdown.p["Global"][loc("resource_Supercoiled_short")] =
      +(bonus * 100).toFixed(2) + "%";
  ...
  if (global.race.universe === "evil" && global.resource.Authority.display) {
    if (global.resource.Authority.amount < 100) {
      ...
      breakdown.p["Global"][global.resource.Authority.name] =
        -(malus * 100).toFixed(2) + "%";
  ...
  if (global.race["untapped"]) {
    if (global.race["untapped"] > 0) {
      let untapped = +(
        global.race.untapped / (global.race.untapped + 20) / 10 +
        0.00024
      ).toFixed(4);
  ...
  if (global.tech["world_control"]) {
    ...
    for (let i = 0; i < 3; i++) {
      if (
        global.civic.foreign[`gov${i}`].occ ||
        global.civic.foreign[`gov${i}`].anx ||
        global.civic.foreign[`gov${i}`].buy
      ) {
        occupy +=
          global.civic.govern.type === "federation"
            ? 5 + govEffect.federation()[0]
            : 5;
      }
    }
    if (occupy > 0) {
  ...
  if (global.race["rejuvenated"]) {
    let decay =
      global.stats.days < 996 ? (1000 - global.stats.days) / 2000 : 0.02;
  ...
  if (global.race["intelligent"] || cyclopsFathom > 0) {
    ...
    if (global.race["intelligent"]) {
      bonus +=
        workerScale(global.civic.scientist.workers, "scientist") *
          traits.intelligent.vars()[1] +
        workerScale(global.civic.professor.workers, "professor") *
          traits.intelligent.vars()[0];
    }
    if (cyclopsFathom > 0) {
      bonus +=
        workerScale(global.civic.scientist.workers, "scientist") *
          traits.intelligent.vars(1)[1] *
          cyclopsFathom +
        workerScale(global.civic.professor.workers, "professor") *
          traits.intelligent.vars(1)[0] *
          cyclopsFathom;
    }
  ...
  if (
    (global.city.ptrait.includes("trashed") ||
      global.race["scavenger"] ||
      (global.race["servants"] && global.race.servants["force_scavenger"])) &&
    global.civic["scavenger"]
  ) {
    let scavenger =
      global.city.ptrait.includes("trashed") || global.race["scavenger"]
        ? workerScale(global.civic.scavenger.workers, "scavenger")
        : 0;
  ...
  if (
    global.race["unfathomable"] &&
    global.city["surfaceDwellers"] &&
    global.city["captive_housing"]
  ) {
    let thralls = 0;
    let rank =
      global.stats.achieve["nightmare"] && global.stats.achieve.nightmare["mg"]
        ? global.stats.achieve.nightmare.mg
        : 0;
  ...
  if (global.city.ptrait.includes("mellow")) {
    breakdown.p["Global"][loc("planet_mellow_bd")] =
      "-" + (100 - planetTraits.mellow.vars()[2] * 100) + "%";
  ...
  if ((global.race["smoldering"] || phoenixFathom > 0) && global.city["hot"]) {
    ...
    if (phoenixFathom > 0) {
      if (global.city["hot"] > 100) {
        ...
        heat +=
          (global.city["hot"] - 100) *
          traits.smoldering.vars(0.25)[2] *
          phoenixFathom;
      } else {
        heat +=
          global.city["hot"] * traits.smoldering.vars(0.25)[1] * phoenixFathom;
      }
    }
  ...
  if (global.race["heat_intolerance"] && global.city["hot"]) {
    let heat = Math.min(
      100,
      global.city["hot"] * traits.heat_intolerance.vars()[0],
    );
  ...
  if (global.race["cold_intolerance"] && global.city["cold"]) {
    let cold = Math.min(
      100,
      global.city["cold"] * traits.cold_intolerance.vars()[0],
    );
  ...
  if (
    ...
    global.resource[global.race.species].amount > jobScale(10)
  ) {
    let chaos =
      (global.resource[global.race.species].amount - jobScale(10)) *
      (global.race["high_pop"] ? 0.25 / traits.high_pop.vars()[0] : 0.25);
  ...
  if (
    capyFathom > 0 ||
    (global.race["calm"] &&
      global.city["meditation"] &&
      global.resource.Zen.display)
  ) {
  ...
  if (
    (races[global.race.species].type === "aquatic" &&
      !["swamp", "oceanic"].includes(global.city.biome)) ||
    (races[global.race.species].type === "fey" &&
      !["forest", "swamp", "taiga"].includes(global.city.biome)) ||
    (races[global.race.species].type === "heat" &&
      !["ashland", "volcanic"].includes(global.city.biome)) ||
    (races[global.race.species].type === "polar" &&
      !["tundra", "taiga"].includes(global.city.biome)) ||
    (races[global.race.species].type === "sand" &&
      !["ashland", "desert"].includes(global.city.biome)) ||
    (races[global.race.species].type === "demonic" &&
      global.city.biome !== "hellscape") ||
    (races[global.race.species].type === "angelic" &&
      global.city.biome !== "eden")
  ) {
    if (!global.race["warlord"]) {
      let unsuited = 1;
      ...
      breakdown.p["Global"][loc("unsuited")] =
        `-${Math.round((1 - unsuited) * 100)}%`;
  ...
  if (global.race["hibernator"] && global.city.calendar.season === 3) {
    ...
    breakdown.p["Global"][loc("morale_winter")] =
      `-${traits.hibernator.vars()[1]}%`;
  }
  ...
  if (global.race.species === "protoplasm") {
    ...
    // Gain RNA & DNA
    if (
      global.evolution["nucleus"] &&
      global["resource"]["DNA"].amount < global["resource"]["DNA"].max
    ) {
    ...
    if (
      ((global.stats.feat["novice"] &&
        global.stats.achieve["apocalypse"] &&
        global.stats.achieve.apocalypse.l > 0) ||
        global["sim"]) &&
      global.race.universe !== "bigbang" &&
      (!global.race.seeded || (global.race.seeded && global.race["chose"]))
    ) {
      let rank = global["sim"]
        ? 5
        : Math.min(
            global.stats.achieve.apocalypse.l,
            global.stats.feat["novice"],
          );
    ...
    if (global["resource"]["RNA"].amount >= 2 && !global.evolution["dna"]) {
      ...
      global.resource.DNA.display = true;
      if (
        global.stats.achieve["mass_extinction"] &&
        global.stats.achieve["mass_extinction"].l > 1
      ) {
      ...
      global["resource"]["RNA"].amount >= 10 &&
      !global.evolution["membrane"]
    ) {
      global.evolution["membrane"] = { count: 0 };
      drawEvolution();
    } else if (
      global["resource"]["DNA"].amount >= 4 &&
      !global.evolution["organelles"]
    ) {
      global.evolution["organelles"] = { count: 0 };
      drawEvolution();
    } else if (
      global.evolution["organelles"] &&
      global.evolution.organelles.count >= 2 &&
      !global.evolution["nucleus"]
    ) {
      ...
      global.evolution["nucleus"] &&
      global.evolution.nucleus.count >= 1 &&
      !global.evolution["eukaryotic_cell"]
    ) {
      global.evolution["eukaryotic_cell"] = { count: 0 };
      drawEvolution();
    } else if (
      global.evolution["eukaryotic_cell"] &&
      global.evolution.eukaryotic_cell.count >= 1 &&
      !global.evolution["mitochondria"]
    ) {
    ...
    if (global.race["quarantine"] && global.race["qDays"]) {
      let qDays =
        1 - (global.race.qDays <= 1000 ? global.race.qDays : 1000) / 1000;
      ...
      if (
        global.race["vax"] &&
        global.tech["focus_cure"] &&
        global.tech.focus_cure >= 4
      ) {
    ...
    if (global.city.calendar.season === 0 && global.city.calendar.year > 0) {
      ...
      if (global.race["chilled"]) {
        ...
        global.city.morale.season = global.race["leathery"]
          ? -traits.leathery.vars()[0]
          : -5;
      }
    } else {
    ...
    if (global.city.calendar.weather === 0) {
      if (global.city.calendar.temp > 0) {
        if (global.city.calendar.wind === 1) {
          ...
          if (global.race["skittish"]) {
            ...
            weather_morale = global.race["leathery"]
              ? -traits.leathery.vars()[0]
              : -5;
          }
        } else {
      ...
      if (global.race["nyctophilia"]) {
        ...
        (global.city.calendar.wind === 0 && global.city.calendar.temp < 2) ||
        (global.city.calendar.wind === 1 && global.city.calendar.temp === 2)
      ) {
        //Still and Not Hot
        // -or-
        //Windy and Hot
    ...
    if (
      global.race["snowy"] &&
      (global.city.calendar.temp !== 0 || global.city.calendar.weather !== 0)
    ) {
      weather_morale -=
    ...
    if (!global.city.ptrait.includes("mellow")) {
      let unemployed =
        global.civic.unemployed.workers /
        (global.race["high_pop"] ? traits.high_pop.vars()[0] : 1);
      morale -= unemployed;
    ...
    if (
      global.civic.hunter.display &&
      (global.race["playful"] || vulFathom > 0)
    ) {
      let val = 0;
      if (vulFathom > 0) {
    ...
    if (
      ...
      if (dealVal) {
        ...
        Object.keys(global.resource).forEach(function (res) {
          if (
            global.resource[res].hasOwnProperty("trade") &&
      ...
      Object.keys(global.resource).forEach(function (res) {
        ...
        if (routes > 0) {
          ...
          const affordable_routes = Math.floor(
            global.resource.Money.amount / (price * time_multiplier),
          );
          ...
          if (routes > 0) {
            ...
            if (global.race["persuasive"]) {
              rate *=
                1 +
                (traits.persuasive.vars()[0] * global.race["persuasive"]) / 100;
            }
            if (global.race["merchant"]) {
              rate *= 1 + traits.merchant.vars()[1] / 100;
            }
            if (
              global.race["ocular_power"] &&
              global.race["ocularPowerConfig"] &&
              global.race.ocularPowerConfig.c
            ) {
          ...
          const affordable_routes = Math.floor(
            global.resource[res].amount / (rate * time_multiplier),
          );
      ...
      Object.keys(global.race.alchemy).forEach(function (res) {
        if (global.race.alchemy[res] > 0) {
          ...
          if (trasmute >= time_multiplier) {
            let rate =
              global.resource[res].basic && global.tech.alchemy >= 2
                ? tradeRatio[res] * 8
                : tradeRatio[res] * 2;
            if (global.race["witch_hunter"]) {
              rate *= 3;
            ...
            if (
              global.stats.achieve["soul_sponge"] &&
              global.stats.achieve.soul_sponge["mg"]
            ) {
            ...
            if (
              ...
              !global.resource[res].basic &&
              global.tech.alchemy >= 2
            ) {
    ...
    if (
      global.galaxy["trade"] &&
      (gal_on.hasOwnProperty("freighter") ||
        gal_on.hasOwnProperty("super_freighter"))
    ) {
      let cap = 0;
      ...
      for (let i = 0; i < offers.length; i++) {
        ...
        if (pirate < 1) {
          if (breakdown.p.consume[imprt_res][loc("galaxy_piracy")]) {
            breakdown.p.consume[imprt_res][loc("galaxy_piracy")] += -(
              (1 - pirate) *
              imp_total
            );
          } else {
      ...
      nf_resources.forEach(function (r) {
        if (global.resource[r].display) {
          ...
          if (vol > 0) {
            ...
            if (modRes(r, -vol)) {
              breakdown.p.consume[r][loc("city_nanite_factory")] = -(
                vol / time_multiplier
              );
              let trait = traits.deconstructor.vars()[0] / 100;
              let nanite_vol = ((vol * atomic_mass[r]) / 100) * trait;
              breakdown.p.consume["Nanite"][global.resource[r].name] =
                nanite_vol / time_multiplier;
    ...
    if (
      global.interstellar["elysanite_sphere"] &&
      ...
      if (global.interstellar.elysanite_sphere.count >= 1000) {
        ...
        output = powerModifier(
          1750 + global.interstellar.elysanite_sphere.count * 18,
        );
      }
      max_power -= output;
      power_grid += output;
      ...
      global.interstellar["orichalcum_sphere"] &&
      ...
      if (global.interstellar.orichalcum_sphere.count >= 100) {
        ...
        output = powerModifier(
          750 + global.interstellar.orichalcum_sphere.count * 8,
        );
      }
      max_power -= output;
      power_grid += output;
      ...
      global.interstellar["dyson_sphere"] &&
      ...
      if (global.interstellar.dyson_sphere.count >= 100) {
        ...
        output = powerModifier(
          175 + global.interstellar.dyson_sphere.count * 5,
        );
      }
      max_power -= output;
      power_grid += output;
      ...
      global.interstellar["dyson"] &&
      global.interstellar.dyson.count >= 1
    ) {
      let output = 0;
    ...
    if (
      ...
      global.interstellar.stellar_engine.count >= 100
    ) {
    ...
    [
      ...
      let c_action =
        generator.r === "city" ? actions.city : actions[space][generator.r];
      let title =
        typeof c_action[generator.s].title === "string"
          ? c_action[generator.s].title
          : c_action[generator.s].title();
      ...
      if (global[region][generator.s] && global[region][generator.s]["on"]) {
        ...
        if (c_action[generator.s].hasOwnProperty("p_fuel")) {
          ...
          for (let j = 0; j < s_fuels.length; j++) {
            ...
            if (["Oil", "Helium_3"].includes(fuel.r) && region !== "city") {
              fuel_cost =
                region === "space"
                  ? +fuel_adjust(fuel_cost, true)
                  : +int_fuel_adjust(fuel_cost);
            }
        ...
        if (p_on[generator.s] !== global[region][generator.s].on) {
          ...
          $(`#${region}-${generator.s} .on`).prop(
            "title",
            `ON ${p_on[generator.s]}/${global[region][generator.s].on}`,
          );
        } else {
      ...
      Object.keys(fuels).forEach(function (fuel) {
        ...
        while (
          consume * time_multiplier >
            global.resource[fuel].amount +
              (global.resource[fuel].diff > 0
    ...
    if (
      global.eden["soul_engine"] &&
      global.tech["asphodel"] &&
      global.tech.asphodel >= 4
    ) {
      let power =
        (support_on["soul_engine"] || 0) *
        actions.eden.eden_asphodel.soul_engine.powered();
      max_power += power;
      power_grid -= power;
    ...
    if (global.space["swarm_satellite"] && global.space["swarm_control"]) {
      ...
      if (
        global.stats.achieve["iron_will"] &&
        global.stats.achieve.iron_will.l >= 1
      ) {
        solar += 0.15;
      }
    ...
    if (
      global.city["mill"] &&
      global.tech["agriculture"] &&
      global.tech["agriculture"] >= 6
    ) {
      let power = powerModifier(
        global.city.mill.on * actions.city.mill.powered(),
      );
      max_power += power;
      power_grid -= power;
    ...
    if (global.city["windmill"] && global.tech["wind_plant"]) {
      let power = powerModifier(
        global.city.windmill.count * actions.city.windmill.powered(),
      );
      max_power += power;
      power_grid -= power;
      power_generated[loc("city_mill_title2")] = -power;
    ...
    if (global.race["powered"]) {
      let citizens =
        traits.powered.vars()[0] * global.resource[global.race.species].amount;
    ...
    if (global.race["replicator"]) {
      global.city["replicator"] = {
        count: global.race.replicator.pow,
        on: global.race.replicator.pow,
      };
    }
    ...
    for (let i = 0; i < p_structs.length; i++) {
      ...
      const region =
        parts[0] === "city" ? parts[0] : convertSpaceSector(parts[0]);
      const c_action =
        parts[0] === "city"
          ? actions.city[struct]
          : actions[region][parts[0]][struct];
      if (global[region][struct]?.on) {
        if (
          checkPowerRequirements(c_action) &&
          (region !== "galaxy" || p_on["s_gate"])
        ) {
      ...
      if (
        global.settings.lowPowerBalance &&
        c_action.hasOwnProperty("powerBalancer")
      ) {
          ...
          if (balValues) {
            balValues.forEach(function (v) {
              ...
              if (v.hasOwnProperty("r") && v.hasOwnProperty("k")) {
                ...
                if (
                  global.resource[v.r]["odif"] &&
                  global.resource[v.r]["odif"] < 0
                ) {
                  global.resource[v.r]["odif"] = 0;
                }
                let diff =
                  global.resource[v.r].diff +
                  (global.resource[v.r]["odif"]
                    ? global.resource[v.r]["odif"]
                    : 0);
                while (
    ...
    for (let i = 0; i < p_structs.length; i++) {
      ...
      if (global[region][struct]?.on) {
        ...
        if (c_action.hasOwnProperty("p_fuel")) {
          ...
          for (let j = 0; j < s_fuels.length; j++) {
            const title =
              typeof c_action.title === "string"
                ? c_action.title
                : c_action.title();
            const fuel = s_fuels[j];
            const fuel_cost =
              ["Oil", "Helium_3"].includes(fuel.r) && region === "space"
                ? fuel_adjust(fuel.a, true)
                : fuel.a;
        ...
        if (p_on[struct] !== global[region][struct].on) {
          ...
          $(`#${region}-${struct} .on`).prop(
            "title",
            `ON ${p_on[struct]}/${global[region][struct].on}`,
          );
          // Remove the reset actions for reset structures that lose power
          if (
            ["matrix", "atmo_terraformer", "ascension_trigger"].includes(struct)
          ) {
    ...
    if (global.space["fob"] && global.space["lander"]) {
      if (p_on["fob"]) {
        ...
        if (support_on["lander"] !== global.space.lander.on) {
          ...
          $(`#space-lander .on`).prop(
            "title",
            `ON ${support_on["lander"]}/${global.space.lander.on}`,
          );
        } else {
    ...
    if (p_on["s_gate"] && p_on["foothold"]) {
      ...
      while (
        consume * time_multiplier > global.resource["Elerium"].amount &&
        consume > 0
      ) {
        consume -= increment;
        p_on["foothold"]--;
    ...
    if (global.race["fasting"]) {
      const foodBuildings = [
        ...
        "space:spaceport",
        "int_:starport",
        "gxy_:starbase" /*, "space:space_station", "space:embassy"*/,
    ...
    [
      ...
      if (global[sup.a][sup.s] && global[sup.a][sup.s].count > 0) {
        ...
        if (actions[sup.a][sup.r][sup.s].hasOwnProperty("support_fuel")) {
          ...
          for (let j = 0; j < s_fuels.length; j++) {
            let fuel = s_fuels[j];
            let fuel_cost = ["Oil", "Helium_3"].includes(fuel.r)
              ? sup.a === "space"
                ? +fuel_adjust(fuel.a, true)
                : +int_fuel_adjust(fuel.a)
              : fuel.a;
            let mb_consume = p_on[sup.s] * fuel_cost;
            breakdown.p.consume[fuel.r][actions[sup.a][sup.r][sup.s].title] =
              -mb_consume;
            ...
            if (p_on[sup.s] < global[sup.a][sup.s].on) {
              ...
              $(`#space-${sup.s} .on`).prop(
                "title",
                `ON ${p_on[sup.s]}/${global[sup.a][sup.s].on}`,
              );
            } else {
        ...
        global[sup.a][sup.s].s_max =
          p_on[sup.s] * actions[sup.a][sup.r][sup.s].support();
        switch (sup.g) {
          case "moon":
            {
              global[sup.a][sup.s].s_max +=
                global.tech["luna"] && global.tech["luna"] >= 2
                  ? p_on["nav_beacon"] *
                    actions.space.spc_home.nav_beacon.support()
                  : 0;
            }
            break;
          case "red":
            {
              global[sup.a][sup.s].s_max +=
                global.tech["mars"] && global.tech["mars"] >= 3
                  ? p_on["red_tower"] *
                    actions.space.spc_red.red_tower.support()
                  : 0;
              global[sup.a][sup.s].s_max +=
                global.tech["luna"] && global.tech["luna"] >= 3
                  ? p_on["nav_beacon"] *
                    actions.space.spc_home.nav_beacon.support()
                  : 0;
            }
          ...
          case "tau_home":
            {
              global[sup.a][sup.s].s_max += p_on["tau_farm"]
                ? p_on["tau_farm"]
                : 0;
            }
            break;
          case "asphodel":
            {
              global[sup.a][sup.s].s_max +=
                (p_on["rectory"] ? p_on["rectory"] : 0) *
                actions.eden.eden_asphodel.rectory.support();
              global[sup.a][sup.s].s_max +=
                (p_on["corruptor"] ? p_on["corruptor"] : 0) *
                actions.eden.eden_asphodel.corruptor.support();
            }
            break;
        }
      }
      ...
      if (global[sup.a][sup.s]) {
        ...
        for (var i = 0; i < area_structs.length; i++) {
          if (global[sup.a][area_structs[i]]) {
            ...
            let supportSize = actions[sup.a][sup.r2][
              area_structs[i]
            ].hasOwnProperty("support")
              ? actions[sup.a][sup.r2][area_structs[i]].support() * -1
              : 1;
            ...
            if (operating * supportSize > remaining_support && !sup.oc) {
              ...
              $(`#${id} .on`).prop(
                "title",
                `ON ${operating}/${global[sup.a][area_structs[i]].on}`,
              );
            } else {
            ...
            if (
              actions[sup.a][sup.r2][area_structs[i]].hasOwnProperty(
                "support_fuel",
              )
            ) {
              let s_fuels =
                actions[sup.a][sup.r2][area_structs[i]].support_fuel();
              if (!Array.isArray(s_fuels)) {
                s_fuels = [s_fuels];
              }
              for (let j = 0; j < s_fuels.length; j++) {
                ...
                breakdown.p.consume[fuel.r][
                  actions[sup.a][sup.r2][area_structs[i]].title
                ] = -mb_consume;
    ...
    if (global.tech["womling_technicians"]) {
      womling_technician =
        1 + p_on["womling_station"] * (global.tech["isolation"] ? 0.3 : 0.08);
    ...
    if (global.space["space_barracks"] && !global.race["fasting"]) {
      ...
      for (let i = 0; i < global.space.space_barracks.on; i++) {
        if (!modRes("Oil", -(time_multiplier * oil_cost))) {
          sm_consume -=
            global.space.space_barracks.on * oil_cost - i * oil_cost;
    ...
    if (p_on["int_factory"] && p_on["int_factory"] > 0) {
      ...
      breakdown.p.consume.Deuterium[loc("interstellar_int_factory_title")] =
        -d_consume;
    }
    ...
    if (support_on["water_freighter"] && support_on["water_freighter"] > 0) {
      ...
      breakdown.p.consume.Helium_3[loc("space_water_freighter_title")] =
        -h_consume;
    }
    ...
    if (
      ...
      global.interstellar["starport"].count > 0
    ) {
      let fuel_cost = +int_fuel_adjust(5);
      let mb_consume = p_on["starport"] * fuel_cost;
      breakdown.p.consume.Helium_3[loc("interstellar_alpha_starport_title")] =
        -mb_consume;
      ...
      global.interstellar.starport.s_max =
        p_on["starport"] * actions.interstellar.int_alpha.starport.support();
      global.interstellar.starport.s_max +=
        p_on["habitat"] * actions.interstellar.int_alpha.habitat.support();
      global.interstellar.starport.s_max +=
        p_on["xfer_station"] *
        actions.interstellar.int_proxima.xfer_station.support();
    }
    ...
    if (global.interstellar["starport"]) {
      ...
      for (var i = 0; i < structs.length; i++) {
        if (global.interstellar[structs[i]]) {
          ...
          if (used_support + operating > global.interstellar.starport.s_max) {
            operating -=
              used_support + operating - global.interstellar.starport.s_max;
            $(`#${id} .on`).addClass("warn");
            $(`#${id} .on`).prop(
              "title",
              `ON ${operating}/${global.interstellar[structs[i]].on}`,
            );
          } else {
            $(`#${id} .on`).removeClass("warn");
            $(`#${id} .on`).prop("title", `ON`);
      ...
      if (
        global.interstellar.hasOwnProperty("mining_droid") &&
        global.interstellar.mining_droid.count > 0
      ) {
    ...
    if (global.galaxy["starbase"] && global.galaxy["starbase"].count > 0) {
      ...
      if (p_on["s_gate"]) {
        global.galaxy.starbase.s_max =
          p_on["starbase"] * actions.galaxy.gxy_gateway.starbase.support();
        if (p_on["gateway_station"]) {
          global.galaxy.starbase.s_max +=
            p_on["gateway_station"] *
            actions.galaxy.gxy_stargate.gateway_station.support();
        }
        if (p_on["telemetry_beacon"]) {
          global.galaxy.starbase.s_max +=
            p_on["telemetry_beacon"] *
            actions.galaxy.gxy_stargate.telemetry_beacon.support();
        }
        if (p_on["ship_dock"]) {
          global.galaxy.starbase.s_max +=
            p_on["ship_dock"] * actions.galaxy.gxy_gateway.ship_dock.support();
        }
      } else {
    ...
    if (global.galaxy["starbase"]) {
      ...
      for (var i = 0; i < gateway_structs.length; i++) {
        if (global.galaxy[gateway_structs[i]]) {
          ...
          let operating_cost =
            -actions.galaxy.gxy_gateway[gateway_structs[i]].support();
          let max_operating = Math.floor(
            (global.galaxy.starbase.s_max - used_support) / operating_cost,
          );
          if (operating > max_operating) {
            ...
            $(`#${id} .on`).prop(
              "title",
              `ON ${operating}/${global.galaxy[gateway_structs[i]].on}`,
            );
          } else {
            $(`#${id} .on`).removeClass("warn");
            $(`#${id} .on`).prop("title", `ON`);
    ...
    if (global.galaxy["foothold"] && global.galaxy.foothold.count > 0) {
      global.galaxy.foothold.s_max =
        p_on["s_gate"] *
        p_on["foothold"] *
        actions.galaxy.gxy_alien2.foothold.support();
    }

    // Guard Post
    if (global.portal["guard_post"]) {
      global.portal.guard_post.s_max =
        global.portal.guard_post.count *
        actions.portal.prtl_ruins.guard_post.support();

      if (global.portal.guard_post.on > 0) {
        let army =
          global.portal.fortress.garrison -
          global.portal.fortress.patrols * global.portal.fortress.patrol_size;
        if (p_on["soul_forge"]) {
    ...
    if (global.portal["harbor"]) {
      global.portal.harbor.s_max =
        p_on["harbor"] * actions.portal.prtl_lake.harbor.support();
    }

    // Purifier
    if (global.portal["purifier"]) {
      global.portal.purifier.s_max = +(
        p_on["purifier"] * actions.portal.prtl_spire.purifier.support()
      ).toFixed(2);
      ...
      for (var i = 0; i < purifier_structs.length; i++) {
        if (global.portal[purifier_structs[i]]) {
          ...
          if (used_support + operating > global.portal.purifier.s_max) {
            operating -=
              used_support + operating - global.portal.purifier.s_max;
            $(`#${id} .on`).addClass("warn");
            $(`#${id} .on`).prop(
              "title",
              `ON ${operating}/${global.portal[purifier_structs[i]].on}`,
            );
          } else {
            $(`#${id} .on`).removeClass("warn");
            $(`#${id} .on`).prop("title", `ON`);
          ...
          used_support +=
            operating *
            -actions.portal.prtl_spire[purifier_structs[i]].support();
    ...
    // Space Station
    if (
      global.space["space_station"] &&
      global.space["space_station"].count > 0
    ) {
      let fuel_cost = +fuel_adjust(2.5, true);
      let ss_consume = p_on["space_station"] * fuel_cost;
      breakdown.p.consume.Helium_3[loc("space_belt_station_title")] =
        -ss_consume;
    ...
    if (global.space["space_station"]) {
      ...
      for (var i = 0; i < belt_structs.length; i++) {
        if (global.space[belt_structs[i]]) {
          ...
          if (
            used_support +
              operating * -actions.space.spc_belt[belt_structs[i]].support() >
            global.space.space_station.s_max
          ) {
            let excess =
              used_support +
              operating * -actions.space.spc_belt[belt_structs[i]].support() -
              global.space.space_station.s_max;
            operating -= Math.ceil(
              excess / -actions.space.spc_belt[belt_structs[i]].support(),
            );
            $(`#${id} .on`).addClass("warn");
            $(`#${id} .on`).prop(
              "title",
              `ON ${operating}/${global.space[belt_structs[i]].on}`,
            );
          } else {
            $(`#${id} .on`).removeClass("warn");
            $(`#${id} .on`).prop("title", `ON`);
    ...
    if (
      global.interstellar["nexus"] &&
      global.interstellar["nexus"].count > 0
    ) {
      let cash_cost = 350;
      ...
      global.interstellar.nexus.s_max =
        p_on["nexus"] * actions.interstellar.int_nebula.nexus.support();
    }
      ...
      for (var i = 0; i < structs.length; i++) {
        if (global.interstellar[structs[i]]) {
          ...
          if (used_support + operating > global.interstellar.nexus.s_max) {
            operating -=
              used_support + operating - global.interstellar.nexus.s_max;
            $(`#${id} .on`).addClass("warn");
            $(`#${id} .on`).prop(
              "title",
              `ON ${operating}/${global.interstellar[structs[i]].on}`,
    ...
    if (global.interstellar["xfer_station"] && p_on["xfer_station"]) {
      ...
      breakdown.p.consume.Uranium[loc("interstellar_xfer_station_title")] =
        -xfer_consume;
    ...
    if (p_on["neutron_miner"] && p_on["neutron_miner"] > 0) {
      ...
      breakdown.p.consume.Helium_3[loc("interstellar_neutron_miner_title")] =
        -out_consume;
    ...
    if (global.interstellar["cruiser"]) {
      ...
      breakdown.p.consume.Helium_3[loc("interstellar_cruiser_title")] =
        -out_consume;
      for (let i = 0; i < global.interstellar["cruiser"].on; i++) {
        if (!modRes("Helium_3", -(time_multiplier * fuel_cost))) {
          out_consume -=
            global.interstellar["cruiser"].on * fuel_cost - i * fuel_cost;
          active -= i;
          break;
        }
      }
    ...
    for (let j = 0; j < galaxy_ship_types.length; j++) {
      ...
      const req = galaxy_ship_types[j].hasOwnProperty("req")
        ? p_on[galaxy_ship_types[j].req] > 0
        : true;
      ...
      for (let i = 0; i < galaxy_ship_types[j].ships.length; i++) {
        ...
        if (global[area][ship]) {
          let operating = 0;
          if (
            global[area][ship].hasOwnProperty("on") &&
            req &&
            (p_on["s_gate"] || area !== "galaxy")
          ) {
            ...
            // Support cost
            const operating_cost = actions[area][region][ship].hasOwnProperty(
              "support",
            )
              ? -actions[area][region][ship].support()
              : 0;
            if (operating_cost > 0) {
              const max_operating = Math.floor(
                (global[area][support_home].s_max - used_support) /
                  operating_cost,
              );
            ...
            if (actions[area][region][ship].hasOwnProperty("ship")) {
              if (
                actions[area][region][ship].ship.civ &&
                global[area][ship].hasOwnProperty("crew")
              ) {
                ...
                if (civPerShip > 0) {
                  ...
                  operating = Math.min(
                    operating,
                    Math.floor(global[area][ship].crew / civPerShip),
                  );
                }
              }

              if (
                actions[area][region][ship].ship.mil &&
                global[area][ship].hasOwnProperty("mil")
              ) {
                ...
                if (milPerShip > 0) {
                  ...
                  operating = Math.min(
                    operating,
                    Math.floor(global[area][ship].mil / milPerShip),
                  );
                }
              }

              if (actions[area][region][ship].ship.hasOwnProperty("helium")) {
                let increment = +int_fuel_adjust(
                  actions[area][region][ship].ship.helium,
                ).toFixed(2);
                let consume = operating * increment;
                while (
                  consume * time_multiplier >
                    global.resource.Helium_3.amount +
                      (global.resource.Helium_3.diff > 0
              ...
              if (
                actions[area][region][ship].ship.hasOwnProperty("deuterium")
              ) {
                let increment = +int_fuel_adjust(
                  actions[area][region][ship].ship.deuterium,
                ).toFixed(2);
                let consume = operating * increment;
                while (
                  consume * time_multiplier >
                    global.resource.Deuterium.amount +
                      (global.resource.Deuterium.diff > 0
      ...
      if (
        support_home &&
        global?.[area]?.[support_home]?.hasOwnProperty("support")
      ) {
        global[area][support_home].support = used_support;
      }
    }

    breakdown.p.consume.Helium_3[loc("galaxy_fuel_consume")] =
      -andromeda_helium;
    breakdown.p.consume.Deuterium[loc("galaxy_fuel_consume")] =
      -andromeda_deuterium;

    global.civic.crew.workers = crew_civ;
    if (global.civic.garrison.hasOwnProperty("crew")) {
      if (
        global.space.hasOwnProperty("shipyard") &&
        global.space.shipyard.hasOwnProperty("ships")
      ) {
        global.space.shipyard.ships.forEach(function (ship) {
          if (
            ship.location !== "spc_dwarf" ||
            (ship.location === "spc_dwarf" && ship.transit > 0)
          ) {
    ...
    Object.keys(job_desc).forEach(function (job) {
      if (global.civic[job]) {
        if (job !== "crew") {
          ...
          if (total > global.resource[global.race.species].amount) {
            global.civic[job].workers -=
              total - global.resource[global.race.species].amount;
          }
        ...
        if (job !== "unemployed" && job !== "hunter" && job !== "forager") {
          ...
          if (
            global.race["sky_lover"] &&
            ["miner", "coal_miner", "crystal_miner", "pit_miner"].includes(job)
          ) {
    ...
    global.civic[global.civic.d_job].workers +=
      global.resource[global.race.species].amount - total;
    ...
    Object.keys(job_desc).forEach(function (job) {
      if (
        job !== "craftsman" &&
        global.civic[job] &&
        global.civic[job].display &&
        global.civic[job].workers < global.civic[job].assigned &&
        global.civic[global.civic.d_job].workers > 0 &&
    ...
    if (global.tech["theatre"] && !global.race["joyless"]) {
      entertainment +=
        workerScale(global.civic.entertainer.workers, "entertainer") *
        global.tech.theatre;
      if (global.race["musical"]) {
        entertainment +=
          workerScale(global.civic.entertainer.workers, "entertainer") *
          traits.musical.vars()[0];
      }
    ...
    if (
      ...
      let val = 0;
      val +=
        global.eden.hasOwnProperty("pillbox") && p_on["pillbox"]
          ? 0.35 * p_on["pillbox"]
          : 0;
      val += global.civic.elysium_miner.workers * 0.15;
      val +=
        global.eden.hasOwnProperty("archive") && p_on["archive"]
          ? 0.4 * p_on["archive"]
          : 0;
    ...
    if (eventActive("summer")) {
      let boost =
        ((global.resource.Thermite.diff * 2.5) /
          (global.resource.Thermite.diff * 2.5 + 500)) *
        500;
    ...
    if (
      global.civic.govern.type === "oligarchy" &&
    ...
    if (
      ((global.civic.govern.type !== "autocracy" &&
        ...
        global.race["immoral"]) &&
      global.civic.garrison.protest + global.civic.garrison.fatigue > 2
    ) {
      let immoral = global.race["immoral"]
        ? 1 + traits.immoral.vars()[0] / 100
        : 1;
      let warmonger = Math.round(
        Math.log2(
          global.civic.garrison.protest + global.civic.garrison.fatigue,
        ) * immoral,
      );
      global.city.morale.warmonger = global.race["immoral"]
        ? warmonger
        : -warmonger;
    ...
    if (global.city["amphitheatre"]) {
      ...
      mBaseCap += athVal
        ? global.city.amphitheatre.count * athVal
    ...
    if (global.tech["superstar"]) {
      ...
      mBaseCap +=
        workerScale(global.civic.entertainer.workers, "entertainer") * mcapval;
    }
    moraleCap = mBaseCap;
    ...
    if (global.tech["monuments"]) {
      ...
      if (global.race["wish"] && global.race["wishStats"]) {
        ...
        if (
          global.interstellar["wonder_gardens"] ||
          global.space["wonder_gardens"] ||
          global.portal["wonder_gardens"]
        ) {
          monuments += 5;
        }
      }
    ...
    if (global.city.morale.current < 100) {
      if (global.race["blissful"]) {
        ...
        breakdown.p["Global"][loc("morale")] =
          +(global.city.morale.current - 100).toFixed(2) + "%";
      }
    } else {
      global_multiplier *= 1 + (global.city.morale.current - 100) / 200;
      breakdown.p["Global"][loc("morale")] =
        +((global.city.morale.current - 100) / 2).toFixed(2) + "%";
    }

    if (global.race["lazy"] && global.city.calendar.temp === 2) {
      breakdown.p["Global"][loc("trait_lazy_bd")] =
        "-" + traits.lazy.vars()[0] + "%";
    ...
    if (global.civic.govern.type === "dictator") {
      breakdown.p["Global"][loc("wish_dictator")] =
        `+${govEffect.dictator()[1]}%`;
      global_multiplier *= 1 + govEffect.dictator()[1] / 100;
    }

    if (global.race["selenophobia"]) {
      let moon =
        global.city.calendar.moon > 14
          ? 28 - global.city.calendar.moon
          : global.city.calendar.moon;
      breakdown.p["Global"][loc("moon_phase")] =
        -moon + traits.selenophobia.vars()[0] + "%";
    ...
    if (global.interstellar["mass_ejector"]) {
      ...
      Object.keys(global.interstellar.mass_ejector).forEach(function (res) {
        if (atomic_mass[res]) {
          ...
          if (ejected > 0) {
            breakdown.p.consume[res][loc("interstellar_blackhole_name")] =
              -ejected;
          }
      ...
      global.interstellar.stellar_engine.mass +=
        (mass / 10000000000) * time_multiplier;
      global.interstellar.stellar_engine.exotic +=
        (exotic / 10000000000) * time_multiplier;
    }
    ...
    if (global.portal["transport"] && global.portal["purifier"]) {
      ...
      let supply = 0;
      let bireme_rating =
        global.blood["spire"] && global.blood.spire >= 2 ? 0.8 : 0.85;
      let cargoSize =
        global.stats.achieve["what_is_best"] &&
        global.stats.achieve.what_is_best.e >= 4
          ? 8
          : 5;
      Object.keys(global.portal.transport.cargo).forEach(function (res) {
        if (supplyValue[res]) {
          ...
          while (
            volume * time_multiplier > global.resource[res].amount &&
            volume > 0
          ) {
          ...
          supply += Number(
            shipped * supplyValue[res].in * time_multiplier * bireme,
          );
        }
      });
      if (
        global.tech["hell_lake"] &&
        global.tech.hell_lake >= 7 &&
        global.tech["railway"]
      ) {
    ...
    if (
      global.race["carnivore"] &&
      !global.race["herbivore"] &&
      !global.race["soul_eater"] &&
      !global.race["artifical"]
    ) {
      if (global.resource["Food"].amount > 10) {
        let rotPercent = traits.carnivore.vars()[0] / 100;
        let rot = +((global.resource["Food"].amount - 10) * rotPercent).toFixed(
          3,
        );
    ...
    if (
      ...
      global.city["farm"] ||
      global.city["soul_well"] ||
      global.city["compost"] ||
      global.city["tourist_center"] ||
      global.city["transmitter"]
    ) {
      let food_base = 0;
      ...
      if (global.race["artifical"]) {
        if (global.city["transmitter"]) {
          food_base =
            ...
            production("psychic_boost", "Food");
          breakdown.p["Food"][loc("city_transmitter")] = food_base + "v";
          global.city.transmitter["lpmod"] =
            production("transmitter") *
            global_multiplier *
            production("psychic_boost", "Food");
        }
        ...
        if (global.race["detritivore"]) {
          if (global.city["compost"]) {
            ...
            if (
              ...
              !global.race["smoldering"]
            ) {
              let lumberIncrement = 0.5;
              let lumber_cost = operating * lumberIncrement;

              while (
                lumber_cost * time_multiplier > global.resource.Lumber.amount &&
                lumber_cost > 0
              ) {
              ...
              breakdown.p.consume.Lumber[loc("city_compost_heap")] =
                -lumber_cost;
              modRes("Lumber", -(lumber_cost * time_multiplier));
            }
            let c_factor = traits.detritivore.vars()[0] / 100;
            let food_compost =
              operating * (1.2 + global.tech["compost"] * c_factor);
            food_compost *=
              global.city.biome === "grassland"
                ? biomes.grassland.vars()[0]
                : 1;
            food_compost *=
              global.city.biome === "savanna" ? biomes.savanna.vars()[0] : 1;
            food_compost *=
              global.city.biome === "ashland" ? biomes.ashland.vars()[0] : 1;
            food_compost *=
              global.city.biome === "volcanic" ? biomes.volcanic.vars()[0] : 1;
            food_compost *=
              global.city.biome === "hellscape"
                ? biomes.hellscape.vars()[0]
                : 1;
            food_compost *= global.city.ptrait.includes("trashed")
              ? planetTraits.trashed.vars()[0]
              : 1;
          ...
          if (global.race["servants"]) {
            ...
            serve_hunt *= servantTrait(
              global.race.servants.jobs.hunter,
              "hunting",
            );
          ...
          if (
            ...
            breakdown.p["Food"][`ᄂ${loc("city_lodge")}`] =
              global.city.lodge.count * 5 + "%";
          }

          if (global.city["soul_well"]) {
            let souls =
              global.city["soul_well"].count *
              (global.race["ghostly"] ? 2 + traits.ghostly.vars()[1] : 2);
          ...
          if (global.city["captive_housing"]) {
            ...
            if (
              global.city.captive_housing.cattle <
                global.city.captive_housing.cattleCap &&
              hunt > 0
            ) {
              ...
              if (
                ...
                global.city.captive_housing.cattle ** 2
              ) {
                global.city.captive_housing.cattle++;
            ...
            if (global.city.captive_housing.cattle > 0) {
              let food =
                (global.city.captive_housing.cattle / 3) *
                production("psychic_boost", "Food");
              breakdown.p["Food"][loc("city_captive_housing_cattle_bd")] =
                food + "v";
              food_base += food;
              if (
                global.resource.Food.amount <
                global.resource.Food.max * 0.01
              ) {
                global.city.captive_housing.cattle--;
                modRes(
                  "Food",
          ...
          if (!global.race["submerged"]) {
            if (global.city.calendar.temp === 0) {
              if (global.city.calendar.weather === 0) {
                weather_multiplier *= global.race["chilled"]
                  ? 1 + traits.chilled.vars()[3] / 100
                  : 0.7;
              } else {
                weather_multiplier *= global.race["chilled"]
                  ? 1 + traits.chilled.vars()[4] / 100
                  : 0.85;
              }
            }
            if (global.city.calendar.weather === 2) {
              weather_multiplier *= global.race["chilled"]
                ? 1 - traits.chilled.vars()[5] / 100
                : 1.1;
            }
          }
          ...
          if (global.race["forager"]) {
            let forage =
              1 +
              (global.tech["foraging"] ? 0.75 * global.tech["foraging"] : 0);
            ...
            if (global.race["servants"]) {
              ...
              serve *= servantTrait(
                global.race.servants.jobs.forager,
                "forager",
              );
              foragers += serve;
            }
          ...
          if (global.race["warlord"]) {
            let food =
              workerScale(global.civic.lumberjack.workers, "farmer") *
              farmerValue(true);
          ...
          if (global.city["farm"]) {
            ...
            let food =
              farmers * farmerValue(true) + farmhands * farmerValue(false);
            ...
            if (global.race["servants"]) {
              ...
              food +=
                servants * farmerValue(true, true) +
                servehands * farmerValue(false, true);
            }
            ...
            if (food > 0) {
              breakdown.p["Food"][`ᄂ${loc("city_mill_title1")}`] =
                (mill_multiplier - 1) * 100 + "%";
              breakdown.p["Food"][`ᄂ${loc("sign_virgo")}+0`] =
                (virgo - 1) * 100 + "%";
              breakdown.p["Food"][`ᄂ${loc("morale_weather")}`] =
                (weather_multiplier - 1) * 100 + "%";
              breakdown.p["Food"][`ᄂ${loc("quarantine")}+0`] =
                (q_multiplier - 1) * 100 + "%";
            }
          }
        }
      ...
      if (global.tauceti["tau_farm"] && p_on["tau_farm"]) {
        ...
        if (food_base > 0) {
          breakdown.p["Food"][`ᄂ${loc("tau_home_colony")}`] =
            (colony_val - 1) * 100 + "%";
        }
      ...
      if (global.tech["military"]) {
        hunting =
          (global.race["herbivore"] && !global.race["carnivore"]) ||
          global.race["artifical"]
            ? 0
            : armyRating(garrisonSize(), "hunting") / 3;
      ...
      if (global.tech["mars"]) {
        biodome =
          support_on["biodome"] *
          workerScale(global.civic.colonist.workers, "colonist") *
          production("biodome", "food") *
          production("psychic_boost", "Food");
        if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
          biodome +=
            support_on["biodome"] *
            production("biodome", "cat_food") *
            production("psychic_boost", "Food");
        }
      }

      breakdown.p["Food"][actions.space.spc_red.biodome.title()] =
        biodome + "v";
      if (biodome > 0) {
        breakdown.p["Food"][`ᄂ${loc("space_syndicate")}+0`] =
          -((1 - red_synd) * 100) + "%";
        breakdown.p["Food"][`ᄂ${loc("space_red_ziggurat_title")}+0`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Food"][`ᄂ${loc("sign_virgo")}+0`] =
          (virgo - 1) * 100 + "%";
      }
      ...
      let generated =
        food_base +
        hunting * q_multiplier +
        biodome * red_synd * zigVal * virgo;
          ...
          case 0:
            food_consume_mod *=
              global.city.calendar.temp === 0
                ? 1
                : 1 - traits.photosynth.vars()[2] / 100;
            break;
          case 1:
      ...
      if (global.city.banquet) {
        if (global.city.banquet.on) {
          banquet *=
            (global.city.banquet.level >= 5 ? 1.02 : 1.022) **
      ...
      if (!global.race["fasting"]) {
        consume =
          (global.resource[global.race.species].amount +
            soldiers -
            (global.civic.unemployed.workers +
              workerScale(global.civic.hunter.workers, "hunter")) *
              0.5) *
          food_consume_mod;
        if (global.race["forager"]) {
        ...
        if (consume * banquet + ravenous >= global.resource.Food.amount) {
          ...
          if (banquet > 1) {
            breakdown.p.consume.Food[`${loc("city_banquet")}`] = -(
              consume *
              (banquet - 1)
            );
          }
        ...
        if (global.space["spaceport"]) {
          spaceport =
            p_on["spaceport"] *
            (global.race["cataclysm"] || global.race["orbit_decayed"] ? 2 : 25);
          breakdown.p.consume.Food[loc("space_red_spaceport_title")] =
            -spaceport;
        }

        if (global.interstellar["starport"]) {
          starport = p_on["starport"] * 100;
          breakdown.p.consume.Food[loc("interstellar_alpha_starport_title")] =
            -starport;
        }
        ...
        if (global.space["space_station"]) {
          space_station =
            p_on["space_station"] * (global.race["cataclysm"] ? 1 : 10);
          breakdown.p.consume.Food[loc("space_belt_station_title")] =
            -space_station;
        }

        if (global.space["space_barracks"] && !global.race["cataclysm"]) {
          space_marines = global.space.space_barracks.on * 10;
          breakdown.p.consume.Food[loc("tech_space_marines_bd")] =
            -space_marines;
        }
      ...
      if (hunting > 0) {
        breakdown.p["Food"][`ᄂ${loc("quarantine")}+1`] =
          (q_multiplier - 1) * 100 + "%";
      }
      if (global.race["fasting"]) {
      ...
      if (!modRes("Food", delta * time_multiplier) || global.race["fasting"]) {
        if (
          ...
          if (global.resource[global.race.species].amount > 0) {
            ...
            if (
              global.portal["dish_life_infuser"] &&
            ...
            if (global.race["fasting"]) {
              ...
              breakdown.p.consume[global.race.species][
                global.resource[global.race.species].name
              ] = -base.toFixed(2);
              breakdown.p.consume[global.race.species][loc("genelab_traits")] =
                (1 - food_consume_mod) * base.toFixed(2);
              breakdown.p.consume[global.race.species][loc("Threshold")] =
                threshold.toFixed(2);
              global.resource[global.race.species].delta =
                -(base * food_consume_mod - threshold) * time_multiplier;
            ...
            if (Math.rand(0, 10) === 0) {
              if (global.race["fasting"]) {
                let starved =
                  (global.resource[global.race.species].amount / 100) *
                    food_consume_mod -
                  threshold;
                if (starved < 0) {
                  starved = 0;
    ...
    if (global.portal["fortress"] && global.portal.fortress.walls < 100) {
      ...
      if (
        global.portal.fortress.repair >=
        actions.portal.prtl_fortress.info.repair()
      ) {
    ...
    if (global.civic.homeless > 0) {
      let missing = Math.min(
        ...
        global.resource[global.race.species].max -
          global.resource[global.race.species].amount,
      );
      ...
      ((fed && global["resource"]["Food"].amount > 0) ||
        global.race["fasting"]) &&
      global["resource"][global.race.species].max >
        global["resource"][global.race.species].amount
    ) {
      if (
        global.race["artifical"] ||
        (global.race["spongy"] && global.city.calendar.weather === 0)
      ) {
        // Do Nothing
      } else if (
        global.race["parasite"] &&
        ...
        let lowerBound = global.tech["reproduction"]
          ? global.tech["reproduction"]
          : 0;
        ...
        if (
          global.tech["reproduction"] &&
          global.tech.reproduction >= 2 &&
          global.city["hospital"]
        ) {
        ...
        if (global.race["promiscuous"]) {
          lowerBound +=
            traits.promiscuous.vars()[0] * global.race["promiscuous"];
        }
        if (global.race["fasting"]) {
    ...
    if (global.space["shipyard"] && global.space.shipyard["ships"]) {
      ...
      global.space.shipyard.ships.forEach(function (ship) {
        if (ship.location !== "spc_dwarf" || ship.transit !== 0) {
          ...
          if (fuel.res && fuel.burn > 0) {
            if (
              fuel.burn * time_multiplier <
              global.resource[fuel.res].amount +
                (global.resource[fuel.res].diff > 0
      ...
      breakdown.p.consume.Helium_3[loc("outer_shipyard_fleet")] =
        -fuels.Helium_3;
    ...
    if (
      global.portal["dish_soul_steeper"] &&
      global.portal["dish_soul_steeper"].on
    ) {
      hunger -=
        (0.03 +
          (global.race["malnutrition"] ? 0.01 : 0) +
          (global.race["angry"] ? -0.01 : 0)) *
        global.portal["dish_soul_steeper"].on;
    }
    ...
    if (global.resource.Furs.display) {
      if (
        ...
        if (hunters > 0) {
          breakdown.p["Furs"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
        }
        modRes(
          "Furs",
          hunters * hunger * global_multiplier * time_multiplier * q_multiplier,
        );

        if (!global.race["soul_eater"] && global.race["evil"]) {
          let reclaimers = workerScale(
            global.civic.lumberjack.workers,
            "lumberjack",
          );
          ...
          if (global.race["servants"]) {
            ...
            serve *= servantTrait(
              global.race.servants.jobs.lumberjack,
              "lumberjack",
            );
            reclaimers += serve;
          }
          ...
          if (reclaimers > 0) {
            breakdown.p["Furs"][`ᄂ${loc("quarantine")}+1`] =
              (q_multiplier - 1) * 100 + "%";
          }
          modRes(
            "Furs",
      ...
      if (hunting > 0) {
        breakdown.p["Furs"][`ᄂ${loc("quarantine")}+2`] =
          (q_multiplier - 1) * 100 + "%";
      }
      modRes(
        "Furs",
        hunting * hunger * global_multiplier * q_multiplier * time_multiplier,
      );

      if (global.race["forager"]) {
        let forage =
          1 + (global.tech["foraging"] ? 0.5 * global.tech["foraging"] : 0);
        let foragers = workerScale(global.civic.forager.workers, "forager");
        foragers *= racialTrait(foragers, "forager");
        ...
        let forage_base =
          foragers * forage * 0.05 * production("psychic_boost", "Furs");
        breakdown.p["Furs"][jobName("forager")] = forage_base + "v";
        if (forage_base > 0) {
          breakdown.p["Furs"][`ᄂ${loc("quarantine")}+3`] =
            (q_multiplier - 1) * 100 + "%";
        }
    ...
    if (
      global.resource.Furs.display &&
      global.tech["isolation"] &&
      global.tauceti["womling_farm"]
    ) {
      let base =
        global.tauceti.womling_farm.farmers *
        production("psychic_boost", "Furs");
    ...
    if (global.race["unfathomable"] && global.civic.hunter.display) {
      ...
      stealable.forEach(function (res) {
        if (global.resource[res].display) {
          let raid =
            (hunters * production("psychic_boost", res) * tradeRatio[res]) / 5;
          ...
          if (raid > 0) {
            breakdown.p[res][`ᄂ${loc("quarantine")}+99`] =
              (q_multiplier - 1) * 100 + "%";
          }
          modRes(
            res,
            raid * hunger * global_multiplier * time_multiplier * q_multiplier,
          );
        }
      });
    }
    ...
    // Knowledge
    {
      //block scope
      let sundial_base =
        global.tech["primitive"] && global.tech["primitive"] >= 3 ? 1 : 0;
      if (global.race["ancient_ruins"]) {
        sundial_base++;
      }
      if (
        global.stats.achieve["extinct_junker"] &&
        global.stats.achieve["extinct_junker"].l >= 1
      ) {
        sundial_base++;
      }
      ...
      let professors_base = workerScale(
        global.civic.professor.workers,
        "professor",
      );
      let prof_impact = global.race["studious"]
        ? global.civic.professor.impact + traits.studious.vars()[0]
        : global.civic.professor.impact;
      let fathom = fathomCheck("elven");
      if (fathom > 0) {
        prof_impact += traits.studious.vars(1)[0] * fathom;
      }
      professors_base *= prof_impact;
      professors_base *= global.race["pompous"]
        ? 1 - traits.pompous.vars()[0] / 100
        : 1;
      professors_base *= racialTrait(
        workerScale(global.civic.professor.workers, "professor"),
        "science",
      );
      ...
      if (
        ...
        if (
          global.arpa.hasOwnProperty("sequence") &&
          global.arpa.sequence.time === null
        ) {
      ...
      let womling = global.tauceti.hasOwnProperty("womling_lab")
        ? global.tauceti.womling_lab.scientist *
          (global.tech["womling_gene"] ? 10 : 8)
        : 0;
      ...
      if (global.race.universe === "magic") {
        ...
        breakdown.p["Knowledge"][
          global.civic?.scientist?.name || jobName("scientist")
        ] = scientist_base + "v";
      }
      breakdown.p["Knowledge"][loc("tau_red_womlings")] = womling + "v";
      breakdown.p["Knowledge"][loc("hunger")] = (hunger - 1) * 100 + "%";
      breakdown.p["Knowledge"][
        global.race["unfathomable"] ? loc("tech_moondial") : loc("tech_sundial")
      ] = sundial_base + "v";
      ...
      if (global.city["library"] || global.race["warlord"]) {
        ...
        let lib_count = global.race["warlord"]
          ? (global.race?.absorbed?.length || 1) * 10
          : global.city.library.count;
        let library_mult = 1 + lib_count * lib_multiplier;
        breakdown.p["Knowledge"][
          global.race["warlord"]
            ? loc("portal_throne_of_evil_title")
            : loc("city_library")
        ] = (library_mult - 1) * 100 + "%";
        delta *= library_mult;
      }
      if (astroSign === "gemini") {
        ...
        breakdown.p["Knowledge"][loc(`sign_${astroSign}`)] =
          (astro_mult - 1) * 100 + "%";
        delta *= astro_mult;
      }
      if (global.tech["isolation"] && support_on["infectious_disease_lab"]) {
        ...
        breakdown.p["Knowledge"][
          actions.tauceti.tau_home.infectious_disease_lab.title()
        ] = (lab_mult - 1) * 100 + "%";
        delta *= lab_mult;
      }
      if (global.civic.govern.type === "technocracy") {
        breakdown.p["Knowledge"][loc("govern_technocracy")] =
          govEffect.technocracy()[2] + "%";
      ...
      if (
        global.tech["tau_gas2"] &&
        global.tech.tau_gas2 >= 6 &&
        (!global.tech["alien_data"] || global.tech.alien_data < 6) &&
        global.tauceti["alien_space_station"] &&
        p_on["alien_space_station"]
      ) {
        ...
        if (
          global.tauceti.alien_space_station.decrypted >=
            (global.race["lone_survivor"] ? 1000000 : 250000000) &&
          !global.tech["alien_data"]
        ) {
          global.tech["alien_data"] = 1;
          messageQueue(
            loc("tau_gas2_alien_station_data1", [loc("tech_dist_womling")]),
            "success",
            false,
            ["progress"],
          );
          ...
          global.tauceti.alien_space_station.decrypted >=
            (global.race["lone_survivor"] ? 2000000 : 500000000) &&
          global.tech["alien_data"] &&
          global.tech.alien_data === 1
        ) {
          ...
          messageQueue(
            loc("tau_gas2_alien_station_data2", [
              loc(
                `tau_gas2_alien_station_data2_r${global.race.tau_food_item || 0}`,
              ),
            ]),
            "success",
            false,
          ...
          global.tauceti.alien_space_station.decrypted >=
            (global.race["lone_survivor"] ? 3000000 : 750000000) &&
          global.tech["alien_data"] &&
          global.tech.alien_data === 2
        ) {
          global.tech.alien_data = 3;
          messageQueue(loc("tau_gas2_alien_station_data3"), "success", false, [
            "progress",
          ]);
          drawTech();
        } else if (
          global.tauceti.alien_space_station.decrypted >=
            (global.race["lone_survivor"] ? 4800000 : 1200000000) &&
          global.tech["alien_data"] &&
          global.tech.alien_data === 3
        ) {
          ...
          messageQueue(
            loc("tau_gas2_alien_station_data4", [
              loc(
                `tau_gas2_alien_station_data4_r${global.race.tau_junk_item || 0}`,
              ),
            ]),
            "success",
            false,
          ...
          global.tauceti.alien_space_station.decrypted >=
            (global.race["lone_survivor"] ? 6000000 : 1500000000) &&
          global.tech["alien_data"] &&
          global.tech.alien_data === 4
        ) {
          global.tech.alien_data = 5;
          messageQueue(loc("tau_gas2_alien_station_data5"), "success", false, [
            "progress",
          ]);
          drawTech();
        } else if (
          global.tauceti.alien_space_station.decrypted >=
            (global.race["lone_survivor"] ? 10000000 : 2500000000) &&
          global.tech["alien_data"] &&
          global.tech.alien_data === 5
        ) {
          ...
          if (global.race["lone_survivor"]) {
            ...
            messageQueue(
              loc("tau_gas2_alien_station_data6_alt"),
              "success",
              false,
              ["progress"],
            );
            ...
            messageQueue(
              loc("tau_gas2_alien_station_data6"),
              "success",
              false,
              ["progress"],
            );
    ...
    if (global.resource.Omniscience.display) {
      if (support_on["research_station"]) {
        ...
        let ghost_gain =
          support_on["research_station"] * ghost_base * 0.0000325 * corruptor;
        breakdown.p["Omniscience"][loc("eden_research_station_title")] =
          ghost_gain + "v";
        if (corruptor > 1) {
          breakdown.p["Omniscience"][`ᄂ${loc("eden_corruptor_title")}`] =
            (corruptor - 1) * 100 + "%";
        }
      ...
      if (global.tech["science"] && global.tech.science >= 23) {
        let scientist = workerScale(
          global.civic.scientist.workers,
          "scientist",
        );
        scientist *= racialTrait(scientist, "science");
        scientist *= global.race["pompous"]
          ? 1 - traits.pompous.vars()[0] / 100
          : 1;

        let sci_gain = scientist * 0.000707;
        breakdown.p["Omniscience"][global.civic.scientist.name] =
          sci_gain + "v";
    ...
    if (global.city["factory"]) {
      let on_factories =
        ...
        (p_on["hell_factory"] || 0) *
          actions.portal.prtl_wasteland.hell_factory.lines() +
      ...
      let max_factories =
        ...
        (global.interstellar["int_factory"]
          ? global.interstellar["int_factory"].on * 2
          : 0) +
        (global.portal["hell_factory"]
          ? global.portal["hell_factory"].on *
            actions.portal.prtl_wasteland.hell_factory.lines()
          : 0) +
        (global.tauceti["tau_factory"]
          ? global.tauceti["tau_factory"].on *
            (global.tech["isolation"] ? 5 : 3)
          : 0);
      let eff = max_factories > 0 ? on_factories / max_factories : 0;
      let remaining = max_factories;

      ["Lux", "Furs", "Alloy", "Polymer", "Nano", "Stanene"].forEach(
        function (res) {
      ...
      let tauBonus = global.tech["isolation"]
        ? 1 + (support_on["colony"] || 0) * 0.5
        : 1;
      ...
      if (global.city.factory["Lux"] && global.city.factory["Lux"] > 0) {
        ...
        while (
          fur_cost * time_multiplier > global.resource.Furs.amount &&
          fur_cost > 0
        ) {
        ...
        let demand =
          highPopAdjust(global.resource[global.race.species].amount) *
          f_rate.Lux.demand[assembly] *
          eff;
      ...
      if (global.city.factory["Furs"] && global.city.factory["Furs"] > 0) {
        ...
        while (
          polymer_cost * time_multiplier > global.resource.Polymer.amount &&
          polymer_cost > 0
        ) {
        ...
        while (
          money_cost * time_multiplier > global.resource.Money.amount &&
          money_cost > 0
        ) {
          polymer_cost -= polymerIncrement;
          money_cost -= moneyIncrement;
        ...
        if (delta > 0) {
          if (tauBonus > 0) {
            breakdown.p["Furs"][`ᄂ${loc("tau_home_colony")}`] =
              (tauBonus - 1) * 100 + "%";
          }
          ...
          if (global.tech["q_factory"]) {
            ...
            delta *= q_bonus;
            breakdown.p["Furs"][`ᄂ${loc("quantum")}`] =
              (q_bonus - 1) * 100 + "%";
          }
        }

        if (global.race["gravity_well"]) {
          breakdown.p["Furs"][`ᄂ${loc("evo_challenge_gravity_well")}+0`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      ...
      if (global.city.factory["Alloy"] && global.city.factory["Alloy"] > 0) {
        let copper_cost =
          global.city.factory.Alloy * f_rate.Alloy.copper[assembly] * eff;
        let aluminium_cost =
          global.city.factory.Alloy * f_rate.Alloy.aluminium[assembly] * eff;
        let workDone = global.city.factory.Alloy;

        while (
          copper_cost * time_multiplier > global.resource.Copper.amount &&
          copper_cost > 0
        ) {
        ...
        while (
          aluminium_cost * time_multiplier > global.resource.Aluminium.amount &&
          aluminium_cost > 0
        ) {
          copper_cost -= f_rate.Alloy.copper[assembly] * eff;
          aluminium_cost -= f_rate.Alloy.aluminium[assembly] * eff;
        ...
        let factory_output =
          workDone *
          f_rate.Alloy.output[assembly] *
          eff *
          production("psychic_boost", "Alloy");
        factory_output = factoryBonus(factory_output);
        ...
        if (global.race["metallurgist"]) {
          factory_output *=
            1 +
            (traits.metallurgist.vars()[0] * global.race["metallurgist"]) / 100;
        }
        ...
        if (delta > 0) {
          if (tauBonus > 0) {
            breakdown.p["Alloy"][`ᄂ${loc("tau_home_colony")}`] =
              (tauBonus - 1) * 100 + "%";
          }

          if (global.race["discharge"] && global.race["discharge"] > 0) {
            delta *= 0.5;
            breakdown.p["Alloy"][`ᄂ${loc("evo_challenge_discharge")}`] =
              "-50%";
          }
          ...
          if (global.tech["q_factory"]) {
            ...
            delta *= q_bonus;
            breakdown.p["Alloy"][`ᄂ${loc("quantum")}`] =
              (q_bonus - 1) * 100 + "%";
          }
        ...
        if (global.race["gravity_well"]) {
          breakdown.p["Alloy"][`ᄂ${loc("evo_challenge_gravity_well")}+0`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      ...
      if (
        ...
        let oilIncrement =
          global.race["kindling_kindred"] || global.race["smoldering"]
            ? f_rate.Polymer.oil_kk[assembly] * eff
            : f_rate.Polymer.oil[assembly] * eff;
        let lumberIncrement =
          global.race["kindling_kindred"] || global.race["smoldering"]
            ? 0
            : f_rate.Polymer.lumber[assembly] * eff;
        ...
        while (
          oil_cost * time_multiplier > global.resource.Oil.amount &&
          oil_cost > 0
        ) {
          lumber_cost -= lumberIncrement;
          oil_cost -= oilIncrement;
        ...
        let factory_output =
          workDone *
          f_rate.Polymer.output[assembly] *
          eff *
          production("psychic_boost", "Polymer");
        factory_output = factoryBonus(factory_output);
        ...
        if (delta > 0) {
          if (tauBonus > 0) {
            breakdown.p["Polymer"][`ᄂ${loc("tau_home_colony")}`] =
              (tauBonus - 1) * 100 + "%";
          }

          if (global.race["discharge"] && global.race["discharge"] > 0) {
            delta *= 0.5;
            breakdown.p["Polymer"][`ᄂ${loc("evo_challenge_discharge")}`] =
              "-50%";
          }
          ...
          if (global.tech["q_factory"]) {
            let q_bonus = (quantum_level - 1) / 2 + 1;
            delta *= q_bonus;
            breakdown.p["Polymer"][`ᄂ${loc("quantum")}`] =
              (q_bonus - 1) * 100 + "%";
          }
        }

        if (global.race["gravity_well"]) {
          breakdown.p["Polymer"][`ᄂ${loc("evo_challenge_gravity_well")}+0`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      ...
      if (p_on["s_gate"] && global.galaxy["raider"] && gal_on["raider"] > 0) {
        let base =
          gal_on["raider"] * 2.3 * production("psychic_boost", "Polymer");
        ...
        if (base > 0) {
          breakdown.p["Polymer"][`ᄂ${loc("galaxy_piracy")}`] =
            -((1 - pirate) * 100) + "%";
          breakdown.p["Polymer"][`ᄂ${loc("space_red_ziggurat_title")}`] =
            (zigVal - 1) * 100 + "%";
        }
        modRes("Polymer", delta * time_multiplier);
      }
      ...
      if (global.city.factory["Nano"] && global.city.factory["Nano"] > 0) {
        ...
        while (
          neutronium_cost * time_multiplier >
            global.resource.Neutronium.amount &&
          neutronium_cost > 0
        ) {
        ...
        while (
          coal_cost * time_multiplier > global.resource.Coal.amount &&
          coal_cost > 0
        ) {
          neutronium_cost -= neutroniumIncrement;
          coal_cost -= coalIncrement;
        ...
        let factory_output =
          workDone *
          f_rate.Nano_Tube.output[assembly] *
          eff *
          production("psychic_boost", "Nano_Tube");
        factory_output = factoryBonus(factory_output);
        ...
        if (delta > 0) {
          if (tauBonus > 0) {
            breakdown.p["Nano_Tube"][`ᄂ${loc("tau_home_colony")}`] =
              (tauBonus - 1) * 100 + "%";
          }

          if (global.race["discharge"] && global.race["discharge"] > 0) {
            delta *= 0.5;
            breakdown.p["Nano_Tube"][`ᄂ${loc("evo_challenge_discharge")}`] =
              "-50%";
          }
          ...
          if (global.tech["q_factory"]) {
            let q_bonus = (quantum_level - 1) / 2 + 1;
            delta *= q_bonus;
            breakdown.p["Nano_Tube"][`ᄂ${loc("quantum")}`] =
              (q_bonus - 1) * 100 + "%";
          }
        ...
        if (global.race["gravity_well"]) {
          breakdown.p["Nano_Tube"][`ᄂ${loc("evo_challenge_gravity_well")}+0`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      ...
      if (
        ...
        while (
          alum_cost * time_multiplier > global.resource.Aluminium.amount &&
          alum_cost > 0
        ) {
        ...
        while (
          nano_cost * time_multiplier > global.resource.Nano_Tube.amount &&
          nano_cost > 0
        ) {
          nano_cost -= nanoIncrement;
          alum_cost -= alumIncrement;
        ...
        breakdown.p.consume.Aluminium[loc("city_factory")] = breakdown.p.consume
          .Aluminium[loc("city_factory")]
          ? breakdown.p.consume.Aluminium[loc("city_factory")] - alum_cost
          : -alum_cost;
        ...
        let factory_output =
          workDone *
          f_rate.Stanene.output[assembly] *
          eff *
          production("psychic_boost", "Stanene");
        factory_output = factoryBonus(factory_output);
        ...
        if (delta > 0) {
          if (tauBonus > 0) {
            breakdown.p["Stanene"][`ᄂ${loc("tau_home_colony")}`] =
              (tauBonus - 1) * 100 + "%";
          }

          if (global.race["discharge"] && global.race["discharge"] > 0) {
            delta *= 0.5;
            breakdown.p["Stanene"][`ᄂ${loc("evo_challenge_discharge")}`] =
              "-50%";
          }
          ...
          if (global.tech["q_factory"]) {
            let q_bonus = (quantum_level - 1) / 2 + 1;
            delta *= q_bonus;
            breakdown.p["Stanene"][`ᄂ${loc("quantum")}`] =
              (q_bonus - 1) * 100 + "%";
          }
        ...
        if (global.race["gravity_well"]) {
          breakdown.p["Stanene"][`ᄂ${loc("evo_challenge_gravity_well")}+0`] =
            -((1 - teamster(1)) * 100) + "%";
        }
    ...
    if (global.resource.Cement.display) {
      let unit_price = global.race["high_pop"]
        ? 3 / traits.high_pop.vars()[0]
        : 3;
      if (global.city.biome === "ashland") {
        unit_price *= biomes.ashland.vars()[1];
      }
      let stone_cost =
        workerScale(global.civic.cement_worker.workers, "cement_worker") *
        unit_price;
      let workDone = workerScale(
        global.civic.cement_worker.workers,
        "cement_worker",
      );
      while (
        stone_cost * time_multiplier > global.resource.Stone.amount &&
        stone_cost > 0
      ) {
      ...
      let tauBonus = global.tech["isolation"]
        ? 1 + (support_on["colony"] || 0) * 0.5
        : 1;
      breakdown.p.consume.Stone[
        loc(
          global.tech["isolation"]
            ? "job_cement_worker_bd"
      ...
      let cement_base =
        global.tech["cement"] >= 4 ? (global.tech.cement >= 7 ? 1.45 : 1.2) : 1;
      cement_base *= global.civic.cement_worker.impact;
      cement_base *= racialTrait(
        workerScale(global.civic.cement_worker.workers, "cement_worker"),
        "factory",
      );
      if (global.city.biome === "ashland") {
      ...
      if (
        global.stats.achieve["lamentis"] &&
        global.stats.achieve.lamentis.l >= 3
      ) {
        cement_base *= 1.1;
      }

      let factory_output =
        workDone * cement_base * production("psychic_boost", "Cement");
      if (global.civic.govern.type === "corpocracy") {
      ...
      if (
        ...
        hell_factory +=
          (p_on["hell_factory"] * 8 * (global.portal.hell_factory.rank - 1)) /
          100;
      }

      let mining_pit = global.tech["isolation"]
        ? 1 + support_on["mining_pit"] * 0.08
        : 1;
      ...
      if (factory_output > 0) {
        if (global.tech["isolation"]) {
          breakdown.p["Cement"][`ᄂ${loc("tau_home_colony")}+0`] =
            (tauBonus - 1) * 100 + "%";
          breakdown.p["Cement"][`ᄂ${loc("tau_home_mining_pit")}+0`] =
            (mining_pit - 1) * 100 + "%";
        }
        breakdown.p["Cement"][`ᄂ${loc("power")}+0`] =
          (powered_mult - 1) * 100 + "%";
        breakdown.p["Cement"][`ᄂ${loc("quarantine")}+0`] =
          (cq_multiplier - 1) * 100 + "%";
      }

      if (hell_factory > 1) {
        breakdown.p["Cement"][`ᄂ${loc("portal_factory_title")}+0`] =
          (hell_factory - 1) * 100 + "%";
      }

      if (
        global.race["discharge"] &&
        global.race["discharge"] > 0 &&
        p_on["cement_plant"] > 0
      ) {
      ...
      let delta =
        factory_output * ai_core * tauBonus * mining_pit * hell_factory;
      if (global.city["cement_plant"]) {
        global.city.cement_plant["cnvay"] = +(
          delta *
          hunger *
          cq_multiplier *
          global_multiplier *
      ...
      if (global.tech["ai_core"] && p_on["citadel"] > 0) {
        breakdown.p["Cement"][loc("interstellar_citadel_effect_bd")] =
          (ai_core - 1) * 100 + "%";
      }
    ...
    if (smelterUnlocked()) {
      ...
      if (p_on["stellar_forge"]) {
        star_forge =
          p_on["stellar_forge"] *
          actions.interstellar.int_neutron.stellar_forge.smelting();
        global.city.smelter.Star = Math.max(
      ...
      if (p_on["hell_forge"]) {
        capacity +=
          p_on["hell_forge"] * actions.portal.prtl_ruins.hell_forge.smelting();
      }
      if (p_on["demon_forge"]) {
        capacity +=
          p_on["demon_forge"] *
          actions.portal.prtl_wasteland.demon_forge.smelting();
      }
      if (p_on["sacred_smelter"]) {
        capacity +=
          p_on["sacred_smelter"] *
          actions.eden.eden_elysium.sacred_smelter.smelting();
      }
      if (p_on["ore_refinery"]) {
        capacity +=
          p_on["ore_refinery"] *
          actions.tauceti.tau_gas.ore_refinery.smelting();
      }
      if (global.space["hell_smelter"]) {
        capacity +=
          global.space.hell_smelter.count *
          actions.space.spc_hell.hell_smelter.smelting();
      }
      if (p_on["geothermal"]) {
        capacity +=
          p_on["geothermal"] * actions.space.spc_hell.geothermal.smelting();
      }
      ...
      if (global.race["forge"]) {
        ...
        global.city.smelter.Oil = Math.max(
          0,
          global.city.smelter.cap -
            global.city.smelter.Star -
      ...
      if (
        ...
        let irid_smelt =
          global.tech["irid_smelting"] ||
          (global.tech["m_smelting"] && global.tech.m_smelting >= 2)
            ? true
            : false;
        if (
          !(global.resource.Iridium.display && irid_smelt) &&
          !(
            global.resource.Steel.display &&
      ...
      let disable_smelters = Math.max(
        0,
        iron_smelter + steel_smelter + iridium_smelter - total_fuel,
      );

      if (consume_wood > 0) {
        let max_operable = Math.max(
          0,
          Math.floor(
            global.resource[fuel_config.l_type].amount /
      ...
      if (consume_coal > 0) {
        let max_operable = Math.max(
          0,
          Math.floor(
            global.resource.Coal.amount /
              (fuel_config.c_cost * time_multiplier),
          ),
        );
      ...
      if (consume_oil > 0) {
        let max_operable = Math.max(
          0,
          Math.floor(
            global.resource.Oil.amount / (fuel_config.o_cost * time_multiplier),
          ),
        );
      ...
      if (inferno_bonus > 0) {
        ...
        let max_operable_oil = Math.floor(
          (global.resource.Oil.amount - consume_oil) /
            (inferno_rate.Oil * time_multiplier),
        );
        let max_operable_coal = Math.floor(
          (global.resource.Coal.amount - consume_coal) /
            (inferno_rate.Coal * time_multiplier),
        );
        let max_operable_infernite = Math.floor(
          global.resource.Infernite.amount /
            (inferno_rate.Infernite * time_multiplier),
        );
        let max_operable = Math.max(
          0,
          Math.min(max_operable_oil, max_operable_coal, max_operable_infernite),
        );
      ...
      breakdown.p.consume[fuel_config.l_type][loc("city_smelter")] =
        -consume_wood;
      ...
      if (
        ...
        modRes("Uranium", ash * time_multiplier);
        // Display on the right side of the breakdown to demonstrate that there is no global production scaling
        breakdown.p.consume["Uranium"][loc("city_coal_ash")] =
          (breakdown.p.consume["Uranium"][loc("city_coal_ash")] ?? 0) + ash;
      }
      ...
      if (global.resource.Steel.display) {
        ...
        while (
          (iron_consume * time_multiplier > global.resource.Iron.amount &&
            iron_consume > 0) ||
          (coal_consume * time_multiplier > global.resource.Coal.amount &&
            coal_consume > 0)
        ) {
          iron_consume -= 2;
          coal_consume -= 0.25;
        ...
        let steel_base = 1;
        if (
          global.stats.achieve["steelen"] &&
          global.stats.achieve["steelen"].l >= 1
        ) {
        ...
        if (
          global.stats.achieve["lamentis"] &&
          global.stats.achieve.lamentis.l >= 2
        ) {
          steel_base *= 1.1;
        }
        for (i = 4; i <= 6; i++) {
        ...
        let smelter_output =
          steel_smelter * steel_base * production("psychic_boost", "Steel");
        if (global.race["pyrophobia"]) {
        ...
        breakdown.p["Steel"][loc("city_shrine")] =
          ((shrineMetal.mult - 1) * 100).toFixed(1) + "%";
        ...
        if (global.tech["titanium"] && global.tech["titanium"] >= 1) {
          let titanium =
            smelter_output * hunger * production("psychic_boost", "Titanium");
          if (star_forge > 0) {
          ...
          breakdown.p["Titanium"][loc("resource_Steel_name")] =
            titanium / divisor + "v";
        }
      }
    }
    ...
    if (
      global[graph_source][graph_struct] &&
      global[graph_source][graph_struct].count > 0
    ) {
      let on_graph = global.race["truepath"]
        ? support_on["g_factory"]
        : global.race["warlord"]
          ? p_on["twisted_lab"]
          : int_on["g_factory"];
      ...
      if (graphene_production > 0) {
        ...
        while (
          consume_coal * time_multiplier > global.resource.Coal.amount &&
          consume_coal > 0
        ) {
          consume_coal -= 25 * eff;
          graphene_production--;
        ...
        while (
          consume_oil * time_multiplier > global.resource.Oil.amount &&
          consume_oil > 0
        ) {
          consume_oil -= 15 * eff;
          graphene_production--;
        ...
        graphene_production *=
          production("g_factory") * production("psychic_boost", "Graphene");

        breakdown.p.consume.Lumber[
          global.race["warlord"]
            ? loc("portal_twisted_lab_title")
            : loc("interstellar_g_factory_bd")
        ] = -consume_wood;
        breakdown.p.consume.Coal[
          global.race["warlord"]
            ? loc("portal_twisted_lab_title")
            : loc("interstellar_g_factory_bd")
        ] = -consume_coal;
        breakdown.p.consume.Oil[
          global.race["warlord"]
            ? loc("portal_twisted_lab_title")
            : loc("interstellar_g_factory_bd")
        ] = -consume_oil;
        ...
        if (
          global.race["warlord"] &&
          global.portal.hasOwnProperty("incinerator") &&
          global.portal.incinerator.rank > 1
        ) {
        ...
        let delta =
          ...
          eff *
          incinerator;
        breakdown.p["Graphene"][
          global.race["warlord"]
            ? loc("portal_twisted_lab_title")
            : loc("interstellar_g_factory_bd")
        ] = graphene_production + "v";
        if (global.tech["isolation"] && graphene_production > 0) {
          delta *= womling_technician;
          if (womling_technician > 1) {
            breakdown.p["Graphene"][`ᄂ${loc("tau_red_womlings")}+0`] =
              (womling_technician - 1) * 100 + "%";
          }
        }

        if (incinerator > 1) {
          breakdown.p["Graphene"][`ᄂ${loc("portal_incinerator_title")}`] =
            (incinerator - 1) * 100 + "%";
        }

        if (graphene_production > 0) {
          breakdown.p["Graphene"][`ᄂ${loc("space_syndicate")}`] =
            -((1 - synd) * 100) + "%";
          breakdown.p["Graphene"][`ᄂ${loc("space_red_ziggurat_title")}`] =
            (zigVal - 1) * 100 + "%";
        }

        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.5;
          breakdown.p["Graphene"][`ᄂ${loc("evo_challenge_discharge")}`] =
            "-50%";
        }

        if (p_on["citadel"] > 0) {
          breakdown.p["Graphene"][loc("interstellar_citadel_effect_bd")] =
            (ai - 1) * 100 + "%";
        }
    ...
    if (global.galaxy["vitreloy_plant"] && p_on["vitreloy_plant"] > 0) {
      ...
      vitreloy_production = Math.min(
        vitreloy_production,
        Math.floor(
          global.resource.Money.amount / (consume_money * time_multiplier),
        ),
      );
      vitreloy_production = Math.min(
        vitreloy_production,
        Math.floor(
          global.resource.Bolognium.amount /
            (consume_bolognium * time_multiplier),
        ),
      );
      vitreloy_production = Math.min(
        vitreloy_production,
        Math.floor(
          global.resource.Stanene.amount / (consume_stanene * time_multiplier),
        ),
      );
      ...
      if (vitreloy_production > 0) {
        ...
        vitreloy_production *=
          production("vitreloy_plant") *
          production("psychic_boost", "Vitreloy");

        breakdown.p.consume.Money[loc("galaxy_vitreloy_plant_bd")] =
          -consume_money;
        breakdown.p.consume.Bolognium[loc("galaxy_vitreloy_plant_bd")] =
          -consume_bolognium;
        breakdown.p.consume.Stanene[loc("galaxy_vitreloy_plant_bd")] =
          -consume_stanene;
        ...
        breakdown.p["Vitreloy"][loc("galaxy_vitreloy_plant_bd")] =
          vitreloy_production + "v";

        if (global.race["discharge"] && global.race["discharge"] > 0) {
          vitreloy_production *= 0.5;
          breakdown.p["Vitreloy"][`ᄂ${loc("evo_challenge_discharge")}`] =
            "-50%";
        }

        if (vitreloy_production > 0) {
          breakdown.p["Vitreloy"][`ᄂ${loc("galaxy_piracy")}+0`] =
            -((1 - pirate) * 100) + "%";
          breakdown.p["Vitreloy"][`ᄂ${loc("space_red_ziggurat_title")}+0`] =
            (zigVal - 1) * 100 + "%";
        }
        modRes(
          "Vitreloy",
    ...
    if (p_on["shadow_mine"]) {
      let attract = p_on["soul_attractor"]
        ? 1 + p_on["soul_attractor"] * (global.tech.pitspawn >= 3 ? 0.2 : 0.1)
        : 1;
      ...
      if (global.resource.Vitreloy.display) {
        ...
        let mine_base =
          p_on["shadow_mine"] * rate * production("psychic_boost", "Vitreloy");
        let mine_delta = mine_base * attract * global_multiplier;

        if (mine_base > 0) {
          breakdown.p["Vitreloy"][loc("portal_shadow_mine_title")] =
            mine_base + "v";
          breakdown.p["Vitreloy"][`ᄂ${loc("portal_soul_attractor_title")}+0`] =
            (attract - 1) * 100 + "%";
        }
      ...
      if (global.resource.Elerium.display) {
        ...
        let mine_base =
          p_on["shadow_mine"] * rate * production("psychic_boost", "Elerium");

        let mine_delta = mine_base * attract * global_multiplier;

        if (mine_base > 0) {
          breakdown.p["Elerium"][loc("portal_shadow_mine_title")] =
            mine_base + "v";
          breakdown.p["Elerium"][`ᄂ${loc("portal_soul_attractor_title")}+0`] =
            (attract - 1) * 100 + "%";
        }
      ...
      if (global.resource.Infernite.display) {
        ...
        let mine_base =
          p_on["shadow_mine"] * rate * production("psychic_boost", "Infernite");

        let mine_delta = mine_base * attract * global_multiplier;

        if (mine_base > 0) {
          breakdown.p["Infernite"][loc("portal_shadow_mine_title")] =
            mine_base + "v";
          breakdown.p["Infernite"][
            `ᄂ${loc("portal_soul_attractor_title")}+0`
          ] = (attract - 1) * 100 + "%";
        }
    ...
    if (p_on["s_gate"] && global.galaxy["raider"] && gal_on["raider"] > 0) {
      let base =
        gal_on["raider"] * 0.05 * production("psychic_boost", "Vitreloy");
      let pirate = piracy("gxy_chthonian");
      let delta = base * global_multiplier * pirate * hunger * zigVal;

      breakdown.p["Vitreloy"][loc("galaxy_raider")] = base + "v";
      if (base > 0) {
        breakdown.p["Vitreloy"][`ᄂ${loc("galaxy_piracy")}+1`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Vitreloy"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
      }
    ...
    if (
      ...
      breakdown.p["Cipher"][`ᄂ${loc("space_syndicate")}+0`] =
        -((1 - synd) * 100) + "%";
      ...
      if (
        global.resource.Cipher.display &&
        global.tech["outer"] &&
        global.tech.outer === 2
      ) {
        global.tech.outer = 3;
    ...
    if (
      !global.tech["isolation"] &&
      global.space["digsite"] &&
      global.space.digsite.count === 100
    ) {
      ...
      let shock_base =
        support_on["shock_trooper"] * production("shock_trooper");
      let tank_base = support_on["tank"] * production("tank");

      if (support_on["shock_trooper"]) {
        breakdown.p["Cipher"][loc("space_shock_trooper_title")] =
          shock_base + "v";
        breakdown.p["Cipher"][`ᄂ${loc("space_syndicate")}+1`] =
          -((1 - synd) * 100) + "%";
      }
      if (support_on["tank"]) {
        breakdown.p["Cipher"][loc("space_tank_title")] = tank_base + "v";
        breakdown.p["Cipher"][`ᄂ${loc("space_syndicate")}+2`] =
          -((1 - synd) * 100) + "%";
      }
    ...
    if (
      global.portal["oven_complete"] &&
      p_on["oven_complete"] &&
      !global.tech["dish_reset"] &&
      global.portal["devilish_dish"].done >= 100
    ) {
    ...
    if (
      ...
      if (base > 0) {
        breakdown.p["Cipher"][`ᄂ${loc("tau_home_colony")}`] =
          (colony_val - 1) * 100 + "%";
      }
    ...
    let e_ship = {};
    if (
      global.tauceti["ore_refinery"] &&
      global.tauceti["mining_ship"] &&
      global.tech["tau_roid"] &&
      global.tech.tau_roid >= 4
    ) {
      global.tauceti.ore_refinery.max =
        global.tauceti.ore_refinery.count * 1000;
      ...
      if (global.tauceti.ore_refinery.fill > 0) {
        ...
        e_ship["iron"] =
          ((raw * c_ratio * (100 - global.tauceti.mining_ship.common)) / 100) *
          production("mining_ship_ore", "iron") *
          production("psychic_boost", "Iron");
        e_ship["aluminium"] =
          ((raw * c_ratio * global.tauceti.mining_ship.common) / 100) *
          production("mining_ship_ore", "aluminium") *
          production("psychic_boost", "Aluminium");
        e_ship["iridium"] =
          ((raw * u_ratio * (100 - global.tauceti.mining_ship.uncommon)) /
            100) *
          production("mining_ship_ore", "iridium") *
          production("psychic_boost", "Iridium");
        e_ship["neutronium"] =
          ((raw * u_ratio * global.tauceti.mining_ship.uncommon) / 100) *
          production("mining_ship_ore", "neutronium") *
          production("psychic_boost", "Neutronium");

        if (global.tech.tau_roid >= 5) {
          e_ship["orichalcum"] =
            ((raw * 0.05 * (100 - global.tauceti.mining_ship.rare)) / 10) *
            production("mining_ship_ore", "orichalcum") *
            production("psychic_boost", "Orichalcum");
          e_ship["elerium"] =
            ((raw * 0.05 * global.tauceti.mining_ship.rare) / 10) *
            production("mining_ship_ore", "elerium") *
            production("psychic_boost", "Elerium");
        }
      }
      ...
      if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
        if (
          ...
          let lumber =
            support_on["biodome"] *
            workerScale(global.civic.colonist.workers, "colonist") *
            production("biodome", "lumber") *
            production("psychic_boost", "Lumber");

          breakdown.p["Lumber"][actions.space.spc_red.biodome.title()] =
            lumber + "v";
          if (lumber > 0) {
            breakdown.p["Lumber"][`ᄂ${loc("space_red_ziggurat_title")}`] =
              (zigVal - 1) * 100 + "%";
          }
          ...
          modRes(
            "Lumber",
            lumber * hunger * global_multiplier * time_multiplier * zigVal,
          );
        }
      } else if (
        global.race["soul_eater"] &&
        ...
        modRes(
          "Lumber",
          hunters * hunger * global_multiplier * time_multiplier,
        );
        modRes(
          "Lumber",
          soldiers * hunger * global_multiplier * time_multiplier,
        );
        ...
        if (reclaimers > 0) {
          breakdown.p["Lumber"][`ᄂ${loc("city_graveyard")}+0`] =
            (graveyard - 1) * 100 + "%";
          breakdown.p["Lumber"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
        }
        breakdown.p["Lumber"][loc("soldiers")] = soldiers + "v";
        if (soldiers > 0) {
          breakdown.p["Lumber"][`ᄂ${loc("quarantine")}+1`] =
            (q_multiplier - 1) * 100 + "%";
        }
        if (global.race["forager"]) {
          let forage = 1;
          ...
          if (forage_base > 0) {
            breakdown.p["Lumber"][`ᄂ${loc("city_graveyard")}+1`] =
              (graveyard - 1) * 100 + "%";
            breakdown.p["Lumber"][`ᄂ${loc("quarantine")}+2`] =
              (q_multiplier - 1) * 100 + "%";
          }
          modRes(
            "Lumber",
        ...
        lumber_base *=
          global.city.biome === "forest" ? biomes.forest.vars()[0] : 1;
        lumber_base *=
          global.city.biome === "savanna" ? biomes.savanna.vars()[2] : 1;
        lumber_base *=
          global.city.biome === "desert" ? biomes.desert.vars()[2] : 1;
        lumber_base *=
          global.city.biome === "swamp" ? biomes.swamp.vars()[2] : 1;
        lumber_base *=
          global.city.biome === "taiga" ? biomes.taiga.vars()[0] : 1;
        lumber_base *= global.civic.lumberjack.impact;
        if (global.race["living_tool"]) {
          lumber_base *=
            traits.living_tool.vars()[0] *
              (global.tech["science"] && global.tech.science > 0
                ? global.tech.science * 0.25
                : 0) +
            1;
        } else {
          lumber_base *=
            (global.tech["axe"] && global.tech.axe > 1
              ? (global.tech.axe - 1) * 0.35
              : 0) + 1;
        }
        ...
        if (lumber_base > 0) {
          breakdown.p["Lumber"][`ᄂ${loc("city_lumber_yard")}`] =
            (lumber_yard - 1) * 100 + "%";
          breakdown.p["Lumber"][`ᄂ${loc("city_sawmill")}`] =
            (sawmills - 1) * 100 + "%";
          breakdown.p["Lumber"][`ᄂ${loc("power")}`] =
            (power_mult - 1) * 100 + "%";
          breakdown.p["Lumber"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
        }
        ...
        if (global.race["forager"]) {
          ...
          let forage_base =
            foragers * forage * 0.25 * production("psychic_boost", "Lumber");
          breakdown.p["Lumber"][jobName("forager")] = forage_base + "v";
          if (lumber_base > 0) {
            breakdown.p["Lumber"][`ᄂ${loc("city_lumber_yard")}`] =
              (lumber_yard - 1) * 100 + "%";
    ...
    let refinery = global.city["metal_refinery"]
      ? global.city["metal_refinery"].count * 6
      : 0;
    ...
    if (global.race["sappy"]) {
      if (
        ...
        if (global.city.hasOwnProperty("basic_housing")) {
          ...
          stone_base *= 1 + grove;
          breakdown.p["Stone"][`ᄂ${housingLabel("small")}`] =
            grove * 100 + "%";
        }
      ...
      let quarriers = global.race["warlord"]
        ? global.civic.miner.workers
        : global.civic.quarry_worker.workers;
      let stone_prod_name = global.race["warlord"]
        ? jobName("miner")
        : loc("workers");
      ...
      if (global.race["living_tool"] || global.race["tusk"]) {
        ...
        let tusk = global.race["tusk"]
          ? 1 +
            (traits.tusk.vars()[0] / 100) *
              (armyRating(jobScale(balance), "army", 0) / balance / 100)
          : 1;
        let lt = global.race["living_tool"]
          ? traits.living_tool.vars()[0] *
              (global.tech["science"] && global.tech.science > 0
                ? global.tech.science * 0.06
                : 0) +
            1
          : 1;
        ...
        stone_base *=
          (global.tech["hammer"] && global.tech["hammer"] > 0
            ? global.tech["hammer"] * 0.4
            : 0) + 1;
      }
      ...
      if (global.race["warlord"] && global.portal["tunneler"]) {
        tunneler =
          1 +
          ((global.portal.tunneler.rank + 3) / 100) *
            global.portal.tunneler.count;
      }
      ...
      if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
        ...
        if (p_on["rock_quarry"]) {
          ...
          quarry_discharge =
            global.race["discharge"] && global.race["discharge"] > 0;
        }
      ...
      if (stone_base > 0) {
        if (zigValStone > 1) {
          breakdown.p["Stone"][`ᄂ${loc("space_red_ziggurat_title")}`] =
            (zigVal - 1) * 100 + "%";
        }
        breakdown.p["Stone"][`ᄂ${loc("city_rock_quarry")}`] =
          (rock_quarry - 1) * 100 + "%";
        breakdown.p["Stone"][`ᄂ${loc("power")}`] =
          (power_mult - 1) * 100 + "%";
        if (quarry_discharge) {
          breakdown.p["Stone"][`ᄂ${loc("evo_challenge_discharge")}`] = "-50%";
        }
        breakdown.p["Stone"][`ᄂ${loc("portal_tunneler_bd")}`] =
          (tunneler - 1) * 100 + "%";
        breakdown.p["Stone"][`ᄂ${loc("quarantine")}+0`] =
          (q_multiplier - 1) * 100 + "%";
      }
      ...
      if (global.race["smoldering"] && global.resource.Chrysotile.display) {
        ...
        if (asbestos_base > 0) {
          if (zigValStone > 1) {
            breakdown.p["Chrysotile"][`ᄂ${loc("space_red_ziggurat_title")}`] =
              (zigVal - 1) * 100 + "%";
          }
          breakdown.p["Chrysotile"][`ᄂ${loc("city_rock_quarry")}`] =
            (rock_quarry - 1) * 100 + "%";
          breakdown.p["Chrysotile"][`ᄂ${loc("power")}`] =
            (power_mult - 1) * 100 + "%";
          if (quarry_discharge) {
            breakdown.p["Chrysotile"][`ᄂ${loc("evo_challenge_discharge")}`] =
              "-50%";
          }
          breakdown.p["Chrysotile"][`ᄂ${loc("portal_tunneler_bd")}`] =
            (tunneler - 1) * 100 + "%";
          breakdown.p["Chrysotile"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
        }
      }
      ...
      if (forage_base > 0) {
        ...
        if (forage_base > 0) {
          breakdown.p["Stone"][`ᄂ${loc("city_rock_quarry")}+1`] =
            (rock_quarry - 1) * 100 + "%";
          breakdown.p["Stone"][`ᄂ${loc("power")}+1`] =
            (power_mult - 1) * 100 + "%";
          if (quarry_discharge) {
            breakdown.p["Stone"][`ᄂ${loc("evo_challenge_discharge")}+1`] =
              "-50%";
          }
          breakdown.p["Stone"][`ᄂ${loc("portal_tunneler_bd")}+1`] =
            (tunneler - 1) * 100 + "%";
          breakdown.p["Stone"][`ᄂ${loc("quarantine")}+1`] =
            (q_multiplier - 1) * 100 + "%";
        }
      }
      ...
      let delta =
        (stone_base * zigValStone + forage_base) * rock_quarry * tunneler;
      if (global.city["rock_quarry"]) {
        global.city.rock_quarry["cnvay"] = +(
          delta *
          hunger *
          q_multiplier *
          global_multiplier *
      ...
      if (
        ...
        (global.city["metal_refinery"].count > 0 ||
          global.race["cataclysm"] ||
          global.race["orbit_decayed"] ||
          global.race["warlord"])
      ) {
        ...
        if (base > 0) {
          ...
          if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
            breakdown.p["Aluminium"][`ᄂ${loc("space_red_ziggurat_title")}`] =
              (zigVal - 1) * 100 + "%";
          }
          breakdown.p["Aluminium"][`ᄂ${loc("city_rock_quarry")}+0`] =
            (rock_quarry - 1) * 100 + "%";
          breakdown.p["Aluminium"][`ᄂ${loc("power")}+0`] =
            (power_mult - 1) * 100 + "%";
          if (quarry_discharge) {
            breakdown.p["Aluminium"][`ᄂ${loc("evo_challenge_discharge")}+0`] =
              "-50%";
          }
          breakdown.p["Aluminium"][`ᄂ${loc("portal_tunneler_bd")}+0`] =
            (tunneler - 1) * 100 + "%";
          breakdown.p["Aluminium"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
        }
        ...
        if (forage_alum_base > 0) {
          ...
          breakdown.p["Aluminium"][`ᄂ${loc("city_rock_quarry")}+1`] =
            (rock_quarry - 1) * 100 + "%";
          breakdown.p["Aluminium"][`ᄂ${loc("power")}+1`] =
            (power_mult - 1) * 100 + "%";
          if (quarry_discharge) {
            breakdown.p["Aluminium"][`ᄂ${loc("evo_challenge_discharge")}+1`] =
              "-50%";
          }
          breakdown.p["Aluminium"][`ᄂ${loc("portal_tunneler_bd")}+1`] =
            (tunneler - 1) * 100 + "%";
          breakdown.p["Aluminium"][`ᄂ${loc("quarantine")}+1`] =
            (q_multiplier - 1) * 100 + "%";
        }
        ...
        delta *=
          ...
          q_multiplier *
          global_multiplier;
        global.city.metal_refinery["cnvay"] = +(
          delta *
          (power_single - 1)
        ).toFixed(5);
        ...
        breakdown.p["Aluminium"][loc("city_shrine")] =
          ((shrineMetal.mult - 1) * 100).toFixed(1) + "%";
    ...
    if (
      global.race["ocular_power"] &&
      global.race["ocularPowerConfig"] &&
      global.race.ocularPowerConfig.p &&
      global.race.ocularPowerConfig.ds > 0
    ) {
      if (
        !global.race.ocularPowerConfig.hasOwnProperty("ticks") ||
        global.race.ocularPowerConfig.ticks <= 0
      ) {
        global.race.ocularPowerConfig["dsl"] = Math.round(
          global.race.ocularPowerConfig.ds / 10,
        );
        global.race.ocularPowerConfig.ds = 0;
        global.race.ocularPowerConfig["ticks"] = Math.round(
          10 / time_multiplier,
        );
      }
    ...
    if (global.resource.Water.display) {
      if (support_on["water_freighter"]) {
        ...
        let base =
          production("water_freighter") *
          support_on["water_freighter"] *
          production("psychic_boost", "Water");
        let delta = base * hunger * global_multiplier * synd * zigVal;

        breakdown.p["Water"][loc("space_water_freighter_title")] = base + "v";
        if (base > 0) {
          breakdown.p["Water"][`ᄂ${loc("space_syndicate")}`] =
            -((1 - synd) * 100) + "%";
          breakdown.p["Water"][`ᄂ${loc("space_red_ziggurat_title")}`] =
            (zigVal - 1) * 100 + "%";
      ...
      if (
        global.tech["isolation"] &&
        global.tauceti["tau_farm"] &&
        p_on["tau_farm"]
      ) {
        let colony_val = 1 + (support_on["colony"] || 0) * 0.5;

        let base =
          production("tau_farm", "water") *
          p_on["tau_farm"] *
          production("psychic_boost", "Water");
        let delta = base * global_multiplier * colony_val;
        ...
        if (base > 0) {
          breakdown.p["Water"][`ᄂ${loc("tau_home_colony")}`] =
            (colony_val - 1) * 100 + "%";
        }
    ...
    if (global.eden["palace"] && p_on["spirit_vacuum"] && global.tech["isle"]) {
      ...
      if (global.tech.isle >= 6 && p_on["spirit_battery"]) {
        ...
        if (
          global.race["warlord"] &&
          global.eden["corruptor"] &&
          global.tech?.asphodel >= 13
        ) {
      ...
      if (
        ...
        global.eden.soul_compactor.count === 1
      ) {
    ...
    if (global.resource.Mana.display) {
          ...
          if (global.race.casting[spell]) {
            if (global.race.casting[spell] > 0) {
              ...
              if (consume_mana_dt > global.resource.Mana.amount) {
                active_rituals[spell] = maxRitualNum(
                  global.resource.Mana.amount,
                  time_multiplier,
                );
              } else {
                active_rituals[spell] = global.race.casting[spell];
              }
              breakdown.p.consume.Mana[loc(`modal_pylon_spell_${spell}`)] =
                -consume_mana;
      ...
      if (
        ...
        if (global.tech["nexus"]) {
          ...
          delta *= 1 + nexus / 100;
          breakdown.p["Mana"][`ᄂ${loc("arpa_projects_nexus_title")}`] =
            nexus + "%";
        }
      ...
      if (global.tech["cleric"] && global.civic.priest.display) {
        let mana_base =
          workerScale(global.civic.priest.workers, "priest") * 0.0025;
        if (global.race["high_pop"]) {
      ...
      if (global.race.universe === "magic" && global.civic.scientist.display) {
        let mana_base =
          workerScale(global.civic.scientist.workers, "scientist") * 0.025;
        if (global.race["high_pop"]) {
          mana_base = highPopAdjust(mana_base);
        }
        mana_base *= darkEffect("magic");
        ...
        if (global.civic.govern.type === "magocracy") {
          ...
          breakdown.p["Mana"][`ᄂ${loc("govern_magocracy")}`] =
            govEffect.magocracy()[0] + "%";
        }
    ...
    if (global.resource.Crystal.display) {
      let crystal_base = workerScale(
        global.civic.crystal_miner.workers,
        "crystal_miner",
      );
      ...
      crystal_base *=
        global.civic.crystal_miner.impact *
        production("psychic_boost", "Crystal");
    ...
    if (global.resource.Copper.display || global.resource.Iron.display) {
      ...
      if (global.race["industrious"]) {
        let bonus =
          1 + (traits.industrious.vars()[0] * global.race["industrious"]) / 100;
        miner_base *= bonus;
      }
      ...
      if (!global.race["living_tool"] && !global.race["tusk"]) {
        miner_base *=
          (global.tech["pickaxe"] && global.tech.pickaxe > 0
            ? global.tech.pickaxe * 0.15
            : 0) + 1;
      }
      if (global.tech["explosives"] && global.tech.explosives >= 2) {
      ...
      if (global.resource.Copper.display) {
        ...
        let copper_base =
          miner_base * copper_mult * production("psychic_boost", "Copper");
        ...
        if (copper_base > 0) {
          breakdown.p["Copper"][`ᄂ${loc("power")}`] =
            (copper_power - 1) * 100 + "%";
          breakdown.p["Copper"][`ᄂ${loc("portal_tunneler_bd")}`] =
            (tunneler - 1) * 100 + "%";
          breakdown.p["Copper"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
          if (
            global.race["discharge"] &&
            global.race["discharge"] > 0 &&
            ...
            breakdown.p["Copper"][`ᄂ${loc("evo_challenge_discharge")}`] =
              "-50%";
          }
        }
        let delta =
        ...
        if (global.race["forager"] && global.tech["dowsing"]) {
          ...
          let forage_base =
            foragers * forage * 0.025 * production("psychic_boost", "Copper");
          if (global.city.geology["Copper"]) {
          ...
          if (forage_base > 0) {
            breakdown.p["Copper"][`ᄂ${loc("quarantine")}+1`] =
              (q_multiplier - 1) * 100 + "%";
          }
          modRes(
            "Copper",
      ...
      if (global.resource.Iron.display) {
        let iron_mult = 1 / 4;
        let iron_base =
          miner_base * iron_mult * production("psychic_boost", "Iron");
        ...
        if (support_on["iron_ship"]) {
          space_iron =
            support_on["iron_ship"] *
            production("iron_ship") *
            production("psychic_boost", "Iron");
          space_iron *= synd;
        }
        ...
        if (iron_base > 0) {
          breakdown.p["Iron"][`ᄂ${loc("portal_tunneler_bd")}`] =
            (tunneler - 1) * 100 + "%";
          breakdown.p["Iron"][`ᄂ${loc("power")}`] =
            (iron_power - 1) * 100 + "%";
          if (
            global.race["discharge"] &&
            global.race["discharge"] > 0 &&
          ...
          breakdown.p["Iron"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
        }
        ...
        if (global.tech["isolation"] && global.race["lone_survivor"]) {
          {
            // Pit Miner
            let miner_base = workerScale(
              global.civic.pit_miner.workers,
              "pit_miner",
            );
            miner_base *= racialTrait(miner_base, "miner");
            let colony_val = 1 + (support_on["colony"] || 0) * 0.5;
            ...
            if (pit_base > 0) {
              breakdown.p["Iron"][`ᄂ${loc("tau_home_colony")}`] =
                (colony_val - 1) * 100 + "%";
            }
          }

          if (
            global.tauceti.hasOwnProperty("womling_mine") &&
            global.tauceti.hasOwnProperty("overseer")
          ) {
            // Womling Mine
            let prod = global.tauceti.overseer.prod / 100;
            let iron_base =
              global.tauceti.womling_mine.miners *
              production("womling_mine", "iron") *
              production("psychic_boost", "Iron");
            breakdown.p["Iron"][loc("tau_red_womlings")] = iron_base + "v";
            womling = iron_base * prod;

            if (iron_base > 0) {
              breakdown.p["Iron"][`ᄂ${loc("tau_red_womling_prod_label")}`] =
                -((1 - prod) * 100) + "%";
            }
          }
        }
        ...
        if (space_iron > 0) {
          breakdown.p["Iron"][`ᄂ${loc("space_syndicate")}`] =
            -((1 - synd) * 100) + "%";
          breakdown.p["Iron"][`ᄂ${loc("space_red_ziggurat_title")}`] =
            (zigVal - 1) * 100 + "%";
          breakdown.p["Iron"][`ᄂ${loc("quarantine")}+1`] =
            (qs_multiplier - 1) * 100 + "%";
        }
        if (global.race["gravity_well"]) {
        ...
        if (global.race["gravity_well"]) {
          breakdown.p["Iron"][`ᄂ${loc("evo_challenge_gravity_well")}+1`] =
            -((1 - teamster(1)) * 100) + "%";
        }

        let eship_iron = e_ship["iron"] ? e_ship.iron * womling_technician : 0;
        let delta =
          (iron_base * tunneler * iron_power * q_multiplier +
            space_iron * qs_multiplier * zigVal +
            eship_iron +
            pit_miner +
            womling) *
          smelter_mult *
          shrineMetal.mult;
        global.city.mine["ipow"] = +(
          iron_base *
          q_multiplier *
          hunger *
          global_multiplier *
        ...
        if (e_ship["iron"] && e_ship.iron > 0) {
          ...
          if (womling_technician > 1) {
            breakdown.p["Iron"][`ᄂ${loc("tau_red_womlings")}+0`] =
              (womling_technician - 1) * 100 + "%";
          }
        }

        breakdown.p["Iron"][loc("city_smelter")] =
          (smelter_mult - 1) * 100 + "%";
        breakdown.p["Iron"][loc("city_shrine")] =
          ((shrineMetal.mult - 1) * 100).toFixed(1) + "%";
        ...
        if (global.race["forager"] && global.tech["dowsing"]) {
          ...
          let forage_base =
            foragers * forage * 0.035 * production("psychic_boost", "Iron");
          if (global.city.geology["Iron"]) {
          ...
          if (forage_base > 0) {
            breakdown.p["Iron"][`ᄂ${loc("quarantine")}+2`] =
              (q_multiplier - 1) * 100 + "%";
          }
          modRes(
            "Iron",
        ...
        if (global.tech["titanium"] && global.tech["titanium"] >= 2) {
          let labor_base =
            highPopAdjust(workerScale(global.civic.miner.workers, "miner")) / 4;
          if (support_on["iron_ship"]) {
          ...
          let divisor = global.tech["titanium"] >= 3 ? 10 : 25;
          modRes("Titanium", (delta * time_multiplier) / divisor);
          breakdown.p["Titanium"][loc("resource_Iron_name")] =
            iron / divisor + "v";
        }
      }
      ...
      if (global.race["sappy"]) {
        ...
        if (
          ...
          breakdown.p["Aluminium"][
            `${global.race["cataclysm"] || global.race["orbit_decayed"] ? structName("mine") : jobName("miner")}+2`
          ] = base + "v";
          if (
            (global.race["cataclysm"] || global.race["orbit_decayed"]) &&
            base > 0 &&
        ...
        if (global.race["smoldering"] && global.resource.Chrysotile.display) {
          let cry_base =
            (miner_base / 2) * production("psychic_boost", "Chrysotile");
...

    {
      ...
      if (global.interstellar["mining_droid"] && miner_droids["alum"] > 0) {
        ...
        breakdown.p["Aluminium"][loc("interstellar_mining_droid_title")] =
          base + "v";
        if (base > 0) {
          breakdown.p["Aluminium"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
            (zigVal - 1) * 100 + "%";
        }
      ...
      if (global.resource.Aluminium.display && global.space["titan_mine"]) {
        ...
        let titan_colonists = p_on["ai_colonist"]
          ? workerScale(global.civic.titan_colonist.workers, "titan_colonist") +
            jobScale(p_on["ai_colonist"])
          : workerScale(global.civic.titan_colonist.workers, "titan_colonist");
        let alum_base =
          production("titan_mine", "aluminium") *
          support_on["titan_mine"] *
          titan_colonists *
          production("psychic_boost", "Aluminium");
        let alum_delta =
          alum_base *
        ...
        breakdown.p["Aluminium"][`${loc("city_mine")}+0`] =
          +alum_base.toFixed(3) + "v";
        if (alum_base > 0) {
          breakdown.p["Aluminium"][`ᄂ${loc("space_syndicate")}`] =
            -((1 - synd) * 100) + "%";
          breakdown.p["Aluminium"][`ᄂ${loc("space_red_ziggurat_title")}+2`] =
            (zigVal - 1) * 100 + "%";
          breakdown.p["Aluminium"][`ᄂ${loc("quarantine")}+2`] =
            (qs_multiplier - 1) * 100 + "%";
        }
      ...
      if (
        global.resource.Aluminium.display &&
        e_ship["aluminium"] &&
        e_ship.aluminium > 0
      ) {
        let alum_delta =
          ...
          womling_technician;
        alum_delta *= 1 + refinery / 100;
        breakdown.p["Aluminium"][loc("tau_roid_mining_ship")] =
          e_ship.aluminium + "v";
        if (womling_technician > 1) {
          breakdown.p["Aluminium"][`ᄂ${loc("tau_red_womlings")}+0`] =
            (womling_technician - 1) * 100 + "%";
        }
        modRes("Aluminium", alum_delta * time_multiplier);
      }
      ...
      if (refinery > 0) {
        ...
        breakdown.p["Aluminium"][`ᄂ${loc("quarantine")}+3`] =
          (q_multiplier - 1) * 100 + "%";
      }
    }
    ...
    if (support_on["red_mine"] && support_on["red_mine"] > 0) {
      ...
      let copper_base =
        support_on["red_mine"] *
        workerScale(global.civic.colonist.workers, "colonist") *
        production("red_mine", "copper").f;
      copper_base *= production("psychic_boost", "Copper");
      breakdown.p["Copper"][loc("space_red_mine_desc_bd", [planetName().red])] =
        copper_base + "v";
      if (copper_base > 0) {
        breakdown.p["Copper"][`ᄂ${loc("space_syndicate")}`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Copper"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Copper"][`ᄂ${loc("quarantine")}+1`] =
          (qs_multiplier - 1) * 100 + "%";
      }
      ...
      let titanium_base =
        ...
        workerScale(global.civic.colonist.workers, "colonist") *
        hunger *
        production("red_mine", "titanium").f;
      titanium_base *= production("psychic_boost", "Titanium");
      breakdown.p["Titanium"][
        loc("space_red_mine_desc_bd", [planetName().red])
      ] = titanium_base + "v";
      if (titanium_base > 0) {
        breakdown.p["Titanium"][`ᄂ${loc("space_syndicate")}`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Titanium"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Titanium"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
      }
      modRes(
        "Titanium",
    ...
    if (shrineBonusActive()) {
      breakdown.p["Copper"][loc("city_shrine")] =
        ((shrineMetal.mult - 1) * 100).toFixed(1) + "%";
      breakdown.p["Titanium"][loc("city_shrine")] =
        ((shrineMetal.mult - 1) * 100).toFixed(1) + "%";
    }
    breakdown.p["Copper"][loc("hunger")] = (hunger - 1) * 100 + "%";

    if (breakdown.p["Uranium"].hasOwnProperty(loc("city_coal_ash"))) {
      breakdown.p["Uranium"][loc("city_coal_ash")] =
        breakdown.p["Uranium"][loc("city_coal_ash")] + "v";
    }

    // Coal
    if (global.resource.Coal.display) {
      let coal_base = workerScale(
        global.race["warlord"]
          ? global.civic.miner.workers
          : global.civic.coal_miner.workers,
        "coal_miner",
      );
      ...
      if (global.race["resilient"]) {
        let bonus =
          1 + (traits.resilient.vars()[0] * global.race["resilient"]) / 100;
        coal_base *= bonus;
      }
      if (!global.race["living_tool"] && !global.race["tusk"]) {
        coal_base *=
          (global.tech["pickaxe"] && global.tech.pickaxe > 0
            ? global.tech.pickaxe * 0.12
            : 0) + 1;
      }
      if (global.tech["explosives"] && global.tech.explosives >= 2) {
      ...
      coal_base *=
        global.civic.coal_miner.impact * production("psychic_boost", "Coal");
      breakdown.p["Coal"][
        global.race["warlord"] ? jobName("miner") : jobName("coal_miner")
      ] = coal_base + "v";
      if (coal_base > 0) {
        breakdown.p["Coal"][`ᄂ${loc("power")}`] = (power_mult - 1) * 100 + "%";
        breakdown.p["Coal"][`ᄂ${loc("portal_tunneler_bd")}`] =
          (tunneler - 1) * 100 + "%";
        breakdown.p["Coal"][`ᄂ${loc("quarantine")}+0`] =
          (q_multiplier - 1) * 100 + "%";
      }
      ...
      if (global.race["cataclysm"] && support_on["iridium_mine"]) {
        coal_base =
          support_on["iridium_mine"] * production("iridium_mine", "coal");
        coal_base *= production("psychic_boost", "Coal");
        breakdown.p["Coal"][loc("space_moon_iridium_mine_title")] =
          coal_base + "v";
        if (coal_base > 0) {
          breakdown.p["Coal"][`ᄂ${loc("space_red_ziggurat_title")}`] =
            (zigVal - 1) * 100 + "%";
          breakdown.p["Coal"][`ᄂ${loc("quarantine")}+0`] =
            (q_multiplier - 1) * 100 + "%";
        }
      ...
      let delta =
        coal_base * tunneler * hunger * q_multiplier * global_multiplier;
      ...
      if (global.interstellar["mining_droid"] && miner_droids["coal"] > 0) {
        let driod_base =
          miner_droids["coal"] * 3.75 * production("psychic_boost", "Coal");
        let driod_delta = driod_base * global_multiplier * zigVal;
        breakdown.p["Coal"][loc("interstellar_mining_droid_title")] =
          driod_base + "v";
        if (driod_base > 0) {
          breakdown.p["Coal"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
            (zigVal - 1) * 100 + "%";
        }
      ...
      if (global.resource.Uranium.display) {
        let uranium =
          (delta / (global.race["cataclysm"] ? 48 : 115)) *
          production("psychic_boost", "Uranium");
        global.city.coal_mine["upow"] = +(
          global.city.coal_mine["cpow"] / (global.race["cataclysm"] ? 48 : 115)
        ).toFixed(5);
        if (global.city.geology["Uranium"]) {
        ...
        breakdown.p["Uranium"][
          global.race["cataclysm"]
            ? loc("space_moon_iridium_mine_title")
            : global.race["warlord"]
              ? jobName("miner")
              : jobName("coal_miner")
        ] = uranium / global_multiplier + "v";

        let u_delta = uranium * tunneler;
        if (u_delta > 0) {
          breakdown.p["Uranium"][`ᄂ${loc("portal_tunneler_bd")}`] =
            (tunneler - 1) * 100 + "%";
        }
    ...
    if (
      global.race["warlord"] &&
      global.portal["tunneler"] &&
      global.portal.tunneler.count > 0
    ) {
      ...
      ["Neutronium", "Adamantite", "Bolognium", "Orichalcum"].forEach(
        function (res) {
          ...
          if (cur_base > 0) {
            breakdown.p[res][`ᄂ${loc("portal_tunneler_bd")}`] =
              (tunneler - 1) * 100 + "%";
    ...
    if (global.interstellar["mining_droid"] && miner_droids["uran"] > 0) {
      let driod_base =
        miner_droids["uran"] * 0.12 * production("psychic_boost", "Uranium");
      let driod_delta = driod_base * global_multiplier * zigVal;
      breakdown.p["Uranium"][loc("interstellar_mining_droid_title")] =
        driod_base + "v";
      if (driod_base > 0) {
        breakdown.p["Uranium"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
      }
    ...
    if (global.space["uranium_mine"] && p_on["uranium_mine"]) {
      ...
      let mine_base =
        p_on["uranium_mine"] *
        production("uranium_mine") *
        production("psychic_boost", "Uranium");
      let mine_delta =
        mine_base * global_multiplier * qs_multiplier * synd * zigVal;
      breakdown.p["Uranium"][
        loc("space_kuiper_mine", [global.resource.Uranium.name])
      ] = mine_base + "v";
      if (mine_base > 0) {
        breakdown.p["Uranium"][`ᄂ${loc("space_syndicate")}`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Uranium"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Uranium"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
      }
    ...
    if (global.resource.Oil.display) {
      ...
      if (global.tauceti["whaling_station"] && global.tauceti["whaling_ship"]) {
        global.tauceti.whaling_station.max =
          global.tauceti.whaling_station.count * 750;
      ...
      let fueled_oil_wells = global.race["warlord"]
        ? global.portal.pumpjack.count
      ...
      let extract_prod = global.space["oil_extractor"]
        ? production("oil_extractor")
        : 0;
      ...
      let delta =
        oil_well * oil_prod_mod +
        oil_extractor * extract_prod_mod +
        whale_oil * womling_technician;
      delta *= hunger * global_multiplier;
      if (global.race["gravity_well"]) {
        delta = teamster(delta);
      }
      ...
      if (global.space["oil_extractor"]) {
        global.space.oil_extractor["lpmod"] =
          production("oil_extractor") * qs_multiplier * synd * zigVal;
      }

      breakdown.p["Oil"][
        global.race["warlord"]
          ? loc("portal_pumpjack_title")
          : loc("city_oil_well")
      ] = oil_well + "v";
      if (oil_well > 0) {
        breakdown.p["Oil"][`ᄂ${loc("quarantine")}+0`] =
          (q_multiplier - 1) * 100 + "%";
      }
      breakdown.p["Oil"][loc("space_gas_moon_oil_extractor_title")] =
        oil_extractor + "v";
      if (oil_extractor > 0) {
        breakdown.p["Oil"][`ᄂ${loc("space_syndicate")}`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Oil"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Oil"][`ᄂ${loc("quarantine")}+1`] =
          (qs_multiplier - 1) * 100 + "%";
      }
      breakdown.p["Oil"][loc("tau_roid_whaling_ship")] = whale_oil + "v";
      if (womling_technician > 1) {
        breakdown.p["Oil"][`ᄂ${loc("tau_red_womlings")}+0`] =
          (womling_technician - 1) * 100 + "%";
      }
      if (global.race["gravity_well"]) {
        breakdown.p["Oil"][`${loc("evo_challenge_gravity_well")}+0`] =
          -((1 - teamster(1)) * 100) + "%";
      }
    ...
    if (support_on["iridium_mine"] || global.race["warlord"]) {
      ...
      if (global.race["warlord"]) {
        ...
        iridium_base =
          support_on["iridium_mine"] * production("iridium_mine", "iridium").f;
      }
      ...
      if (iridium_base > 0) {
        breakdown.p["Iridium"][`ᄂ${loc("city_smelter")}+0`] =
          iridium_smelter * 100 + "%";
        breakdown.p["Iridium"][`ᄂ${loc("portal_tunneler_bd")}+0`] =
          (tunneler - 1) * 100 + "%";
        breakdown.p["Iridium"][`ᄂ${loc("space_syndicate")}+0`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Iridium"][`ᄂ${loc("space_red_ziggurat_title")}+0`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Iridium"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
        if (global.race["gravity_well"]) {
          breakdown.p["Iridium"][`ᄂ${loc("evo_challenge_gravity_well")}+0`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      }
    ...
    if (support_on["iridium_ship"]) {
      let iridium_base =
        support_on["iridium_ship"] * production("iridium_ship");
      iridium_base *= production("psychic_boost", "Iridium");
      let synd = syndicate("spc_belt");
      let delta =
        iridium_base *
      ...
      if (iridium_base > 0) {
        breakdown.p["Iridium"][`ᄂ${loc("city_smelter")}+1`] =
          iridium_smelter * 100 + "%";
        breakdown.p["Iridium"][`ᄂ${loc("space_syndicate")}+1`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Iridium"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Iridium"][`ᄂ${loc("quarantine")}+1`] =
          (qs_multiplier - 1) * 100 + "%";
        if (global.race["gravity_well"]) {
          breakdown.p["Iridium"][`ᄂ${loc("evo_challenge_gravity_well")}+1`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      }
      modRes("Iridium", delta * time_multiplier);
    ...
    if (
      p_on["s_gate"] &&
      global.resource.Adamantite.display &&
      global.galaxy["armed_miner"] &&
      gal_on["armed_miner"] > 0
    ) {
      let base =
        gal_on["armed_miner"] * 0.65 * production("psychic_boost", "Iridium");
      ...
      if (base > 0) {
        breakdown.p["Iridium"][`ᄂ${loc("galaxy_ore_processor")}`] =
          -((1 - foothold) * 100) + "%";
        breakdown.p["Iridium"][`ᄂ${loc("city_smelter")}+2`] =
          iridium_smelter * 100 + "%";
        breakdown.p["Iridium"][`ᄂ${loc("galaxy_piracy")}`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Iridium"][`ᄂ${loc("space_red_ziggurat_title")}+2`] =
          (zigVal - 1) * 100 + "%";
        if (global.race["gravity_well"]) {
          breakdown.p["Iridium"][`ᄂ${loc("evo_challenge_gravity_well")}+2`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      }
      modRes("Iridium", delta * time_multiplier);
    ...
    if (
      global.resource.Iridium.display &&
      e_ship["iridium"] &&
      e_ship.iridium > 0
    ) {
      let iridium_delta =
      ...
      breakdown.p["Iridium"][loc("tau_roid_mining_ship")] =
        e_ship.iridium + "v";
      breakdown.p["Iridium"][`ᄂ${loc("city_smelter")}+3`] =
        iridium_smelter * 100 + "%";
      if (womling_technician > 1) {
        breakdown.p["Iridium"][`ᄂ${loc("tau_red_womlings")}+0`] =
          (womling_technician - 1) * 100 + "%";
      }
      if (global.race["gravity_well"]) {
        breakdown.p["Iridium"][`ᄂ${loc("evo_challenge_gravity_well")}+3`] =
          -((1 - teamster(1)) * 100) + "%";
      }
    ...
    // Helium 3
    if (
      (global.space["moon_base"] && support_on["helium_mine"]) ||
      global.race["warlord"]
    ) {
      let helium_base =
        (global.race["warlord"]
          ? global.portal.pumpjack.count
          : support_on["helium_mine"]) * production("helium_mine").f;
      ...
      breakdown.p["Helium_3"][
        global.race["warlord"]
          ? loc("portal_pumpjack_title")
          : loc("space_moon_helium_mine_title")
      ] = helium_base + "v";
      if (helium_base > 0) {
        breakdown.p["Helium_3"][`ᄂ${loc("space_syndicate")}+0`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Helium_3"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Helium_3"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
        if (global.race["gravity_well"]) {
          breakdown.p["Helium_3"][`ᄂ${loc("evo_challenge_gravity_well")}+0`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      }
    ...
    if (global.space["gas_mining"] && p_on["gas_mining"]) {
      ...
      let delta =
        gas_mining * hunger * global_multiplier * synd * qs_multiplier * zigVal;
      if (global.race["gravity_well"]) {
        delta = teamster(delta);
      }
      ...
      if (gas_mining > 0) {
        breakdown.p["Helium_3"][`ᄂ${loc("space_syndicate")}+1`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Helium_3"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Helium_3"][`ᄂ${loc("quarantine")}+1`] =
          (qs_multiplier - 1) * 100 + "%";
        if (global.race["gravity_well"]) {
          breakdown.p["Helium_3"][`ᄂ${loc("evo_challenge_gravity_well")}+1`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      }
      modRes("Helium_3", delta * time_multiplier);
    ...
    if (p_on["refueling_station"]) {
      let gas_mining =
        p_on["refueling_station"] * production("refueling_station");
      gas_mining *= production("psychic_boost", "Helium_3");
      let delta = gas_mining * hunger * global_multiplier * womling_technician;

      breakdown.p["Helium_3"][loc("tau_gas_refueling_station_title")] =
        gas_mining + "v";
      if (womling_technician > 1) {
        breakdown.p["Helium_3"][`ᄂ${loc("tau_red_womlings")}+0`] =
          (womling_technician - 1) * 100 + "%";
      }
      modRes("Helium_3", delta * time_multiplier);
    }
    ...
    if (global.interstellar["harvester"] && int_on["harvester"]) {
      ...
      breakdown.p["Helium_3"][loc("interstellar_harvester_title")] =
        gas_mining + "v";
      if (gas_mining > 0) {
        breakdown.p["Helium_3"][`ᄂ${loc("space_red_ziggurat_title")}+2`] =
          (zigVal - 1) * 100 + "%";
        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.5;
          breakdown.p["Helium_3"][`ᄂ${loc("evo_challenge_discharge")}`] =
            "-50%";
        }
      }
      ...
      if (global.tech["ram_scoop"]) {
        let deut_mining =
          int_on["harvester"] * production("harvester", "deuterium");
        deut_mining *= production("psychic_boost", "Deuterium");
        let deut_delta = deut_mining * hunger * global_multiplier * zigVal;

        breakdown.p["Deuterium"][loc("interstellar_harvester_title")] =
          deut_mining + "v";
        if (deut_mining > 0) {
          breakdown.p["Deuterium"][`ᄂ${loc("space_red_ziggurat_title")}`] =
            (zigVal - 1) * 100 + "%";
        }
    ...
    if (p_on["s_gate"] && global.galaxy["raider"] && gal_on["raider"] > 0) {
      let base =
        gal_on["raider"] * 0.65 * production("psychic_boost", "Deuterium");
      let pirate = piracy("gxy_chthonian");
      let delta = base * global_multiplier * pirate * hunger * zigVal;

      breakdown.p["Deuterium"][loc("galaxy_raider")] = base + "v";
      if (base > 0) {
        breakdown.p["Deuterium"][`ᄂ${loc("galaxy_piracy")}`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Deuterium"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
      }
    ...
    if (p_on["outpost"]) {
      ...
      breakdown.p["Neutronium"][loc("space_gas_moon_outpost_bd")] =
        p_values.b * psy * p_on["outpost"] + "v";
      if (global.tech["drone"]) {
        breakdown.p["Neutronium"][`ᄂ${loc("tech_worker_drone")}`] =
          p_values.d * 100 + "%";
      }
      let synd = syndicate("spc_gas_moon");
      ...
      if (p_values.b > 0) {
        breakdown.p["Neutronium"][`ᄂ${loc("space_syndicate")}+0`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Neutronium"][`ᄂ${loc("space_red_ziggurat_title")}+0`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Neutronium"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.5;
          global.space.outpost["lpmod"] *= 0.5;
          breakdown.p["Neutronium"][`ᄂ${loc("evo_challenge_discharge")}+0`] =
            "-50%";
        }
        if (global.race["gravity_well"]) {
          breakdown.p["Neutronium"][
            `ᄂ${loc("evo_challenge_gravity_well")}+0`
          ] = -((1 - teamster(1)) * 100) + "%";
        }
      }
    ...
    if (p_on["neutron_miner"]) {
      let n_base =
        p_on["neutron_miner"] *
        production("neutron_miner") *
        production("psychic_boost", "Neutronium");
      let delta = n_base * hunger * global_multiplier * zigVal;
      breakdown.p["Neutronium"][loc("interstellar_neutron_miner_bd")] =
        n_base + "v";
      global.interstellar.neutron_miner["lpmod"] =
        production("neutron_miner") * hunger * global_multiplier * zigVal;

      if (n_base > 0) {
        breakdown.p["Neutronium"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.5;
          global.interstellar.neutron_miner["lpmod"] *= 0.5;
          breakdown.p["Neutronium"][`ᄂ${loc("evo_challenge_discharge")}+1`] =
            "-50%";
        }
      }
    ...
    if (p_on["s_gate"] && global.galaxy["raider"] && gal_on["raider"] > 0) {
      let base =
        gal_on["raider"] * 0.8 * production("psychic_boost", "Neutronium");
      let pirate = piracy("gxy_chthonian");
      let delta = base * global_multiplier * pirate * hunger * zigVal;

      breakdown.p["Neutronium"][loc("galaxy_raider")] = base + "v";
      if (base > 0) {
        breakdown.p["Neutronium"][`ᄂ${loc("galaxy_piracy")}`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Neutronium"][`ᄂ${loc("space_red_ziggurat_title")}+2`] =
          (zigVal - 1) * 100 + "%";
      }
      modRes("Neutronium", delta * time_multiplier);
    }
    ...
    if (global.space["neutronium_mine"] && p_on["neutronium_mine"]) {
      ...
      let mine_base =
        p_on["neutronium_mine"] *
        production("neutronium_mine") *
        production("psychic_boost", "Neutronium");
      let mine_delta =
        mine_base * global_multiplier * qs_multiplier * synd * zigVal;
      breakdown.p["Neutronium"][
      ...
      if (mine_base > 0) {
        breakdown.p["Neutronium"][`ᄂ${loc("space_syndicate")}+1`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Neutronium"][`ᄂ${loc("space_red_ziggurat_title")}+3`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Neutronium"][`ᄂ${loc("quarantine")}+1`] =
          (qs_multiplier - 1) * 100 + "%";
      }
    ...
    if (
      global.resource.Neutronium.display &&
      e_ship["neutronium"] &&
      e_ship.neutronium > 0
    ) {
      let neutronium_delta =
        e_ship.neutronium * global_multiplier * womling_technician;
      breakdown.p["Neutronium"][loc("tau_roid_mining_ship")] =
        e_ship.neutronium + "v";
      if (womling_technician > 1) {
        breakdown.p["Neutronium"][`ᄂ${loc("tau_red_womlings")}+0`] =
          (womling_technician - 1) * 100 + "%";
      }
    ...
    if (support_on["elerium_ship"]) {
      let elerium_base =
        support_on["elerium_ship"] *
        production("elerium_ship") *
        production("psychic_boost", "Elerium");
      let synd = syndicate("spc_belt");
      let delta =
        elerium_base *
      ...
      if (elerium_base > 0) {
        breakdown.p["Elerium"][`ᄂ${loc("space_syndicate")}+0`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Elerium"][`ᄂ${loc("space_red_ziggurat_title")}+0`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Elerium"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.75;
          breakdown.p["Elerium"][`ᄂ${loc("evo_challenge_discharge")}`] =
            "-25%";
        }
        if (global.race["gravity_well"]) {
          breakdown.p["Elerium"][`ᄂ${loc("evo_challenge_gravity_well")}+1`] =
            -((1 - teamster(1)) * 100) + "%";
        }
      }
    ...
    if (int_on["elerium_prospector"]) {
      let elerium_base =
        int_on["elerium_prospector"] *
        production("elerium_prospector") *
        production("psychic_boost", "Elerium");
      let delta = elerium_base * hunger * global_multiplier * zigVal;
      breakdown.p["Elerium"][loc("interstellar_elerium_prospector_bd")] =
        elerium_base + "v";
      if (elerium_base > 0) {
        breakdown.p["Elerium"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
      }
      modRes("Elerium", delta * time_multiplier);
    }
    ...
    if (global.space["elerium_mine"] && p_on["elerium_mine"]) {
      ...
      let mine_base =
        p_on["elerium_mine"] *
        production("elerium_mine") *
        production("psychic_boost", "Elerium");
      let mine_delta =
        mine_base * global_multiplier * qs_multiplier * synd * hunger * zigVal;
      breakdown.p["Elerium"][
        loc("space_kuiper_mine", [global.resource.Elerium.name])
      ] = mine_base + "v";
      if (mine_base > 0) {
        breakdown.p["Elerium"][`ᄂ${loc("space_syndicate")}+1`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Elerium"][`ᄂ${loc("space_red_ziggurat_title")}+2`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Elerium"][`ᄂ${loc("quarantine")}+1`] =
          (qs_multiplier - 1) * 100 + "%";
      }
      modRes("Elerium", mine_delta * time_multiplier);
    }
    ...
    if (
      global.resource.Elerium.display &&
      e_ship["elerium"] &&
      e_ship.elerium > 0
    ) {
      let elerium_delta =
        e_ship.elerium * global_multiplier * womling_technician;
      breakdown.p["Elerium"][loc("tau_roid_mining_ship")] =
        e_ship.elerium + "v";
      if (womling_technician > 1) {
        breakdown.p["Elerium"][`ᄂ${loc("tau_red_womlings")}+0`] =
          (womling_technician - 1) * 100 + "%";
      }
    ...
    if (
      global.resource.Adamantite.display &&
      global.interstellar["mining_droid"] &&
      miner_droids["adam"] > 0
    ) {
      let driod_base =
        miner_droids["adam"] *
        0.075 *
        production("psychic_boost", "Adamantite");
      let driod_delta =
        driod_base * shrineMetal.mult * global_multiplier * zigVal;
      breakdown.p["Adamantite"][loc("interstellar_mining_droid_title")] =
        driod_base + "v";
      if (driod_base > 0) {
        if (global.interstellar["processing"] && int_on["processing"]) {
          ...
          if (
            global.tech["ai_core"] &&
            global.tech["ai_core"] >= 2 &&
            p_on["citadel"] > 0
          ) {
            rate += p_on["citadel"] * 0.02;
          }
          let bonus = int_on["processing"] * rate;
          breakdown.p["Adamantite"][
            `ᄂ${loc("interstellar_processing_title")}`
          ] = bonus * 100 + "%";

          if (global.race["discharge"] && global.race["discharge"] > 0) {
            bonus *= 0.5;
            breakdown.p["Adamantite"][`ᄂ${loc("evo_challenge_discharge")}`] =
              "-50%";
          }
          driod_delta *= 1 + bonus;
        }
        breakdown.p["Adamantite"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
      }
    ...
    if (
      ...
      if (base > 0) {
        breakdown.p["Adamantite"][`ᄂ${loc("galaxy_ore_processor")}`] =
          -((1 - foothold) * 100) + "%";
        breakdown.p["Adamantite"][`ᄂ${loc("galaxy_piracy")}`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Adamantite"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
      }
    ...
    if (global.resource.Adamantite.display && global.space["titan_mine"]) {
      ...
      let titan_colonists = p_on["ai_colonist"]
        ...
        : workerScale(global.civic.titan_colonist.workers, "titan_colonist");
      let adam_base =
        production("titan_mine", "adamantite") *
        support_on["titan_mine"] *
        titan_colonists *
        production("psychic_boost", "Adamantite");
      let adam_delta =
      ...
      if (adam_base > 0) {
        breakdown.p["Adamantite"][`ᄂ${loc("space_syndicate")}`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Adamantite"][`ᄂ${loc("space_red_ziggurat_title")}+2`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Adamantite"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
      }
    ...
    // Infernite
    if (global.resource.Infernite.display) {
      let workers = global.race["warlord"]
        ? global.tech?.hellspawn >= 2 && global.portal?.tunneler?.count >= 1
          ? global.civic.miner.workers
          : 0
        : global.civic.hell_surveyor.workers;
      if (workers > 0) {
        ...
        let surveyor_base =
          workerScale(highPopAdjust(workers), "hell_surveyor") *
          rate *
          production("psychic_boost", "Infernite");

        let sensors = 1;
        if (global.tech["infernite"] >= 2 && p_on["sensor_drone"]) {
          let drone_rate =
            global.tech.infernite >= 4
        ...
        let surveyor_delta =
          surveyor_base * tunneler * sensors * runBonus * global_multiplier;

        breakdown.p["Infernite"][
          global.race["warlord"] ? jobName("miner") : jobName("hell_surveyor")
        ] = surveyor_base + "v";
        breakdown.p["Infernite"][`ᄂ${loc("portal_sensor_drone_title")}`] =
          (sensors - 1) * 100 + "%";
        breakdown.p["Infernite"][`ᄂ${loc("portal_tunneler_bd")}`] =
          (tunneler - 1) * 100 + "%";
    ...
    if (
      p_on["s_gate"] &&
      global.resource.Bolognium.display &&
      global.galaxy["bolognium_ship"] &&
      gal_on["bolognium_ship"] > 0
    ) {
      let base =
        gal_on["bolognium_ship"] *
        production("bolognium_ship") *
        production("psychic_boost", "Bolognium");
      ...
      if (base > 0) {
        breakdown.p["Bolognium"][`ᄂ${loc("galaxy_piracy")}+0`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Bolognium"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.5;
          breakdown.p["Bolognium"][`ᄂ${loc("evo_challenge_discharge")}+0`] =
            "-50%";
        }
      }
    ...
    if (global.eden["asphodel_harvester"] && support_on["asphodel_harvester"]) {
      let powder_base =
        support_on["asphodel_harvester"] *
        production("asphodel_harvester", "powder");
      powder_base *= production("psychic_boost", "Asphodel_Powder");
      let delta = powder_base * hunger * global_multiplier;

      breakdown.p["Asphodel_Powder"][loc("eden_asphodel_harvester_title")] =
        powder_base + "v";
      ...
      if (global.tech.asphodel >= 5) {
        ...
        delta *= penalty;
        if (penalty <= 1) {
          breakdown.p["Asphodel_Powder"][
            `ᄂ${loc("eden_asphodel_hostility")}+0`
          ] = -((1 - penalty) * 100) + "%";
        } else {
          breakdown.p["Asphodel_Powder"][
            `ᄂ${loc("eden_mech_station_overkill")}+0`
          ] = (penalty - 1) * 100 + "%";
        }
      }
    ...
    if (global.resource.Elysanite.display) {
      if (global.civic.elysium_miner.display) {
        let miner_base = workerScale(
          global.civic.elysium_miner.workers,
          "elysium_miner",
        );
    ...
    if (global.civic.pit_miner.display) {
      if (tauEnabled()) {
...

        {
          ...
          if (bol_base > 0) {
            breakdown.p["Bolognium"][`ᄂ${loc("tau_home_colony")}`] =
              (colony_val - 1) * 100 + "%";
          }
...

        {
          ...
          if (stone_base > 0) {
            breakdown.p["Stone"][`ᄂ${loc("tau_home_colony")}`] =
              (colony_val - 1) * 100 + "%";
          }
        ...
        if (global.race["smoldering"]) {
          ...
          if (cry_base > 0) {
            breakdown.p["Chrysotile"][`ᄂ${loc("tau_home_colony")}`] =
              (colony_val - 1) * 100 + "%";
            breakdown.p["Chrysotile"][loc("hunger")] = (hunger - 1) * 100 + "%";
          }
          modRes("Chrysotile", delta * time_multiplier);
...

        {
          // Adamantite
          let adam_base =
            miner_base * production("psychic_boost", "Adamantite");
          adam_base *= production("mining_pit", "adamantite");
          let delta =
            adam_base * shrineMetal.mult * global_multiplier * colony_val;

          breakdown.p["Adamantite"][jobName("pit_miner")] = adam_base + "v";
          if (adam_base > 0) {
            breakdown.p["Adamantite"][`ᄂ${loc("tau_home_colony")}`] =
              (colony_val - 1) * 100 + "%";
          }
        ...
        if (global.tech["isolation"]) {
          {
            // Copper
            let copper_base =
              miner_base * production("psychic_boost", "Copper");
            copper_base *= production("mining_pit", "copper");
            let delta =
              copper_base * shrineMetal.mult * global_multiplier * colony_val;

            breakdown.p["Copper"][jobName("pit_miner")] = copper_base + "v";
            if (copper_base > 0) {
              breakdown.p["Copper"][`ᄂ${loc("tau_home_colony")}`] =
                (colony_val - 1) * 100 + "%";
            }
...

          {
            ...
            if (coal_base > 0) {
              breakdown.p["Coal"][`ᄂ${loc("tau_home_colony")}`] =
                (colony_val - 1) * 100 + "%";
            }
          ...
          if (global.race["lone_survivor"]) {
            // Aluminium
            let alum_base =
              miner_base * production("psychic_boost", "Aluminium");
            alum_base *= production("mining_pit", "aluminium");
            let delta =
              alum_base * shrineMetal.mult * global_multiplier * colony_val;

            breakdown.p["Aluminium"][jobName("pit_miner")] = alum_base + "v";
            if (alum_base > 0) {
              breakdown.p["Aluminium"][`ᄂ${loc("tau_home_colony")}`] =
                (colony_val - 1) * 100 + "%";
            }
        ...
        if (miner_base > 0) {
          materials_bd[`ᄂ${loc("tau_home_colony")}`] =
            (colony_val - 1) * 100 + "%";
        }
    ...
    if (global.tauceti["tau_farm"] && p_on["tau_farm"]) {
      ...
      if (!global.race["kindling_kindred"] && !global.race["smoldering"]) {
        ...
        if (lumber_base > 0) {
          breakdown.p["Lumber"][`ᄂ${loc("tau_home_colony")}`] =
            (colony_val - 1) * 100 + "%";
        }
    ...
    if (shrineBonusActive()) {
      breakdown.p["Adamantite"][loc("city_shrine")] =
        ((shrineMetal.mult - 1) * 100).toFixed(1) + "%";
    }
    ...
    if (
      ...
      if (base > 0) {
        breakdown.p["Bolognium"][`ᄂ${loc("galaxy_ore_processor")}`] =
          -((1 - foothold) * 100) + "%";
        breakdown.p["Bolognium"][`ᄂ${loc("galaxy_piracy")}+1`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Bolognium"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.5;
          breakdown.p["Bolognium"][`ᄂ${loc("evo_challenge_discharge")}+1`] =
            "-50%";
        }
      }
    ...
    if (
      p_on["s_gate"] &&
      global.resource.Orichalcum.display &&
      global.galaxy["excavator"] &&
      p_on["excavator"] > 0
    ) {
      let base =
        p_on["excavator"] *
      ...
      let pirate = piracy("gxy_chthonian");
      let delta = base * global_multiplier * pirate * zigVal;
      global.galaxy.excavator["lpmod"] =
        production("excavator") * global_multiplier * pirate * zigVal;

      breakdown.p["Orichalcum"][loc("galaxy_excavator")] = base + "v";
      if (base > 0) {
        breakdown.p["Orichalcum"][`ᄂ${loc("galaxy_piracy")}`] =
          -((1 - pirate) * 100) + "%";
        breakdown.p["Orichalcum"][`ᄂ${loc("space_red_ziggurat_title")}`] =
          (zigVal - 1) * 100 + "%";
        if (global.race["discharge"] && global.race["discharge"] > 0) {
          delta *= 0.5;
          global.galaxy.excavator["lpmod"] *= 0.5;
          breakdown.p["Orichalcum"][`ᄂ${loc("evo_challenge_discharge")}`] =
            "-50%";
        }
      }
    ...
    if (global.space["orichalcum_mine"] && p_on["orichalcum_mine"]) {
      ...
      let mine_base =
        p_on["orichalcum_mine"] *
        production("orichalcum_mine") *
        production("psychic_boost", "Orichalcum");
      let mine_delta =
        mine_base * global_multiplier * qs_multiplier * synd * zigVal;
      breakdown.p["Orichalcum"][
        loc("space_kuiper_mine", [global.resource.Orichalcum.name])
      ] = mine_base + "v";
      if (mine_base > 0) {
        breakdown.p["Orichalcum"][`ᄂ${loc("space_syndicate")}`] =
          -((1 - synd) * 100) + "%";
        breakdown.p["Orichalcum"][`ᄂ${loc("space_red_ziggurat_title")}+1`] =
          (zigVal - 1) * 100 + "%";
        breakdown.p["Orichalcum"][`ᄂ${loc("quarantine")}+0`] =
          (qs_multiplier - 1) * 100 + "%";
      }
    ...
    if (
      ...
      let orichalcum_delta =
        e_ship.orichalcum * global_multiplier * womling_technician;
      breakdown.p["Orichalcum"][loc("tau_roid_mining_ship")] =
        e_ship.orichalcum + "v";
      if (womling_technician > 1) {
        breakdown.p["Orichalcum"][`ᄂ${loc("tau_red_womlings")}+0`] =
          (womling_technician - 1) * 100 + "%";
      }
    ...
    // Womling Production
    if (
      global.race["truepath"] &&
      global.tech["tau_red"] &&
      global.tech.tau_red >= 5
    ) {
      if (global.tauceti["womling_mine"] && global.tauceti["overseer"]) {
        let miner_base =
          global.tauceti.womling_mine.miners *
          production("womling_mine", "unobtainium") *
          production("psychic_boost", "Unobtainium");
        let prod = global.tauceti.overseer.prod / 100;
        let miner_delta = miner_base * prod * global_multiplier;

        breakdown.p["Unobtainium"][loc("tau_red_womlings")] = miner_base + "v";
        if (miner_base > 0) {
          breakdown.p["Unobtainium"][`ᄂ${loc("tau_red_womling_prod_label")}`] =
            -((1 - prod) * 100) + "%";
        }
        ...
        if (global.tech["isolation"]) {
          ...
          let titanium_base =
            global.tauceti.womling_mine.miners *
            production("womling_mine", "titanium") *
            production("psychic_boost", "Titanium");
          breakdown.p["Titanium"][loc("tau_red_womlings")] =
            titanium_base + "v";
          let titanium_delta =
            titanium_base * prod * shrineMetal.mult * global_multiplier;

          if (titanium_base > 0) {
            breakdown.p["Titanium"][`ᄂ${loc("tau_red_womling_prod_label")}`] =
              -((1 - prod) * 100) + "%";
          }
          ...
          if (global.race["lone_survivor"]) {
            let copper_base =
              global.tauceti.womling_mine.miners *
              production("womling_mine", "copper") *
              production("psychic_boost", "Copper");
            breakdown.p["Copper"][loc("tau_red_womlings")] = copper_base + "v";
            let copper_delta =
              copper_base * prod * shrineMetal.mult * global_multiplier;

            if (copper_delta > 0) {
              breakdown.p["Copper"][`ᄂ${loc("tau_red_womling_prod_label")}`] =
                -((1 - prod) * 100) + "%";
            }
            ...
            let alumina_base =
              global.tauceti.womling_mine.miners *
              production("womling_mine", "aluminium") *
              production("psychic_boost", "Aluminium");
            breakdown.p["Aluminium"][loc("tau_red_womlings")] =
              alumina_base + "v";
            let alumina_delta =
              alumina_base * prod * shrineMetal.mult * global_multiplier;

            if (alumina_base > 0) {
              breakdown.p["Aluminium"][
                `ᄂ${loc("tau_red_womling_prod_label")}`
              ] = -((1 - prod) * 100) + "%";
            }
            ...
            let iridium_base =
              global.tauceti.womling_mine.miners *
              production("womling_mine", "iridium") *
              production("psychic_boost", "Iridium");
            breakdown.p["Iridium"][loc("tau_red_womlings")] =
              iridium_base + "v";
            let iridium_delta =
              iridium_base *
              prod *
            ...
            if (iridium_base > 0) {
              breakdown.p["Iridium"][`ᄂ${loc("tau_red_womling_prod_label")}`] =
                -((1 - prod) * 100) + "%";
            }
            modRes("Iridium", iridium_delta * time_multiplier);
            ...
            let neutronium_base =
              global.tauceti.womling_mine.miners *
              production("womling_mine", "neutronium") *
              production("psychic_boost", "Neutronium");
            breakdown.p["Neutronium"][loc("tau_red_womlings")] =
              neutronium_base + "v";
            let neutronium_delta =
              neutronium_base * prod * hunger * global_multiplier;
    ...
    if (shrineBonusActive()) {
      breakdown.p["Iridium"][loc("city_shrine")] =
        ((shrineMetal.mult - 1) * 100).toFixed(1) + "%";
    }
    ...
    if (global.tech["currency"] >= 1) {
      ...
      let temple_mult = 1;
      if (
        global.tech["anthropology"] &&
        global.tech["anthropology"] >= 4 &&
        !global.race["truepath"]
      ) {
      ...
      let delta =
        (income_base - upkeep) * temple_mult * hunger * getShrineResult.mult;
      delta *= global_multiplier;

      breakdown.p["Money"][loc("morale_tax")] = income_base + "v";
      if (income_base > 0) {
        breakdown.p["Money"][`ᄂ${loc("civics_spy_purchase_bd")}`] =
          -upkeep + "v";
        breakdown.p["Money"][
          global.race["cataclysm"] || global.race["orbit_decayed"]
            ? `ᄂ${loc("space_red_ziggurat_title")}`
            : `ᄂ${structName("temple")}`
        ] = (temple_mult - 1) * 100 + "%";
        breakdown.p["Money"][`ᄂ${loc("city_shrine")}`] =
          (getShrineResult.mult - 1) * 100 + "%";
      }
    ...
    if (
      ...
      modRes(
        "Money",
        +(cash * time_multiplier * global_multiplier * hunger).toFixed(2),
      );
    ...
    if (
      ...
      let tourism = 0;
      let amp =
        global.tech["monument"] && global.tech.monument >= 3 && p_on["s_gate"]
          ? 3
          : 1;
      if (global.city["amphitheatre"]) {
        tourism +=
      ...
      if (global.city["casino"]) {
        tourism +=
          global.city["tourist_center"].on * global.city.casino.count * 5 * amp;
      }
      ...
      if (
        global.city["trade"] &&
        global.stats.achieve["banana"] &&
        global.stats.achieve.banana.l >= 4
      ) {
        tourism +=
          global.city["tourist_center"].on * global.city.trade.count * 3 * amp;
      }
      let piousVal = govActive("pious", 1);
      if (piousVal && global.city["temple"]) {
        tourism +=
          global.city["tourist_center"].on * templeCount() * piousVal * amp;
      }
      if (global.civic.govern.type === "corpocracy") {
      ...
      if (astroSign === "aquarius") {
        breakdown.p["Money"][`ᄂ${loc("sign_aquarius")}`] =
          astroVal("aquarius")[0] + "%";
      }
      modRes(
        "Money",
        +(tourism * time_multiplier * global_multiplier * hunger).toFixed(2),
      );
      rawCash += tourism * global_multiplier * hunger;
    }

    if (
      global.portal["bazaar"] &&
      global.portal["spire"] &&
      global.tech["monuments"]
    ) {
      let monuments = global.tech.monuments;
      if (
        global.race["wish"] &&
      ...
      let revenue =
        global.portal.bazaar.count * monuments * global.portal.spire.count;
      revenue *= production("psychic_cash");

      breakdown.p["Money"][loc("portal_bazaar_title")] =
        Math.round(revenue) + "v";
      if (astroSign === "aquarius") {
      ...
      modRes(
        "Money",
        +(revenue * time_multiplier * global_multiplier * hunger).toFixed(2),
      );
    ...
    if (global.tauceti["tau_cultural_center"]) {
      ...
      revenue *= production("psychic_cash");
      breakdown.p["Money"][loc("tech_cultural_center")] =
        Math.round(revenue) + "v";
      if (astroSign === "aquarius") {
        revenue *= 1 + astroVal("aquarius")[0] / 100;
        breakdown.p["Money"][`ᄂ${loc("sign_aquarius")}`] =
    ...
    if (global.tech["tau_junksale"]) {
      ...
      let culture = p_on["tau_cultural_center"]
        ? 1 + p_on["tau_cultural_center"] * 0.08
        : 1;
      breakdown.p["Money"][loc("tau_red_womling_village")] =
        Math.round(revenue) + "v";
      breakdown.p["Money"][`ᄂ${loc("tech_cultural_center")}+1`] =
        (culture - 1) * 100 + "%";
      modRes(
        "Money",
        +(
    ...
    if (
      ...
      global.tech["teamster"] &&
      global.tech.teamster >= 2
    ) {
      let teamsters = global.civic.teamster.workers;
      let revenue = teamsters * rawCash * 0.01;
      breakdown.p["Money"][jobName("teamster")] =
        Math.round(revenue / global_multiplier) + "v";
    ...
    if (
      ...
      let merchsales =
        global.resource[global.race.species].amount * templeCount() * 0.08;
      breakdown.p["Money"][structName("temple")] = merchsales + "v";
      modRes(
        "Money",
        +(merchsales * global_multiplier * time_multiplier).toFixed(2),
      );
    ...
    if (global.race["truepath"] && global.tauceti["overseer"]) {
      let rate =
        (global.tauceti.overseer.loyal + global.tauceti.overseer.morale) / 200;
      ...
      breakdown.p["Money"][`ᄂ${loc("tech_cultural_center")}`] =
        (culture - 1) * 100 + "%";
      modRes("Money", +(delta * time_multiplier).toFixed(2));
      rawCash += delta;
    }
    ...
    if (global.tech["foundry"]) {
      let craft_costs = global.race["resourceful"]
        ? 1 - traits.resourceful.vars()[0] / 100
        : 1;
      ...
      Object.keys(crafting_costs).forEach(function (craft) {
        ...
        if (
          ...
          global.race.servants.hasOwnProperty("sjobs") &&
          global.race.servants.sjobs.hasOwnProperty(craft)
        ) {
        ...
        let volume = Math.floor(
          global.resource[crafting_costs[craft][0].r].amount /
            ((crafting_costs[craft][0].a * speed * craft_costs) / 140),
        );
        for (let i = 1; i < crafting_costs[craft].length; i++) {
          let temp = Math.floor(
            global.resource[crafting_costs[craft][i].r].amount /
              ((crafting_costs[craft][i].a * speed * craft_costs) / 140),
          );
          if (temp < volume) {
            volume = temp;
          }
        ...
        for (let i = 0; i < crafting_costs[craft].length; i++) {
          ...
          if (
            typeof crafting_usage[crafting_costs[craft][i].r] === "undefined"
          ) {
            crafting_usage[crafting_costs[craft][i].r] =
              final / time_multiplier;
          } else {
            crafting_usage[crafting_costs[craft][i].r] +=
              final / time_multiplier;
          }
        ...
        modRes(
          craft,
          ((craft_ratio * volume * speed * time_multiplier) / 140) *
            production("psychic_boost", craft),
        );
      });

      Object.keys(crafting_usage).forEach(function (used) {
        if (crafting_usage[used] > 0) {
          breakdown.p.consume[used][jobName("craftsman")] =
            -crafting_usage[used];
        }
      });
    }
    ...
    // Detect new unlocks
    if (
      !global.settings.showResearch &&
      (global.resource.Lumber.amount >= 5 || global.resource.Stone.amount >= 6)
    ) {
    ...
    if (global.race["decay"]) {
      Object.keys(tradeRatio).forEach(function (res) {
        if (global.resource[res].amount > 50) {
          let decay = +(
            (global.resource[res].amount - 50) *
            (0.001 * tradeRatio[res])
          ).toFixed(3);
    ...
    if (global.resource.Asphodel_Powder.display) {
      if (global.resource.Asphodel_Powder.amount > 0) {
        let decay = +(global.resource.Asphodel_Powder.amount * 0.0045).toFixed(
          3,
        );

        let stabilize = 0.92;
        if (
          p_on["ascension_trigger"] &&
          global.eden.hasOwnProperty("encampment") &&
          global.eden.encampment.asc
        ) {
          let heatSink =
            actions.interstellar.int_sirius.ascension_trigger.heatSink();
        ...
        breakdown.p.consume.Asphodel_Powder[loc("evo_challenge_decay")] =
          -decay;
      } else {
  ...
  if (
    ...
    global.civic.garrison.workers < global.civic.garrison.max
  ) {
    let rate = 2.5;
    if (global.race["high_pop"]) {
    ...
    if (global.city["boot_camp"]) {
      ...
      rate *=
        1 +
        (global.race["orbit_decayed"] && global.space["space_barracks"]
          ? global.space.space_barracks.on
          : global.city.boot_camp.count) *
          train;
    }
    if (
      global.tech["celestial_warfare"] &&
      global.tech.celestial_warfare >= 5 &&
      global.eden["bunker"]
    ) {
      let train = 0.1;
      if (global.blood["lust"]) {
    ...
    if (global.race["brute"]) {
      global.civic.garrison.rate +=
        (traits.brute.vars()[1] / 40) * time_multiplier;
    }
    let fathom = fathomCheck("orc");
    if (fathom > 0) {
      global.civic.garrison.rate +=
        (traits.brute.vars(1)[1] / 40) * fathom * time_multiplier;
    }
    ...
    while (global.civic.garrison.progress >= 100) {
      ...
      if (
        ...
        global.portal.fortress["assigned"] &&
        global.portal.fortress.garrison < global.portal.fortress.assigned
      ) {
        global.portal.fortress.garrison++;
      }
    }
  ...
  if (global.portal["carport"]) {
    if (global.portal.carport.damaged > 0) {
      ...
      global.portal.carport.damaged = Math.min(
        global.portal.carport.damaged,
        jobScale(global.portal.carport.count),
      );
    } else {
      if ($("#portal-carport .count").hasClass("has-text-alert")) {
  ...
  Object.keys(global.resource).forEach(function (res) {
    if (
      ...
      global.resource[res].max >= 0
    ) {
      global.resource[res].amount = global.resource[res].max;
    }
    if (
      global["resource"][res].rate > 0 ||
      (global["resource"][res].rate === 0 && global["resource"][res].max === -1)
    ) {
...
function midLoop() {
  ...
  if (global.race.species === "protoplasm") {
    ...
    if (global.evolution["membrane"]) {
      let effect = global.evolution["mitochondria"]
        ? global.evolution["mitochondria"].count * 5 + 5
        : 5;
      caps["RNA"] += global.evolution["membrane"].count * effect;
    }
    if (global.evolution["eukaryotic_cell"]) {
      let effect = global.evolution["mitochondria"]
        ? global.evolution["mitochondria"].count * 10 + 10
        : 10;
    ...
    if (
      global.race.hasOwnProperty("psychicPowers") &&
      global.race.psychicPowers.hasOwnProperty("channel")
    ) {
    ...
    if (
      ...
      if (global.space["space_barracks"]) {
        let gain =
          global.space.space_barracks.on * (global.race["cataclysm"] ? 2 : 1);
        caps.Authority += gain;
        breakdown.c.Authority[loc("space_red_space_barracks_title")] =
          gain + "v";
      }
      ...
      if (global.race["pet"]) {
        ...
        caps.Authority += pet;
        breakdown.c.Authority[loc(`event_pet_${global.race.pet.type}_owner`)] =
          pet + "v";
      }

      global.resource.Authority.amount =
        global.race["cataclysm"] || global.race["orbit_decayed"]
          ? 90
          : global.race["lone_survivor"]
            ? 100
      ...
      if (
        (global.race["lone_survivor"] || global.tech["isolation"]) &&
        global.tauceti["colony"] &&
        support_on["colony"]
      ) {
        global.resource.Authority.amount += support_on["colony"] * 5;
      }

      global.resource.Authority.amount = Math.floor(
        global.resource.Authority.amount,
      );
    ...
    if (global.race["unfathomable"] && global.city["captive_housing"]) {
      ...
      if (
        usedCap < global.city.captive_housing.raceCap &&
    ...
    if (global.race["psychic"]) {
      if (
        ...
        if (
          global.race.psychicPowers.boostTime < 0 ||
          global.race.psychicPowers.boostTime > 360
        ) {
          global.race.psychicPowers.boostTime = 0;
        }
      }
      if (
        ...
        if (
          global.race.psychicPowers.assaultTime < 0 ||
          global.race.psychicPowers.assaultTime > 360
        ) {
          global.race.psychicPowers.assaultTime = 0;
        }
      }
      if (
        ...
        if (
          global.race.psychicPowers.cash < 0 ||
          global.race.psychicPowers.cash > 360
        ) {
          global.race.psychicPowers.cash = 0;
        }
      }
    ...
    if (global.city["captive_housing"]) {
      ...
      global.city.captive_housing.raceCap =
        houses *
        (global.tech["unfathomable"] && global.tech.unfathomable >= 3 ? 3 : 2);
    ...
    if (global.city["wharf"]) {
      ...
      breakdown.c.Crates[loc("city_wharf")] =
        global.city.wharf.count * vol + "v";
      caps["Containers"] += global.city.wharf.count * vol;
      breakdown.c.Containers[loc("city_wharf")] =
        global.city.wharf.count * vol + "v";
    }
    if (global.space["munitions_depot"]) {
      let vol = 25;
      caps["Crates"] += global.space.munitions_depot.count * vol;
      breakdown.c.Crates[loc("tech_munitions_depot")] =
        global.space.munitions_depot.count * vol + "v";
      caps["Containers"] += global.space.munitions_depot.count * vol;
      breakdown.c.Containers[loc("tech_munitions_depot")] =
        global.space.munitions_depot.count * vol + "v";
    }
    if (global.interstellar["cargo_yard"]) {
      caps["Crates"] += global.interstellar.cargo_yard.count * 50;
      breakdown.c.Crates[loc("interstellar_cargo_yard_title")] =
        global.interstellar.cargo_yard.count * 50 + "v";
      caps["Containers"] += global.interstellar.cargo_yard.count * 50;
      breakdown.c.Containers[loc("interstellar_cargo_yard_title")] =
        global.interstellar.cargo_yard.count * 50 + "v";
    ...
    if (global.interstellar["neutron_miner"] && p_on["neutron_miner"]) {
      ...
      caps["Neutronium"] += gain;
      breakdown.c.Neutronium[loc("interstellar_neutron_miner_title")] =
        gain + "v";
    }
    if (global.city["storage_yard"]) {
      let size = global.tech.container >= 3 ? 20 : 10;
      if (
        global.stats.achieve["pathfinder"] &&
        global.stats.achieve.pathfinder.l >= 1
      ) {
        size += 10;
      }
      if (global.tech["world_control"]) {
      ...
      breakdown.c.Crates[loc("city_storage_yard")] =
        global.city["storage_yard"].count * size + "v";
    }
    if (global.space["garage"]) {
      let g_vol =
        global.tech["particles"] >= 4 ? 20 + global.tech["supercollider"] : 20;
      if (global.tech["world_control"] || global.race["cataclysm"]) {
        g_vol += 10;
      }
      caps["Containers"] += global.space.garage.count * g_vol;
      breakdown.c.Containers[loc("space_red_garage_title")] =
        global.space.garage.count * g_vol + "v";
      if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
        caps["Crates"] += global.space.garage.count * g_vol;
        breakdown.c.Crates[loc("space_red_garage_title")] =
          global.space.garage.count * g_vol + "v";
      }
    }
    if (global.tech["tp_depot"]) {
      caps["Containers"] += global.tech.tp_depot * 50;
      breakdown.c.Containers[loc("galaxy_gateway_depot")] =
        global.tech.tp_depot * 50 + "v";
      caps["Crates"] += global.tech.tp_depot * 50;
      breakdown.c.Crates[loc("galaxy_gateway_depot")] =
        global.tech.tp_depot * 50 + "v";
    }
    if (global.city["warehouse"]) {
      ...
      if (
        global.stats.achieve["pathfinder"] &&
        global.stats.achieve.pathfinder.l >= 2
      ) {
        volume += 10;
      }
      if (global.tech["world_control"]) {
      ...
      breakdown.c.Containers[loc("city_warehouse")] =
        global.city["warehouse"].count * volume + "v";
    }
    if (global.city["rock_quarry"]) {
      let gain = BHStorageMulti(
        global.city.rock_quarry.count * spatialReasoning(100),
      );
      caps["Stone"] += gain;
    ...
    if (global.city["lumber_yard"]) {
      let gain = BHStorageMulti(
        global.city.lumber_yard.count * spatialReasoning(100),
      );
      caps["Lumber"] += gain;
      breakdown.c.Lumber[loc("city_lumber_yard")] = gain + "v";
    } else if (global.city["graveyard"]) {
      let gain = BHStorageMulti(
        global.city.graveyard.count * spatialReasoning(100),
      );
      caps["Lumber"] += gain;
    ...
    if (global.city["sawmill"]) {
      let gain = BHStorageMulti(
        global.city.sawmill.count * spatialReasoning(200),
      );
      caps["Lumber"] += gain;
    ...
    if (global.portal["dig_demon"] && global.race["warlord"]) {
      let demons =
        global.portal.dig_demon.on *
        actions.portal.prtl_wasteland.dig_demon.citizens();
      caps[global.race.species] += demons;
      breakdown.c[global.race.species][loc("portal_dig_demon_title")] =
        demons + "v";
      lCaps["miner"] += demons;
    ...
    if (global.tauceti["tauceti_casino"]) {
      ...
      if (global.tech["isolation"]) {
        ...
        let pop =
          p_on["tauceti_casino"] *
          actions.tauceti.tau_home.tauceti_casino.citizens();
        caps[global.race.species] += pop;
    ...
    if (global.city["garrison"]) {
      lCaps["garrison"] +=
        global.city.garrison.on * actions.city.garrison.soldiers();
    }
    if (global.space["space_barracks"] && !global.race["fasting"]) {
      let soldiers = actions.space.spc_red.space_barracks.soldiers();
      lCaps["garrison"] += Math.round(
        global.space.space_barracks.on * soldiers,
      );
    }
    if (global.interstellar["cruiser"]) {
    ...
    if (
      global.eden["fire_support_base"] &&
      global.eden.fire_support_base.count === 100
    ) {
      lCaps["garrison"] +=
        actions.eden.eden_elysium.fire_support_base.soldiers();
    }
    if (
      global.race["orbit_decayed"] &&
      global.space.hasOwnProperty("red_mine")
    ) {
      lCaps["miner"] += jobScale(support_on["red_mine"]);
      lCaps["coal_miner"] += jobScale(support_on["red_mine"]);
    }
    if (!global.tech["world_control"]) {
      let occ_amount = jobScale(
      ...
      for (let i = 2; i >= 0; i--) {
        if (global.civic.foreign[`gov${i}`].occ) {
          ...
          if (lCaps["garrison"] < 0) {
            ...
            messageQueue(
              loc("civics_garrison_autodeoccupy_desc", [govTitle(i)]),
              "danger",
              false,
              ["spy"],
            );
    ...
    if (
      ...
      breakdown.c.Slave[
        loc("city_slave_housing", [global.resource.Slave.name])
      ] = global.city.slave_pen.count * 4 + "v";
    ...
    if (global.race["calm"] && global.city["meditation"]) {
      ...
      breakdown.c.Zen[loc("city_meditation")] =
        global.city.meditation.count * traits.calm.vars()[0] + "v";
      global.resource.Zen.amount =
        global.resource[global.race.species].amount * 2 +
        global.civic.garrison.workers;
      if (global.resource.Zen.amount > global.resource.Zen.max) {
        global.resource.Zen.amount = global.resource.Zen.max;
      }
      let zen =
        global.resource.Zen.amount / (global.resource.Zen.amount + 5000);
      breakdown.c.Zen[loc("trait_calm_desc")] = `+${(zen * 100).toFixed(2)}%`;
    }
    if (global.city["basic_housing"]) {
      let pop =
        global.city.basic_housing.count * actions.city.basic_housing.citizens();
      caps[global.race.species] += pop;
    ...
    if (global.tauceti["tau_housing"] && global.tech["isolation"]) {
      let pop =
        ...
        actions.tauceti.tau_home.tau_housing.citizens();
      caps[global.race.species] += pop;
      breakdown.c[global.race.species][housingLabel("small")] = pop + "v";
    }
    ...
    if (
      p_on["s_gate"] &&
      global.galaxy["consulate"] &&
      global.galaxy.consulate.count >= 1
    ) {
    ...
    if (p_on["s_gate"] && p_on["embassy"] && global.galaxy["dormitory"]) {
      let pop =
        p_on["dormitory"] * actions.galaxy.gxy_gorddon.dormitory.citizens();
      caps[global.race.species] += pop;
      breakdown.c[global.race.species][loc("galaxy_dormitory")] = pop + "v";
    }
    if (p_on["arcology"]) {
      let pop =
        p_on["arcology"] * actions.portal.prtl_ruins.arcology.citizens();
      caps[global.race.species] += pop;
      breakdown.c[global.race.species][loc("portal_arcology_title")] =
        pop + "v";
      lCaps["garrison"] +=
        p_on["arcology"] * actions.portal.prtl_ruins.arcology.soldiers();

      caps["Containers"] += p_on["arcology"] * Math.round(quantum_level) * 10;
      breakdown.c.Containers[loc("portal_arcology_title")] =
        p_on["arcology"] * Math.round(quantum_level) * 10 + "v";
      caps["Crates"] += p_on["arcology"] * Math.round(quantum_level) * 10;
      breakdown.c.Crates[loc("portal_arcology_title")] =
        p_on["arcology"] * Math.round(quantum_level) * 10 + "v";

      let sup = hellSupression("ruins");
      let money =
        p_on["arcology"] * spatialReasoning(bank_vault() * 8 * sup.supress);
      caps["Money"] += money;
    ...
    if (global.portal["bazaar"] && global.portal["spire"]) {
      let containers =
        global.portal.bazaar.count * global.portal.spire.count * 8;
      ...
      let money = spatialReasoning(
        (bank_vault() *
          global.portal.spire.count *
          global.portal.bazaar.count) /
          3,
      );
      caps["Money"] += money;
    ...
    if (support_on["colony"]) {
      ...
      breakdown.c.Containers[loc("tau_home_colony")] =
        support_on["colony"] * containers + "v";
      caps["Crates"] += support_on["colony"] * containers;
      breakdown.c.Crates[loc("tau_home_colony")] =
        support_on["colony"] * containers + "v";

      let pop =
        support_on["colony"] * actions.tauceti.tau_home.colony.citizens();
      caps[global.race.species] += pop;
      breakdown.c[global.race.species][loc("tau_home_colony")] = pop + "v";
    }
    if (p_on["operating_base"]) {
      lCaps["garrison"] +=
        Math.min(support_on["operating_base"], p_on["operating_base"]) *
        actions.space.spc_enceladus.operating_base.soldiers();
    }
    if (p_on["fob"]) {
    ...
    if (global.space["living_quarters"]) {
      let gain = Math.round(
        support_on["living_quarters"] *
          actions.space.spc_red.living_quarters.citizens(),
      );
    ...
    if (
      support_on["biodome"] &&
      (global.race["artifical"] || global.race["orbit_decayed"])
    ) {
      let gain =
        support_on["biodome"] *
        spatialReasoning(global.race["artifical"] ? 500 : 100);
      caps["Food"] += gain;
      breakdown.c.Food[loc("space_red_signal_tower_title")] = gain + "v";
    }
    if (global.space["titan_quarters"]) {
      let gain = Math.round(
        support_on["titan_quarters"] *
          actions.space.spc_titan.titan_quarters.citizens(),
      );
      caps[global.race.species] += gain;
    ...
    if (global.interstellar["habitat"] && p_on["habitat"]) {
      let pop =
        p_on["habitat"] * actions.interstellar.int_alpha.habitat.citizens();
      caps[global.race.species] += pop;
      breakdown.c[global.race.species][loc("interstellar_habitat_title")] =
        pop + "v";
    }
    if (global.interstellar["luxury_condo"] && p_on["luxury_condo"]) {
      let cit =
        p_on["luxury_condo"] *
        actions.interstellar.int_alpha.luxury_condo.citizens();
    ...
    if (global.portal["hovel"]) {
      let cit =
        global.portal.hovel.count *
        actions.portal.prtl_wasteland.hovel.citizens();
      caps[global.race.species] += cit;
    ...
    if (global.city["shed"]) {
      ...
      let label =
        global.tech["storage"] <= 2
          ? loc("city_shed_title1")
          : global.tech["storage"] >= 4
            ? loc("city_shed_title3")
            : loc("city_shed_title2");
      for (const res of actions.city.shed.res()) {
        if (global.resource[res].display) {
          let gain =
            global.city.shed.count *
            spatialReasoning(actions.city.shed.val(res) * multiplier);
          caps[res] += gain;
    ...
    if (global.interstellar["warehouse"]) {
      ...
      for (const res of actions.interstellar.int_alpha.warehouse.res()) {
        if (global.resource[res].display) {
          let gain =
            global.interstellar.warehouse.count *
            spatialReasoning(
              actions.interstellar.int_alpha.warehouse.val(res) * multiplier,
            );
          caps[res] += gain;
          breakdown.c[res][label] = gain + "v";
        }
    ...
    if (global.eden["warehouse"]) {
      ...
      for (const res of actions.eden.eden_asphodel.warehouse.res()) {
        if (global.resource[res].display) {
          let gain =
            global.eden.warehouse.count *
            spatialReasoning(
              actions.eden.eden_asphodel.warehouse.val(res) * multiplier,
            );
          caps[res] += gain;
          breakdown.c[res][label] = gain + "v";
        }
    ...
    if (global.portal["warehouse"]) {
      ...
      if (
        ...
        multiplier *=
          1 +
          (p_on["corruptor"] || 0) * (global.tech.asphodel >= 13 ? 0.16 : 0.12);
      }
      let label =
        global.tech["storage"] <= 2
          ? loc("city_shed_title1")
      ...
      for (const res of actions.portal.prtl_wasteland.warehouse.res()) {
        if (global.resource[res].display) {
          let gain =
            global.portal.warehouse.count *
            spatialReasoning(
              actions.portal.prtl_wasteland.warehouse.val(res) * multiplier,
            );
          caps[res] += gain;
          breakdown.c[res][label] = gain + "v";
        }
      ...
      let cc_gain =
        global.portal.warehouse.count *
        (65 + global.portal.warehouse.rank * 35);
      caps["Crates"] += cc_gain;
      ...
      for (const res of actions.space.spc_titan.storehouse.res()) {
        if (global.resource[res].display) {
          ...
          let gain =
            ...
            spatialReasoning(
              actions.space.spc_titan.storehouse.val(res) *
                (heavy ? h_multiplier : multiplier),
            );
          caps[res] += gain;
          breakdown.c[res][label] = gain + "v";
        }
    ...
    if (global.tauceti["repository"]) {
      ...
      for (const res of actions.tauceti.tau_home.repository.res()) {
        if (global.resource[res].display) {
          let gain =
            ...
            spatialReasoning(
              actions.tauceti.tau_home.repository.val(res) * multiplier,
            );
          caps[res] += gain;
          breakdown.c[res][label] = gain + "v";
        }
      ...
      if (global.tech["isolation"]) {
        ...
        breakdown.c.Containers[loc("tech_repository")] =
          global.tauceti.repository.count * containers + "v";
        caps["Crates"] += global.tauceti.repository.count * containers;
        breakdown.c.Crates[loc("tech_repository")] =
          global.tauceti.repository.count * containers + "v";
      }
    }
    ...
    if (global.galaxy["gateway_depot"]) {
      ...
      breakdown.c.Crates[loc("galaxy_gateway_depot")] =
        global.galaxy.gateway_depot.count * containers + "v";
      ...
      if (global.resource.Uranium.display) {
        let gain =
          global.galaxy.gateway_depot.count *
          spatialReasoning(3000 * multiplier);
        caps["Uranium"] += gain;
      ...
      if (global.resource.Nano_Tube.display) {
        let gain =
          global.galaxy.gateway_depot.count *
          spatialReasoning(250000 * multiplier);
        caps["Nano_Tube"] += gain;
      ...
      if (global.resource.Neutronium.display) {
        let gain =
          global.galaxy.gateway_depot.count *
          spatialReasoning(9001 * multiplier);
        caps["Neutronium"] += gain;
      ...
      if (global.resource.Infernite.display) {
        let gain =
          global.galaxy.gateway_depot.count *
          spatialReasoning(6660 * multiplier);
        caps["Infernite"] += gain;
    ...
    if (
      global.resource.Infernite.display &&
      global.portal["fortress"] &&
      !global.race["warlord"]
    ) {
      ...
      for (const res of actions.space.spc_red.garage.res()) {
        if (global.resource[res].display) {
          ...
          let gain =
            ...
            spatialReasoning(
              actions.space.spc_red.garage.val(res) *
                (heavy ? h_multiplier : multiplier),
            );
          caps[res] += gain;
          breakdown.c[res][label] = gain + "v";
    ...
    if (global.portal["harbor"] && p_on["harbor"]) {
      ...
      if (
        ...
        multiplier *=
          1 +
          (p_on["corruptor"] || 0) * (global.tech.asphodel >= 13 ? 0.12 : 0.1);
      }
      ...
      for (const res of actions.portal.prtl_lake.harbor.res()) {
        if (global.resource[res].display) {
          let gain =
            p_on["harbor"] *
            spatialReasoning(
              actions.portal.prtl_lake.harbor.val(res) * multiplier,
            );
          caps[res] += gain;
          breakdown.c[res][label] = gain + "v";
        }
    ...
    if (global.city["silo"]) {
      let gain = BHStorageMulti(
        global.city["silo"].count * spatialReasoning(500),
      );
      caps["Food"] += gain;
    ...
    if (global.city["compost"]) {
      let gain = BHStorageMulti(
        global.city["compost"].count * spatialReasoning(200),
      );
      caps["Food"] += gain;
    ...
    if (global.city["soul_well"]) {
      let gain = BHStorageMulti(
        global.city["soul_well"].count * spatialReasoning(500),
      );
      caps["Food"] += gain;
    ...
    if (global.city["smokehouse"]) {
      let gain = BHStorageMulti(
        global.city["smokehouse"].count * spatialReasoning(100),
      );
      caps["Food"] += gain;
    ...
    if (global.space["propellant_depot"]) {
      let gain =
        global.space["propellant_depot"].count * spatialReasoning(1250);
      gain *= global.tech["world_control"] ? 1.5 : 1;
      caps["Oil"] += gain;
      ...
      if (global.resource["Helium_3"].display) {
        ...
        gain *= global.tech["world_control"] ? 1.5 : 1;
        caps["Helium_3"] += gain;
        breakdown.c.Helium_3[loc("space_home_propellant_depot_title")] =
          gain + "v";
      }
    }
    ...
    if (p_on["refueling_station"]) {
      ...
      breakdown.c.Helium_3[loc("tau_gas_refueling_station_title")] =
        h_gain + "v";
    ...
    if (p_on["xfer_station"]) {
      ...
      if (global.resource.Deuterium.display) {
        ...
        breakdown.c.Deuterium[loc("interstellar_xfer_station_title")] =
          deuterium_gain + "v";
      }
    }
    ...
    if (
      global.city["temple"] &&
      global.genes["ancients"] &&
      global.genes["ancients"] >= 2
    ) {
      lCaps["priest"] += jobScale(templeCount());
    }
    if (
      global.space["ziggurat"] &&
      global.genes["ancients"] &&
      global.genes["ancients"] >= 4
    ) {
    ...
    if (global.city["university"]) {
      let gain =
        actions.city.university.knowVal() * global.city.university.count;
    ...
    if (global.city["wardenclyffe"]) {
      ...
      if (global.tech["supercollider"]) {
        let ratio =
          global.tech["tp_particles"] ||
          (global.tech["particles"] && global.tech["particles"] >= 3)
            ? 12.5
            : 25;
    ...
    if (global.race["logical"]) {
      let factor = global.tech.hasOwnProperty("high_tech")
        ? global.tech.high_tech
        : 0;
      factor += global.tech.hasOwnProperty("science") ? global.tech.science : 0;
      let gain =
        global.resource[global.race.species].amount *
        traits.logical.vars()[1] *
        factor;
      caps["Knowledge"] += gain;
    ...
    if (global.portal["sensor_drone"]) {
      let gain =
        p_on["sensor_drone"] * (global.tech.infernite >= 6 ? 2500 : 1000);
      caps["Knowledge"] += gain;
    ...
    if (global.space["satellite"]) {
      let gain =
        ...
        (global.race["cataclysm"] || global.race["orbit_decayed"] ? 2000 : 750);
      if (
        (global.race["cataclysm"] || global.race["orbit_decayed"]) &&
        global.tech["supercollider"]
      ) {
        let ratio =
          global.tech["tp_particles"] ||
          (global.tech["particles"] && global.tech["particles"] >= 3)
    ...
    if (
      global.space["zero_g_lab"] &&
      Math.min(support_on["zero_g_lab"], p_on["zero_g_lab"]) > 0
    ) {
    ...
    if (
      global.portal["twisted_lab"] &&
      global.portal.twisted_lab.count > 0 &&
      global.race["absorbed"]
    ) {
    ...
    if (global.resource.Omniscience.display) {
      if (global.eden["research_station"]) {
        ...
        let gain =
          (support_on["research_station"] || 0) * Math.round(777 * corruptor);
        caps["Omniscience"] += gain;
        breakdown.c.Omniscience[loc("eden_research_station_title")] =
          gain + "v";
      }

      if (
        global.race["warlord"] &&
        global.portal["mortuary"] &&
        global.portal["corpse_pile"]
      ) {
        let gain =
          global.portal.corpse_pile.count *
          (p_on["mortuary"] || 0) *
          (p_on["encampment"] || 0) *
          2;
        caps["Omniscience"] += gain;
    ...
    if (global.portal["archaeology"]) {
      ...
      caps["Knowledge"] +=
        workerScale(global.civic.archaeologist.workers, "archaeologist") * gain;
      breakdown.c.Knowledge[loc("portal_archaeology_bd")] =
        workerScale(global.civic.archaeologist.workers, "archaeologist") *
          gain +
        "v";
    }
      ...
      if (global.tech.xeno >= 7) {
        for (let j = 0; j < galaxy_ship_types.length; j++) {
          ...
          for (const ship of gatewayArmada) {
            crew +=
              global.galaxy.defense[region][ship] *
              (actions[area]["gxy_gateway"][ship].ship.civ() +
                actions[area]["gxy_gateway"][ship].ship.mil());
          }

          for (let i = 0; i < galaxy_ship_types[j].ships.length; i++) {
            const ship = galaxy_ship_types[j].ships[i];
            if (
              !gatewayArmada.includes(ship) &&
              actions[area][region][ship].hasOwnProperty("ship") &&
              gal_on[ship]
            ) {
    ...
    if (
      global.city["bank"] ||
      (global.race["cataclysm"] && p_on["spaceport"])
    ) {
      let vault =
        global.race["cataclysm"] || global.race["orbit_decayed"]
          ? bank_vault() * 4
      ...
      if (global.interstellar["exchange"]) {
        ...
        let g_vault = spatialReasoning(
          int_on["exchange"] * ((vault * banks) / 18),
        );
        if (global.race["inflation"]) {
          g_vault *= 2;
        }
    ...
    if (global.space["titan_bank"]) {
      ...
      let gain = banks * spatialReasoning(vault);
      caps["Money"] += gain;
      breakdown.c.Money[`${planetName().titan} ${loc("city_bank")}`] =
        gain + "v";
    }
    ...
    if (
      global.city["casino"] ||
      global.space["spc_casino"] ||
      global.tauceti["tauceti_casino"] ||
      global.portal["hell_casino"]
    ) {
      let casinos = 0;
      ...
      if (
        global.tauceti["tauceti_casino"] &&
        global.tauceti.tauceti_casino.count > 0
      ) {
    ...
    if (global.galaxy["resort"]) {
      let vault =
        p_on["resort"] *
        spatialReasoning(global.tech["world_control"] ? 1875000 : 1500000);
      caps["Money"] += vault;
    ...
    if (global.tech["banking"] >= 4) {
      ...
      caps["Money"] += gain;
      breakdown.c.Money[
        global.tech.banking >= 14
          ? loc("tech_crypto_currency")
          : loc("tech_bonds")
      ] = gain + "v";
    }
    if (p_on["moon_base"]) {
    ...
    if (support_on["exotic_lab"]) {
      ...
      if (
        (global.race["cataclysm"] || global.race["orbit_decayed"]) &&
        global.portal["sensor_drone"] &&
        global.tech["science"] >= 14
      ) {
    ...
    if (support_on["research_station"]) {
      ...
      let gain =
        support_on["research_station"] *
        highPopAdjust(global.civic.ghost_trapper.workers) *
        sci;
      caps["Knowledge"] += gain;
    ...
    if (p_on["elerium_contain"]) {
      ...
      caps["Elerium"] += el_gain;
      breakdown.c.Elerium[loc("space_dwarf_elerium_contain_title")] =
        el_gain + "v";
    }
    ...
    if (p_on["demon_forge"]) {
      lCaps["craftsman"] += jobScale(
        p_on["demon_forge"] *
          actions.portal.prtl_wasteland.demon_forge.crafters(),
      );
    }
    if (
      global.tech["elysium"] &&
      global.tech.elysium >= 18 &&
      p_on["sacred_smelter"]
    ) {
    ...
    if (global.portal["carport"]) {
      lCaps["hell_surveyor"] +=
        jobScale(global.portal.carport.count) - global.portal.carport.damaged;
    }
    if (p_on["archaeology"]) {
    ...
    if (p_on["nexus"]) {
      ...
      caps["Deuterium"] += deuterium_gain;
      breakdown.c.Deuterium[loc("interstellar_nexus_title")] =
        deuterium_gain + "v";
    ...
    if (p_on["s_gate"] && global.galaxy["gateway_station"]) {
      ...
      caps["Deuterium"] += deuterium_gain;
      breakdown.c.Deuterium[loc("galaxy_gateway_station")] =
        deuterium_gain + "v";
    ...
    if (global.city["trade"]) {
      let routes =
        global.race["nomadic"] || global.race["xenophobic"]
          ? global.tech.trade
          : global.tech.trade + 1;
    ...
    if (global.tech["railway"]) {
      let routes = 0;
      if (global.race["cataclysm"] || global.race["orbit_decayed"]) {
        routes = global.space["gps"]
          ? Math.floor(global.space.gps.count / 3)
          : 0;
      } else if (global.race["warlord"]) {
        routes = 5;
      } else {
        routes = global.city["storage_yard"]
          ? Math.floor(global.city.storage_yard.count / 6)
          : 0;
      }
      if (
        global.stats.achieve["banana"] &&
        global.stats.achieve.banana.l >= 2
      ) {
        routes++;
      }
      global.city.market.mtrade += global.tech["railway"] * routes;
      breakdown.t_route[loc("arpa_projects_railway_title")] =
        global.tech["railway"] * routes;
    }
    ...
    if (global.tauceti["mining_pit"]) {
      lCaps["pit_miner"] += jobScale(
        support_on["mining_pit"] * (global.tech["isolation"] ? 6 : 8),
      );
      caps["Materials"] += support_on["mining_pit"] * 1000000;
    }

    if (
      global.civic.torturer.display &&
      global.tech["unfathomable"] &&
      global.tech.unfathomable >= 2
    ) {
    ...
    if (global.race["universe"] === "magic" && global.race["witch_hunter"]) {
      ...
      if (global.portal.hasOwnProperty("soul_capacitor")) {
        ...
        breakdown.c.Sus[loc("portal_soul_capacitor_title")] =
          capacitors / 3 + "v";
        sus += capacitors / 3;
      }

      if (
        global.tech["roguemagic"] &&
        global.tech.roguemagic >= 3 &&
        global.city["conceal_ward"]
      ) {
      ...
      if (sus >= 50 && !global.race["witch_hunter_warned"]) {
        ...
        messageQueue(loc("witch_hunter_warning"), "danger", false, [
          "progress",
        ]);
      } else if (
        sus >= 80 &&
        global.race["witch_hunter_warned"] &&
        global.race.witch_hunter_warned === 1
      ) {
        global.race.witch_hunter_warned = 2;
        messageQueue(loc("witch_hunter_warning2"), "danger", false, [
          "progress",
        ]);
      }
    ...
    if (global.galaxy["super_freighter"]) {
      breakdown.gt_route[loc("galaxy_super_freighter")] =
        gal_on["super_freighter"] * 5;
    }
    if (global.galaxy["bolognium_ship"]) {
      lCaps["crew"] +=
        global.galaxy.bolognium_ship.on *
        actions.galaxy.gxy_gateway.bolognium_ship.ship.civ();
    }
    if (global.galaxy["scout_ship"]) {
      lCaps["crew"] +=
        global.galaxy.scout_ship.on *
        actions.galaxy.gxy_gateway.scout_ship.ship.civ();
    }
    if (global.galaxy["corvette_ship"]) {
      lCaps["crew"] +=
        global.galaxy.corvette_ship.on *
        actions.galaxy.gxy_gateway.corvette_ship.ship.civ();
    }
    if (global.galaxy["frigate_ship"]) {
      lCaps["crew"] +=
        global.galaxy.frigate_ship.on *
        actions.galaxy.gxy_gateway.frigate_ship.ship.civ();
    }
    if (global.galaxy["cruiser_ship"]) {
      lCaps["crew"] +=
        global.galaxy.cruiser_ship.on *
        actions.galaxy.gxy_gateway.cruiser_ship.ship.civ();
    }
    if (global.galaxy["dreadnought"]) {
      lCaps["crew"] +=
        global.galaxy.dreadnought.on *
        actions.galaxy.gxy_gateway.dreadnought.ship.civ();
    }
    if (global.galaxy["freighter"]) {
      lCaps["crew"] +=
        global.galaxy.freighter.on *
        actions.galaxy.gxy_gorddon.freighter.ship.civ();
    }
    if (global.galaxy["super_freighter"]) {
      lCaps["crew"] +=
        global.galaxy.super_freighter.on *
        actions.galaxy.gxy_alien1.super_freighter.ship.civ();
    }
    if (global.galaxy["armed_miner"]) {
      lCaps["crew"] +=
        global.galaxy.armed_miner.on *
        actions.galaxy.gxy_alien2.armed_miner.ship.civ();
    }
    if (global.galaxy["scavenger"]) {
      lCaps["crew"] +=
        global.galaxy.scavenger.on *
        actions.galaxy.gxy_alien2.scavenger.ship.civ();
    }
    if (global.portal["transport"]) {
      lCaps["crew"] +=
        global.portal.transport.on *
        actions.portal.prtl_lake.transport.ship.civ();
    }
    ...
    if (global.tauceti["infectious_disease_lab"]) {
      ...
      breakdown.c.Knowledge[
        actions.tauceti.tau_home.infectious_disease_lab.title()
      ] = p_on["infectious_disease_lab"] * gain + "v";

      if (global.tech["isolation"]) {
        let el_gain =
          support_on["infectious_disease_lab"] * spatialReasoning(375);
        caps["Elerium"] += el_gain;
        breakdown.c.Elerium[
          actions.tauceti.tau_home.infectious_disease_lab.title()
        ] = el_gain + "v";
    ...
    if (
      ...
      if (global.race["womling_friend"]) {
        loyal +=
          25 + support_on["overseer"] * actions.tauceti.tau_red.overseer.val();
        morale +=
          75 +
          support_on["womling_fun"] * actions.tauceti.tau_red.womling_fun.val();
      } else if (global.race["womling_god"]) {
        loyal +=
          75 + support_on["overseer"] * actions.tauceti.tau_red.overseer.val();
        morale +=
          40 +
          support_on["womling_fun"] * actions.tauceti.tau_red.womling_fun.val();
      } else if (global.race["womling_lord"]) {
        loyal +=
          support_on["overseer"] * actions.tauceti.tau_red.overseer.val();
        morale +=
          30 +
          support_on["womling_fun"] * actions.tauceti.tau_red.womling_fun.val();
      }
      ...
      let vil_pop =
        global.tech["womling_pop"] && global.tech.womling_pop >= 2 ? 6 : 5;
      ...
      if (support_on["womling_lab"]) {
        ...
        let gain =
          scientist * Math.round((25000 * global.tauceti.overseer.prod) / 100);
        caps["Knowledge"] += gain;
        breakdown.c.Knowledge[loc("interstellar_laboratory_title")] =
          gain + "v";

        if (Math.rand(0, 10) < global.tauceti.womling_lab.scientist) {
          global.tauceti.womling_lab.tech += Math.rand(
            0,
            global.tauceti.womling_lab.scientist + 1,
          );
          let expo =
            global.stats.achieve["overlord"] &&
            global.stats.achieve.overlord.l >= 5
              ? 4.9
              : 5;
          if (global.race["lone_survivor"]) {
            expo -= 0.1;
          ...
          if (
            global.tauceti.womling_lab.tech >=
            Math.round((global.tech.womling_tech + 2) ** expo)
          ) {
            global.tech.womling_tech++;
            global.tauceti.womling_lab.tech = 0;
            messageQueue(
              loc("tau_red_womling_advancement", [global.tech.womling_tech]),
              "advanced",
              false,
              ["progress"],
            );
    ...
    let pop_loss =
      global.resource[global.race.species].amount - caps[global.race.species];
    if (pop_loss > 0) {
      if (
        global.race["orbit_decayed"] &&
        global.stats.days === global.race["orbit_decay"]
      ) {
        messageQueue(loc("tragic_death", [pop_loss]), "danger");
      } else {
        messageQueue(
          loc(pop_loss === 1 ? "abandon1" : "abandon2", [pop_loss]),
          "danger",
        );
    ...
    if (
      global.eden["fortress"] &&
      global.tech.hasOwnProperty("celestial_warfare")
    ) {
    ...
    let tempCrates = caps["Crates"],
      tempContainers = caps["Containers"];
    Object.keys(caps).forEach(function (res) {
    ...
    breakdown.c.Containers[loc("crates_used")] =
      caps["Containers"] - tempContainers + "v";
    ...
    Object.keys(lCaps).forEach(function (job) {
      ...
      if (global.race["servants"]) {
        if (
          global.civic[job].max === -1 &&
          !global.race.servants.jobs.hasOwnProperty(job)
        ) {
        ...
        if (global.race.servants.jobs.hasOwnProperty(job)) {
          if (
            !global.civic[job].display &&
            (job !== "scavenger" || not_scavanger_jobs_avail > 0)
          ) {
            global.race.servants.jobs[job] = 0;
          } else {
          ...
          if (
            total_servants > global.race.servants.max &&
            global.race.servants.jobs[job] > 0
          ) {
    ...
    if (unlock_servants) {
      ...
      global.race.servants["force_scavenger"] =
        not_scavanger_jobs_avail === 0 ? true : false;
      global.race.servants.used = total_servants;
    }

    if (
      global.race["servants"] &&
      global.race.servants.hasOwnProperty("smax") &&
      global.race.servants.smax > 0
    ) {
      let used = 0;
    ...
    if (global.race["gravity_well"]) {
      ...
      if (global.city["oil_well"]) {
        teamster +=
          global.city.oil_well.count *
          (global.tech["teamster"] && global.tech.teamster >= 3 ? 0 : 2);
      }
    ...
    if (global.civic.space_miner.display && global.space["space_station"]) {
      global.space.space_station.s_max = workerScale(
        global.civic.space_miner.workers,
        "space_miner",
      );
    }
          ...
          switch (global.civic.foreign[`gov${i}`].act) {
            case "influence":
              if (Math.floor(seededRandom(0, 4 + spyCatchMod)) === 0) {
                ...
                let covert = Math.floor(
                  seededRandom(
                    global.tech["spy"] >= 5 ? 2 : 1,
                    global.tech["spy"] >= 5 ? 8 : 6,
                  ),
                );
                ...
                messageQueue(
                  loc("civics_spy_influence_success", [govTitle(i), covert]),
                  "success",
                  false,
                  ["spy"],
                );
            ...
            case "sabotage":
              if (Math.floor(seededRandom(0, 3 + spyCatchMod)) === 0) {
                ...
                messageQueue(
                  loc("civics_spy_sabotage_success", [govTitle(i), covert]),
                  "success",
                  false,
                  ["spy"],
                );
            ...
            case "incite":
              if (
                Math.floor(seededRandom(0, 2 + Math.floor(spyCatchMod / 2))) ===
                0
              ) {
                spyCaught(i);
              } else {
                ...
                messageQueue(
                  loc("civics_spy_incite_success", [govTitle(i), covert]),
                  "success",
                  false,
                  ["spy"],
                );
            ...
            case "annex":
              ...
              messageQueue(
                loc("civics_spy_annex_success", [govTitle(i)]),
                "success",
                false,
                ["spy"],
              );
            ...
            case "purchase":
              ...
              messageQueue(
                loc("civics_spy_purchase_success", [govTitle(i)]),
                "success",
                false,
                ["spy"],
              );
    ...
    if (global.race["banana"]) {
      ...
      if (
        global.resource[exporting] &&
        global.resource[exporting].trade <= -500
      ) {
    ...
    for (let i = 0; i < spc_locations.length; i++) {
      ...
      Object.keys(actions[location]).forEach(function (region) {
        Object.keys(actions[location][region]).forEach(function (action) {
          let s_region =
            actions[location][region][action] &&
            actions[location][region][action].hasOwnProperty("region")
              ? actions[location][region][action].region
              : location;
          if (
            (global[s_region][action] ||
              actions[location][region][action].grant) &&
            actions[location][region][action] &&
            actions[location][region][action].cost
          ) {
            ...
            if (global[s_region][action]) {
              global[s_region][action]["time"] = timeFormat(
                timeCheck(c_action),
              );
            }
          }
    ...
    if (global.space["swarm_control"]) {
      global.space.swarm_control.s_max =
        global.space.swarm_control.count *
        actions.space.spc_sun.swarm_control.support();
    }
    ...
    if (global.arpa["sequence"] && global.arpa.sequence.on && gene_sequence) {
      ...
      global.arpa.sequence.progress =
        global.arpa.sequence.max - global.arpa.sequence.time;
      if (global.arpa.sequence.time <= 0) {
        ...
        if (global.race["adaptable"]) {
          ...
          global.arpa.sequence.max = Math.floor(
            global.arpa.sequence.max * adapt,
          );
        }
        global.arpa.sequence.progress = 0;
        global.arpa.sequence.time = global.arpa.sequence.max;
        if (global.tech["genetics"] === 2) {
          messageQueue(loc("genome", [flib("name")]), "success", false, [
            "progress",
          ]);
          ...
          let gene_multi =
            1 + (global.genes["synthesis"] ? global.genes["synthesis"] : 0);
    ...
    if (global.city["foundry"]) {
      ...
      Object.keys(craft_costs).forEach(function (craft) {
        while (
          global.city.foundry[craft] > fworkers &&
          global.city.foundry[craft] > 0
        ) {
    ...
    if (
      global.tech["asteroid"] &&
      global.tech.asteroid === 4 &&
      global.resource.Elerium.amount === 0
    ) {
      modRes("Elerium", 1, true);
    }
    ...
    if (
      ...
      if (Math.rand(0, 100) <= p_on["outpost"]) {
        ...
        messageQueue(
          loc("discover_oil", [planetName().gas_moon]),
          "info",
          false,
          ["progress"],
        );
    ...
    if (
      global.portal.hasOwnProperty("mechbay") &&
      global.tech["hell_spire"] &&
      ...
      if (
        global.portal.hasOwnProperty("spire") &&
        global.portal.spire.count >= 50 &&
        !global.tech["edenic"] &&
        Object.keys(global.pillars).length >= 10
      ) {
        messageQueue(loc("eden_purify_well_msg", [50]), "info", false, [
          "progress",
        ]);
      ...
      let progress = 0;
      let mechSkips = global.eden["mech_station"]
        ? global.eden.mech_station.mechs
        : 0;
      for (let i = 0; i < global.portal.mechbay.active; i++) {
        let mech = global.portal.mechbay.mechs[i];
        if (mechSkips > 0 && mech.size !== "collector") {
          ...
          if (
            global.portal.hasOwnProperty("waygate") &&
            global.tech.hasOwnProperty("waygate") &&
      ...
      if (
        ...
        global.portal.waygate.time =
          progress === 0
            ? timeFormat(-1)
            : timeFormat((100 - global.portal.waygate.progress) / progress);
        global.portal.spire.time = timeFormat(-1);
      } else {
        global.portal.spire.progress += progress;
        global.portal.spire.time =
          progress === 0
            ? timeFormat(-1)
            : timeFormat((100 - global.portal.spire.progress) / progress);
      ...
      if (global.portal.spire.progress >= 100) {
        ...
        if (
          global.portal.spire.count > global.stats.spire[affix][`s${rank - 1}`]
        ) {
          global.stats.spire[affix][`s${rank - 1}`] = global.portal.spire.count;
        }
        if (
          !global.stats.spire[affix].hasOwnProperty(global.portal.spire.boss) ||
          rank > global.stats.spire[affix][global.portal.spire.boss]
        ) {
          global.stats.spire[affix][global.portal.spire.boss] = rank;
        }

        if (
          (global.portal.spire.boss === "djinni" &&
            global.race.species === "djinn") ||
    ...
    if (global.race["fasting"] && global.portal["oven_complete"]) {
      ...
      if (p_on["oven_complete"]) {
        ...
        if (
          ...
          let mult =
            ...
            (global.race["malnutrition"] ? 0.01 : 0) +
            (global.race["angry"] ? -0.01 : 0);
          let working = Math.min(
            global.portal["dish_soul_steeper"].on,
            Math.floor(hunger / mult),
          );
          progress *= 1 + 0.05 * (global.portal["spire"].count - 1) * working;
        }
        global.portal["devilish_dish"].done += progress;
        global.portal["devilish_dish"].done = Math.min(
          global.portal["devilish_dish"].done,
          100,
        );
        global.portal["devilish_dish"].count = Math.floor(
          global.portal["devilish_dish"].done,
        );
        if (
          global.portal["devilish_dish"].done >= 0.05 &&
          global.tech["dish"] === 3
        ) {
      ...
      global.portal["devilish_dish"].time =
        progress === 0
          ? timeFormat(-1)
          : timeFormat((100 - global.portal["devilish_dish"].done) / progress);
    }
    ...
    if (global.tech["r_queue"] && global.r_queue.display) {
      ...
      for (let i = 0; i < global.r_queue.queue.length; i++) {
        Object.keys(actions.tech[global.r_queue.queue[i].type].reqs).forEach(
          function (req) {
            ...
            if (
              (!global.tech[req] ||
                global.tech[req] <
                  actions.tech[global.r_queue.queue[i].type].reqs[req]) &&
              (!q_techs[req] ||
                (q_techs[req] &&
                  q_techs[req].v <
                    actions.tech[global.r_queue.queue[i].type].reqs[req]))
    ...
    if (
      global.arpa.sequence &&
      global.arpa.sequence["auto"] &&
      global.tech["genetics"] &&
  ...
  if (global.tech["queue"] && global.queue.display) {
      ...
      if (struct.action === "arpa") {
        ...
        if (
          checkAffordable(
            t_action,
            true,
            t_action["doNotAdjustCost"] ? true : false,
            true,
          )
        ) {
          struct.cna = false;
    ...
    if (idx >= 0 && c_action && !global.queue.pause) {
      ...
      if (arpa) {
        ...
        if (buildArpa(global.queue.queue[idx].type, 100, true, true)) {
          messageQueue(loc("build_success", [label]), "success", false, [
            "queue",
            "building_queue",
          ]);
        ...
        c_action.hasOwnProperty("type") &&
        c_action.type === "tp-ship"
      ) {
        ...
        let report_in = c_action["queue_complete"]
          ? c_action.queue_complete()
          : 1;
        for (let i = 0; i < attempts; i++) {
          if (c_action.action({ isQueue: true }) !== false) {
            ...
            if (report_in - i <= 1) {
              messageQueue(
                loc("build_success", [global.queue.queue[idx].label]),
                "success",
                false,
    ...
    for (let i = 0; i < global.queue.queue.length; i++) {
      used_slots += Math.ceil(
        global.queue.queue[i].q / global.queue.queue[i].qs,
      );
      if (used_slots > global.queue.max) {
        let remaining =
          Math.ceil(global.queue.queue[i].q / global.queue.queue[i].qs) -
          (used_slots - global.queue.max);
        if (remaining === 0) {
        ...
        this.className.split(/\s+/).forEach(function (cls) {
          if (cls.startsWith(`res-`)) {
            ...
            if (global.resource.hasOwnProperty(res)) {
              ...
              let fail_max =
                global.resource[res].max >= 0 &&
                res_val > global.resource[res].max
                  ? true
                  : false;
              let avail = elm.attr(`data-ok`)
                ? elm.attr(`data-ok`)
                : "has-text-dark";
              if (
                global.resource[res].amount + global.resource[res].diff <
                  res_val ||
                fail_max
              ) {
                ...
                elm.hasClass("has-text-danger") ||
                elm.hasClass("has-text-alert")
              ) {
...

  {
    ...
    if (global.settings.q_resize !== "manual") {
      ...
      if (
        ["auto", "grow"].includes(global.settings.q_resize) &&
    ...
    if ($(`#msgQueue`).hasClass("right")) {
      ...
      $(`#resources`).height(
        `calc(100vh - 5rem - ${buildHeight}px - ${msgHeight}px)`,
      );
  ...
  if ($(`#mechList`).length > 0) {
    $(`#mechList`).css(
      "height",
      `calc(100vh - 11.5rem - ${$(`#mechAssembly`).height()}px)`,
    );
  }
  if ($(`#shipList`).length > 0) {
    $(`#shipList`).css(
      "height",
      `calc(100vh - 11.5rem - ${$(`#shipPlans`).height()}px)`,
    );
  }
}
...
function longLoop() {
  ...
  if (global.race.species !== "protoplasm") {
    if (
      global.settings.tabLoad ||
      (global.settings.civTabs === 2 && global.settings.govTabs === 2)
    ) {
      let grids = gridDefs();
      ...
      Object.keys(grids).forEach(function (grid) {
        grids[grid].l.forEach(function (struct) {
          ...
          let c_action =
            parts[0] === "city"
              ? actions.city[parts[1]]
              : actions[space][parts[0]][parts[1]];
          let breaker = $(`#pg${c_action.id}${grid}`);

          if (
            grids[grid].s &&
            (breaker.length === 0 ||
              (gridEnabled(c_action, region, parts[0], parts[1]) &&
                breaker.hasClass("inactive")))
          ) {
    ...
    if (global.portal["fortress"] && !global.race["warlord"]) {
      ...
      global.race["warlord"] &&
      global.portal["minions"] &&
      global.portal.minions.count > 0
    ) {
      hellguard();
    }
    ...
    if (global.race["unstable"]) {
      if (
        global.resource[global.race.species].amount > 0 &&
        Math.rand(0, 100) < traits.unstable.vars()[0]
      ) {
        let bound = Math.ceil(
          (global.resource[global.race.species].amount ** 0.9 *
            traits.unstable.vars()[1]) /
            100,
        );
    ...
    if (
      global.race["blubber"] &&
      global.resource[global.race.species].amount >= 50
    ) {
      let oldAge = Math.rand(
        0,
        1 + Math.floor(global.resource[global.race.species].amount / 50),
      );
      blubberFill(oldAge);
    }
    ...
    if (global.tech["currency"] && global.tech["currency"] >= 2) {
      ...
      Object.keys(resource_values).forEach(function (res) {
        let r_val = global.race["truepath"]
          ? resource_values[res] * 2
          : resource_values[res];
        if (
          res === "Copper" &&
          global.tech["high_tech"] &&
    ...
    if (global.civic.garrison.wounded > 0) {
      let healed = global.race["regenerative"]
        ? traits.regenerative.vars()[0]
        : 1;
      ...
      if (
        global.race["cannibalize"] &&
        global.city["s_alter"] &&
        global.city.s_alter.regen > 0
      ) {
        hc >= 20
          ? (hc *= 1 + traits.cannibalize.vars()[0] / 100)
          : (hc += Math.floor(traits.cannibalize.vars()[0] / 5));
      }
      let mantisFathom = fathomCheck("mantis");
      if (mantisFathom > 0) {
        hc >= 20
          ? (hc *= 1 + (traits.cannibalize.vars(1)[0] / 100) * mantisFathom)
          : (hc += Math.floor(
              (traits.cannibalize.vars(1)[0] / 5) * mantisFathom,
            ));
      }
      if (global.race["high_pop"]) {
    ...
    if (global.civic.garrison["m_use"] && global.civic.garrison.m_use > 0) {
      let merc_bound =
        global.tech["mercs"] && global.tech["mercs"] >= 2 ? 3 : 4;
      let max_merc_roll = global.race["high_pop"]
        ? traits.high_pop.vars()[0]
        : 1;
      let num_restore = 0;
      ...
      global.civic.garrison.m_use = Math.max(
        0,
        global.civic.garrison.m_use - num_restore,
      );
    }
        ...
        switch (global.city.biome) {
          ...
          case "volcanic":
            if (global.city.calendar.season === 1) {
              ...
              Math.rand(0, 2) === 0 &&
              temp < 2 &&
              !global.city.ptrait.includes("permafrost")
            ) {
              temp++;
            }
            break;
    ...
    if (
      ...
      let deterioration =
        Math.floor(50000000 / (1 + global.race.mutation)) - global.stats.days;
      if (global.race.deterioration === 0 && deterioration < 40000000) {
        ...
        messageQueue(
          loc("deterioration1", [flib("name"), death_clock]),
          "danger",
          false,
          ["progress"],
        );
        ...
        let death_clock = Math.round(deterioration / orbitLength());
        messageQueue(
          loc("deterioration2", [flib("name"), death_clock]),
          "danger",
          false,
          ["progress"],
        );
        ...
        let death_clock = Math.round(deterioration / orbitLength());
        messageQueue(
          loc("deterioration3", [flib("name"), death_clock]),
          "danger",
          false,
          ["progress"],
        );
        ...
        let death_clock = Math.round(deterioration / orbitLength());
        messageQueue(
          loc("deterioration4", [flib("name"), death_clock]),
          "danger",
          false,
          ["progress"],
        );
        ...
        global.tech["decay"] = 1;
        messageQueue(loc("deterioration5", [flib("name")]), "danger", false, [
          "progress",
        ]);
        drawTech();
      }
    ...
    if (
      ...
      if (Math.rand(0, 5) === 0) {
        ...
        messageQueue(
          loc("galaxy_piracy_msg", [races[global.galaxy.alien2.id].name]),
          "info",
          false,
          ["progress"],
        );
    ...
    if (
      global.portal["archaeology"] &&
      global.tech.hasOwnProperty("hell_ruins") &&
      global.tech.hell_ruins >= 2 &&
      !global.tech["hell_vault"]
    ) {
      let sup = hellSupression("ruins");
      let value = 250000;
      if (global.race["high_pop"]) {
    ...
    if (global.tech["syndicate"] && global.race["truepath"]) {
      ...
      Object.keys(regions).forEach(function (region) {
        if (
          regions[region].info.hasOwnProperty("syndicate") &&
          regions[region].info.syndicate()
        ) {
          let cap = regions[region].info.hasOwnProperty("syndicate_cap")
            ? regions[region].info.syndicate_cap()
            : 500;
      ...
      if (
        ...
        global.space.shipyard.ships.forEach(function (ship) {
          ...
          if (
            global.tech.hasOwnProperty("eris_scan") &&
            ship.location === "spc_eris" &&
            ship.transit === 0
          ) {
            eScan += sensorRange(ship);
          }
          if (
            global.tech.hasOwnProperty("tauceti") &&
            ship.location === "tauceti" &&
            ship.transit === 0
          ) {
        ...
        if (
          global.tech.hasOwnProperty("eris_scan") &&
          global.tech.hasOwnProperty("eris") &&
          global.tech.eris === 1 &&
          eScan > 50
        ) {
          ...
          if (global.tech.eris_scan >= 100) {
            ...
            global.tech.eris = 2;
            messageQueue(
              loc("space_eris_scan", [planetName().eris]),
              "info",
              false,
              ["progress"],
            );
        ...
        if (global.space.hasOwnProperty("position")) {
          Object.keys(spacePlanetStats).forEach(function (planet) {
            if (global.space.position.hasOwnProperty(planet)) {
              let orbit =
                spacePlanetStats[planet].orbit === -1
                  ? orbitLength()
    ...
    if (
      !global.race["warlord"] &&
      (global.stats.matrix > 0 || global.stats.retire > 0) &&
      !global.race["servants"] &&
      Math.rand(0, 25) === 0
    ) {
      let womlings =
        Math.min(global.stats.matrix, 100) +
        Math.min(global.stats.retire, 100) +
        Math.min(global.stats.eden, 100);
      let skilled = Math.min(
        Math.min(global.stats.matrix, global.stats.retire),
        100,
      );
      skilled +=
        global.stats.achieve["pathfinder"] &&
        global.stats.achieve.pathfinder.l >= 5
          ? 2
          : 0;
      if (
        global.stats.achieve["overlord"] &&
    ...
    if (
      global.race["truepath"] &&
      global.tech["focus_cure"] &&
      global.tech.focus_cure >= 2 &&
      global.tauceti["infectious_disease_lab"]
    ) {
      if (global.tauceti.infectious_disease_lab.cure < 100) {
        ...
        if (races[global.race.species].type === "synthetic") {
          messageQueue(loc("tech_decode_virus_msg2s"), "info", false, [
            "progress",
          ]);
        } else {
          messageQueue(loc("tech_decode_virus_msg2"), "info", false, [
            "progress",
          ]);
        }
      } else if (
        global.tech.focus_cure >= 4 &&
        global.race.hasOwnProperty("vax")
      ) {
        let med = global.race["artifical"]
          ? global.city.boot_camp.count
        ...
        if (global.tech.focus_cure === 4 && global.race.vax < 25) {
          ...
          messageQueue(loc("tech_vaccine_campaign_msg1"), "info", false, [
            "progress",
          ]);
          ...
          messageQueue(loc("tech_vaccine_campaign_msg2"), "info", false, [
            "progress",
          ]);
          removeTask("assemble");
          defineGovernor();
    ...
    if (global.race["infiltrator"]) {
      let tech_source = global.tech["world_control"]
        ? `trait_infiltrator_steal_alt`
        : `trait_infiltrator_steal`;
      let know_adjust = traits.infiltrator.vars()[0] / 100;
      if (
        global.resource.Knowledge.max >=
          actions.tech.steel.cost.Knowledge() * know_adjust &&
        !global.race["steelen"] &&
        global.tech["smelting"] &&
        global.tech.smelting === 1
      ) {
        messageQueue(loc(tech_source, [loc("tech_steel")]), "info", false, [
          "progress",
        ]);
      ...
      if (
        global.resource.Knowledge.max >=
          actions.tech.electricity.cost.Knowledge() * know_adjust &&
        global.tech["high_tech"] &&
        global.tech.high_tech === 1
      ) {
        messageQueue(
          loc(tech_source, [loc("tech_electricity")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        global.resource.Knowledge.max >=
          actions.tech.electronics.cost.Knowledge() * know_adjust &&
        global.tech["high_tech"] &&
        global.tech.high_tech === 3 &&
        global.tech["titanium"]
      ) {
        messageQueue(
      ...
      if (
        global.resource.Knowledge.max >=
          actions.tech.fission.cost.Knowledge() * know_adjust &&
        global.tech["high_tech"] &&
        global.tech.high_tech === 4 &&
        global.tech["uranium"]
      ) {
        messageQueue(loc(tech_source, [loc("tech_fission")]), "info", false, [
          "progress",
        ]);
      ...
      if (
        global.resource.Knowledge.max >=
          actions.tech.rocketry.cost.Knowledge() * know_adjust &&
        global.tech["high_tech"] &&
        global.tech.high_tech === 6
      ) {
        messageQueue(loc(tech_source, [loc("tech_rocketry")]), "info", false, [
          "progress",
        ]);
        ...
        if (global.race["truepath"] && !global.tech["rival"]) {
          global.tech["rival"] = 1;
          messageQueue(
            loc(`civics_rival_unlocked`, [govTitle(3)]),
            "info",
            false,
            ["progress", "combat"],
          );
      ...
      if (
        global.resource.Knowledge.max >=
          actions.tech.artifical_intelligence.cost.Knowledge() * know_adjust &&
        global.tech["high_tech"] &&
        global.tech.high_tech === 9
      ) {
        messageQueue(
          loc(tech_source, [loc("tech_artificial_intelligence")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        global.resource.Knowledge.max >=
          actions.tech.quantum_computing.cost.Knowledge() * know_adjust &&
        global.tech["high_tech"] &&
        global.tech.high_tech === 10 &&
        global.tech["nano"]
      ) {
        messageQueue(
          loc(tech_source, [loc("tech_quantum_computing")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        global.resource.Knowledge.max >=
          actions.tech[
            global.race["truepath"] ? "virtual_reality_tp" : "virtual_reality"
          ].cost.Knowledge() *
            know_adjust &&
        global.tech["high_tech"] &&
        global.tech.high_tech === 11 &&
        global.tech["stanene"] &&
        ((global.tech["infernite"] &&
          global.tech["alpha"] &&
          global.tech["alpha"] >= 2) ||
          global.race["truepath"])
      ) {
        messageQueue(
          loc(tech_source, [loc("tech_virtual_reality")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (global.race["truepath"]) {
        if (
          global.resource.Knowledge.max >=
            actions.tech.quantium.cost.Knowledge() * know_adjust &&
          global.tech["supercollider"] &&
          global.tech.supercollider >= 10 &&
          global.tech["enceladus"] &&
          global.tech.enceladus >= 3 &&
          !global.tech["quantium"]
        ) {
          messageQueue(
            loc(tech_source, [loc("tech_quantium")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          global.resource.Knowledge.max >=
            actions.tech.alien_biotech.cost.Knowledge() * know_adjust &&
          global.tech["genetics"] &&
          global.tech.genetics >= 8 &&
          global.tech["kuiper"] &&
          !global.tech["biotech"]
          ...
          messageQueue(
            loc(tech_source, [loc("tech_alien_biotech")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          global.resource.Knowledge.max >=
            actions.tech.shields.cost.Knowledge() * know_adjust &&
          global.tech["high_tech"] &&
          global.tech.high_tech === 13
        ) {
          messageQueue(loc(tech_source, [loc("tech_shields")]), "info", false, [
            "progress",
          ]);
        ...
        if (
          global.resource.Knowledge.max >=
            actions.tech.ai_core.cost.Knowledge() * know_adjust &&
          global.tech["high_tech"] &&
          global.tech.high_tech === 14 &&
          global.tech["blackhole"] &&
          global.tech["blackhole"] >= 3
        ) {
          messageQueue(loc(tech_source, [loc("tech_ai_core")]), "info", false, [
            "progress",
          ]);
        ...
        if (
          global.resource.Knowledge.max >=
            actions.tech.graphene_processing.cost.Knowledge() * know_adjust &&
          global.tech["ai_core"] &&
          global.tech.ai_core === 2
        ) {
          messageQueue(
            loc(tech_source, [loc("tech_graphene_processing")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          global.resource.Knowledge.max >=
            actions.tech.nanoweave.cost.Knowledge() * know_adjust &&
          global.tech["science"] &&
          global.tech.science >= 18 &&
          !global.tech["nanoweave"]
        ) {
          messageQueue(
            loc(tech_source, [loc("tech_nanoweave")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          global.resource.Knowledge.max >=
            actions.tech.orichalcum_analysis.cost.Knowledge() * know_adjust &&
          global.tech["high_tech"] &&
          global.tech.high_tech === 16 &&
          global.tech["chthonian"] &&
          global.tech["chthonian"] >= 3
        ) {
          messageQueue(
            loc(tech_source, [loc("tech_orichalcum_analysis")]),
            "info",
            false,
            ["progress"],
          );
          messageQueue(loc("tech_orichalcum_analysis_result"), "info", false, [
            "progress",
          ]);
        ...
        if (
          global.resource.Knowledge.max >=
            actions.tech.infernium_fuel.cost.Knowledge() * know_adjust &&
          global.tech["smelting"] &&
          global.tech.smelting === 7 &&
          global.tech["hell_ruins"] &&
          global.tech["hell_ruins"] >= 4
        ) {
          messageQueue(
            loc(tech_source, [loc("tech_infernium_fuel")]),
            "info",
            false,
            ["progress"],
          );
    ...
    if (moldFathom > 0) {
      ...
      let know_adjust =
        1 - ((100 - traits.infiltrator.vars(1)[0]) * moldFathom) / 100;
      if (
        moldFathom >= 0.02 &&
        global.resource.Knowledge.max >=
          actions.tech.smelting.cost.Knowledge() * know_adjust &&
        checkTechRequirements("smelting", false) &&
        !global.tech["smelting"]
      ) {
        messageQueue(loc(tech_source, [loc("tech_smelting")]), "info", false, [
          "progress",
        ]);
        global.tech["smelting"] = 1;
      ...
      if (
        moldFathom >= 0.04 &&
        global.resource.Knowledge.max >=
          actions.tech.dynamite.cost.Knowledge() * know_adjust &&
        checkTechRequirements("dynamite", false) &&
        global.tech["explosives"] &&
        global.tech.explosives === 1
      ) {
        messageQueue(loc(tech_source, [loc("tech_dynamite")]), "info", false, [
          "progress",
        ]);
      ...
      if (
        moldFathom >= 0.08 &&
        global.resource.Knowledge.max >=
          actions.tech.portland_cement.cost.Knowledge() * know_adjust &&
        checkTechRequirements("portland_cement", false) &&
        global.tech["cement"] &&
        global.tech.cement === 3
      ) {
        messageQueue(
          loc(tech_source, [loc("tech_portland_cement")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        moldFathom >= 0.12 &&
        global.resource.Knowledge.max >=
          actions.tech.oxygen_converter.cost.Knowledge() * know_adjust &&
        checkTechRequirements("oxygen_converter", false) &&
        global.tech["smelting"] &&
        global.tech.smelting === 4
      ) {
        messageQueue(
          loc(tech_source, [loc("tech_oxygen_converter")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        moldFathom >= 0.15 &&
        global.resource.Knowledge.max >=
          actions.tech.machinery.cost.Knowledge() * know_adjust &&
        checkTechRequirements("machinery", false) &&
        global.tech["foundry"] &&
        global.tech.foundry === 6
      ) {
        messageQueue(loc(tech_source, [loc("tech_machinery")]), "info", false, [
          "progress",
        ]);
      ...
      if (
        moldFathom >= 0.2 &&
        global.resource.Knowledge.max >=
          actions.tech.uranium_storage.cost.Knowledge() * know_adjust &&
        checkTechRequirements("uranium_storage", false) &&
        global.tech["uranium"] &&
        global.tech.uranium === 1
      ) {
        messageQueue(
          loc(tech_source, [loc("tech_uranium_storage")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        moldFathom >= 0.25 &&
        global.resource.Knowledge.max >=
          actions.tech.synthetic_fur.cost.Knowledge() * know_adjust &&
        checkTechRequirements("synthetic_fur", false) &&
        !global.tech["synthetic_fur"]
      ) {
        messageQueue(
          loc(tech_source, [actions.tech.synthetic_fur.title()]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        moldFathom >= 0.35 &&
        global.resource.Knowledge.max >=
          actions.tech.rover.cost.Knowledge() * know_adjust &&
        checkTechRequirements("rover", false) &&
        global.tech["space_explore"] &&
        global.tech.space_explore === 1
      ) {
        messageQueue(loc(tech_source, [loc("tech_rover")]), "info", false, [
          "progress",
        ]);
      ...
      if (
        moldFathom >= 0.4 &&
        global.resource.Knowledge.max >=
          actions.tech.starcharts.cost.Knowledge() * know_adjust &&
        checkTechRequirements("starcharts", false) &&
        global.tech["space_explore"] &&
        global.tech.space_explore === 3
      ) {
        messageQueue(
          loc(late_tech_source, [loc("tech_starcharts")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (
        moldFathom >= 0.5 &&
        global.resource.Knowledge.max >=
          actions.tech.nano_tubes.cost.Knowledge() * know_adjust &&
        checkTechRequirements("nano_tubes", false) &&
        !global.tech["nano"]
      ) {
        messageQueue(
          loc(late_tech_source, [loc("tech_nano_tubes")]),
          "info",
          false,
          ["progress"],
        );
      ...
      if (global.race["truepath"]) {
        if (
          moldFathom >= 0.65 &&
          global.resource.Knowledge.max >=
            actions.tech.stanene_tp.cost.Knowledge() * know_adjust &&
          checkTechRequirements("stanene_tp", false) &&
          !global.tech["stanene"]
        ) {
          messageQueue(
            loc(late_tech_source, [loc("tech_stanene")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          moldFathom >= 0.8 &&
          global.resource.Knowledge.max >=
            actions.tech.anitgrav_bunk.cost.Knowledge() * know_adjust &&
          checkTechRequirements("anitgrav_bunk", false) &&
          global.tech["marines"] &&
          global.tech.marines === 1
        ) {
          messageQueue(
            loc(late_tech_source, [loc("tech_anitgrav_bunk")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          moldFathom >= 0.65 &&
          global.resource.Knowledge.max >=
            actions.tech.stanene.cost.Knowledge() * know_adjust &&
          checkTechRequirements("stanene", false) &&
          !global.tech["stanene"]
        ) {
          messageQueue(
            loc(late_tech_source, [loc("tech_stanene")]),
        ...
        if (
          moldFathom >= 0.78 &&
          global.resource.Knowledge.max >=
            actions.tech.hydroponics.cost.Knowledge() * know_adjust &&
          checkTechRequirements("hydroponics", false) &&
          global.tech["mars"] &&
          global.tech.mars === 5
        ) {
          messageQueue(
            loc(late_tech_source, [loc("tech_hydroponics")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          moldFathom >= 0.92 &&
          global.resource.Knowledge.max >=
            actions.tech.orichalcum_panels.cost.Knowledge() * know_adjust &&
          checkTechRequirements("orichalcum_panels", false) &&
          global.tech["swarm"] &&
          global.tech.swarm === 5
        ) {
          messageQueue(
            loc(late_tech_source, [loc("tech_orichalcum_panels")]),
            "info",
            false,
            ["progress"],
          );
        ...
        if (
          moldFathom >= 1 &&
          global.resource.Knowledge.max >=
            actions.tech.cybernetics.cost.Knowledge() * know_adjust &&
          checkTechRequirements("cybernetics", false) &&
          global.tech["high_tech"] &&
          global.tech.high_tech === 17
        ) {
          messageQueue(
            loc(late_tech_source, [loc("tech_cybernetics")]),
            "info",
            false,
            ["progress"],
          );
    ...
    if (
      global.race["truepath"] &&
      global.tech["tauceti"] &&
      global.tech.tauceti === 3 &&
      global.space.hasOwnProperty("jump_gate") &&
      global.tauceti.hasOwnProperty("jump_gate") &&
      global.space.jump_gate.count >= 100 &&
      global.tauceti.jump_gate.count >= 100
    ) {
    ...
    if (
      ...
      if (
        global.tech.tauceti === 5 &&
        !global.tech["plague"] &&
        Math.rand(0, 50) === 0
      ) {
        global.tech["plague"] = 1;
        messageQueue(loc("tau_plague", [govTitle(3)]), "info", false, [
          "progress",
        ]);
      } else if (
        global.tech["plague"] &&
        global.tech["tau_roid"] &&
        global.tech["tau_whale"]
      ) {
        if (
          global.tech.plague === 1 &&
          (global.tech.tau_roid >= 4 || global.tech.tau_whale >= 2) &&
          Math.rand(0, 50) === 0
        ) {
          global.tech.plague = 2;
          global.race["quarantine"] = 1;
          global.race["qDays"] = 0;
          messageQueue(loc("tau_plague2", [govTitle(3)]), "info", false, [
            "progress",
          ]);
        } else if (
          global.tech.plague === 2 &&
          global.tech.tau_roid >= 5 &&
          ...
          global.race["qDays"] = 0;
          messageQueue(
            loc("tau_plague3", [govTitle(3), races[global.race.species].home]),
            "info",
            false,
            ["progress"],
          );
          ...
          if (global.tech.plague < 5 && Math.rand(0, 50) === 0) {
            ...
            messageQueue(
              loc("tau_plague5b", [races[global.race.species].home]),
              "info",
              false,
              ["progress"],
            );
          ...
          global.race["qDays"] = 0;
          messageQueue(
            loc("tau_plague5a", [races[global.race.species].home]),
            "info",
            false,
            ["progress"],
          );
    ...
    if (
      ...
      global.interstellar["stellar_engine"] &&
      global.interstellar.stellar_engine.exotic >= 0.025
    ) {
      ...
      if (
        !global.race.governor.config.hasOwnProperty("trash") ||
        (global.race.governor.config.hasOwnProperty("trash") &&
          !global.race.governor.config.trash["stab"])
      ) {
        messageQueue(loc("interstellar_blackhole_unstable"), "danger", false, [
          "progress",
        ]);
      }
      drawTech();
    ...
    if (
      ...
      messageQueue(loc("outer_syndicate", [govTitle(4)]), "info", false, [
        "progress",
      ]);
    ...
    if (global.race["orbit_decay"]) {
      if (!global.race["orbit_decayed"]) {
        $(`#infoTimer`).html(
          `T-${global.race["orbit_decay"] - global.stats.days}`,
        );
      }
      orbitDecayed();
    }
    if (
      global.race["truepath"] &&
      global.city.ptrait.includes("kamikaze") &&
      orbitLength() <= 10
    ) {
  ...
  if (!global.race.seeded || (global.race.seeded && global.race["chose"])) {
    ...
    if (global.race.species !== "protoplasm") {
      if (Math.rand(0, global.m_event.t) === 0) {
        ...
        if (event_pool.length > 0) {
          let event =
            event_pool[Math.floor(seededRandom(0, event_pool.length))];
          let msg = events[event].effect();
    ...
    if (
      global.race["witch_hunter"] &&
      global.resource.Sus.amount >= 50 &&
      global.civic.scientist.workers > 0
    ) {
    ...
    if (
      global.stats.achieve["endless_hunger"] &&
      global.city.banquet &&
      global.city.banquet.on
    ) {
      global.city.banquet.strength++;
  ...
  if (
    ...
    (global.stats.days + global.stats.tdays) % 100000 === 99999
  ) {
...
function buildGene(blockGeneBuffer = false) {
  ...
  if (
    ...
    global.resource.Knowledge.amount >= global.resource.Knowledge.max - buffer
  ) {
    global.resource.Knowledge.amount -= 200000;
    let gene = global.genes["synthesis"]
      ? sythMap[global.genes["synthesis"]]
      : 1;
    global.resource.Genes.amount += gene;
  }
}
...
function diffCalc(res, period) {
  ...
  global.resource[res].diff = +(
    global.resource[res].delta /
    (period / sec)
  ).toFixed(2);
  global.resource[res].delta = 0;

  if (
    global.resource[res].hasOwnProperty("gen") &&
    global.resource[res].hasOwnProperty("gen_d")
  ) {
    global.resource[res].gen = +(
      global.resource[res].gen_d /
      (period / sec)
    ).toFixed(2);
  ...
  if (global.race["decay"]) {
    if (global.resource[res].diff < 0) {
      if (
        global.resource[res].diff >=
        breakdown.p.consume[res][loc("evo_challenge_decay")]
      ) {
      ...
      (el.hasClass("has-text-danger") || el.hasClass("has-text-warning"))
    ) {
      el.removeClass("has-text-danger");
      el.removeClass("has-text-warning");
    }
...
function resourceAlt() {
  [
    "#resources > .resource",
    ".tab-item > .market-item",
    "#galaxyTrade > .market-item",
  ].forEach(function (id) {
    let alt = false;
...
function spyCaught(i) {
  let escape =
    global.race["elusive"] || Math.floor(seededRandom(0, 3)) === 0
      ? true
      : false;
  ...
  if (!escape && Math.floor(seededRandom(0, 4)) === 0) {
    messageQueue(loc("event_spy_sellout", [govTitle(i)]), "danger", false, [
      "spy",
    ]);
    ...
    messageQueue(
      loc(escape ? "event_spy_fail" : "event_spy", [govTitle(i)]),
      "danger",
      false,
      ["spy"],
    );
  ...
  $.ajax({
    ...
    success: function (res) {
      if (
        ...
        $("#topBar .version > a").html(
          `<span class="has-text-warning">${loc(`update_avail`)}</span> v` +
            global.version +
            revision,
        );
      }

...
if (global.genes['geneReset'] && !global.genes.geneReset['found']){
    ...
    // Add mutation discoveries, seeding Tactical for saves without a gene library.
    let owed = global.race['mutation'] || 0;
    let found = 0;
    for (let i=0; i<owed; i++){
        if (i === 0 && findMinorTrait('tactical')){ found++; continue; }
...
function fastLoop(){
    ...
    if (global.race.species === 'protoplasm'){
        ...
        if (global.tech['anthropology'] && global.tech['anthropology'] >= 4 && global.race['truepath']){
            let merchsales = hugeAdjust(global.resource[global.race.species].amount) * templeCount() * 0.08 * templeOutputBonus();

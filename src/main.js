...
function fastLoop(){
    ...
    if ((global.city.ptrait.includes('trashed') || global.race['scavenger'] || global.race['scrounger'] || (global.race['servants'] && global.race.servants['force_scavenger'])) && global.civic['scavenger']){
        let scavenger = global.city.ptrait.includes('trashed') || global.race['scrounger'] || global.race['scavenger'] ? workerScale(global.civic.scavenger.workers,'scavenger') : 0;
        ...
        if (scavenger > 0){
            ...
            breakdown.p['Global'][job_data.scavenger.name()] = bonus+'%';
            global_multiplier *= 1 + (bonus / 100);
        }
    }
    if (global.race['unfathomable'] && global.city['surfaceDwellers'] && global.city['captive_housing']){
        ...
        if (global.city.hasOwnProperty('surfaceDwellers')){
            ...
            if (thralls > hugeAdjust(global.civic.torturer.workers) * rank / 2){
                let unsupervised = thralls - hugeAdjust(global.civic.torturer.workers) * rank / 2;
    ...
    if (global.civic.govern.type === 'anarchy' && global.resource[global.race.species].amount > Math.floor(jobScale(10) / hugeScale(1))){
        let chaos = (global.resource[global.race.species].amount - Math.floor(jobScale(10) / hugeScale(1))) * (0.25 * hugeScale(1) / jobScale(1));
    ...
    if (global.race.species === 'protoplasm'){
        ...
        if (!global.city.ptrait.includes('mellow')){
            let unemployed = global.civic.unemployed.workers / jobScale(1) * hugeScale(1);
            morale -= unemployed;
        ...
        if (global.civic.hunter.display && (global.race['playful'] || vulFathom > 0)){
            ...
            stress -= hugeAdjust(highPopAdjust(global.civic.hunter.workers)) / divisor;
        }
            ...
            Object.keys(global.race.alchemy).forEach(function (res){
                if (global.race.alchemy[res] > 0){
                    ...
                    if (trasmute >= time_multiplier){
                        ...
                        modRes(res,trasmute * time_multiplier * rate, false, supplyRegionKey('city'));
                        modRes('Mana', -(trasmute * time_multiplier));
                        modRes('Crystal', -(trasmute * 0.15 * time_multiplier), false, supplyRegionKey('city'));
                        totMana -= trasmute;
        ...
        if (global.galaxy['trade'] && (gal_on.hasOwnProperty('freighter') || gal_on.hasOwnProperty('super_freighter'))){
            ...
            for (let i=0; i<offers.length; i++){
                ...
                imprt_vol = hugeAdjust(imprt_vol);
                exprt_vol = hugeAdjust(exprt_vol);
        ...
        if (global.race['deconstructor'] && global.city['nanite_factory']){
            nf_resources.forEach(function(r){
                if (global.resource[r].display){
                    let vol = hugeAdjust(global.city.nanite_factory[r] * time_multiplier);
                    if (vol > 0){
                        if (global.resource[r].amount < vol){
                            vol = global.resource[r].amount;
                        }
                        if (modRes(r,-(vol), false, supplyRegionKey('city'))){
        ...
        if(global.race['artifical'] && global.race['iceage']){
            let output = 25;
            max_power -= output;
            power_grid += output;
            power_generated[loc('underground_old_device')] = output;
        }
        ...
        if (global.interstellar['elysanite_sphere'] && global.interstellar.elysanite_sphere.count > 0){
            ...
            if (global.interstellar.elysanite_sphere.count >= 1000){
                output = powerModifier(22500, true);
            }
            else {
                output = powerModifier(1750 + (global.interstellar.elysanite_sphere.count * 18), true);
            }
            max_power -= output;
            power_grid += output;
            ...
            if (global.interstellar.orichalcum_sphere.count >= 100){
                output = powerModifier(1750, true);
            }
            else {
                output = powerModifier(750 + (global.interstellar.orichalcum_sphere.count * 8), true);
            }
            max_power -= output;
            power_grid += output;
            power_generated[loc('interstellar_dyson_sphere_title')] = output;
            ...
            if (global.interstellar.dyson_sphere.count >= 100){
                output = powerModifier(750, true);
            }
            else {
                output = powerModifier(175 + (global.interstellar.dyson_sphere.count * 5), true);
            }
            max_power -= output;
            power_grid += output;
            power_generated[loc('interstellar_dyson_sphere_title')] = output;
            ...
            if (global.interstellar.dyson.count >= 100){
                output = powerModifier(175, true);
            }
            else {
                output = powerModifier(global.interstellar.dyson.count * 1.25, true);
            }
            max_power -= output;
            power_grid += output;
        ...
        if (global.underground['core_tap_perk']){
            let output = -(global.underground['core_tap_perk'].count * actions.underground.cave_perk.core_tap_perk.powered());
            max_power -= output;
            power_grid += output;
        ...
        if (global.race['wish'] && global.race['wishStats'] && global.race.wishStats.potato){
            let power = powerModifier(global.race.wishStats.potato, true);
            max_power -= power;
            power_grid += power;
        ...
        if (global.space['fob'] && global.space['lander']){
            if (p_on['fob']){
                ...
                for (let i=0; i<support_on['lander']; i++){
                    if (!modRes('Oil', -(time_multiplier * fuel), false, 'spc_triton')){
        ...
        if (p_on['s_gate'] && p_on['foothold']){
            ...
            modRes('Elerium', -(number), false, 'int_blackhole');
        }
        ...
        if (global.tech['womling_technicians']){
            womling_technician = 1 + hugeAdjust(p_on['womling_station'] * (global.tech['isolation'] ? 0.30 : 0.08));
        ...
        if (global.space['space_barracks'] && !global.race['fasting']){
            let oil_cost = +hugeAdjust(fuel_adjust(2,true));
            ...
            for (let i=0; i<global.space.space_barracks.on; i++){
                if (!modRes('Oil', -(time_multiplier * oil_cost), false, 'spc_red')){
        ...
        if (p_on['red_factory'] && p_on['red_factory'] > 0){
            let h_consume = p_on['red_factory'] * hugeAdjust(fuel_adjust(1,true));
            modRes('Helium_3',-(h_consume * time_multiplier), false, 'spc_red');
            breakdown.p.consume.Helium_3[structName('factory')] = -(h_consume);
        }

        if (p_on['int_factory'] && p_on['int_factory'] > 0){
            let d_consume = p_on['int_factory'] * hugeAdjust(int_fuel_adjust(5));
            modRes('Deuterium',-(d_consume * time_multiplier), false, 'int_alpha');
        ...
        if (global.space['fob'] && p_on['fob']){
            ...
            for (let i=0; i<p_on['fob']; i++){
                if (!modRes('Helium_3', -(time_multiplier * fuel_cost), false, 'spc_triton')){
        ...
        if (p_on['outpost'] && p_on['outpost'] > 0){
            let fuel_cost = +hugeAdjust(fuel_adjust(2,true));
            ...
            for (let i=0; i<p_on['outpost']; i++){
                if (!modRes('Oil', -(time_multiplier * fuel_cost), false, 'spc_gas_moon')){
        ...
        if (p_on['neutron_miner'] && p_on['neutron_miner'] > 0){
            let fuel_cost = +hugeAdjust(int_fuel_adjust(3));
            ...
            for (let i=0; i<p_on['neutron_miner']; i++){
                if (!modRes('Helium_3', -(time_multiplier * fuel_cost), false, 'int_neutron')){
        ...
        if (global.interstellar['cruiser']){
            let fuel_cost = +hugeAdjust(int_fuel_adjust(6));
            ...
            for (let i=0; i<global.interstellar['cruiser'].on; i++){
                if (!modRes('Helium_3', -(time_multiplier * fuel_cost), false, 'int_proxima')){
        ...
        if (global.eden['pillbox']){
            let pillsize = hugeScale(jobStack(10));
            if (p_on['pillbox']){
        ...
        if (global.race['replicator'] && p_on['replicator']){
            // Both lines are fed from the power actually delivered, so a brownout thins them together.
            replicatorLines(p_on['replicator']).forEach(function(line){
                if (line.pow <= 0 || ['Asphodel_Powder','Elysanite'].includes(line.res)){ return; }
                let vol = replicator(line.res,line.pow);
                // The UI stops the two lines sharing a resource, but a save predating that rule can still
                // have them matched — accumulate rather than overwrite so the tooltip stays honest.
                let bd = breakdown.p.consume[line.res][loc('tau_replicator_db')] || 0;
                breakdown.p.consume[line.res][loc('tau_replicator_db')] = bd + vol;
                modRes(line.res, time_multiplier * vol);
            });
        }
                    ...
                    if (global[area][ship].hasOwnProperty('on') && req && (p_on['s_gate'] || area !== 'galaxy')){
                        ...
                        if (actions[area][region][ship].hasOwnProperty('ship')){
                            if (actions[area][region][ship].ship.civ && global[area][ship].hasOwnProperty('crew')){
                                ...
                                if (civPerShip > 0){
                                    ...
                                    if (global[area][ship].crew < operating * civPerShip){
                                        ...
                                        global.civic.crew.workers -= civPerShip;
                                        global[area][ship].crew -= civPerShip;*/
                                        let difference = global[area][ship].crew - (operating * civPerShip);
                                        global.civic[global.civic.d_job].workers += difference;
                                        global.civic.crew.workers -= difference;
                                        global[area][ship].crew -= difference;
                                    }
                            ...
                            if (actions[area][region][ship].ship.hasOwnProperty('helium')){
                                let increment = +hugeAdjust(int_fuel_adjust(actions[area][region][ship].ship.helium)).toFixed(2);
                                ...
                                modRes('Helium_3', -(consume * time_multiplier), false, 'int_blackhole');
                                andromeda_helium += consume;
                            }

                            if (actions[area][region][ship].ship.hasOwnProperty('deuterium')){
                                let increment = +hugeAdjust(int_fuel_adjust(actions[area][region][ship].ship.deuterium)).toFixed(2);
                                let consume = operating * increment;
                                ...
                                modRes('Deuterium', -(consume * time_multiplier), false, 'int_blackhole');
        ...
        if(global.underground['color_garden']){
            ...
            let water_consumption = workers;
            if(water_consumption * time_multiplier > global.resource.Water.amount){
                workers = global.resource.Water.amount / 1.5;
            }
            ...
            global.underground['color_garden'].mushrooms *= 0.9902 ** time_multiplier;
            
            let effect = Math.floor(global.underground['color_garden'].mushrooms) * actions.underground.depths.color_garden.mushroom_effect();
            global.city.morale.gardener = +effect.toFixed(1);
            morale += effect;
        }

        if(p_on['bonfire']){
            global.city.morale.under_bonfire = hugeAdjust(p_on['bonfire']) * 2;
            morale += hugeAdjust(p_on['bonfire']) * 2;
        }
        else{
            global.city.morale.under_bonfire = 0;
        }
        if(p_on['under_casino']){
            global.city.morale.casino = hugeAdjust(p_on['under_casino']) * 2;
            morale += hugeAdjust(p_on['under_casino']) * 2;
        }
        else{
        ...
        if (global.race['nostalgic']){
            ...
            let penalty = traits.nostalgic.vars()[0] * -factor;
            global.city.morale.nostalgic = penalty;
            morale += global.city.morale.nostalgic;
        }
        else{
            global.city.morale.nostalgic = 0;
        }
        ...
        if (support_on['vr_center'] && !global.race['joyless']){
            ...
            global.city.morale.vr = hugeAdjust(support_on['vr_center']) * vr_morale;
            morale += hugeAdjust(support_on['vr_center']) * vr_morale;
        }
        else {
            global.city.morale.vr = 0;
        }
        if (int_on['zoo'] && !global.race['fasting']){
            global.city.morale.zoo = hugeAdjust(int_on['zoo']) * 5;
            morale += hugeAdjust(int_on['zoo']) * 5;
        }
        else if(p_on['surface_zoo']){
            let info = global.surface.overview;
            if(info && Math.min(global.surface.herbivores?.count, global.surface.carnivores?.count, global.surface.scavengers?.count) >= 10){
                morale += hugeAdjust(p_on['surface_zoo']) * 4;
                global.city.morale.zoo = hugeAdjust(p_on['surface_zoo']) * 4;
            }
        }
        else{
        ...
        if (p_on['tavern'] && !global.race['joyless']){
            global.city.morale.tavern = hugeAdjust(p_on['tavern']) * hugeAdjust(p_on['shadow_mine']) * 0.35;
            morale += hugeAdjust(p_on['tavern']) * hugeAdjust(p_on['shadow_mine']) * 0.35;
        }
        else {
            global.city.morale.tavern = 0;
        }
        if (support_on['bliss_den'] && !global.race['joyless']){
            global.city.morale.bliss_den = hugeAdjust(support_on['bliss_den']) * 8;
            morale += hugeAdjust(support_on['bliss_den']) * 8;
        }
        else {
        ...
        if (p_on['restaurant'] && !global.race['fasting'] && !global.race['joyless']){
            let val = 0;
            val += global.eden.hasOwnProperty('pillbox') && p_on['pillbox'] ? 0.35 * hugeAdjust(p_on['pillbox']) : 0;
            val += hugeAdjust(global.civic.elysium_miner.workers) * 0.15;
            val += global.eden.hasOwnProperty('archive') && p_on['archive'] ? 0.4 * hugeAdjust(p_on['archive']) : 0;
            global.city.morale.restaurant = hugeAdjust(p_on['restaurant']) * val;
            morale += hugeAdjust(p_on['restaurant']) * val;
        }
        else {
        ...
        mBaseCap += global.tauceti['tauceti_casino'] ? hugeAdjust(p_on['tauceti_casino']) : 0;
        mBaseCap += global.portal['hell_casino'] ? hugeAdjust(p_on['hell_casino']) : 0;

        if (global.city['amphitheatre']){
            let athVal = govActive('athleticism',0);
            mBaseCap += athVal ? hugeAdjust(global.city.amphitheatre.count * athVal) : hugeAdjust(global.city.amphitheatre.count);
        }
        if(global.underground['color_garden']){
            mBaseCap += hugeAdjust(global.underground['color_garden'].count) * 2;
        }
        if (support_on['vr_center']){
            mBaseCap += hugeAdjust(support_on['vr_center']) * 2;
        }
        if (int_on['zoo'] && !global.race['fasting']){
            mBaseCap += hugeAdjust(int_on['zoo']) * 2;
        }
        if (support_on['bliss_den']){
            mBaseCap += hugeAdjust(support_on['bliss_den']) * 2;
        }
        if (p_on['resort']){
            mBaseCap += hugeAdjust(p_on['resort']) * 2;
        }
        ...
        if(p_on['bonfire']){
            moraleCap += hugeAdjust(p_on['bonfire']);
        }
        if(p_on['surface_zoo']){
            let info = global.surface.overview;
            if(info && Math.min(info.herbivores, info.carnivores, info.scavengers) >= 10){
                moraleCap += hugeAdjust(p_on['surface_zoo']) * 3;
            }
        }
        ...
        if (global.interstellar['mass_ejector']){
            ...
            Object.keys(global.interstellar.mass_ejector).forEach(function (res){
                if (atomic_mass[res]){
                    ...
                    let volume = actions.interstellar.int_blackhole.mass_ejector.volume();
                    if (total + ejected > p_on['mass_ejector'] * volume){
                        ejected = p_on['mass_ejector'] * volume - total;
                    }
                    total += ejected;
                    ...
                    modRes(res, -(time_multiplier * ejected), false, 'int_blackhole');
        ...
        if (global.portal['transport'] && global.portal['purifier']){
            ...
            let cargoSize = actions.portal.prtl_lake.transport.cargo_size();
            Object.keys(global.portal.transport.cargo).forEach(function (res){
                if (supplyValue[res]){
                    ...
                    let volume = shipped * hugeAdjust(supplyValue[res].out);
                    while (volume * time_multiplier > global.resource[res].amount && volume > 0){
                        volume -= hugeAdjust(supplyValue[res].out);
                        shipped--;
                    }
                    if (volume > 0){
                    ...
                    let bireme = 1 - (bireme_rating ** hugeAdjust(gal_on['bireme'] || 0));

                    modRes(res, -(time_multiplier * volume), false, 'prtl_lake');
                    supply += Number(shipped * hugeAdjust(supplyValue[res].in) * time_multiplier * bireme);
                }
            });
        ...
        if (global.race['carnivore'] && !global.race['herbivore'] && !global.race['soul_eater'] && !global.race['artifical']){
            if (global.resource['Food'].amount > 10){
                ...
                if (global.city['smokehouse']){
                    rot *= 0.9 ** hugeAdjust(global.city.smokehouse.count);
                }
                modRes('Food', -(rot * time_multiplier), false, 'spc_home');
        ...
        if (global.race['gnawer']){
            ...
            if (global.resource[res].display){
                ...
                // Chewed by the citizens themselves, so each zone pays for the mouths it houses.
                let gnawed = zoneTally();
                for (const zone of citizenZones()){
                    gnawed.add(zone, -res_cost * citizenShare(zone));
                }
                gnawed.apply(res, time_multiplier);
            }
        }
        ...
        if (global.resource[global.race.species].amount >= 1 || global.city['farm'] || global.city['soul_well'] || global.city['compost'] || global.city['tourist_center'] || global.city['transmitter']){
            ...
            if (global.race['artifical']){
                ...
                if (global.race['detritivore'] && !global.race['iceage']){
                    ...
                    breakdown.p['Food'][job_data.hunter.name()] = food_hunt + 'v';

                    if (global.race['carnivore'] && global.city['lodge'] && food_hunt > 0){
                        food_hunt *= 1 + hugeAdjust(global.city.lodge.count / 20);
                        breakdown.p['Food'][`ᄂ${loc('city_lodge')}`] = (global.city.lodge.count * 5) + '%';
                    }

                    if (global.city['soul_well']){
                        let souls = hugeAdjust(global.city['soul_well'].count) * (global.race['ghostly'] ? (2 + traits.ghostly.vars()[1]) : 2);
                    ...
                    if (global.city['captive_housing']){
                        ...
                        if (global.city.captive_housing.cattle > 0){
                            ...
                            if (global.resource.Food.amount < global.resource.Food.max * 0.01){
                                global.city.captive_housing.cattle--;
                                modRes('Food', 1000 * production('psychic_boost','Food') * global_multiplier, true, supplyRegionKey('city'));
                                global.stats.cattle++;
                            }
                        }
                    }
                    ...
                    if (global.race['forager']){
                        ...
                        breakdown.p['Food'][job_data.forager.name()] = food_forage + 'v';
                        food_base += food_forage;
                    }

                    if (global.race['warlord']){
                        let food = workerScale(global.civic.lumberjack.workers,'farmer') * farmerValue(true);
                        breakdown.p['Food'][job_data.lumberjack.name()] = food + 'v';
                        food_base += food;
                    }
                    ...
                    if (global.city['farm']){
                        ...
                        if (global.city['mill']){
                            ...
                            mill_multiplier += hugeAdjust(working * mill_bonus);
                        }
            ...
            if(global.underground['mushroom_farm'] || global.underground['under_transmitter']){
                let powered = p_on['under_transmitter'] || 0;
                let farmers = workerScale(global.civic.farmer.workers + jobScale(powered),'farmer');
                let farmhands = 0;
                let farm_count = (global.race['artifical'] ? global.underground['under_transmitter']?.count : global.underground['mushroom_farm']?.count) + powered;
                ...
                if (global.race['servants']){
                    let servants = global.race.servants.jobs.farmer;
                    let servehands = 0;
                    let open = farm_count - (farmers / (jobScale(1)));
                    if (servants > open){
                        servehands = servants - open;
                        servants = open;
                    }
                ...
                breakdown.p['Food'][job_data.farmer.name()] = (food) + 'v';
                breakdown.p.consume.Water[job_data.farmer.name()] = -(water_use);
                food_base += (food * virgo * production('psychic_boost','Food'));
                modRes('Water', -(water_use * time_multiplier));
            ...
            if(p_on['surface_farm']){
                let produce = p_on['surface_farm'] * production('surface_farm');
                let virgo = astroSign === 'virgo' ? 1 + (astroVal('virgo')[0] / 100) : 1;
                breakdown.p['Food'][structName('surface_farm')] = (produce) + 'v';
                food_base += produce * virgo * production('psychic_boost', 'food');
            ...
            if (global.tauceti['tau_farm'] && p_on['tau_farm']){
                ...
                modRes('Food', delta * time_multiplier, false, 'tau_home');
            }
            ...
            // Track food production by its producing zone.
            let grownAt = zoneTally()
                .add(supplyRegionKey('city'), food_base)
                .add(supplyRegionKey('city'), hunting * q_multiplier)
                .add(supplyZone('space:biodome'), biodome * red_synd * zigVal * virgo)
                .add(supplyZone('space:nitrogen_harvester'), nitrogen_food * zigVal * virgo)
                .scale(global_multiplier);
            let generated = grownAt.total();
            ...
            let restaurant = 0;
            if(!global.race['fasting']){
                consume = (global.resource[global.race.species].amount + soldiers - ((global.civic.unemployed.workers + global.civic.hunter.workers) * 0.5)) * food_consume_mod;
                if (global.race['forager']){
                    consume -= global.civic.forager.workers;
                }
                if(global.race['ravenous']){ //todo: update this for the supply system. It currently increases food consumption for every colony based on food stored at home
                    ravenous = (global.resource.Food.amount / traits.ravenous.vars()[1]);
                }
                breakdown.p.consume.Food[flib('name')] = -(consume + ravenous);
                if(global.city.banquet && global.city.banquet.on){ //todo: same issues as ravenous ^
                ...
                if (global.interstellar['zoo']){
                    zoo = hugeAdjust(int_on['zoo']) * 12000;
                    breakdown.p.consume.Food[loc('tech_zoo')] = -(zoo);
                }

                if (global.eden['restaurant']){
                    restaurant = hugeAdjust(p_on['restaurant']) * 250000;
            ...
            // Charge food consumption to the zone where it occurs.
            for (const zone of citizenZones()){
                grownAt.add(zone, -(consume + ravenous + tourism) * citizenShare(zone));
            }
            grownAt.add(supplyZone('space:spaceport'), -spaceport)
                   .add(supplyZone('interstellar:starport'), -starport)
                   .add(supplyZone('galaxy:starbase'), -starbase)
                   .add(supplyZone('space:space_station'), -space_station)
                   .add(supplyZone('space:space_barracks'), -space_marines)
                   .add(supplyZone('galaxy:embassy'), -embassy)
                   .add(supplyZone('interstellar:zoo'), -zoo)
                   .add(supplyZone('eden:restaurant'), -restaurant);
            ...
            if (!foodApplied || global.race['fasting']){
                if (global.race['anthropophagite'] && global.resource[global.race.species].amount > 1 && !global.race['fasting']){
                    const rescuePools = localStarvation ? starvingPools : [supplyRegionKey('city')];
                    for (const pool of rescuePools){
                        if (global.resource[global.race.species].amount <= 1){ break; }
                        if (localStarvation ? starveZone(pool) : (global.resource[global.race.species].amount--, true)){
                            modRes('Food', 10000 * traits.anthropophagite.vars()[0], false, pool);
                    ...
                    // An isolated hungry outpost must not mark the home world as unfed. Connected
                    // regions share a pool, and therefore share this outcome.
                    fed = localStarvation ? !starvingPools.includes(supplyPool(supplyRegionKey('city'))) : false;
                    if(global.resource[global.race.species].amount > 0){
                        ...
                        if(global.race['fasting']){
                            meditators = hugeAdjust(highPopAdjust(global.civic.meditator.workers)) * job_data.meditator.impact();
                        }
                        if (global.race['atrophy']){
                            atrophy = traits.atrophy.vars()[0];
                        }
                        if(global.portal['dish_life_infuser'] && global.portal['dish_life_infuser'].on){
                            infusion = 0.95 ** hugeAdjust(global.portal['dish_life_infuser'].on);
                        }
                        ...
                        if (Math.rand(0, 10) === 0){
                            if(global.race['fasting']){
                                ...
                                citizenDeath(starved);
                            }
                            else if (localStarvation){
                                for (const pool of starvingPools){
                                    if (starveZone(pool)){
            ...
            if (global.race['anthropophagite'] && global.resource[global.race.species].amount > 1 && Math.rand(0,400) === 0){
                global.resource[global.race.species].amount--;
                modRes('Food', 10000 * traits.anthropophagite.vars()[0], false, supplyRegionKey('city'));
                global.stats.murders++;
                citizenDeath(1);
            }
        }
        ...
        if (global.portal['fortress'] && global.portal.fortress.walls < 100){
            ...
            if (paid > 0){
                modRes('Stone', -paid, false, supplyRegionKey('city'));
        ...
        if (global.civic.homeless > 0){
            ...
            if (global.race['artifical'] || (global.race['spongy'] && global.city.calendar.weather === 0)){
            ...
            else if (global.race['parasite'] && global.city.calendar.wind === 0 && !global.race['cataclysm'] && !global.race['orbit_decayed'] && !global.race['iceage']){
                // Do Nothing
            }
                ...
                if (global.race['parasite'] && (global.race['cataclysm'] || global.race['orbit_decayed'] || global.race['iceage'])){
        ...
        if(global.portal['dish_soul_steeper'] && global.portal['dish_soul_steeper'].on){
            hunger -= (0.03 + (global.race['malnutrition'] ? 0.01 : 0) + (global.race['angry'] ? -0.01 : 0)) * hugeAdjust(global.portal['dish_soul_steeper'].on);
        }
        hunger = Math.max(hunger, 0);

        // Furs
        if (global.resource.Furs.display){
            let watch_tower = 1 + hugeAdjust(0.04 * support_on['watch_tower'] || 0);
            if (global.race['evil'] || global.race['artifical'] || global.race['unfathomable']){
                ...
                breakdown.p['Furs'][job_data.hunter.name()] = hunters  + 'v';
                if (hunters > 0){
                    breakdown.p['Furs'][`ᄂ${loc('quarantine')}+0`] = ((q_multiplier - 1) * 100) + '%';
                    breakdown.p['Furs'][`ᄂ${loc('surface_watch_tower')}+0`] = (watch_tower * 100) + '%';
                }
                modRes('Furs', hunters * hunger * watch_tower * global_multiplier * time_multiplier * q_multiplier, false, supplyRegionKey('city'));

                if (!global.race['soul_eater'] && global.race['evil'] && !global.race['iceage']){
                    ...
                    breakdown.p['Furs'][job_data.lumberjack.name()] = reclaimers  + 'v';
                    if (reclaimers > 0){
                        breakdown.p['Furs'][`ᄂ${loc('quarantine')}+1`] = ((q_multiplier - 1) * 100) + '%';
                    }
                    modRes('Furs', reclaimers * hunger * global_multiplier * time_multiplier * q_multiplier, false, supplyRegionKey('city'));
                }
            }
            ...
            if (hunting > 0){
                ...
                breakdown.p['Furs'][`ᄂ${loc('surface_watch_tower')}+2`] = ((watch_tower - 1) * 100) + '%';
            }
            modRes('Furs', hunting * hunger * watch_tower * global_multiplier * q_multiplier * time_multiplier, false, supplyRegionKey('city'));
            ...
            if (global.race['forager']){
                ...
                breakdown.p['Furs'][job_data.forager.name()] = forage_base + 'v';
                if (forage_base > 0){
                    breakdown.p['Furs'][`ᄂ${loc('quarantine')}+3`] = ((q_multiplier - 1) * 100) + '%';
                }
                modRes('Furs', forage_base * hunger * q_multiplier * time_multiplier, false, supplyRegionKey('city'));
            }
        }
        ...
        if (global.resource.Furs.display && global.tech['isolation'] && global.tauceti['womling_farm']){
            ...
            modRes('Furs', delta, false, 'tau_red');
        }
        ...
        if (global.resource.Furs.display && geneRank('moltskin') > 0){
            ...
            modRes('Furs', shed * hunger * global_multiplier * time_multiplier, false, supplyRegionKey('city'));
        }

        if ((global.race['unfathomable'] && global.civic.hunter.display) || (global.race['scrounger'] && global.civic.scavenger.display)){
            let weapons = weaponTechModifer();
            let hunters = 0;
            let scavengers = 0;
            if (global.race['unfathomable']){
                let raiders = workerScale(global.civic.hunter.workers,'hunter');
                raiders *= racialTrait(raiders,'hunting');
                hunters += raiders;
                if (global.race['servants']){
            ...
            if (global.race['scrounger']){
                let scav = workerScale(global.civic.scavenger.workers,'hunter');
                scav *= racialTrait(scav,'hunting');
                scav *= traits.scrounger.vars()[0] / 100;
                scavengers += scav;
                if (global.race['servants']){
                    let serve = jobScale(global.race.servants.jobs.scavenger);
                    serve *= servantTrait(global.race.servants.jobs.scavenger,'hunting');
                    serve *= traits.scrounger.vars()[0] / 100;
                    scavengers += highPopAdjust(serve);
                }
            }
            ...
            stealable.forEach(function(res){
                if (global.resource[res].display){
                    let total = 0;
                    for (let i=0;i<2;i++){
                        let type = [hunters, scavengers][i];
                        let raid = type * production('psychic_boost',res) * tradeRatio[res] / 5;
                        ...
                        breakdown.p[res][i === 0 ? job_data.hunter.name() : job_data.scavenger.name()] = raid  + 'v';
                        if (raid > 0){
                            breakdown.p[res][`ᄂ${loc('quarantine')}+99`] = ((q_multiplier - 1) * 100) + '%';
                        }
                        total += raid;
                    }
                    modRes(res, total * hunger * global_multiplier * time_multiplier * q_multiplier, false, supplyRegionKey('city'));
                }
            });
        }
        ...
        { //block scope
            ...
            if(global.race['iceage']){
                sundial_base = Math.max(1, sundial_base / 2);
                if(global.tech['science'] >= 2){
                    sundial_base += 2;
                }
                if(global.tech['science'] >= 5){
                    if(Math.rand(0, actions.underground.cave.stone_slab.breakthrough_chance()) === 0){
                        global.underground['stone_slab'].breakthrough += 1;
                    }
                    sundial_base += global.underground['stone_slab'].breakthrough * 0.5;
            ...
            let prof_impact = global.race['studious'] ? job_data.professor.impact() + traits.studious.vars()[0] : job_data.professor.impact();
            ...
            if (global.tech['anthropology'] && global.tech['anthropology'] >= 3){
                professors_base *= 1 + hugeAdjust(faithTempleCount() * 0.05);
            }
            ...
            if (global.tech['science'] >= 6 && global.city['wardenclyffe']){
                ...
                scientist_base *= 1 + (professor * hugeAdjust(p_on['wardenclyffe']) * 0.01 * infiltratorFactor('city','wardenclyffe'));
            }
            if (global.space['satellite']){
                scientist_base *= 1 + hugeAdjust(global.space.satellite.count * 0.01);
            }
            ...
            breakdown.p['Knowledge'][job_data.professor.name()] = professors_base + 'v';
            if (global.race.universe === 'magic'){
                breakdown.p['Knowledge'][loc('job_wizard')] = scientist_base + 'v';
            }
            else {
                breakdown.p['Knowledge'][job_data.scientist.name()] = scientist_base + 'v';
            }
            breakdown.p['Knowledge'][loc('tau_red_womlings')] = womling + 'v';
            breakdown.p['Knowledge'][loc('hunger')] = ((hunger - 1) * 100) + '%';
            breakdown.p['Knowledge'][global.race['iceage'] ? loc('rock_carving') : global.race['unfathomable'] ? loc('tech_moondial') : loc('tech_sundial')] = sundial_base + 'v';

            if (global.race['iceage'] && global.tech['science'] >= 3){
                sundial_base *= 1 + hugeAdjust(0.05 * jobScale(global.civic.professor.workers));
                breakdown.p['Knowledge'][`ᄂ${job_data.professor.name()}`] = 5 * jobScale(global.civic.professor.workers) + '%';
            }
            if (p_on['under_biolab']){
                let biolab = actions.underground.industry.under_biolab.bio_effect();
                sundial_base *= 1 + (biolab / 100 * p_on['under_biolab']);
                breakdown.p['Knowledge'][`ᄂ${loc('city_biolab')}`] = +(biolab * p_on['under_biolab']).toFixed(2) + '%';
            }
            ...
            if (global.city['library'] || global.race['warlord']){
                ...
                let library_mult = 1 + hugeAdjust(lib_count * lib_multiplier * (global.race['warlord'] ? 1 : infiltratorFactor('city','library')));
            ...
            if (global.tech['isolation'] && support_on['infectious_disease_lab']){
                let lab_mult = 1 + hugeAdjust(support_on['infectious_disease_lab']) * 0.75 * infiltratorFactor('tau_home','infectious_disease_lab');
        ...
        if (global.resource.Omniscience.display){
            if (support_on['research_station']){
                ...
                let ghost_gain = hugeAdjust(support_on['research_station']) * ghost_base * 0.0000325 * corruptor;
            ...
            if (global.tech['science'] && global.tech.science >= 23){
                ...
                breakdown.p['Omniscience'][job_data.scientist.name()] = sci_gain + 'v';
        ...
        if (global.city['factory']){
            ...
            let eff = hugeAdjust(max_factories > 0 ? on_factories / max_factories : 0);
            ...
            if (global.city.factory['Furs'] && global.city.factory['Furs'] > 0){
                ...
                if (delta > 0){
                    ...
                    if (techBonus > 1){
                        ...
                        breakdown.p['Furs'][`ᄂ${job_data.technician.name()}`] = ((techBonus - 1) * 100) + '%';
            ...
            if (global.city.factory['Alloy'] && global.city.factory['Alloy'] > 0){
                ...
                if (delta > 0){
                    ...
                    if (techBonus > 1){
                        ...
                        breakdown.p['Alloy'][`ᄂ${job_data.technician.name()}`] = ((techBonus - 1) * 100) + '%';
            ...
            if (global.city.factory['Polymer'] && global.city.factory['Polymer'] > 0){
                let oilIncrement = global.race['kindling_kindred'] || global.race['smoldering'] || global.race['iceage'] ? f_rate.Polymer.oil_kk[assembly] * eff : f_rate.Polymer.oil[assembly] * eff;
                let lumberIncrement = global.race['kindling_kindred'] || global.race['smoldering'] || global.race['iceage'] ? 0 : f_rate.Polymer.lumber[assembly] * eff;
                ...
                if (delta > 0){
                    ...
                    if (techBonus > 1){
                        ...
                        breakdown.p['Polymer'][`ᄂ${job_data.technician.name()}`] = ((techBonus - 1) * 100) + '%';
            ...
            if (p_on['s_gate'] && global.galaxy['raider'] && gal_on['raider'] > 0){
                ...
                modRes('Polymer', delta * time_multiplier, false, 'gxy_chthonian');
            }
            ...
            if (global.city.factory['Nano'] && global.city.factory['Nano'] > 0){
                ...
                if (delta > 0){
                    ...
                    if (techBonus > 1){
                        ...
                        breakdown.p['Nano_Tube'][`ᄂ${job_data.technician.name()}`] = ((techBonus - 1) * 100) + '%';
        ...
        if (global.resource.Cement.display){
            ...
            if (workDone > 0){
                workDone = Math.min(workDone, global.resource.Stone.amount / unit_price / time_multiplier);
            }
            ...
            if(global.race['iceage']){
                workDone = Math.min(workDone, global.resource.Water.amount / unit_price / time_multiplier);
                let water_cost = workDone * unit_price;
                breakdown.p.consume.Water[loc('city_cement_plant_bd')] = -(water_cost);
                modRes('Water', -(water_cost * time_multiplier));
            }
            let stone_cost = workDone * unit_price;
            ...
            modRes('Stone', -(stone_cost * time_multiplier), false, 'tau_home');
            ...
            // (not like flier ever reaches this point anyways since cement is never visible)
            ...
            if (global.city.powered && p_on['cement_plant']){
                ...
                powered_mult += hugeAdjust(p_on['cement_plant'] * rate);
                power_single += rate;
            }

            if (global.city.powered && p_on['under_foundry']){
                let rate = global.tech['cement'] >= 6 ? 0.08 : 0.05;
                powered_mult += hugeAdjust(p_on['under_foundry'] * rate);
                power_single += rate;
            }
            ...
            if (global.tech['ai_core'] && p_on['citadel'] > 0){
                ...
                ai_core += hugeAdjust(p_on['citadel'] * ai);
            }

            let hell_factory = 1;
            if (global.race['warlord'] && global.portal['hell_factory'] && p_on['hell_factory'] > 0){
                hell_factory += hugeAdjust(p_on['hell_factory']) * 8 * (global.portal.hell_factory.rank - 1) / 100
            }

            let mining_pit = global.tech['isolation'] ? 1 + hugeAdjust(support_on['mining_pit'] * 0.08 * infiltratorFactor('tau_home','mining_pit')) : 1;
            ...
            if(support_on['crater_fabrication']){
                fabrication += 0.02 * hugeAdjust(highPopAdjust(global.civic.crater_worker.workers)) * hugeAdjust(support_on['crater_fabrication']);
            }
            ...
            if (techBonus > 1){
                breakdown.p['Cement'][`ᄂ${job_data.technician.name()}+0`] = ((techBonus - 1) * 100) + '%';
            }
            ...
            let delta = factory_output * ai_core * tauBonus * mining_pit * hell_factory * techBonus;
            ...
            modRes('Cement', delta * time_multiplier, false, supplyRegionKey('city'));
        }
        ...
        if (smelterUnlocked()){
            ...
            if (global.race['forge']){
                ...
                global.city.smelter.Oil = Math.max(0, global.city.smelter.cap - global.city.smelter.Star - global.city.smelter.Inferno - global.city.smelter.Super);
            }
            ...
            let consume_super = global.city.smelter.Super * fuel_config.s_cost;
            ...
            let super_bonus = global.city.smelter.Super;
            ...
            if (consume_super > 0){
                let max_operable = Math.max(0, Math.floor(global.resource.Super_Fuel.amount / (fuel_config.s_cost * time_multiplier)));
                if (max_operable < super_bonus){
                    disable_smelters += super_bonus - max_operable;
                    consume_super = max_operable * fuel_config.s_cost;
                    super_bonus = max_operable;
                }
            }
            ...
            if (super_bonus){
                iron_smelter *= 1 + (super_bonus / 100);
                iridium_smelter *= 1 + (super_bonus / 100);
            }
            if(p_on['core_forge']){
                ...
                iron_smelter *= 1 + hugeAdjust(p_on['core_forge'] / 20 * mineshaft_effect);
                iridium_smelter *= 1 + hugeAdjust(p_on['core_forge'] / 20 * mineshaft_effect);
            }
            ...
            iron_smelter = hugeAdjust(iron_smelter);
            iridium_smelter = hugeAdjust(iridium_smelter);
            ...
            bdShareUse('Super_Fuel', loc('city_smelter'), -(consume_super), smelterAt);
            ...
            applyShare('Super_Fuel', -(consume_super), smelterAt, time_multiplier);
            ...
            if (global.resource.Steel.display){
                let i_increment = hugeAdjust(2);
                let c_increment = hugeAdjust(0.25);
                let iron_consume = steel_smelter * i_increment;
                let coal_consume = steel_smelter * c_increment;
                while ((iron_consume * time_multiplier > global.resource.Iron.amount && iron_consume > 0) || (coal_consume * time_multiplier > global.resource.Coal.amount && coal_consume > 0)){
                    iron_consume -= i_increment;
                    coal_consume -= c_increment;
                    steel_smelter--;
                }
                ...
                if (super_bonus){
                    steel_smelter *= 1 + (super_bonus / 100);
                }
                if (dirtVal){
                ...
                if(p_on['core_forge']){
                    ...
                    steel_smelter *= 1 + hugeAdjust(p_on['core_forge'] / 20 * mineshaft_effect);
                }
                if(global.underground['smelter_perk']){
                ...
                steel_smelter = hugeAdjust(steel_smelter);
        ...
        graph_plants.forEach(function(plant){
            ...
            let eff = max_graph > 0 ? hugeAdjust(plant.active || 0) / max_graph : 0;
            ...
            while (consume_wood * time_multiplier > fuel_on_hand('Lumber') && consume_wood > 0){
                consume_wood -= 350 * eff;
                graphene_production--;
            }
            while (consume_coal * time_multiplier > fuel_on_hand('Coal') && consume_coal > 0){
                consume_coal -= 25 * eff;
                graphene_production--;
            }
            while (consume_oil * time_multiplier > fuel_on_hand('Oil') && consume_oil > 0){
                consume_oil -= 15 * eff;
                graphene_production--;
            }
            ...
            modRes('Oil', -(consume_oil * time_multiplier), false, plant_at);
            ...
            if (global.tech['ai_core'] >= 3){
                ...
                ai += graph * hugeAdjust(p_on['citadel']);
            }
            ...
            if (plant.incin && global.portal.hasOwnProperty('incinerator') && global.portal.incinerator.rank > 1){
                ...
                incinerator += rank * 15 * hugeAdjust(global.portal.incinerator.on) / 100;
            }
            ...
            modRes('Graphene', delta * time_multiplier, false, plant_at);
        });
        ...
        if (global.galaxy['vitreloy_plant'] && p_on['vitreloy_plant'] > 0){
            ...
            let consume_money = hugeAdjust(50000);
            let consume_bolognium = hugeAdjust(2.5);
            let consume_stanene = hugeAdjust(100);
            ...
            if (vitreloy_production > 0){
                ...
                modRes('Bolognium', -(consume_bolognium * time_multiplier), false, 'gxy_alien1');
                modRes('Stanene', -(consume_stanene * time_multiplier), false, 'gxy_alien1');
                ...
                modRes('Vitreloy', vitreloy_production * hunger * global_multiplier * pirate * time_multiplier * zigVal, false, 'gxy_alien1');
            }
        }

        if (p_on['shadow_mine']){
            let attract = p_on['soul_attractor'] ? 1 + hugeAdjust(p_on['soul_attractor'] * (global.tech.pitspawn >= 3 ? 0.2 : 0.1)) : 1;
            ...
            if (global.resource.Vitreloy.display){ // Vitreloy
                ...
                modRes('Vitreloy', mine_delta * time_multiplier, false, 'prtl_pit');
            }
            ...
            if (global.resource.Elerium.display){ // Elerium
                ...
                modRes('Elerium', mine_delta * time_multiplier, false, 'prtl_pit');
            }
            ...
            if (global.resource.Infernite.display){ // Infernite
                ...
                modRes('Infernite', mine_delta * time_multiplier, false, 'prtl_pit');
            }
        }

        if (p_on['s_gate'] && global.galaxy['raider'] && gal_on['raider'] > 0){
            let base = hugeAdjust(gal_on['raider']) * 0.05 * production('psychic_boost','Vitreloy');
            let pirate = piracy('gxy_chthonian');
            let delta = base * global_multiplier * pirate * hunger * zigVal;
            ...
            modRes('Vitreloy', delta * time_multiplier, false, 'gxy_chthonian');
        }
        ...
        if (global.tech['isolation'] && global.tauceti['alien_outpost'] && p_on['alien_outpost']){
            ...
            let decoder_val = 1 + hugeAdjust(decoders * quantum_level / 100 * infiltratorFactor('tau_home','data_decoder'));
        ...
        if (global.tauceti['ore_refinery'] && global.tauceti['mining_ship'] && global.tech['tau_roid'] && global.tech.tau_roid >= 4){
            global.tauceti.ore_refinery.max = Math.floor(hugeAdjust(global.tauceti.ore_refinery.count) * 1000);

            // Refine Ore
            if (global.tauceti.ore_refinery.fill > 0){
                let raw = hugeAdjust(p_on['ore_refinery']) * production('ore_refinery');
                ...
                raw /= hugeAdjust(1); //removed as production() already has its own hugeAdjust
        ...
        if (global.tauceti['synthesizer'] && p_on['synthesizer'] && support_on['mining_ship']){
            ...
            let positronium = p_on['synthesizer'] * base * hugeAdjust(support_on['mining_ship']);
        ...
        { //block scope
            if (support_on['nitrogen_harvester'] && !global.race['kindling_kindred'] && !global.race['smoldering']){
                ...
                modRes('Lumber', lumber * hunger * global_multiplier * time_multiplier * zigVal, false, 'spc_venus');
            }
            ...
            if (global.race['cataclysm'] || global.race['orbit_decayed']){
                if (global.tech['mars'] && support_on['biodome'] && !global.race['kindling_kindred'] && !global.race['smoldering']){
                    ...
                    modRes('Lumber', lumber * hunger * global_multiplier * time_multiplier * zigVal, false, 'spc_red');
                }
            }
            else if (global.race['soul_eater'] && global.race.species !== 'wendigo' && global.race['evil'] && !global.race['iceage']){
                let weapons = weaponTechModifer();
                let hunters = workerScale(global.civic.hunter.workers,'hunter');
                hunters *= racialTrait(hunters,'hunting');
                ...
                breakdown.p['Lumber'][job_data.hunter.name()] = hunters  + 'v';
                breakdown.p['Lumber'][loc('soldiers')] = soldiers  + 'v';
                breakdown.p['Lumber'][loc('hunger')] = ((hunger - 1) * 100) + '%';
                modRes('Lumber', hunters * hunger * global_multiplier * time_multiplier, false, supplyRegionKey('city'));
                modRes('Lumber', soldiers * hunger * global_multiplier * time_multiplier, false, supplyRegionKey('city'));
            }
            else if (global.race['evil'] && !global.race['iceage']){
                let reclaimers = workerScale(global.civic.lumberjack.workers,'lumberjack');
                reclaimers *= racialTrait(reclaimers,'lumberjack');
                ...
                if (global.city['graveyard']){
                    graveyard += hugeAdjust(global.city['graveyard'].count) * 0.08;
                }

                let soldiers = armyRating(garrisonSize(),'hunting') / 5;
                soldiers *= production('psychic_boost','Lumber');

                breakdown.p['Lumber'][job_data.lumberjack.name()] = reclaimers  + 'v';
                if (reclaimers > 0){
                ...
                if (global.race['forager']){
                    ...
                    breakdown.p['Lumber'][job_data.forager.name()] = forage_base  + 'v';
                    if (forage_base > 0){
                        breakdown.p['Lumber'][`ᄂ${loc('city_graveyard')}+1`] = ((graveyard - 1) * 100) + '%';
                        breakdown.p['Lumber'][`ᄂ${loc('quarantine')}+2`] = ((q_multiplier - 1) * 100) + '%';
                    }
                    modRes('Lumber', forage_base * hunger * graveyard * global_multiplier * q_multiplier * time_multiplier, false, supplyRegionKey('city'));
                }
                breakdown.p['Lumber'][loc('hunger')] = ((hunger - 1) * 100) + '%';
                modRes('Lumber', reclaimers * hunger * graveyard * global_multiplier * q_multiplier * time_multiplier, false, supplyRegionKey('city'));
                modRes('Lumber', soldiers * hunger * global_multiplier * q_multiplier * time_multiplier, false, supplyRegionKey('city'));
            }
            else {
                let workers = workerScale(global.civic.lumberjack.workers,'lumberjack');
                let lumber_base = racialTrait(workers,'lumberjack');
                ...
                lumber_base *= job_data.lumberjack.impact();
                ...
                if (global.city['sawmill']){
                    ...
                    sawmills *= hugeAdjust(global.city.sawmill.count * saw * sawmill_kept) + 1;
                }
                let power_mult = 1;
                let power_single = 1;
                if (global.city.powered && global.city.sawmill && p_on['sawmill']){
                    power_mult += hugeAdjust(p_on['sawmill'] * 0.04 * sawmill_kept);
                    power_single += 0.04;
                }
                let lumber_yard = 1;
                if (global.city['lumber_yard']){
                    lumber_yard += hugeAdjust(global.city['lumber_yard'].count) * 0.02 * infiltratorFactor('city','lumber_yard');
                }
                let woodcutter = 1;
                if(support_on['woodcutter']){
                    woodcutter += hugeAdjust(support_on['woodcutter']) * 0.08;
                }
                let ecosystem = 1;
                if(global.surface.scavengers?.count){
                    ecosystem = actions.surface.ecosystem.scavengers.tree_effect();
                }

                breakdown.p['Lumber'][job_data.lumberjack.name()] = lumber_base + 'v';
                if (lumber_base > 0){
                    breakdown.p['Lumber'][`ᄂ${loc('city_lumber_yard')}`] = ((lumber_yard - 1) * 100) + '%';
                    breakdown.p['Lumber'][`ᄂ${loc('surface_woodcutter')}`] = ((woodcutter - 1) * 100) + '%';
                    breakdown.p['Lumber'][`ᄂ${loc('surface_ecosystem')}`] = ((ecosystem - 1) * 100) + '%';
                ...
                if(global.race['iceage']){
                    ...
                    if(global.surface.trees){
                        let tree_info = ecosystemInfo.trees;
                        let info = global.surface.overview;
                        let reserved_trees = info.area * (info.trees_reserved / 100) / tree_info.size * ecoMinorTraitEffect('trees', 'compact');
                        let avail_trees = Math.max(0, global.surface.trees.count - reserved_trees);
                ...
                if (global.race['forager'] && !global.race['iceage']){
                    let forage = 1;
                    let foragers = workerScale(global.civic.forager.workers,'forager');
                    foragers *= racialTrait(foragers,'forager');
                    ...
                    modRes('Lumber', forage_base * hunger * q_multiplier * sawmills * lumber_yard * power_mult * global_multiplier * time_multiplier, false, supplyRegionKey('city'));
                }

                breakdown.p['Lumber'][loc('hunger')] = ((hunger - 1) * 100) + '%';
                modRes('Lumber', delta * time_multiplier, false, supplyRegionKey('city'));
            }
        }

        let refinery_kept = infiltratorFactor('city','metal_refinery');
        let refinery = global.city['metal_refinery'] ? hugeAdjust(global.city['metal_refinery'].count * 6 * refinery_kept) : 0;
        ...
        if (global.race['sappy'] && !global.race['iceage']){
            if (global.tech['mining'] && global.resource[global.race.species].amount > 0){
                ...
                stone_base = hugeAdjust(stone_base);
                ...
                if (global.city.hasOwnProperty('basic_housing')){
                    let grove = hugeAdjust(global.city.basic_housing.count) * 0.025;
                    stone_base *= 1 + grove;
            ...
            stone_base *= job_data.quarry_worker.impact();
            ...
            if(global.underground['under_mine']){
                underground_quarry += global.underground['under_mine'].count * 0.03;
            ...
            if (global.race['warlord'] && global.portal['tunneler']){
                tunneler = 1 + (global.portal.tunneler.rank + 3) / 100 * hugeAdjust(global.portal.tunneler.count);
            }
            ...
            if (global.race['cataclysm'] || global.race['orbit_decayed'] || global.tech['resettle']){
                ...
                rock_quarry += hugeAdjust(global.city['rock_quarry'].count) * 0.02 * quarry_kept;
                if (p_on['rock_quarry']){
                    power_single += 0.04;
                    power_mult += hugeAdjust(p_on['rock_quarry'] * 0.04 * quarry_kept);
            ...
            if (forage_base > 0){
                breakdown.p['Stone'][job_data.forager.name()] = forage_base + 'v';
                if (forage_base > 0){
            ...
            if (global.race['smoldering'] && global.resource.Chrysotile.display){
                ...
                modRes('Chrysotile', (a_delta * time_multiplier) * geneBonus('fireweave'), false, supplyRegionKey('city'));
            }
            ...
            if (global.city['metal_refinery'] && (global.city['metal_refinery'].count > 0 || global.race['cataclysm'] || global.race['orbit_decayed'] || global.race['warlord'])){
                ...
                if (forage_alum_base > 0){
                    ...
                    breakdown.p['Aluminium'][job_data.forager.name()] = forage_alum_base + 'v';
                ...
                delta *= rock_quarry * tunneler * shrineMetal.mult * mworks.Aluminium * hunger * q_multiplier * global_multiplier;
                ...
                if (global.tech['alumina'] >= 2){
                    refinery += hugeAdjust(p_on['metal_refinery']) * 6 * q_multiplier * refinery_kept;
                    let ref_single = hugeAdjust(6) * q_multiplier / 100 * refinery_kept;
                ...
                modRes('Aluminium', delta * time_multiplier, false, supplyRegionKey('city'));
            }
        }
        ...
        if (global.resource.Water.display){
            if (support_on['water_freighter']){
                ...
                modRes('Water', delta * time_multiplier, false, 'spc_enceladus');
            }
            ...
            if (global.tech['isolation'] && global.tauceti['tau_farm'] && p_on['tau_farm']){
                ...
                modRes('Water', delta * time_multiplier, false, 'tau_home');
            }
            ...
            if(global.civic.water_collector.workers){
                ...
                let ice_collectors = 1 + hugeAdjust((global.underground['ice_collector']?.count || 0) * 0.04);
                ...
                breakdown.p['Water'][`ᄂ${loc('underground_ice_collector')}`] = -((1 - ice_collectors) * 100) + '%';
                modRes('Water', delta * time_multiplier);
            }

            if(p_on['water_pump']){
                let prod = production('water_pump') * p_on['water_pump'] * production('psychic_boost','Water');
                let ice_collectors = 1 + hugeAdjust((global.underground['ice_collector']?.count || 0) * 0.04);
                breakdown.p['Water'][loc('underground_water_pump')] = prod + 'v';
                breakdown.p['Water'][`ᄂ${loc('underground_ice_collector')}+0`] = -((1 - ice_collectors) * 100) + '%';
                modRes('Water', prod * time_multiplier * ice_collectors * global_multiplier);
            }
        }
        ...
        [
            ...
            if(global[building.r][building.b]){
                ...
                if(global[building.r][building.b].on > active){
                    struct.addClass('warn');
                    struct.prop('title',`ON ${active}/${global[building.r][building.b].on}`);
                }
                else {
        ...
        if(support_on['crater_drill']){
            let iron_base = production('crater_drill','iron') * production('psychic_boost', 'Iron');
            iron_base *= workerScale(global.civic.crater_worker.workers,'crater_worker') * support_on['crater_drill'];
            let copper_base = production('crater_drill','copper') * production('psychic_boost', 'Copper');
            copper_base *= workerScale(global.civic.crater_worker.workers,'crater_worker') * support_on['crater_drill'];
            let coal_base = production('crater_drill','coal') * production('psychic_boost', 'Coal');
            coal_base *= workerScale(global.civic.crater_worker.workers,'crater_worker') * support_on['crater_drill'];
            let uranium_base = production('crater_drill','uranium') * production('psychic_boost', 'Uranium');
            uranium_base *= workerScale(global.civic.crater_worker.workers,'crater_worker') * support_on['crater_drill'];
            let smelter_mult = 1 + (iron_smelter * 0.1);
            let refinery = 1;
            if(p_on['core_refinery']){
                let mineshaft_effect = 1 + (actions.underground.cave.mineshaft.full_depth() - 200000) * 0.00003;
                if(mineshaft_effect < 1){
                    mineshaft_effect = 1;
                }
                refinery += 0.05 * hugeAdjust(p_on['core_refinery']) * mineshaft_effect;
            }
            breakdown.p['Iron'][loc('surface_crater_drill')] = (iron_base) + 'v';
            if (iron_base > 0){
                breakdown.p['Iron'][`ᄂ${loc('underground_core_refinery')}+0`] = ((refinery - 1) * 100) + '%';
            }
            breakdown.p['Copper'][loc('surface_crater_drill')] = (copper_base) + 'v';
            if (copper_base > 0){
                breakdown.p['Copper'][`ᄂ${loc('underground_core_refinery')}+0`] = ((refinery - 1) * 100) + '%';
            }
            breakdown.p['Coal'][loc('surface_crater_drill')] = (coal_base) + 'v';
            if (coal_base > 0){
                breakdown.p['Coal'][`ᄂ${loc('underground_core_refinery')}+0`] = ((refinery - 1) * 100) + '%';
            }
            breakdown.p['Uranium'][loc('surface_crater_drill')] = (uranium_base) + 'v';
            if (uranium_base > 0 && global.tech['surface_uranium'] >= 3){
                uranium_base *= refinery;
                breakdown.p['Uranium'][`ᄂ${loc('underground_core_refinery')}+0`] = ((refinery - 1) * 100) + '%';
            }
            
            modRes('Iron', iron_base * shrineMetal.mult * smelter_mult * refinery * global_multiplier * time_multiplier * hunger);
            modRes('Copper', copper_base * shrineMetal.mult * refinery * global_multiplier * time_multiplier * hunger);
            modRes('Coal', coal_base * refinery * global_multiplier * time_multiplier * hunger);
            modRes('Uranium', uranium_base * global_multiplier * time_multiplier * hunger);
        }
        
        if(global.surface['overview']){
            surfaceEcosystemVisual(); //updates numbers on ecosystem area and water
        }

        if (global.eden['palace'] && p_on['spirit_vacuum'] && global.tech['isle']){
            let drain = 1653439 * hugeAdjust(p_on['spirit_vacuum']);
            if (global.tech.isle >= 6 && p_on['spirit_battery']){
                let battery = hugeAdjust(p_on['spirit_battery'] || 0);
                let boost = 0.08;
                if (global.race['warlord'] && global.eden['corruptor'] && global.tech?.asphodel >= 13){
                    boost *= 1 + hugeAdjust(p_on['corruptor'] || 0) * 0.03;
                }
        ...
        if (global.resource.Mana.display){
            ...
            if (global.city['pylon'] || global.space['pylon'] || global.tauceti['pylon'] || global.underground['pylon']){
                let mana_base = 0;
                let name = 'city_pylon';
                if (global.underground['pylon']){
                    mana_base = global.underground.pylon.count * 0.015;
                }
                ...
                mana_base = hugeAdjust(mana_base);

                let delta = mana_base * hunger * global_multiplier;
                breakdown.p['Mana'][loc(name)] = mana_base+'v';

                if (global.tech['cleric'] >= 2 && global.underground['archaeological_dig']){
                    delta *= 1 + (global.underground['archaeological_dig'].relics / 500);
                    breakdown.p['Mana'][`ᄂ${loc('tech_magic_relics')}`] = +(global.underground['archaeological_dig'].relics / 5).toFixed(1) + '%';
                }
                if (global.tech['nexus']){
                    let nexus = global.tech['nexus'] * 5;
                    delta *= 1 + hugeAdjust(nexus / 100);
        ...
        if (global.resource.Crystal.display){
            ...
            crystal_base *= job_data.crystal_miner.impact() * production('psychic_boost','Crystal');

            breakdown.p['Crystal'][job_data.crystal_miner.name()] = crystal_base + 'v';
            ...
            modRes('Crystal', delta * time_multiplier, false, supplyRegionKey('city'));
        }
        ...
        if (global.resource.Copper.display || global.resource.Iron.display){
            ...
            miner_base *= job_data.miner.impact();
            if (global.race['tough']){
            ...
            if (global.city['mine']['on']){
                power_mult += hugeAdjust(p_on['mine'] * 0.05);
                pow_single += 1.05;
            }
            if(global.underground['under_mine']?.on){
                power_mult += hugeAdjust(p_on['under_mine'] * 0.05);
                pow_single += 1.05;
            }
            ...
            if (global.resource.Copper.display){
                ...
                breakdown.p['Copper'][job_data.miner.name()] = (copper_base) + 'v';
                if (copper_base > 0){
                    breakdown.p['Copper'][`ᄂ${loc('power')}`] = ((copper_power - 1) * 100) + '%';
                    breakdown.p['Copper'][`ᄂ${loc('portal_tunneler_bd')}`] = ((tunneler - 1) * 100) + '%';
                    breakdown.p['Copper'][`ᄂ${loc('underground_core_refinery')}`] = ((refinery - 1) * 100) + '%';
                ...
                let delta = copper_base * shrineMetal.mult * mworks.Copper * tunneler * refinery * hunger * q_multiplier * global_multiplier;
                global.city.mine['cpow'] = +(delta * (cop_single - 1)).toFixed(5);
                delta *= copper_power;

                modRes('Copper', delta * time_multiplier, false, 'spc_home');
                ...
                if (global.race['forager'] && global.tech['dowsing']){
                    ...
                    breakdown.p['Copper'][job_data.forager.name()] = forage_base  + 'v';
                    if (forage_base > 0){
                        breakdown.p['Copper'][`ᄂ${loc('quarantine')}+1`] = ((q_multiplier - 1) * 100) + '%';
                    }
                    modRes('Copper', forage_base * mworks.Copper * hunger * global_multiplier * q_multiplier * time_multiplier, false, supplyRegionKey('city'));
                }
            }
            ...
            if (global.resource.Iron.display){
                ...
                breakdown.p['Iron'][job_data.miner.name()] = (iron_base) + 'v';
                if (iron_base > 0){
                    ...
                    breakdown.p['Iron'][`ᄂ${loc('underground_core_refinery')}`] = ((refinery - 1) * 100) + '%';
                ...
                if (global.tech['isolation'] && global.race['lone_survivor']){
                    { // Pit Miner
                        ...
                        breakdown.p['Iron'][job_data.pit_miner.name()] = pit_base + 'v';
                        if (pit_base > 0){
                ...
                breakdown.p['Iron'][job_data.space_miner.name()] = space_iron + 'v';
                if (space_iron > 0){
                ...
                let ironAt = zoneTally()
                    .add(supplyRegionKey('city'), iron_base * tunneler * iron_power * refinery * q_multiplier)
                    .add(supplyZone('space:iron_ship'), space_iron * qs_multiplier * zigVal)
                    .add(supplyZone('tauceti:mining_ship'), eship_iron)
                    .add(supplyZone('tauceti:mining_pit'), pit_miner)
                    .add(supplyZone('tauceti:womling_mine'), womling)
                    .scale(smelter_mult * shrineMetal.mult * mworks.Iron * hunger * global_multiplier);
                let delta = ironAt.total();
                global.city.mine['ipow'] = +(iron_base * q_multiplier * hunger * global_multiplier * refinery * (iron_single - 1)).toFixed(5);
                ...
                if (global.race['forager'] && global.tech['dowsing']){
                    ...
                    breakdown.p['Iron'][job_data.forager.name()] = forage_base  + 'v';
                    if (forage_base > 0){
                        breakdown.p['Iron'][`ᄂ${loc('quarantine')}+2`] = ((q_multiplier - 1) * 100) + '%';
                    }
                    modRes('Iron', forage_base * mworks.Iron * hunger * global_multiplier * q_multiplier * time_multiplier, false, supplyRegionKey('city'));
                }

                breakdown.p['Iron'][loc('hunger')] = ((hunger - 1) * 100) + '%';
                ironAt.apply('Iron', time_multiplier);
            ...
            if (global.race['sappy']){
                ...
                if ((global.city['metal_refinery'] && global.city['metal_refinery'].count > 0) || global.race['cataclysm'] || global.race['orbit_decayed']){
                    let base = 0;
                    let dug_at = supplyRegionKey('city');
                    if (global.race['cataclysm'] || global.race['orbit_decayed']){
                        dug_at = supplyZone('space:red_mine');
                        if (global.tech['mars'] && support_on['red_mine']){
                    ...
                    modRes('Aluminium', delta * time_multiplier, false, dug_at);
                }
                ...
                if (global.race['smoldering'] && global.resource.Chrysotile.display){
                    ...
                    breakdown.p['Chrysotile'][job_data.miner.name()] = (cry_base) + 'v';
                    if (cry_base > 0){
                    ...
                    breakdown.p['Chrysotile'][loc('hunger')] = ((hunger - 1) * 100) + '%';
                    modRes('Chrysotile', (delta * time_multiplier) * geneBonus('fireweave'), false, supplyRegionKey('city'));
                }
            }
        }

        {
            ...
            if (global.interstellar['mining_droid'] && miner_droids['alum'] > 0){
                let base = hugeAdjust(miner_droids['alum']) * 2.75 * production('psychic_boost','Aluminium');
                let delta = base * shrineMetal.mult * mworks.Aluminium * global_multiplier * zigVal;
                delta *= 1 + (refinery / 100);
                ...
                modRes('Aluminium', delta * time_multiplier, false, 'int_alpha');
            }

            // Aluminium Titan Mines
            if (global.resource.Aluminium.display && global.space['titan_mine']){
                let synd = syndicate('spc_titan');
                let titan_colonists = p_on['ai_colonist'] ? workerScale(global.civic.titan_colonist.workers,'titan_colonist') + hugeAdjust(jobScale(p_on['ai_colonist'])) : workerScale(global.civic.titan_colonist.workers,'titan_colonist');
                ...
                modRes('Aluminium', alum_delta * time_multiplier, false, 'spc_titan');
            }

            // Aluminium Extractor Ship
            if (global.resource.Aluminium.display && e_ship['aluminium'] && e_ship.aluminium > 0){
                let alum_delta = e_ship.aluminium * shrineMetal.mult * mworks.Aluminium * global_multiplier * womling_technician;
                alum_delta *= 1 + (refinery / 100);
                ...
                modRes('Aluminium', alum_delta * time_multiplier, false, 'tau_roid');
            }
        ...
        if (support_on['red_mine'] && support_on['red_mine'] > 0){
            ...
            modRes('Copper', copper_base * shrineMetal.mult * mworks.Copper * time_multiplier * global_multiplier * qs_multiplier * hunger * synd * zigVal, false, 'spc_red');
            ...
            modRes('Titanium', (titanium_base * shrineMetal.mult * mworks.Titanium * time_multiplier * global_multiplier * qs_multiplier * synd * zigVal) * geneBonus('assayer'), false, 'spc_red');
        }
        if (shrineBonusActive()){
        ...
        if (global.resource.Coal.display){
            ...
            if(p_on['core_refinery']){
                ...
                refinery += 0.05 * p_on['core_refinery'] * mineshaft_effect;
            }
            ...
            if (global.underground['under_coal_mine']?.on){
                power_mult += (p_on['under_coal_mine'] * 0.05);
                coal_single += 0.05;
            }
            ...
            if (coal_base > 0){
                ...
                breakdown.p['Coal'][`ᄂ${loc('underground_core_refinery')}`] = ((refinery - 1) * 100) + '%';
            ...
            let coal_at = supplyRegionKey('city');
            if (global.race['cataclysm'] && support_on['iridium_mine']){
                coal_at = supplyZone('space:iridium_mine');
            ...
            let delta = coal_base * tunneler * refinery * hunger * q_multiplier * global_multiplier;
            ...
            if (global.interstellar['mining_droid'] && miner_droids['coal'] > 0){
                let droid_base = hugeAdjust(miner_droids['coal']) * 3.75 * production('psychic_boost','Coal');
                ...
                modRes('Coal', droid_delta * time_multiplier, false, 'int_alpha');
            }

            modRes('Coal', delta * time_multiplier, false, coal_at);

            // Uranium (from coal miners)
            if (global.resource.Uranium.display && !global.race['iceage']){
                ...
                breakdown.p['Uranium'][global.race['cataclysm'] ? loc('space_moon_iridium_mine_title') : (global.race['warlord'] ? job_data.miner.name() : job_data.coal_miner.name())] = uranium / global_multiplier + 'v';
                ...
                modRes('Uranium', (u_delta * time_multiplier) * geneBonus('abyssal'), false, coal_at);
            }
        }
        ...
        if (global.race['warlord'] && global.portal['tunneler'] && global.portal.tunneler.count > 0){
            ...
            ['Neutronium','Adamantite','Bolognium','Orichalcum'].forEach(function(res){
                ...
                breakdown.p[res][job_data.miner.name()] = cur_base + 'v';
                if (cur_base > 0){
                ...
                modRes(res, delta * time_multiplier, false, 'prtl_wasteland');
            });
        }

        // Space Uranium
        if (global.interstellar['mining_droid'] && miner_droids['uran'] > 0){
            let droid_base = hugeAdjust(miner_droids['uran']) * 0.12 * production('psychic_boost','Uranium');
            let droid_delta = droid_base * global_multiplier * zigVal;
            ...
            modRes('Uranium', (droid_delta * time_multiplier) * geneBonus('abyssal'), false, 'int_alpha');
        }
        ...
        if (global.space['uranium_mine'] && p_on['uranium_mine']){
            ...
            modRes('Uranium', (mine_delta * time_multiplier) * geneBonus('abyssal'), false, 'spc_makemake');
        }
        ...
        if (global.resource.Oil.display){
            ...
            let oilAt = zoneTally()
                .add(global.race['warlord'] ? supplyZone('portal:pumpjack') : supplyRegionKey('city'), oil_well * oil_prod_mod)
                .add(supplyZone('space:oil_extractor'), oil_extractor * extract_prod_mod)
                .add(supplyZone('industry:oil_pump'), oil_pump * pump_prod_mod)
                .add(supplyZone('tauceti:whaling_ship'), whale_oil * womling_technician)
                .scale(hunger * global_multiplier);
            let delta = oilAt.total();
            if (global.race['gravity_well']){
                // teamster() is applied to the total, so pass its effect back as a ratio to keep the split exact.
                let hauled = teamster(delta);
                oilAt.scale(delta !== 0 ? hauled / delta : 1);
                delta = hauled;
            }
            ...
            breakdown.p['Oil'][loc('underground_oil_pump')] = oil_pump + 'v';
        ...
        if (support_on['iridium_mine'] || global.race['warlord']){
            ...
            breakdown.p['Iridium'][global.race['warlord'] ? job_data.miner.name() : loc('space_moon_iridium_mine_title')] = iridium_base + 'v';
            if (iridium_base > 0){
            ...
            modRes('Iridium', (delta * time_multiplier) * geneBonus('assayer'), false, 'spc_moon');
        }
        ...
        if (support_on['iridium_ship']){
            ...
            breakdown.p['Iridium'][job_data.space_miner.name()] = iridium_base + 'v';
            if (iridium_base > 0){
            ...
            modRes('Iridium', (delta * time_multiplier) * geneBonus('assayer'), false, 'spc_belt');
        }
        ...
        if (p_on['s_gate'] && global.resource.Adamantite.display && global.galaxy['armed_miner'] && gal_on['armed_miner'] > 0){
            ...
            modRes('Iridium', (delta * time_multiplier) * geneBonus('assayer'), false, 'gxy_alien2');
        }
        ...
        if (global.resource.Iridium.display && e_ship['iridium'] && e_ship.iridium > 0){
            ...
            modRes('Iridium', (iridium_delta * time_multiplier) * geneBonus('assayer'), false, supplyZone('tauceti:mining_ship'));
        }
        ...
        if ((global.space['moon_base'] && support_on['helium_mine']) || global.race['warlord']){
            ...
            modRes('Helium_3', (delta * time_multiplier) * geneBonus('resilient') * geneBonus('abyssal'), false, 'spc_moon');
        }
        ...
        if (global.space['gas_mining'] && p_on['gas_mining']){
            ...
            modRes('Helium_3', (delta * time_multiplier) * geneBonus('resilient') * geneBonus('abyssal'), false, 'spc_gas');
        }
        ...
        if (p_on['refueling_station']){
            ...
            modRes('Helium_3', (delta * time_multiplier) * geneBonus('resilient') * geneBonus('abyssal'), false, 'tau_gas');
        }
        ...
        if (global.interstellar['harvester'] && int_on['harvester']){
            ...
            modRes('Helium_3', (delta * time_multiplier) * geneBonus('resilient') * geneBonus('abyssal'), false, 'int_nebula');
            ...
            if (global.tech['ram_scoop']){
                ...
                modRes('Deuterium', deut_delta * time_multiplier, false, 'int_nebula');
        ...
        if (p_on['s_gate'] && global.galaxy['raider'] && gal_on['raider'] > 0){
            ...
            modRes('Deuterium', delta * time_multiplier, false, 'gxy_chthonian');
        }
        ...
        if (p_on['outpost']){
            ...
            modRes('Neutronium', delta * time_multiplier, false, 'spc_gas_moon');
        }
        ...
        if (p_on['neutron_miner']){
            ...
            modRes('Neutronium', delta * time_multiplier, false, 'int_neutron');
        }
        ...
        if (p_on['s_gate'] && global.galaxy['raider'] && gal_on['raider'] > 0){
            ...
            modRes('Neutronium', delta * time_multiplier, false, 'gxy_chthonian');
        }
        ...
        if (global.space['neutronium_mine'] && p_on['neutronium_mine']){
            ...
            modRes('Neutronium', mine_delta * time_multiplier, false, 'spc_makemake');
        }
        ...
        if (global.resource.Neutronium.display && e_ship['neutronium'] && e_ship.neutronium > 0){
            ...
            modRes('Neutronium', neutronium_delta * time_multiplier, false, 'tau_roid');
        }
        ...
        if (support_on['elerium_ship']){
            ...
            if (global.race['gravity_well']){ delta = teamster(delta); }
            breakdown.p['Elerium'][job_data.space_miner.name()] = elerium_base + 'v';
            ...
            modRes('Elerium', delta * time_multiplier, false, 'spc_belt');
        }
        ...
        if (int_on['elerium_prospector']){
            ...
            modRes('Elerium', delta * time_multiplier, false, 'int_nebula');
        }
        ...
        if (global.space['elerium_mine'] && p_on['elerium_mine']){
            ...
            modRes('Elerium', mine_delta * time_multiplier, false, 'spc_makemake');
        }
        ...
        if (global.resource.Elerium.display && e_ship['elerium'] && e_ship.elerium > 0){
            ...
            modRes('Elerium', elerium_delta * time_multiplier, false, 'tau_roid');
        }
        ...
        if (global.resource.Adamantite.display && global.interstellar['mining_droid'] && miner_droids['adam'] > 0){
            let droid_base = hugeAdjust(miner_droids['adam']) * 0.075 * production('psychic_boost','Adamantite');
            ...
            modRes('Adamantite', droid_delta * time_multiplier, false, 'int_alpha');
        }

        if (p_on['s_gate'] && global.resource.Adamantite.display && global.galaxy['armed_miner'] && gal_on['armed_miner'] > 0){
            let base = hugeAdjust(gal_on['armed_miner']) * 0.23 * production('psychic_boost','Adamantite');
            let foothold = 1 + hugeAdjust((gal_on['ore_processor'] ?? 0) * 0.1);
            let pirate = piracy('gxy_alien2');
            ...
            modRes('Adamantite', delta * time_multiplier, false, 'gxy_alien2');
        }
        ...
        if (global.resource.Adamantite.display && global.space['titan_mine']){
            ...
            modRes('Adamantite', adam_delta * time_multiplier, false, 'spc_titan');
        }
        ...
        if (global.tech['resettle'] && global.resource.Stone.display && global.space['titan_mine']){
            ...
            if (stone_base > 0){
                ...
                breakdown.p['Stone'][`ᄂ${loc('quarantine')}`] = ((qs_multiplier - 1) * 100) + '%';
            }
            modRes('Stone', stone_delta * time_multiplier, false, 'spc_titan');
        }
        ...
        if (global.tech['resettle'] && global.resource.Chrysotile.display && global.space['titan_mine']){
            ...
            modRes('Chrysotile', (cry_delta * time_multiplier) * geneBonus('fireweave'), false, 'spc_titan');
        }
        ...
        if (global.resource.Infernite.display){
            ...
            if (workers > 0){
                ...
                if (global.tech['infernite'] >= 2 && p_on['sensor_drone']){
                    ...
                    sensors = 1 + hugeAdjust(p_on['sensor_drone'] * drone_rate);
                }
                ...
                breakdown.p['Infernite'][global.race['warlord'] ? job_data.miner.name() : job_data.hell_surveyor.name()] = surveyor_base + 'v';
                ...
                modRes('Infernite', surveyor_delta * time_multiplier, false, supplyZone('portal:sensor_drone'));
            }
        ...
        if (p_on['s_gate'] && global.resource.Bolognium.display && global.galaxy['bolognium_ship'] && gal_on['bolognium_ship'] > 0){
            ...
            modRes('Bolognium', delta * time_multiplier, false, 'gxy_gateway');
        }
        ...
        if (global.eden['asphodel_harvester'] && support_on['asphodel_harvester']){
            ...
            modRes('Asphodel_Powder', delta * time_multiplier, false, 'eden_asphodel');
        }
        ...
        if (global.resource.Elysanite.display){
            if (global.civic.elysium_miner.display){
                ...
                breakdown.p['Elysanite'][job_data.elysium_miner.name()] = miner_base + 'v';

                let delta = miner_base;
                delta *= hunger * global_multiplier;

                modRes('Elysanite', delta * time_multiplier, false, 'eden_elysium');
            }
        }
        ...
        if (global.civic.pit_miner.display){
            if (tauEnabled()){
                ...
                miner_base *= racialTrait(miner_base,'miner');
                let colony_val = 1 + hugeAdjust((support_on['colony'] || 0) * 0.5);
                let pit_at = supplyZone('tauceti:mining_pit');
                ...
                { // Bolognium
                    ...
                    breakdown.p['Bolognium'][job_data.pit_miner.name()] = bol_base + 'v';
                    if (bol_base > 0){
                        breakdown.p['Bolognium'][`ᄂ${loc('tau_home_colony')}`] = ((colony_val - 1) * 100) + '%';
                    }

                    modRes('Bolognium', delta * time_multiplier, false, pit_at);
                }
                ...
                { // Stone
                    ...
                    breakdown.p['Stone'][job_data.pit_miner.name()] = stone_base + 'v';
                    if (stone_base > 0){
                        breakdown.p['Stone'][`ᄂ${loc('tau_home_colony')}`] = ((colony_val - 1) * 100) + '%';
                    }

                    modRes('Stone', delta * time_multiplier, false, pit_at);
                }
                ...
                if (global.race['smoldering']){ // Chrysotile
                    ...
                    breakdown.p['Chrysotile'][job_data.pit_miner.name()] = cry_base + 'v';
                    if (cry_base > 0){
                        breakdown.p['Chrysotile'][`ᄂ${loc('tau_home_colony')}`] = ((colony_val - 1) * 100) + '%';
                        breakdown.p['Chrysotile'][loc('hunger')] = ((hunger - 1) * 100) + '%';
                    }
                    modRes('Chrysotile', (delta * time_multiplier) * geneBonus('fireweave'), false, pit_at);
                }
                ...
                { // Adamantite
                    ...
                    breakdown.p['Adamantite'][job_data.pit_miner.name()] = adam_base + 'v';
                    if (adam_base > 0){
                        breakdown.p['Adamantite'][`ᄂ${loc('tau_home_colony')}`] = ((colony_val - 1) * 100) + '%';
                    }

                    modRes('Adamantite', delta * time_multiplier, false, pit_at);
                }
                ...
                if (global.tech['isolation']){
                    { // Copper
                        ...
                        breakdown.p['Copper'][job_data.pit_miner.name()] = copper_base + 'v';
                        if (copper_base > 0){
                            breakdown.p['Copper'][`ᄂ${loc('tau_home_colony')}`] = ((colony_val - 1) * 100) + '%';
                        }

                        modRes('Copper', delta * time_multiplier, false, pit_at);
                    }
                    ...
                    { // Coal
                        ...
                        breakdown.p['Coal'][job_data.pit_miner.name()] = coal_base + 'v';
                        if (coal_base > 0){
                            breakdown.p['Coal'][`ᄂ${loc('tau_home_colony')}`] = ((colony_val - 1) * 100) + '%';
                        }

                        modRes('Coal', delta * time_multiplier, false, pit_at);
                    }
                    ...
                    if (global.race['lone_survivor']){ // Aluminium
                        ...
                        let delta = alum_base * shrineMetal.mult * mworks.Aluminium * global_multiplier * colony_val;

                        breakdown.p['Aluminium'][job_data.pit_miner.name()] = alum_base + 'v';
                        if (alum_base > 0){
                            breakdown.p['Aluminium'][`ᄂ${loc('tau_home_colony')}`] = ((colony_val - 1) * 100) + '%';
                        }

                        modRes('Aluminium', delta * time_multiplier, false, pit_at);
                    }
                }
            }
            else {
                ...
                materials_bd[job_data.pit_miner.name()] = miner_base + 'v';
                if (miner_base > 0){
        ...
        if (global.tauceti['tau_farm'] && p_on['tau_farm']){
            ...
            if (!global.race['kindling_kindred'] && !global.race['smoldering']){
                ...
                modRes('Lumber', delta * time_multiplier, false, 'tau_home');
            }
        }
        ...
        if (p_on['s_gate'] && global.resource.Bolognium.display && global.galaxy['armed_miner'] && gal_on['armed_miner'] > 0){
            let base = hugeAdjust(gal_on['armed_miner']) * 0.032 * production('psychic_boost','Bolognium');
            let foothold = 1 + hugeAdjust((gal_on['ore_processor'] ?? 0)* 0.1);
            let pirate = piracy('gxy_alien2');
            ...
            modRes('Bolognium', delta * time_multiplier, false, 'gxy_alien2');
        }
        ...
        if (p_on['s_gate'] && global.resource.Orichalcum.display && global.galaxy['excavator'] && p_on['excavator'] > 0){
            ...
            modRes('Orichalcum', delta * time_multiplier, false, 'gxy_chthonian');
        }
        ...
        if (global.space['orichalcum_mine'] && p_on['orichalcum_mine']){
            ...
            modRes('Orichalcum', mine_delta * time_multiplier, false, 'spc_makemake');
        }
        ...
        if (global.resource.Orichalcum.display && e_ship['orichalcum'] && e_ship.orichalcum > 0){
            ...
            modRes('Orichalcum', orichalcum_delta * time_multiplier, false, 'tau_roid');
        }
        ...
        if (global.race['truepath'] && global.tech['tau_red'] && global.tech.tau_red >= 5){
            if (global.tauceti['womling_mine'] && global.tauceti['overseer']){
                ...
                modRes('Unobtainium', miner_delta * time_multiplier, false, 'tau_red');
                ...
                if (global.tech['isolation']){
                    ...
                    modRes('Uranium', (uranium_delta * time_multiplier) * geneBonus('abyssal'), false, 'tau_red');
                    ...
                    let titanium_delta = titanium_base * prod * shrineMetal.mult * mworks.Titanium * global_multiplier;

                    if (titanium_base > 0){
                        breakdown.p['Titanium'][`ᄂ${loc('tau_red_womling_prod_label')}`] = -((1 - prod) * 100) + '%';
                    }
                    modRes('Titanium', (titanium_delta * time_multiplier) * geneBonus('assayer'), false, 'tau_red');
                    ...
                    if (global.race['lone_survivor']){
                        ...
                        modRes('Copper', copper_delta * time_multiplier, false, 'tau_red');
                        ...
                        let alumina_delta = alumina_base * prod * shrineMetal.mult * mworks.Aluminium * global_multiplier;

                        if (alumina_base > 0){
                            breakdown.p['Aluminium'][`ᄂ${loc('tau_red_womling_prod_label')}`] = -((1 - prod) * 100) + '%';
                        }
                        modRes('Aluminium', alumina_delta * time_multiplier, false, 'tau_red');
                        ...
                        modRes('Iridium', (iridium_delta * time_multiplier) * geneBonus('assayer'), false, 'tau_red');
                        ...
                        modRes('Neutronium', neutronium_delta * time_multiplier, false, 'tau_red');
                    }
                }
            }
        }
        ...
        // Breakdown rows list in insertion order, so these metalworks lines are written here rather than
        // up with the rest of their sources — this is past the pit miner, the last one added.
        if (mworksOn){
            ...
            breakdown.p['Aluminium'][loc('space_metalworks_title')] = ((mworks.Aluminium - 1) * 100).toFixed(1) + '%';
            breakdown.p['Titanium'][loc('space_metalworks_title')] = ((mworks.Titanium - 1) * 100).toFixed(1) + '%';
        }
        ...
        if (global.tech['currency'] >= 1){
            ...
            if (fed && global.tech['banking'] && global.tech['banking'] >= 2){
                let impact = +workerScale(job_data.banker.impact(),'banker');
            ...
            let extra_income = 0;
            if (govActive('extravagant',0) && (global.city['apartment'] || support_on['surface_apartment'])){
            ...
            if (global.tech['anthropology'] && global.tech['anthropology'] >= 4 && !global.race['truepath']){
                temple_mult += hugeAdjust(faithTempleCount()) * 0.025;
            }
        ...
        if (global.tech['gambling'] && (p_on['casino'] || p_on['spc_casino'] || p_on['tauceti_casino'] || p_on['hell_casino'] || p_on['under_casino'])){
            let casinos = 0;
            ...
            if (p_on['under_casino']){ casinos += p_on['under_casino']; }
        ...
        if (global.city['tourist_center'] && global.city['tourist_center'].on && !global.race['fasting'] && !global.race['warlord']){
            ...
            amp = hugeAdjust(amp);
            if (global.city['amphitheatre']){
                tourism += global.city['tourist_center'].on * hugeAdjust(global.city.amphitheatre.count) * amp;
            }
            if (global.city['casino']){
                tourism += global.city['tourist_center'].on * hugeAdjust(global.city.casino.count) * 5 * amp;
            }
            if (global.space['spc_casino']){
                tourism += global.city['tourist_center'].on * hugeAdjust(global.space.spc_casino.count) * 5 * amp;
            }
            if (global.tech['monuments']){
                let monuments = global.tech.monuments;
                if (global.race['wish'] && global.race['wishStats']){
            ...
            if (global.city['trade'] && global.stats.achieve['banana'] && global.stats.achieve.banana.l >= 4){
                tourism += global.city['tourist_center'].on * hugeAdjust(global.city.trade.count) * 3 * amp;
            }
            let piousVal = govActive('pious',1);
            if (piousVal && global.city['temple']){
                tourism += global.city['tourist_center'].on * hugeAdjust(templeCount()) * piousVal * amp;
            }
            if (global.civic.govern.type === 'corpocracy'){
        ...
        if(p_on['surface_zoo']){
            let income = 0.2 * (global.surface.trees?.count || 0) * ecoMinorTraitEffect('trees', 'playful');
            income += 0.5 * (global.surface.herbivores?.count || 0) * ecoMinorTraitEffect('herbivores', 'playful');
            income += 1.5 * (global.surface.carnivores?.count || 0) * ecoMinorTraitEffect('carnivores', 'playful');
            income += 0.28 * (global.surface.scavengers?.count || 0) * ecoMinorTraitEffect('scavengers', 'playful');
            income *= hugeAdjust(p_on['surface_zoo']);
            income *= production('psychic_cash');
            breakdown.p['Money'][loc('surface_zoo')] = Math.round(income) + 'v';
            modRes('Money', +(income * time_multiplier * global_multiplier * hunger).toFixed(2));
            rawCash += income * global_multiplier * hunger;
        }
        ...
        if (global.portal['bazaar'] && global.portal['spire'] && global.tech['monuments']){
            ...
            let revenue = hugeAdjust(global.portal.bazaar.count) * monuments * global.portal.spire.count;
        ...
        if (global.tauceti['tau_cultural_center']){
            ...
            if (global.tauceti['tauceti_casino']){
                revenue += hugeAdjust(p_on['tau_cultural_center']) * hugeAdjust(p_on['tauceti_casino']) * 20;
            }
            if (global.tech['monuments']){
                let monuments = global.tech.monuments;
                if (global.race['wish'] && global.race['wishStats']){
                ...
                revenue += hugeAdjust(p_on['tau_cultural_center']) * monuments * 5;
            }
            if (global.tech['tau_culture'] && global.tech.tau_culture >= 2){
                revenue += hugeAdjust(p_on['tau_cultural_center']) * hugeAdjust(support_on['colony']) * 15;
            }
            if (global.civic.govern.type === 'corpocracy'){
        ...
        if (global.tech['tau_junksale']){
            let revenue = hugeAdjust(support_on['womling_village']) * 40;
            let culture = p_on['tau_cultural_center'] ? 1 + hugeAdjust(p_on['tau_cultural_center'] * 0.08) : 1;
        ...
        if (global.race['gravity_well'] && global.tech['teamster'] && global.tech.teamster >= 2){
            ...
            breakdown.p['Money'][job_data.teamster.name()] = Math.round(revenue / global_multiplier) + 'v';
        ...
        if (global.tech['anthropology'] && global.tech['anthropology'] >= 4 && global.race['truepath']){
            let merchsales = hugeAdjust(global.resource[global.race.species].amount) * hugeAdjust(templeCount()) * 0.08;
        ...
        if (global.race['truepath'] && global.tauceti['womling_market']){
            let base = global.city.market.mtrade * hugeAdjust(support_on['womling_market'] || 0);
            if (base > 0){
        ...
        if (global.tech['foundry']){
            ...
            const bench_seats = {
                foundry: global.city['foundry'] ? jobStack(global.city.foundry.count) : 0,
                fabrication: support_on['fabrication'] ? jobStack(support_on['fabrication']) : 0,
                tau_factory: global.tech['isolation'] && support_on['tau_factory'] ? jobStack(support_on['tau_factory'] * 5) : 0
            };
        ...
        if (global.race['decay']){
            Object.keys(tradeRatio).forEach(function (res) {
                if (global.resource[res].amount > 50){
                    ...
                    if (partitioned(res) && global.resource[res].amount > 0){
                        // Rot is proportional to what is sitting there, so each pool loses its own share.
                        let held = global.resource[res].amount;
                        let rot = zoneTally();
                        for (const pool of supplyPools()){
                            rot.add(pool, -decay * (regAmount(res, pool) / held));
                        }
                        rot.apply(res, time_multiplier);
                    }
                    else {
        ...
        if (global.resource.Asphodel_Powder.display){
            if (global.resource.Asphodel_Powder.amount > 0){
                ...
                if (global.race['warlord'] && global.eden['corruptor'] && p_on['corruptor']){
                    stabilize -= 0.004 * hugeAdjust(p_on['corruptor']);
                }
                ...
                modRes('Asphodel_Powder', -(decay * time_multiplier), false, 'eden_asphodel');
    ...
    if (global.portal['carport']){
        ...
        if (global.portal.carport.damaged > 0){
            ...
            global.portal.carport.damaged = Math.min(global.portal.carport.damaged, jobStack(global.portal.carport.count));
        }
        else {
...
function midLoop(){
    ...
    if (global.race.species === 'protoplasm'){
        ...
        // Moon Phase
        if (!global.race['orbit_decayed'] && (!global.race['iceage'] || global.tech['surface'])){
        ...
        var regCapAccum = {};
        function addCap(res, val, at, label){
            if (res !== global.race.species){
                //val = hugeAdjust(val); already handled in spatialStorage
            }
            else if (global.race['high_pop']){
                // High Pop houses a fractional number per building; the stack as a whole is rounded.
                val = Math.round(val);
            }
            caps[res] += val;
            // Resolve a building’s storage zone from its supply metadata.
            const zone = supplyZone(at);
            if (!regCapAccum[res]){ regCapAccum[res] = {}; }
            regCapAccum[res][zone] = (regCapAccum[res][zone] || 0) + val;
            // Record storage sources per zone for the regional breakdown.
            if (label !== undefined){
                if (breakdown.c[res]){ breakdown.c[res][label] = val + 'v'; }
                if (!breakdown.creg[zone]){ breakdown.creg[zone] = {}; }
                if (!breakdown.creg[zone][res]){ breakdown.creg[zone][res] = {}; }
                breakdown.creg[zone][res][label] = val + 'v';
            }
        }
        ...
        if (global.race.universe === 'evil' && global.tech['primitive'] && global.tech.primitive >= 3){
            ...
            if (global.city['garrison']){
                let gain = hugeAdjust(global.city.garrison.on) * 0.5;
                caps.Authority += gain;
                breakdown.c.Authority[actions.city.garrison.title()] = gain+'v';
            }
            if (global.underground['hunting_lodge']){
                let gain = hugeAdjust(global.underground.hunting_lodge.on) * 0.5;
                caps.Authority += gain;
            ...
            if ((global.race['lone_survivor'] || global.tech['isolation']) && global.tauceti['colony'] && support_on['colony']){
                global.resource.Authority.amount += hugeAdjust(support_on['colony'] * 5);
            }
        ...
        if (global.city['captive_housing']){
            let houses = hugeAdjust(global.city.captive_housing.count);
            global.city.captive_housing.raceCap = Math.floor(houses * (global.tech['unfathomable'] && global.tech.unfathomable >= 3 ? 3 : 2));
            global.city.captive_housing.cattleCap = Math.floor(houses * 5);
        }
        if (global.city['farm']){
            if (global.tech['farm']){
                let pop = global.city.farm.count * actions.city.farm.citizens();
                addCap(global.race.species, pop, 'city:farm', loc('city_farm'));
            }
        }
        if (global.city['wharf']){
            ...
            addCap('Crates', (global.city.wharf.count * vol), 'city:wharf', loc('city_wharf'));
            addCap('Containers', (global.city.wharf.count * vol), 'city:wharf', loc('city_wharf'));
        }
        if (global.space['munitions_depot']){
            let vol = 25;
            addCap('Crates', (global.space.munitions_depot.count * vol), 'space:munitions_depot', loc('tech_munitions_depot'));
            addCap('Containers', (global.space.munitions_depot.count * vol), 'space:munitions_depot', loc('tech_munitions_depot'));
        }
        if (global.interstellar['cargo_yard']){
            addCap('Crates', (global.interstellar.cargo_yard.count * 50), 'interstellar:cargo_yard', loc('interstellar_cargo_yard_title'));
            addCap('Containers', (global.interstellar.cargo_yard.count * 50), 'interstellar:cargo_yard', loc('interstellar_cargo_yard_title'));
        }
        if (global.city['storage_yard']){
            ...
            addCap('Crates', (global.city['storage_yard'].count * size), 'city:storage_yard', loc('city_storage_yard'));
        }
        if (global.space['garage']){
            ...
            addCap('Containers', (global.space.garage.count * g_vol), 'space:garage', loc('space_red_garage_title'));
            if (global.race['cataclysm'] || global.race['orbit_decayed']){
                addCap('Crates', (global.space.garage.count * g_vol), 'space:garage', loc('space_red_garage_title'));
            }
        }
        ...
        if (global.city['warehouse']){
            ...
            addCap('Containers', (global.city['warehouse'].count * volume), 'city:warehouse', loc('city_warehouse'));
        }
        if (global.city['mine']){
            lCaps['miner'] += jobStack(global.city.mine.count);
        }
        if (global.underground['under_mine']){
            lCaps['miner'] += jobStack(global.underground.under_mine.count);
        }
        if(global.underground['ice_collector']){
            lCaps['water_collector'] += jobStack(global.underground['ice_collector'].count);
        }
        if(p_on['core_mine']){
            lCaps['core_miner'] += jobStack(p_on['core_mine']);
        }
        if (global.portal['dig_demon'] && global.race['warlord']){
            ...
            addCap(global.race.species, demons, 'portal:dig_demon', loc('portal_dig_demon_title'));
            lCaps['miner'] += demons;
        ...
        if (global.city['coal_mine']){
            lCaps['coal_miner'] += jobStack(global.city.coal_mine.count);
        }
        if(global.underground['under_coal_mine']){
            lCaps['coal_miner'] += jobStack(global.underground.under_coal_mine.count);
        }
        if (global.city['bank']){
            lCaps['banker'] += jobStack(global.city.bank.count);
        }
        if(global.underground['vault']){
            lCaps['banker'] += jobStack(global.underground.vault.count);
        }
        if (global.city['amphitheatre']){
            let athVal = govActive('athleticism',1);
            lCaps['entertainer'] += jobStack(athVal ? (global.city.amphitheatre.count * athVal) : global.city.amphitheatre.count);
        }
        if(global.underground['color_garden']){
            lCaps['iceage_gardener'] += jobStack(global.underground.color_garden.count);
        }
        if (global.city['casino']){
            if (global.tech['theatre'] && !global.race['joyless']){
                lCaps['entertainer'] += jobStack(global.city.casino.count);
            }
        }
        if (global.space['spc_casino']){
            if (global.tech['theatre'] && !global.race['joyless']){
                lCaps['entertainer'] += jobStack(global.space.spc_casino.count);
            }
            if (decayPerks()){
                lCaps['banker'] += jobStack(global.space.spc_casino.count);
            }
        }
        if (global.portal['hell_casino']){
            if (global.tech['theatre'] && !global.race['joyless']){
                lCaps['entertainer'] += jobStack(global.portal.hell_casino.count * 3);
            }
            lCaps['banker'] += jobStack(global.portal.hell_casino.count);
        }
        if (global.tauceti['tauceti_casino']){
            if (global.tech['theatre'] && !global.race['joyless']){
                lCaps['entertainer'] += jobStack(global.tauceti.tauceti_casino.count);
            }
            if (global.tech['isolation']){
                lCaps['banker'] += jobStack(global.tauceti.tauceti_casino.count);

                let pop = p_on['tauceti_casino'] * actions.tauceti.tau_home.tauceti_casino.citizens();
                addCap(global.race.species, pop, 'tauceti:tauceti_casino', structName('casino'));
            }
        }
        if (global.galaxy['resort']){
            if (global.tech['theatre'] && !global.race['joyless']){
                lCaps['entertainer'] += jobStack(p_on['resort'] * 2);
            }
        }
        if (global.space['survey_resort']){
            lCaps['entertainer'] += jobStack(p_on['survey_resort'] || 0);
        }
        if (global.space['comedy_club']){
            lCaps['entertainer'] += jobStack(support_on['comedy_club'] || 0);
        }
        if (global.city['cement_plant']){
            lCaps['cement_worker'] += jobStack(global.city.cement_plant.count * 2);
        }
        if (global.underground['under_foundry']){
            lCaps['cement_worker'] += jobStack(global.underground.under_foundry.count);
        }
        if (global.eden['eden_cement']){
            let ec = p_on['eden_cement'] || 0;
            lCaps['cement_worker'] += jobStack(ec * 5);
        }
        if (global.race['orbit_decayed'] && p_on['red_factory']){
            lCaps['cement_worker'] += jobStack(p_on['red_factory']);
        }
        if (global.race['parasite'] && !global.tech['isolation']){
            lCaps['garrison'] += jobStack(traits.parasite.vars()[0]);
        }
        if (global.city['garrison']){
        ...
        if (global.underground['hunting_lodge_perk']){
            lCaps['garrison'] += global.underground.hunting_lodge_perk.count * actions.underground.cave_perk.hunting_lodge_perk.soldiers();
        }
        if (support_on['watch_tower']){
            lCaps['garrison'] += support_on['watch_tower'] * actions.surface.wastes.watch_tower.soldiers();
        }
        ...
        if (global.race['wish'] && global.race['wishStats']){
            lCaps['garrison'] += jobStack(global.race.wishStats.troop);
        }
        ...
        if (global.race['orbit_decayed'] && global.space.hasOwnProperty('red_mine')){
            lCaps['miner'] += jobStack(support_on['red_mine']);
            lCaps['coal_miner'] += jobStack(support_on['red_mine']);
        }
        if (!global.tech['world_control']){
        ...
        if (global.race['slaver'] && global.tech['slaves'] && global.city['slave_pen']) {
            caps['Slave'] = Math.floor(hugeAdjust(global.city.slave_pen.count * 4));
            breakdown.c.Slave[loc('city_slave_housing',[global.resource.Slave.name])] = Math.floor(hugeAdjust(global.city.slave_pen.count * 4)) + 'v';
        ...
        if (global.race['calm'] && global.city['meditation']) {
            caps['Zen'] = Math.floor(hugeAdjust(global.city.meditation.count * traits.calm.vars()[0]));
            breakdown.c.Zen[loc('city_meditation')] = Math.floor(hugeAdjust(global.city.meditation.count * traits.calm.vars()[0])) + 'v';
        ...
        if (global.city['basic_housing']){
            ...
            addCap(global.race.species, pop, 'city:basic_housing', housingLabel('small'));
        }
        if(global.underground['hollow']){
            ...
            addCap(global.race.species, pop, 'underground:hollow', loc('underground_hollow'));
        }
        if (global.tauceti['tau_housing'] && global.tech['isolation']){
            ...
            addCap(global.race.species, pop, 'tauceti:tau_housing', housingLabel('small'));
        }
        if (global.city['cottage']){
            let pop = global.city.cottage.count * actions.city.cottage.citizens();
            addCap(global.race.species, pop, 'city:cottage', housingLabel('medium'));
        }
        if (global.underground['stone_house']){
            ...
            addCap(global.race.species, pop, 'underground:stone_house', loc('underground_stone_house'));
        }
        if (global.city['apartment']){
            let pop = p_on['apartment'] * actions.city.apartment.citizens();
            addCap(global.race.species, pop, 'city:apartment', housingLabel('large'));
        }
        if (support_on['surface_apartment']){
            let pop = support_on['surface_apartment'] * actions.surface.wastes.surface_apartment.citizens();
            addCap(global.race.species, pop, 'surface:surface_apartment', `${housingLabel('large')}+0`);
        }
        if (global.underground['apartment_perk']){
            ...
            addCap(global.race.species, pop, 'underground:apartment_perk', `${housingLabel('large')}+1`);
        }
        if (global.eden['rectory']){
            let pop = p_on['rectory'] * actions.eden.eden_asphodel.rectory.citizens();
            addCap(global.race.species, pop, 'eden:rectory', loc(`eden_rectory_title`));
        }
        ...
        if (global.portal['bazaar'] && global.portal['spire']){
            ...
            addCap('Containers', containers, 'portal:bazaar', loc('portal_bazaar_title'));
            addCap('Crates', containers, 'portal:bazaar', loc('portal_bazaar_title'));
        }
        ...
        if (support_on['colony']){
            ...
            addCap(global.race.species, pop, 'tauceti:colony', loc('tau_home_colony'));
        }
        ...
        if (global.space['living_quarters']){
            ...
            addCap(global.race.species, gain, 'space:living_quarters');
            lCaps['colonist'] += jobStack(support_on['living_quarters']);
        ...
        if (global.space['titan_quarters']){
            ...
            addCap(global.race.species, gain, 'space:titan_quarters');
            lCaps['titan_colonist'] += jobStack(support_on['titan_quarters']);
        ...
        if (global.space['cloud_quarters']){
            ...
            addCap(global.race.species, gain, 'space:cloud_quarters', `${planetName().venus}`);
        }

        if (global.interstellar['habitat'] && p_on['habitat']){
            let pop = p_on['habitat'] * actions.interstellar.int_alpha.habitat.citizens();
            addCap(global.race.species, pop, 'interstellar:habitat', loc('interstellar_habitat_title'));
        }
        if (global.interstellar['luxury_condo'] && p_on['luxury_condo']){
            ...
            addCap(global.race.species, cit, 'interstellar:luxury_condo', loc('tech_luxury_condo'));
        }
        if (support_on['work_station']){
            let cit = Math.round(support_on['work_station'] * actions.surface.crater.work_station.citizens());
            addCap(global.race.species, cit, 'surface:work_station', loc('surface_work_station'));
            lCaps['crater_worker'] += jobStack(support_on['work_station']);
        }
        if (global.city['lodge']){
            let cit = global.city.lodge.count * actions.city.lodge.citizens();
            addCap(global.race.species, cit, 'city:lodge', loc('city_lodge'));
        }
        if (global.portal['hovel']){
            let cit = global.portal.hovel.count * actions.portal.prtl_wasteland.hovel.citizens();
            addCap(global.race.species, cit, 'portal:hovel', loc('portal_hovel_title'));
        }
        if (global.resource.Infernite.display && global.portal['fortress'] && !global.race['warlord']){
            let gain = spatialReasoning(1000);
            // The fortress in Hell, not the one in Eden — they share a name and nothing else.
        ...
        if (global.tauceti['repository']){
            if (global.tech['isolation']){
                ...
                addCap('Crates', (global.tauceti.repository.count * containers), 'tauceti:repository', loc('tech_repository'));
            }
        }

        if (global.galaxy['gateway_depot']){
            let containers = global.tech['world_control'] ? 150 : 100;
            addCap('Crates', (global.galaxy.gateway_depot.count * containers), 'galaxy:gateway_depot', loc('galaxy_gateway_depot'));
            addCap('Containers', (global.galaxy.gateway_depot.count * containers), 'galaxy:gateway_depot', loc('galaxy_gateway_depot'));
        }
        ...
        if (global.space['university'] && support_on['university']){
            ...
            let gain = uni.knowVal() * support_on['university'];
            lCaps['professor'] += jobStack(support_on['university'] * uni.professors());
            caps['Knowledge'] += gain;
        ...
        if(global.underground['stone_slab']){
            ...
            lCaps['professor'] += jobStack(global.underground.stone_slab.count);
            caps['Knowledge'] += gain;
            breakdown.c.Knowledge[loc('underground_stone_slab')] = gain+'v';
        }
        if (global.race['lone_survivor'] && global.tauceti['alien_outpost']){
            lCaps['professor'] += jobStack(global.tauceti.alien_outpost.count);
        }
        if (global.city['library']){
            let gain = global.city['library'].count * actions.city.library.knowVal();
            caps['Knowledge'] += gain;
            breakdown.c.Knowledge[loc('city_library')] = gain+'v';
        }
        if (global.city['wardenclyffe']){
            lCaps['scientist'] += jobStack(global.city['wardenclyffe'].count);
            let power = p_on['wardenclyffe'] || 0;
            let gain = (global.city['wardenclyffe'].count - power) * actions.city.wardenclyffe.knowVal();
            gain += power * actions.city.wardenclyffe.knowVal(true);
            caps['Knowledge'] += gain;
        ...
        if (global.portal['sensor_drone']){
            let gain = p_on['sensor_drone'] * actions.portal.prtl_badlands.sensor_drone.knowVal();
            caps['Knowledge'] += gain;
            breakdown.c.Knowledge[loc('portal_sensor_drone_title')] = gain+'v';
        }
        if (global.space['satellite']){
            let gain = global.space.satellite.count * actions.space.spc_home.satellite.knowVal();
            caps['Knowledge'] += gain;
            breakdown.c.Knowledge[loc('space_home_satellite_title')] = gain+'v';
        }
        if (global.space['observatory'] && global.space.observatory.count > 0){
            let gain = support_on['observatory'] * actions.space.spc_moon.observatory.knowVal();
            caps['Knowledge'] += gain;
            breakdown.c.Knowledge[loc('space_moon_observatory_title')] = gain+'v';

            if (global.race['cataclysm']){
                lCaps['professor'] += jobStack(support_on['observatory']);
            }
        }
        ...
        if (global.interstellar['laboratory'] && int_on['laboratory'] > 0){
            if (global.tech.science >= 16){
                lCaps['scientist'] += jobStack(int_on['laboratory']);
            }
            let gain = int_on['laboratory'] * actions.interstellar.int_alpha.laboratory.knowVal();
            caps['Knowledge'] += gain;
        ...
        if (global.city['biolab']){
            let gain = p_on['biolab'] * actions.city.biolab.knowVal();
            caps['Knowledge'] += gain;
        ...
        if (global.space['zero_g_lab'] && Math.min(support_on['zero_g_lab'],p_on['zero_g_lab']) > 0){
            ...
            let gain = Math.round(using * actions.space.spc_enceladus.zero_g_lab.knowVal());
            caps['Knowledge'] += gain;
        ...
        if (global.resource.Omniscience.display){
            ...
            if (global.race['warlord'] && global.portal['mortuary'] && global.portal['corpse_pile']){
                ...
                let gain = hugeAdjust(p_on['encampment'] || 0) * omniscience;
                caps['Omniscience'] += gain;
                breakdown.c.Omniscience[loc('eden_encampment_title')] = gain+'v';
            }

            if (global.eden['archive']){
                let gain = hugeAdjust(p_on['archive'] || 0) * 1013;
                caps['Omniscience'] += gain;
        ...
        if(global.underground['archaeological_dig']){
            let know = global.underground['archaeological_dig'].relics * actions.underground.industry.archaeological_dig.knowVal();
            caps['Knowledge'] += know;
        ...
        if (p_on['space_station']){
            lCaps['space_miner'] += jobStack(p_on['space_station'] * 3);
        }
        if (support_on['exotic_lab']){
            ...
            if (global.race['cataclysm'] || global.tech['resettle'] || global.race['orbit_decayed']){
                lCaps['scientist'] += jobStack(support_on['exotic_lab']);
            }
        }
        ...
        if (global.tech['isolation'] && support_on['infectious_disease_lab']){
            lCaps['professor'] += jobStack(support_on['infectious_disease_lab'] * 2);
            lCaps['scientist'] += jobStack(support_on['infectious_disease_lab']);
        }

        if (global.race['warlord'] && p_on['twisted_lab']){
            lCaps['professor'] += jobStack(p_on['twisted_lab'] * 3);
            lCaps['scientist'] += jobStack(p_on['twisted_lab'] * 2);
        }

        if (global.race['wish'] && global.race['wishStats'] && global.race.wishStats.prof){
            lCaps['scientist'] += jobStack(global.race.wishStats.prof);
        }
        ...
        if (support_on['fabrication'] && global.race['cataclysm']){
            lCaps['cement_worker'] += jobStack(support_on['fabrication']);
        }
        if (global.tech['isolation'] && support_on['tau_factory']){
            lCaps['cement_worker'] += jobStack(support_on['tau_factory'] * 2);
        }
        if (support_on['crater_fabrication']){
            lCaps['cement_worker'] += jobStack(support_on['crater_fabrication'] * 2);
        }
        ...
        if (actions.space.spc_venus.descender.operating() && support_on['industrial_complex']){
            lCaps['technician'] += jobStack(support_on['industrial_complex'] * actions.space.spc_venus.industrial_complex.technicians());
        }
        if (global.race['warlord'] && p_on['hell_factory']){
            lCaps['cement_worker'] += jobStack(p_on['hell_factory'] * 5);
        }
        if (p_on['womling_station']){
            lCaps['cement_worker'] += jobStack(p_on['womling_station'] * 1);
        }
        if (global.portal['carport']){
            lCaps['hell_surveyor'] += jobStack(global.portal.carport.count) - global.portal.carport.damaged;
        }
        if (p_on['archaeology']){
            lCaps['archaeologist'] += jobStack(p_on['archaeology'] * 2);
        }
        if(global.underground['archaeological_dig']){
            lCaps['archaeologist'] += jobStack(global.underground['archaeological_dig'].count);
        }
        if (support_on['ectoplasm_processor']){
            lCaps['ghost_trapper'] += jobStack(support_on['ectoplasm_processor'] * 5);
        }
        if (p_on['elysanite_mine']){
            lCaps['elysium_miner'] += jobStack(p_on['elysanite_mine'] * 2);
        }
        if (p_on['s_gate'] && p_on['telemetry_beacon']){
            let base_val = actions.galaxy.gxy_stargate.telemetry_beacon.knowVal();
            let gain = hugeAdjust(p_on['telemetry_beacon'] ** 2) * base_val;
            caps['Knowledge'] += gain;
        ...
        if (global.city['trade']){
            global.city.market.mtrade += global.city.trade.count * actions.city.trade.routes();
            breakdown.t_route[loc('city_trade')] = global.city.trade.count * actions.city.trade.routes();
        ...
        if (global.underground['trade']){
            global.city.market.mtrade += global.underground.trade.count * actions.underground.depths.trade.routes();
        ...
        global.city.market.mtrade = Math.floor(global.city.market.mtrade);

        if (global.tauceti['mining_pit']){
            lCaps['pit_miner'] += jobStack(support_on['mining_pit'] * (global.tech['isolation'] ? 6 : 8));
        }
        ...
        if (global.race['universe'] === 'magic' && global.race['witch_hunter']){
            ...
            if (global.civic.scientist.workers > 0){
                ...
                wiz = hugeAdjust(wiz);
                breakdown.c.Sus[loc('job_wizard')] = wiz+'v';
                sus += wiz;
            }
            ...
            if (global.city['pylon'] || global.space['pylon'] || global.tauceti['pylon']){
                ...
                else if(global.underground['pylon']){
                    p_count = global.underground.pylon.count;
                }
        ...
        if (global.tauceti['infectious_disease_lab']){
            let gain = support_on['infectious_disease_lab'] * actions.tauceti.tau_home.infectious_disease_lab.knowVal();
        ...
        if (global.race['truepath'] && global.tauceti['overseer'] && global.tech['tau_red'] && global.tech.tau_red >= 5){
            ...
            //injured amount can be above the population. This is to prevent shutting off housing to reduce injuries instantly.
            if (global.race['womling_friend']){
                loyal += 25 + hugeAdjust(support_on['overseer'] * actions.tauceti.tau_red.overseer.val());
                morale += 75 + hugeAdjust(support_on['womling_fun'] * actions.tauceti.tau_red.womling_fun.val());
            }
            else if (global.race['womling_god']){
                loyal += 75 + hugeAdjust(support_on['overseer'] * actions.tauceti.tau_red.overseer.val());
                morale += 40 + hugeAdjust(support_on['womling_fun'] * actions.tauceti.tau_red.womling_fun.val());
            }
            else if (global.race['womling_lord']){
                loyal += hugeAdjust(support_on['overseer'] * actions.tauceti.tau_red.overseer.val());
                morale += 30 + hugeAdjust(support_on['womling_fun'] * actions.tauceti.tau_red.womling_fun.val());
            }

            let vil_pop = womlingVillagePop();
            pop = Math.floor(support_on['womling_village'] * vil_pop);
            let farmers = Math.floor(hugeAdjust(support_on['womling_farm'] * 2));
            ...
            let unemployed = Math.max(0, pop - farmers - injured);

            let scientist = 0;
            if (support_on['womling_lab']){
                scientist = Math.floor(hugeAdjust(support_on['womling_lab']));
            ...
            let miners = Math.floor(hugeAdjust(support_on['womling_mine'] * 6));
            ...
            if (support_on['womling_craftworks']){
                artisan = Math.floor(support_on['womling_craftworks'] * womlingArtisansPer());
        ...
        if (p_on['world_controller']){
            ...
            if (global.interstellar['far_reach'] && p_on['far_reach'] > 0){
                boost += hugeAdjust(p_on['far_reach']) * 0.01;
            }
        ...
        if (global.underground['stone_slab_perk']){
            let gain = Math.round(caps['Knowledge'] * 0.02 * global.underground['stone_slab_perk'].count);
            lCaps['professor'] += jobStack(global.underground['stone_slab_perk'].count);
            caps['Knowledge'] += gain;
            breakdown.c.Knowledge[loc('underground_stone_slab')] = gain+'v';
        }
        ...
        // because of its reliance on Knowledge. This part is excluded from the storage structs that most other buildings make use of
        ...
        const regional = supplyMode() !== 'global';
        // Worked out once for the whole pass: which pools exist is a walk of every structure in the
        // game, and it cannot change between one resource and the next.
        if (regional){ refreshPools(); }
        Object.keys(caps).forEach(function (res){
            // Keep structural storage separate before adding assigned stacks.
            let structural = caps[res];
            ...
            // Add each active region’s base storage to the global total.
            if (regional && atomic_mass[res]){
                caps[res] += regionBaseTotal(res);
            }
            global.resource[res].max = caps[res];
            // Where that storage actually is. Only worth working out once the pools are live, and
            // only for the resources that are split between them.
            if (regional && atomic_mass[res]){
                setRegCaps(res, structural, regCapAccum[res], create_value, container_value);
                clampPools(res);
                // Include assigned stacks in each region’s storage breakdown.
                bdStacks(res, create_value, container_value);
            }
        ...
        if (global.race['gravity_well']){
            ...
            if (global.city['oil_well']){
                teamster += hugeAdjust(global.city.oil_well.count) * (global.tech['teamster'] && global.tech.teamster >= 3 ? 0 : 2);
            }

            if (global.city['factory'] && p_on['factory']){
                teamster += hugeAdjust(p_on['factory']) * 2;
            }

            if (global.space['red_factory'] && p_on['red_factory']){
                teamster += hugeAdjust(p_on['red_factory']) * 2;
            }

            if (global.space['moon_base'] && support_on['iridium_mine']){
                teamster += hugeAdjust(support_on['iridium_mine']) * 2;
            }

            if (global.space['moon_base'] && support_on['helium_mine']){
                teamster += hugeAdjust(support_on['helium_mine']);
            }

            if (global.tech['mars'] && support_on['red_mine']){
                teamster += hugeAdjust(support_on['red_mine']) * 3;
            }

            if (p_on['outpost']){
                teamster += hugeAdjust(p_on['outpost']) * 3;
            }
        ...
        if (global.portal.hasOwnProperty('transport')){
            let max = 0;
            if (gal_on['transport']){
                max = gal_on['transport'] * actions.portal.prtl_lake.transport.cargo_size();
            }
        ...
        if (global.portal.hasOwnProperty('purifier')){
            ...
            if (spire_on['base_camp']){
                port_value *= 1 + hugeAdjust(spire_on['base_camp'] * 0.4);
            }
            if (spire_on['port']){
                max += hugeAdjust(spire_on['port']) * port_value;
            }
            ...
            for (let i = gatewayArmada.length - 1; i >= 0; i--){
                ...
                for (let j = galaxyRegions.length - 1; j >= 0; j--){
                    ...
                    if (global.galaxy.defense.hasOwnProperty(region)){
                        ...
                        //humongous means the amount of ships that each building adds is multiplied
                        count += global.galaxy.defense[region][ship];
                        if (count > hugeScale(gal_on[ship])){
                            let overflow = count - hugeScale(gal_on[ship]);
                ...
                if (count < hugeScale(gal_on[ship])){
                    let underflow = hugeScale(gal_on[ship]) - count;
        ...
        if (belt_mining > 0 && global.tech['asteroid'] && global.tech['asteroid'] === 3){
            if (Math.rand(0,250) <= belt_mining){
                ...
                modRes('Elerium',1,true,supplyZone('space:elerium_ship'));
                drawTech();
        ...
        if(global.race['fasting'] && global.portal['oven_complete']){
            ...
            if(p_on['oven_complete']){
                progress = 0.00025;
                if(global.portal['dish_life_infuser'] && global.portal['dish_life_infuser'].on){
                    progress *= 1 + hugeAdjust(0.15 * global.portal['dish_life_infuser'].on);
                }
                if(global.portal['dish_soul_steeper'] && global.portal['dish_soul_steeper'].on && global.portal['spire']){
                    ...
                    let working = Math.min(hugeAdjust(global.portal['dish_soul_steeper'].on), Math.floor(hunger / mult));
        ...
        if(global.tech['surface'] >= 10 && (global.tech['crater'] || 0) < 1){
            let progress = 0.005 + hugeAdjust(0.002 * support_on['watch_tower']);
            global.surface['watch_tower'].explore += progress;
            if(global.surface['watch_tower'].explore >= 100){
                global.tech['crater'] = 1;
                global.surface['watch_tower'].explore = 100;
                messageQueue(loc('tech_surface_expedition_result'),'info',false,['progress']);
                renderSurface();
                drawTech();
            }
        }
        ...
        if(global.underground['mineshaft']){ //ice age mineshaft mechanics
            ...
            if(global.tech['mineshaft'] < 5 && global.underground['mineshaft'].depth >= 200000){
                global.underground['mineshaft'].depth = 200000;
            }
...
function rollDayEvents(astroSign){
    ...
    if (global.race.species !== 'protoplasm'){
        if (Math.rand(0,global.m_event.t) === 0){
            ...
            if (!global.race['pet'] && ((global.race['catnip'] && global.race.catnip >= 1.33) || (global.race['anise'] && global.race.anise >= 1.33))){
                event_pool = ['pet'];
            }
            if (event_pool.length > 0){
                let event = event_pool[Math.floor(seededRandom(0,event_pool.length))];
...
function longLoop(){
    ...
    if (global.race.species !== 'protoplasm'){
        ...
        if (global.race['mourning'] && global.race['mourning_total']){
            global.race['mourning_total'] *= 0.99;
        }
        ...
        if (global.city.calendar.day > 0){
            ...
            while (global.city.calendar.day > orbitLength() && !global.race['iceage']){
            ...
            if (global.race['cataclysm'] || global.race['orbit_decayed']){
                ...
                if(Math.rand(0,5) === 0){
                    if(global.tech['mineshaft_depth'] < 2){
                        //96% for colder, 3.5% for neutral, 0.5% for hotter
                        if(Math.rand(0, 25) > 0){
                            temp_adjust = -1;
                        }
                        else if(Math.rand(0, 8) > 0){
                            temp_adjust = 0;
                        }
                        else{
                            temp_adjust = 1;
                        ...
                        //92% for colder, 6% for neutral, 2% for hotter
                        if(Math.rand(0, 25) > 1){
                            temp_adjust = -1;
                        }
                        else if(Math.rand(0, 4) > 0){
                            temp_adjust = 0;
                        }
                        else{
                            temp_adjust = 1;
                        ...
                        //85% for colder, 10% for neutral, 5% for hotter
                        if(Math.rand(0, 20) > 2){
                            temp_adjust = -1;
                        }
                        else if(Math.rand(0, 3) > 0){
                            temp_adjust = 0;
                        }
                        else{
                            temp_adjust = 1;
                ...
                if((temp_adjust === -1 && global.city.calendar.temp > 0) || (temp_adjust === 1 && global.city.calendar.temp < 2)){
                    global.city.calendar.temp += temp_adjust;
                }
                if(global.city.calendar.temp === 0){
                    global.city.calendar.weather = 0;
                }
                else{
                    global.city.calendar.weather = 1; //no snow
                }
                global.city.calendar.wind = 0;
                global.city.calendar.season = 3;
            }
                ...
                if (sky === 0){
                    ...
                    if (global.race['darkness']){
                        if (Math.random() < 1 / Math.max(1, 7 - traits.darkness.vars()[0])){
                            global.city.calendar.weather = 1;
                        }
                    }
                }
        ...
        if (!global.tech['genesis'] && global.race.deterioration >= 1 && global.tech['high_tech'] && global.tech['high_tech'] >= 10 && !global.race['iceage']){
        ...
        if(global.underground['archaeological_dig'] && global.civic.archaeologist.workers >= 1){
            if(Math.rand(0, actions.underground.industry.archaeological_dig.relic_chance()) === 0){
                global.underground['archaeological_dig'].relics++;
            }
        }
        ...
        if (!global.race['warlord'] && (global.stats.matrix > 0 || global.stats.retire > 0) && (!global.race['iceage'] || global.tech['surface']) && !(global.race['servants'] && global.race.servants['prestige']) && Math.rand(0,25) === 0){
        ...
        if (global.race['truepath'] && global.tech['focus_cure'] && global.tech.focus_cure >= 2 && global.tauceti['infectious_disease_lab']){
            if (global.tauceti.infectious_disease_lab.cure < 100){
                let labs = hugeAdjust(support_on['infectious_disease_lab'] || 0) / 100;
                if (labs > 1){ labs = 1; }
        ...
        if (global.race['infiltrator']){
            ...
            if (global.resource.Knowledge.max >= (actions.tech.electricity.cost.Knowledge() * know_adjust) && global.tech['high_tech'] && global.tech.high_tech === 1){
                ...
                if(!global.race['iceage']){
                    ...
                    initStruct(actions.underground.industry.under_coal_power);
                }
            ...
            if (global.resource.Knowledge.max >= (actions.tech.fission.cost.Knowledge() * know_adjust) && global.tech['high_tech'] && global.tech.high_tech === 4 && global.tech['uranium'] && !global.race['iceage']){
            ...
            if (global.resource.Knowledge.max >= (actions.tech.rocketry.cost.Knowledge() * know_adjust) && global.tech['high_tech'] && global.tech.high_tech === 6 && !global.race['iceage']){
        ...
        if (moldFathom > 0){
            ...
            if (moldFathom >= 0.02 && global.resource.Knowledge.max >= (actions.tech.smelting.cost.Knowledge() * know_adjust) && checkTechRequirements('smelting',false) && !global.tech['smelting']){
                ...
                if(global.race['iceage']){
                    initStruct(actions.city.metal_refinery);
                    initStruct(actions.underground.depths.smelter);
                }
                if (global.race['steelen']){
                    global.tech['smelting'] = 2;
                }
            ...
            if (moldFathom >= 0.20 && global.resource.Knowledge.max >= (actions.tech.uranium_storage.cost.Knowledge() * know_adjust) && checkTechRequirements('uranium_storage',false) && global.tech['uranium'] && global.tech.uranium === 1){
                ...
                if(global.race['iceage']){
                    initStruct(actions.surface.crater.critical_storage);
                }
                drawTech();
            }
        ...
        if (global.space['shipyard'] && global.tech['resettle'] && global.tech.resettle >= 3){
            ...
            // Survey the rocky moons for tungsten.
            checkTungstenSurvey();
        ...
        if(global.city.ptrait.includes('kamikaze') && !global.race['cataclysm'] && !global.race['orbit_decayed'] && !global.race['warlord'] && !global.tech['thrusters'] && !global.tech['isolation'] && (global.tech['corrupt'] >= 2 || global.tech['tau_red'] >= 3) && Math.rand(0, 100) === 0){
            let oddity = 0;
            if(!(global.race.species === 'hybrid' ? global.custom.race1?.hybrid || [] :
                races[global.race.species].hybrid || [races[global.race.species].type]).includes('primordial')){
                    //overcomplicated check that just checks whether your current race does not have the primordial genus, including hybrid main or secondary genus
                    oddity++;
            }
            if(global.civic.scientist.workers === 0){
                oddity++;
...
function healSoldiers(astroSign){
    ...
    // Regenerative heals a fractional number of soldiers a day; the fraction is a chance at one more.
    let regen = jobScale(global.race['regenerative'] ? traits.regenerative.vars()[0] : 1);
    let healed = Math.floor(regen);
    if (Math.random() < regen - healed){
        healed++;
    }
...
function diffCalc(res,period){
    ...
    // Production by zone
    if (supplyMode() !== 'global' && atomic_mass[res]){
        const delta = regDelta(res), diff = regDiff(res);
        for (const zone in delta){
            diff[zone] = +(delta[zone] / (period / sec)).toFixed(2);
            delta[zone] = 0;
        }
    }
...
function steelCheck(){
    if (global.resource.Steel.display === false && Math.rand(0,1250) === 0){
        global.resource.Steel.display = true;
        modRes('Steel', 1, true, supplyRegionKey('city'));

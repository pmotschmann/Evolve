...
export const craftingRatio = (function () {
  ...
  return function (res, type, recalc) {
    if (recalc) {
      let noEarth =
        global.race["cataclysm"] || global.race["orbit_decayed"] ? true : false;
      crafting = {
        general: {
          add: [],
          multi: [],
      ...
      if (global.tech["foundry"] >= 3) {
        Object.keys(crafting).forEach(function (resource) {
          if (
            global.city.foundry[resource] &&
            global.city.foundry[resource] > 1
          ) {
      ...
      if (global.portal["demon_forge"] && p_on["demon_forge"]) {
        crafting.general.add.push({
          ...
          auto:
            (p_on["demon_forge"] *
              actions.portal.prtl_wasteland.demon_forge.crafting()) /
            100,
        });
      }
      ...
      if (global.space["fabrication"] && support_on["fabrication"]) {
        crafting.general.add.push({
          ...
          manual:
            support_on["fabrication"] *
            global.civic.colonist.workers *
            (noEarth ? highPopAdjust(0.05) : highPopAdjust(0.02)),
          auto:
            support_on["fabrication"] *
            global.civic.colonist.workers *
            (noEarth ? highPopAdjust(0.05) : highPopAdjust(0.02)),
      ...
      if (global.tauceti["tau_factory"] && support_on["tau_factory"]) {
        crafting.general.add.push({
          ...
          manual: 0,
          auto:
            support_on["tau_factory"] * (global.tech["isolation"] ? 2.75 : 0.9),
        });
      }
      if (
      ...
      if (
        (support_on["zero_g_lab"] && p_on["zero_g_lab"]) ||
        (support_on["infectious_disease_lab"] && p_on["infectious_disease_lab"])
      ) {
      ...
      if (
        global.tech["alien_crafting"] &&
        support_on["infectious_disease_lab"] &&
        p_on["infectious_disease_lab"]
      ) {
        let qCraft =
          1 +
          0.65 *
      ...
      if (global.race["ambidextrous"]) {
        crafting.general.add.push({
          ...
          manual:
            (traits.ambidextrous.vars()[0] * global.race["ambidextrous"]) / 100,
          auto:
            (traits.ambidextrous.vars()[0] * global.race["ambidextrous"]) / 100,
        });
      }
      ...
      if (
        ...
        global.stats.achieve.lamentis.l >= 1
      ) {
        crafting.general.multi.push({
      ...
      if (global.race["ambidextrous"]) {
        crafting.general.multi.push({
          ...
          auto:
            1 +
            (traits.ambidextrous.vars()[1] * global.race["ambidextrous"]) / 100,
        });
      }
      ...
      if (crafting["general"]) {
        ...
        for (let i = 0; i < crafting[res].add.length; i++) {
          ...
          add_bd[curr.name] =
            curr[type] + (add_bd[curr.name] ? add_bd[curr.name] : 0);
          multiplier += curr[type];
        }
        ...
        for (let i = 0; i < crafting[res].multi.length; i++) {
          ...
          multi_bd[curr.name] =
            curr[type] *
              (1 + (multi_bd[curr.name] ? +multi_bd[curr.name] : 0)) -
            1;
          multiplier *= curr[type];
        }
      }
...
export function drawResourceTab(tab) {
  if (tab === "market") {
    if (
      ...
      (global.settings.civTabs !== 4 || global.settings.marketTabs !== 0)
    ) {
      return;
    }
    ...
    if (
      !global.settings.tabLoad &&
      (global.settings.civTabs !== 4 || global.settings.marketTabs !== 1)
    ) {
      return;
    }
    ...
    if (
      !global.settings.tabLoad &&
      (global.settings.civTabs !== 4 || global.settings.marketTabs !== 2)
    ) {
      return;
    }
    ...
    if (
      !global.settings.tabLoad &&
      (global.settings.civTabs !== 4 || global.settings.marketTabs !== 3)
    ) {
      return;
    }
    ...
    if (
      !global.settings.tabLoad &&
      (global.settings.civTabs !== 4 || global.settings.marketTabs !== 4)
    ) {
      return;
    }
...
function loadResource(name, wiki, max, rate, tradable, stackable, color) {
  ...
  if (!global.resource[name].hasOwnProperty("value")) {
    global.resource[name]["value"] = global.race["truepath"]
      ? resource_values[name] * 2
  ...
  if (global.resource[name].max === -1 || global.resource[name].max === -2) {
    res_container = $(
      `<div id="res${name}" class="resource crafted" v-show="display"><div><h3 class="res has-text-${color}">{{ name | namespace }}</h3><span id="cnt${name}" class="count">{{ amount | diffSize }}</span></div></div>`,
    );
  } else {
    res_container = $(
      `<div id="res${name}" class="resource${global.settings.resBar[name] ? ` showBar` : ``}" v-show="display" :style="{ '--percent-full': (bar && max > 0 ? (amount/max)*100 : 0) + '%' }"><div><h3 class="res has-text-${color} bar" @click="toggle('${name}')">{{ name | namespace }}</h3><span id="cnt${name}" class="count">{{ amount | size }} / {{ max | size }}</span></div></div>`,
    );
  }
  ...
  if (stackable) {
    res_container.append(
      $(
        `<span><span tabindex="0" id="con${name}" v-if="showTrigger()" class="interact has-text-success" @click="trigModal" role="button" aria-label="Open crate management for ${global.resource[name].name}">+</span></span>`,
      ),
    );
  } else if (
    max !== -1 ||
    (max === -1 && rate === 0 && global.race["no_craft"]) ||
    name === "Scarletite" ||
    name === "Quantium"
  ) {
  ...
  if (
    ...
    res_container.append(
      $(
        `<span id="inc${name}" class="diff" :aria-label="resRate('${name}')">{{ diff | diffSize }} /s</span>`,
      ),
    );
  } else if (
    max === -1 &&
    !global.race["no_craft"] &&
    name !== "Scarletite" &&
    name !== "Quantium"
  ) {
    ...
    for (let i = 0; i < inc.length; i++) {
      craft.append(
        $(
          `<span id="inc${name}${inc[i]}"><a @click="craft('${name}',${inc[i]})" aria-label="craft ${inc[i]} ${global.resource[name].name}" role="button">+<span class="craft" data-val="${inc[i]}">${inc[i]}</span></a></span>`,
        ),
      );
    }
    craft.append(
      $(
        `<span id="inc${name}A"><a @click="craft('${name}','A')" aria-label="craft max ${global.resource[name].name}" role="button">+<span class="craft" data-val="${"A"}">A</span></a></span>`,
      ),
    );
    infopops = true;
    ...
    res_container.append(
      $(
        `<span id="inc${name}" class="diff" :aria-label="resRate('${name}')">{{ diff | diffSize }}</span>`,
      ),
    );
  } else {
    res_container.append($(`<span></span>`));
    ...
    methods: {
      ...
      craft(res, vol) {
        if (!global.race["no_craft"]) {
          ...
          let volume = Math.floor(
            global.resource[craft_costs[res][0].r].amount /
              craft_costs[res][0].a,
          );
          for (let i = 1; i < craft_costs[res].length; i++) {
            let temp = Math.floor(
              global.resource[craft_costs[res][i].r].amount /
                craft_costs[res][i].a,
            );
            if (temp < volume) {
              volume = temp;
      ...
      craftCost(res, vol) {
        ...
        for (let i = 0; i < craft_costs[res].length; i++) {
          ...
          costs =
            costs +
            `<div>${global.resource[craft_costs[res][i].r].name} ${num}</div>`;
        }
        return costs;
      },
      toggle(res) {
  ...
  if (infopops) {
    ...
    for (let i = 0; i < inc.length; i++) {
      let extra = function () {
        ...
        popper.append(
          $(
            `<div class="has-text-info">${loc("manual_crafting_hover_bonus", [bonus.toLocaleString(), global.resource[res].name])}</div>`,
          ),
        );

        let craft_costs = craftCost(true);
        let crafts = $(
          `<div><span class="has-text-success">${loc("manual_crafting_hover_craft")} </span></div>`,
        );
        let num_crafted = 0;
        if (typeof vol !== "number") {
          ...
          crafts.append(
            $(
              `<span class="has-text-advanced">${sizeApproximation((bonus / 100) * num_crafted, 1)} ${global.resource[res].name}</span>`,
            ),
          );
        } else {
          ...
          crafts.append(
            $(
              `<span class="has-text-advanced"><span class="craft" data-val="${sizeApproximation((bonus / 100) * vol)}">${total_crafted}</span> ${global.resource[res].name}</span>`,
            ),
          );
        }
        let costs = $(
          `<div><span class="has-text-danger">${loc("manual_crafting_hover_use")} </span></div>`,
        );
        for (let i = 0; i < craft_costs[res].length; i++) {
          costs.append(
            $(
              `<span class="craft-elm has-text-caution">${sizeApproximation(num_crafted * craft_costs[res][i].a, 1)} ${global.resource[craft_costs[res][i].r].name}</span>`,
            ),
          );
  ...
  if (stackable) {
    popover(`con${name}`, function () {
      var popper = $(
        `<div>${loc("resource_Crates_plural")} ${global.resource[name].crates}</div>`,
      );
      if (global.tech["steel_container"]) {
        popper.append(
          $(
            `<div>${loc("resource_Containers_plural")} ${global.resource[name].containers}</div>`,
          ),
        );
      }
      return popper;
  ...
  if (
    (name !== global.race.species || global.race["fasting"]) &&
    name !== "Crates" &&
    name !== "Containers" &&
    max !== -1
  ) {
...
export function setResourceName(name) {
  if (name === global.race.species) {
    ...
    global.resource[name].name =
      name === "Money" ? "$" : loc(`resource_${name}_name`);
  }
...
function loadSpecialResource(name, color) {
  ...
  var res_container = $(
    `<div id="res${name}" class="resource" v-show="count"><div><span class="res has-text-${color}">${loc(`resource_${name}_name`)}</span><span class="count">{{ count | round }}</span></div></div>`,
  );
  $("#resources").append(res_container);
  ...
  popover(`res${name}`, function () {
    ...
    switch (name) {
      case "Plasmid":
        {
          let potential =
            ...
            (global.race["wish"] && global.race["wishStats"]
              ? global.race.wishStats.plas
              : 0);
          let active = global.race["no_plasmid"]
            ? Math.min(potential, global.prestige.Plasmid.count)
            : global.prestige.Plasmid.count;
          desc.append(
            $(
              `<span>${loc(`resource_${name}_desc`, [active, +(plasmidBonus("plasmid") * 100).toFixed(2)])}</span>`,
            ),
          );
          if (
            global.genes["store"] &&
            (global.race.universe !== "antimatter" ||
              global.genes["bleed"] >= 3)
          ) {
            let plasmidSpatial = spatialReasoning(1, "plasmid");
            if (plasmidSpatial > 1) {
              desc.append(
                $(
                  `<span> ${loc(`resource_Plasmid_desc2`, [+((plasmidSpatial - 1) * 100).toFixed(2)])}</span>`,
                ),
              );
            }
          }
      ...
      case "AntiPlasmid":
        {
          desc.append(
            $(
              `<span>${loc(`resource_${name}_desc`, [global.prestige.AntiPlasmid.count, +(plasmidBonus("antiplasmid") * 100).toFixed(2)])}</span>`,
            ),
          );
          ...
          if (
            ...
            if (antiSpatial > 1) {
              desc.append(
                $(
                  `<span> ${loc(`resource_Plasmid_desc2`, [+((antiSpatial - 1) * 100).toFixed(2)])}</span>`,
                ),
              );
            }
          }
      ...
      case "Phage":
        {
          desc.append(
            $(
              `<span>${loc(global.prestige.AntiPlasmid.count > 0 ? `resource_Phage_desc2` : `resource_Phage_desc`, [250 + global.prestige.Phage.count])}</span>`,
            ),
          );
          ...
          if (global.genes["store"] && global.genes["store"] >= 4) {
            if (phageSpatial > 1) {
              desc.append(
                $(
                  `<span> ${loc(`resource_Plasmid_desc2`, [+((phageSpatial - 1) * 100).toFixed(2)])}</span>`,
                ),
              );
            }
          }
      ...
      case "Dark":
        {
          switch (global.race.universe) {
            case "standard":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_s`, [+((darkEffect("standard") - 1) * 100).toFixed(2)])}</span>`,
                ),
              );
              break;
            ...
            case "evil":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_e`, [+((darkEffect("evil") - 1) * 100).toFixed(2), +((darkEffect("evil", true) - 1) * 100).toFixed(2)])}</span>`,
                ),
              );
              break;
            ...
            case "micro":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_m`, [darkEffect("micro", false), darkEffect("micro", true)])}</span>`,
                ),
              );
              break;
            ...
            case "heavy":
              ...
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_h`, [+(space * 100).toFixed(4), +(int * 100).toFixed(4)])}</span>`,
                ),
              );
              break;
            ...
            case "antimatter":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_a`, [+((darkEffect("antimatter") - 1) * 100).toFixed(2)])}</span>`,
                ),
              );
              break;
            ...
            case "magic":
              desc.append(
                $(
                  `<span>${loc(`resource_${name}_desc_mg`, [loc("resource_Mana_name"), +((darkEffect("magic") - 1) * 100).toFixed(2)])}</span>`,
                ),
              );
              break;
          }
      ...
      case "Harmony":
        desc.append(
          $(
            `<span>${loc(`resource_${name}_desc`, [global.race.universe === "standard" ? 0.1 : 1, harmonyEffect()])}</span>`,
          ),
        );
        break;
      ...
      case "AICore":
        {
          let bonus = +(
            (1 - 0.99 ** global.prestige.AICore.count) *
            100
          ).toFixed(2);
          desc.append(
            $(`<span>${loc(`resource_${name}_desc`, [bonus])}</span>`),
          );
        }
        break;
      ...
      case "Supercoiled":
        {
          ...
          desc.append(
            $(
              `<span>${loc(`resource_${name}_desc`, [+bonus.toFixed(2)])}</span>`,
            ),
          );
          if (
            global.genes.hasOwnProperty("trader") &&
            global.genes.trader >= 2
          ) {
            let trade = (coiled / (coiled + 500)) * 100;
            desc.append(
              $(
                `<span> ${loc(`resource_${name}_trade_desc`, [+trade.toFixed(2)])}</span>`,
              ),
            );
          }
        }
...
function importRouteEnabled(route) {
  let routeCap =
    global.tech.currency >= 6 ? 1000000 : global.tech.currency >= 4 ? 100 : 25;
  if (global.resource[route].trade >= routeCap) {
    return false;
  }
  return true;
...
export function marketItem(mount, market_item, name, color, full) {
  ...
  if (full) {
    market_item.append(
      $(`<h3 class="res has-text-${color}">{{ r.name | namespace }}</h3>`),
    );
  }
  ...
  if (!global.race["no_trade"]) {
    market_item.append(
      $(
        `<span class="buy"><span class="has-text-success">${loc("resource_market_buy")}</span></span>`,
      ),
    );
    market_item.append(
      $(
        `<span tabindex="0" role="button" aria-label="${loc("resource_market_buy")} ${global.resource[name].name}" class="order" @click="purchase('${name}')">\${{ r.value | buy }}</span>`,
      ),
    );

    market_item.append(
      $(
        `<span class="sell"><span class="has-text-danger">${loc("resource_market_sell")}</span></span>`,
      ),
    );
    market_item.append(
      $(
        `<span tabindex="0" role="button" aria-label="${loc("resource_market_sell")} ${global.resource[name].name}" class="order" @click="sell('${name}')">\${{ r.value | sell }}</span>`,
      ),
    );
  }
  ...
  if (
    ...
    let trade = $(
      `<span class="trade" v-show="m.active"><span class="has-text-warning">${loc("resource_market_routes")}</span></span>`,
    );
    market_item.append(trade);
    trade.append(
      $(
        `<b-tooltip :label="aSell('${name}')" position="is-bottom" size="is-small" multilined animated><span tabindex="0" role="button" aria-label="export ${global.resource[name].name}" class="sub has-text-danger" @click="autoSell('${name}')"><span>-</span></span></b-tooltip>`,
      ),
    );
    trade.append(
      $(
        `<span class="current" v-html="$options.filters.trade(r.trade)"></span>`,
      ),
    );
    trade.append(
      $(
        `<b-tooltip :label="aBuy('${name}')" position="is-bottom" size="is-small" multilined animated><span tabindex="0" role="button" aria-label="import ${global.resource[name].name}" class="add has-text-success" @click="autoBuy('${name}')"><span>+</span></span></b-tooltip>`,
      ),
    );
    trade.append(
      $(
        `<span tabindex="0" role="button" class="zero has-text-advanced" @click="zero('${name}')">${loc("cancel_routes")}</span>`,
      ),
    );
  ...
  vBind({
    ...
    methods: {
      aSell(res) {
        let unit =
          tradeRatio[res] === 1
            ? loc("resource_market_unit")
            : loc("resource_market_units");
      ...
      purchase(res) {
        if (!global.race["no_trade"] && !global.settings.pause) {
          ...
          if (amount > 0) {
            ...
            global.resource[res].value += Number(
              (amount / Math.rand(1000, 10000)).toFixed(2),
            );
          }
        }
      },
        ...
        if (!global.race["no_trade"] && !global.settings.pause) {
          ...
          let amount = Math.floor(
            Math.min(
              ...
              (global.resource.Money.max - global.resource.Money.amount) /
                price,
            ),
          );
          if (amount > 0) {
      ...
      autoBuy(res, keyMult = keyMultiplier()) {
        for (let i = 0; i < keyMult; i++) {
          ...
          if (global.resource[res].trade >= 0) {
            if (
              importRouteEnabled(res) &&
              global.city.market.trade < global.city.market.mtrade
            ) {
...
function initGalaxyTrade() {
  ...
  $("#market").append(
    $(
      `<div id="galaxyTrade" v-show="t.xeno && t.xeno >= 5" class="market-header galaxyTrade"><h2 class="is-sr-only">${loc("galaxy_trade")}</h2></div>`,
    ),
  );
  galacticTrade();
}
...
export function galacticTrade(modal) {
  ...
  if (global.galaxy["trade"]) {
    galaxyTrade.append(
      $(
        `<div class="market-item trade-header"><span class="has-text-special">${loc("galaxy_trade")}</span></div>`,
      ),
    );
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      offer.append(
        $(
          `<span class="offer-item has-text-success">${global.resource[offers[i].buy.res].name}</span>`,
        ),
      );
      offer.append(
        $(
          `<span class="offer-vol has-text-advanced">+{{ '${i}' | t_vol }}/s</span>`,
        ),
      );

      offer.append(
        $(
          `<span class="offer-item has-text-danger">${global.resource[offers[i].sell.res].name}</span>`,
        ),
      );
      offer.append(
        $(
          `<span class="offer-vol has-text-caution">-{{ '${i}' | s_vol }}/s</span>`,
        ),
      );

      let trade = $(
        `<span class="trade"><span class="has-text-warning">${loc("resource_market_routes")}</span></span>`,
      );
      offer.append(trade);

      let assign = loc("galaxy_freighter_assign", [
        global.resource[offers[i].buy.res].name,
        global.resource[offers[i].sell.res].name,
      ]);
      let unassign = loc("galaxy_freighter_unassign", [
        global.resource[offers[i].buy.res].name,
        global.resource[offers[i].sell.res].name,
      ]);
      trade.append(
        $(
          `<b-tooltip :label="desc('${unassign}')" position="is-bottom" size="is-small" multilined animated><span tabindex="0" role="button" aria-label="${unassign}" class="sub has-text-danger" @click="less('${i}')"><span>-</span></span></b-tooltip>`,
        ),
      );
      trade.append($(`<span class="current">{{ g.f${i} }}</span>`));
      trade.append(
        $(
          `<b-tooltip :label="desc('${assign}')" position="is-bottom" size="is-small" multilined animated><span tabindex="0" role="button" aria-label="${assign}" class="add has-text-success" @click="more('${i}')"><span>+</span></span></b-tooltip>`,
        ),
      );
      trade.append(
        $(
          `<span tabindex="0" role="button" class="zero has-text-advanced" @click="zero('${i}')">${loc("cancel_routes")}</span>`,
        ),
      );
    }

    let totals = $(
      `<div class="market-item trade-offer"><div id="galacticTradeTotal"><span class="tradeTotal"><span class="has-text-caution">${loc("resource_market_galactic_trade_routes")}</span> {{ g.cur }} / {{ g.max }}</span></div></div>`,
    );
    totals.append(
      $(
        `<span tabindex="0" role="button" class="zero has-text-advanced" @click="zero()">${loc("cancel_all_routes")}</span>`,
      ),
    );
      ...
      if (breakdown.hasOwnProperty("gt_route")) {
        Object.keys(breakdown.gt_route).forEach(function (k) {
          if (breakdown.gt_route[k] > 0) {
            bd.append(
              `<div class="modal_bd"><span class="has-text-warning">${k}</span> <span>+${breakdown.gt_route[k]}</span></div>`,
            );
          }
        });
      }
      bd.append(
        `<div class="modal_bd ${global.galaxy.trade.max > 0 ? "sum" : ""}"><span class="has-text-caution">${loc("resource_market_galactic_trade_routes")}</span> <span>${global.galaxy.trade.max}</span></div>`,
      );
      return bd;
    },
    {
...
export function containerItem(mount, market_item, name, color) {
  ...
  if (global.resource.Crates.display) {
    let crate = $(
      `<span class="trade"><span class="has-text-warning">${global.resource.Crates.name}</span></span>`,
    );
    market_item.append(crate);

    crate.append(
      $(
        `<span tabindex="0" role="button" aria-label="remove ${global.resource[name].name} ${global.resource.Crates.name}" class="sub has-text-danger" @click="subCrate('${name}')"><span>&laquo;</span></span>`,
      ),
    );
    crate.append(
      $(
        `<span class="current" v-html="$options.filters.cCnt(crates,'${name}')"></span>`,
      ),
    );
    crate.append(
      $(
        `<span tabindex="0" role="button" aria-label="add ${global.resource[name].name} ${global.resource.Crates.name}" class="add has-text-success" @click="addCrate('${name}')"><span>&raquo;</span></span>`,
      ),
    );
  }
  ...
  if (global.resource.Containers.display) {
    let container = $(
      `<span class="trade"><span class="has-text-warning">${global.resource.Containers.name}</span></span>`,
    );
    market_item.append(container);

    container.append(
      $(
        `<span tabindex="0" role="button" aria-label="remove ${global.resource[name].name} ${global.resource.Containers.name}" class="sub has-text-danger" @click="subCon('${name}')"><span>&laquo;</span></span>`,
      ),
    );
    container.append(
      $(
        `<span class="current" v-html="$options.filters.trick(containers)"></span>`,
      ),
    );
    container.append(
      $(
        `<span tabindex="0" role="button" aria-label="add ${global.resource[name].name} ${global.resource.Containers.name}" class="add has-text-success" @click="addCon('${name}')"><span>&raquo;</span></span>`,
      ),
    );
  }
  ...
  vBind({
    ...
    filters: {
      trick(v) {
        if (
          name === "Stone" &&
          global.resource[name].crates === 10 &&
          global.resource[name].containers === 31
        ) {
      ...
      cCnt(ct, res) {
        if (
          (res === "Food" && !global.race["artifical"]) ||
          (global.race["artifical"] && res === "Coal") ||
          res === "Souls"
        ) {
...
export function tradeSellPrice(res) {
  ...
  if (global.tech["railway"]) {
    let boost =
      global.stats.achieve["banana"] && global.stats.achieve.banana.l >= 1
        ? 0.03
        : 0.02;
...
export function craftingPopover(id, res, type, extra) {
  popover(
    ...
    function () {
      let bd = $(
        `<div class="resBreakdown"><div class="has-text-info">{{ res.name | namespace }}</div></div>`,
      );
      ...
      if (type === "auto" && breakdown.p[res]) {
        Object.keys(breakdown.p[res]).forEach(function (mod) {
          ...
          if (val != 0 && !isNaN(val)) {
            ...
            mod = mod.replace(/'/g, "\\'");
            col1.append(
              `<div class="modal_bd"><span>${label}</span><span class="has-text-${type}">{{ ${[res]}['${mod}'] | translate }}</span></div>`,
            );
          }
        });
      }
      Object.keys(craft_total.multi_bd).forEach(function (mod) {
        ...
        if (val != 0 && !isNaN(val)) {
          ...
          mod = mod.replace(/'/g, "\\'");
          col1.append(
            `<div class="modal_bd"><span>${label}</span><span class="has-text-${type}">{{ craft.multi_bd['${mod}'] | translate }}</span></div>`,
          );
        }
      });
      ...
      let title = $(
        `<div class="has-text-info">${loc(`craft_tools_multi`)}</div>`,
      );
      col2.append(title);
      let count = 0;
      Object.keys(craft_total.add_bd).forEach(function (mod) {
        ...
        if (val != 0 && !isNaN(val)) {
          ...
          mod = mod.replace(/'/g, "\\'");
          col2.append(
            `<div class="modal_bd"><span>${label}</span><span class="has-text-${type}">{{ craft.add_bd['${mod}'] | translate }}</span></div>`,
          );
        }
      });
      if (count > 0) {
      ...
      if (breakdown.p.consume && breakdown.p.consume[res]) {
        ...
        Object.keys(breakdown.p.consume[res]).forEach(function (mod) {
          ...
          if (val != 0 && !isNaN(val)) {
            ...
            mod = mod.replace(/'/g, "\\'");
            col3.append(
              `<div class="modal_bd"><span>${label}</span><span class="has-text-${type}">{{ consume.${res}['${mod}'] | fix | translate }}</span></div>`,
            );
          }
        });
        if (count > 0) {
      ...
      if (
        ...
        bd.append(
          `<div class="modal_bd sum"><span>${loc("to_empty")}</span><span class="has-text-danger">{{ res.amount | counter }}</span></div>`,
        );
      }
  ...
  popover(
    ...
    function () {
      ...
      if (type === "p" && name === global.race.species) {
        bd = $(
          `<div class="resBreakdown"><div class="has-text-info">${loc("starvation_resist")}</div></div>`,
        );
      }
      let table = $(`<div class="parent"></div>`);
      bd.append(table);
      ...
      if (
        breakdown[type][name] &&
        !(global.race.species === name && type === "p")
      ) {
        let col1 = $(`<div></div>`);
        table.append(col1);
        let types = [name];
          ...
          if (breakdown[type][t]) {
            Object.keys(breakdown[type][t]).forEach(function (mod) {
              ...
              if (val != 0 && !isNaN(val)) {
                ...
                mod = mod.replace(/'/g, "\\'");
                col1.append(
                  `<div class="modal_bd"><span>${label}</span><span class="has-text-${type}">{{ ${t}['${mod}'] | translate }}</span></div>`,
                );
              }
            });
          }
      ...
      if (breakdown[type].consume && breakdown[type].consume[name]) {
        ...
        Object.keys(breakdown[type].consume[name]).forEach(function (mod) {
          ...
          if (val != 0 && !isNaN(val)) {
            ...
            mod = mod.replace(/'/g, "\\'");
            col2.append(
              `<div class="modal_bd"><span>${label}</span><span class="has-text-${type}">{{ consume.${name}['${mod}'] | fix | translate }}</span></div>`,
            );
          }
        });
        if (count > 0) {
      ...
      if (type === "p" && name !== global.race.species) {
        ...
        bd.append(
          `<div class="modal_bd sum"><span>{{ res.diff | direction }}</span><span class="has-text-${dir}">{{ res.amount | counter }}</span></div>`,
        );
      }
...
function loadRouteCounter() {
  ...
  var market_item = $(
    `<div id="tradeTotal" v-show="active" class="market-item"><div id="tradeTotalPopover"><span class="tradeTotal${no_market}"><span class="has-text-caution">${loc("resource_market_trade_routes")}</span> <span v-html="$options.filters.tdeCnt(trade)"></span> / {{ mtrade }}</span></div></div>`,
  );
  market_item.append(
    $(
      `<span tabindex="0" role="button" class="zero has-text-advanced" @click="zero()">${loc("cancel_all_routes")}</span>`,
      ...
      tdeCnt(ct) {
        ...
        if (
          ((ct === 100 &&
            ...
            (ct === 10 &&
              (global.tech["isolation"] || global.race["cataclysm"]))) &&
          egg17.length > 0
        ) {
          return "10" + egg17;
        }
      ...
      if (breakdown.hasOwnProperty("t_route")) {
        Object.keys(breakdown.t_route).forEach(function (k) {
          if (breakdown.t_route[k] > 0) {
            bd.append(
              `<div class="modal_bd"><span class="has-text-warning">${k}</span> <span>+${breakdown.t_route[k]}</span></div>`,
            );
          }
        });
      }
      bd.append(
        `<div class="modal_bd ${global.city.market.mtrade > 0 ? "sum" : ""}"><span class="has-text-caution">${loc("resource_market_trade_routes")}</span> <span>${global.city.market.mtrade}</span></div>`,
      );
      return bd;
    },
    {
...
function loadContainerCounter() {
  ...
  var market_item = $(
    `<div id="crateTotal" class="market-item"><span v-show="cr.display" class="crtTotal"><span class="has-text-warning">${global.resource.Crates.name}</span><span>{{ cr.amount }} / {{ cr.max }}</span></span><span v-show="cn.display" class="cntTotal"><span class="has-text-warning">${global.resource.Containers.name}</span><span>{{ cn.amount }} / {{ cn.max }}</span></span></div>`,
  );
  $("#resStorage").append(market_item);
...
function buildCrateLabel() {
  let material =
    global.race["kindling_kindred"] || global.race["smoldering"]
      ...
      : global.resource["Plywood"]
        ? global.resource.Plywood.name
        : global.resource.Plywood.name;
...
function buildCrate(num) {
  ...
  if (
    global.resource[material].amount >= cost * keyMutipler &&
    global.resource.Crates.amount < global.resource.Crates.max
  ) {
...
function buildContainer(num) {
  ...
  if (
    keyMutipler + global.resource.Containers.amount >
    global.resource.Containers.max
  ) {
    keyMutipler =
      global.resource.Containers.max - global.resource.Containers.amount;
  }
  if (global.resource["Steel"].amount < 125 * keyMutipler) {
    keyMutipler = Math.floor(global.resource["Steel"].amount / 125);
  }
  if (
    global.resource["Steel"].amount >= 125 * keyMutipler &&
    global.resource.Containers.amount < global.resource.Containers.max
  ) {
...
function drawModal(name) {
  $("#modalBox").append(
    $(
      '<p id="modalBoxTitle" class="has-text-warning modalTitle">{{ name }} - {{ amount | size }}/{{ max | size }}</p>',
    ),
  );
  ...
  if (
    (name === "Food" && !global.race["artifical"]) ||
  ...
  crates.append(
    $(
      `<div class="crateHead"><span>${loc("resource_modal_crate_owned")} {{ crates.amount }}/{{ crates.max }}</span><span>${loc("resource_modal_crate_assigned")} {{ res.crates }}</span></div>`,
    ),
  );

  let buildCr = $(
    `<button class="button construct" @click="buildCrate()">${loc("resource_modal_crate_construct")}</button>`,
  );
  let removeCr = $(
    `<button class="button unassign" @click="subCrate('${name}')">${loc("resource_modal_crate_unassign")}</button>`,
  );
  let addCr = $(
    `<button class="button assign" @click="addCrate('${name}')">${loc("resource_modal_crate_assign")}</button>`,
  );
  ...
  if (global.resource.Containers.display) {
    let containers = $(
      '<div id="modalContainers" class="crates divide"></div>',
    );
    body.append(containers);

    containers.append(
      $(
        `<div class="crateHead"><span>${loc("resource_modal_container_owned")} {{ containers.amount }}/{{ containers.max }}</span><span>${loc("resource_modal_container_assigned")} {{ res.containers }}</span></div>`,
      ),
    );

    let buildCon = $(
      `<button class="button construct" @click="buildContainer()">${loc("resource_modal_container_construct")}</button>`,
    );
    let removeCon = $(
      `<button class="button unassign" @click="removeContainer('${name}')">${loc("resource_modal_container_unassign")}</button>`,
    );
    let addCon = $(
      `<button class="button assign" @click="addContainer('${name}')">${loc("resource_modal_container_assign")}</button>`,
    );
...
export function crateValue() {
  let create_value =
    global.tech["container"] && global.tech["container"] >= 2 ? 500 : 350;
  ...
  create_value *= global.stats.achieve["blackhole"]
    ? 1 + global.stats.achieve.blackhole.l * 0.05
    : 1;
...
export function containerValue() {
  let container_value =
    global.tech["steel_container"] && global.tech["steel_container"] >= 3
      ? 1200
      : 800;
  ...
  container_value *= global.stats.achieve["blackhole"]
    ? 1 + global.stats.achieve.blackhole.l * 0.05
    : 1;
...
function initMarket() {
  ...
  let market = $(
    `<div id="market-qty" class="market-header"><h2 class="is-sr-only">${loc("resource_market")}</h2></div>`,
  );
  clearElement($("#market"));
...
function initStorage() {
  ...
  let store = $(
    `<div id="createHead" class="storage-header"><h2 class="is-sr-only">${loc("tab_storage")}</h2></div>`,
  );
  ...
  if (global.resource["Crates"] && global.resource["Containers"]) {
    store.append(
      $(
        `<b-tooltip :label="buildCrateDesc()" position="is-bottom" class="crate" animated multilined><button :aria-label="buildCrateDesc()" v-show="cr.display" class="button" @click="crate">${loc("resource_modal_crate_construct")}</button></b-tooltip>`,
      ),
    );
    store.append(
      $(
        `<b-tooltip :label="buildContainerDesc()" position="is-bottom" class="container" animated multilined><button :aria-label="buildContainerDesc()" v-show="cn.display" class="button" @click="container">${loc("resource_modal_container_construct")}</button></b-tooltip>`,
      ),
    );
...
function loadMarket() {
  ...
  if (!global.race["no_trade"]) {
    market.append(
      $(`<h3 class="is-sr-only">${loc("resource_trade_qty")}</h3>`),
    );
    market.append(
      $(
        `<b-field class="market"><span tabindex="0" class="button has-text-danger" role="button" aria-label="${loc("resource_trade_qty")} decrease" @click="less">-</span><b-numberinput :input="val()" min="1" :max="limit()" v-model="qty" :controls="false" aria-label="${loc("resource_trade_qty")}"></b-numberinput><span tabindex="0" class="button has-text-success" role="button" aria-label="${loc("resource_trade_qty")} increase" @click="more">+</span></b-field>`,
      ),
    );
  }
...
function initEjector() {
  ...
  if (global.interstellar["mass_ejector"]) {
    let ejector = $(
      `<div id="eject" class="market-item"><h3 class="res has-text-warning">${loc("interstellar_mass_ejector_vol")}</h3></div>`,
    );
    ...
    eject.append(
      $(
        `<span>{{ total }} / {{ on | max }}{{ on | real }}</span><span class="mass">${loc("interstellar_mass_ejector_mass")}: {{ mass | approx }} kt/s</span>`,
      ),
    );
...
export function loadEjector(name, color) {
  ...
  if (atomic_mass[name] && global.interstellar["mass_ejector"]) {
    if (
      ...
      (name === "Elerium" || name === "Infernite")
    ) {
      color = "caution";
    }
    let ejector = $(
      `<div id="eject${name}" class="market-item" v-show="r.display"><h3 class="res has-text-${color}">${global.resource[name].name}</h3></div>`,
    );
    $("#resEjector").append(ejector);
    ...
    res.append(
      $(
        `<span tabindex="0" role="button" aria-label="eject less ${global.resource[name].name}" class="sub has-text-danger" @click="ejectLess('${name}')"><span>&laquo;</span></span>`,
      ),
    );
    res.append($(`<span class="current">{{ e.${name} }}</span>`));
    res.append(
      $(
        `<span tabindex="0" role="button" aria-label="eject more ${global.resource[name].name}" class="add has-text-success" @click="ejectMore('${name}')"><span>&raquo;</span></span>`,
      ),
    );

    res.append(
      $(
        `<span class="mass">${loc("interstellar_mass_ejector_per")}: <span class="has-text-warning">${atomic_mass[name]}</span> kt</span>`,
      ),
    );
    ...
    vBind({
      ...
      methods: {
        ejectMore(r) {
          let keyMutipler = keyMultiplier();
          if (
            keyMutipler + global.interstellar.mass_ejector.total >
            p_on["mass_ejector"] * 1000
          ) {
            keyMutipler =
              p_on["mass_ejector"] * 1000 -
              global.interstellar.mass_ejector.total;
          }
...
function initSupply() {
  ...
  if (global.portal["transport"]) {
    let supply = $(
      `<div id="spireSupply"><h3 class="res has-text-warning pad">${loc("portal_transport_supply")}</h3></div>`,
    );
...
export function loadSupply(name, color) {
  ...
  if (supplyValue[name] && global.portal["transport"]) {
    let ejector = $(
      `<div id="supply${name}" class="market-item" v-show="r.display"><h3 class="res has-text-${color}">${global.resource[name].name}</h3></div>`,
    );
    ...
    res.append(
      $(
        `<span tabindex="0" role="button" aria-label="eject less ${loc("resource_" + name + "_name")}" class="sub has-text-danger" @click="supplyLess('${name}')"><span>&laquo;</span></span>`,
      ),
    );
    res.append($(`<span class="current">{{ e.${name} }}</span>`));
    res.append(
      $(
        `<span tabindex="0" role="button" aria-label="eject more ${loc("resource_" + name + "_name")}" class="add has-text-success" @click="supplyMore('${name}')"><span>&raquo;</span></span>`,
      ),
    );
    ...
    res.append(
      $(
        `<span class="mass">${loc("portal_transport_item", [`<span class="has-text-caution">${volume}</span>`, `<span class="has-text-success">${supplyValue[name].in}</span>`])}</span>`,
      ),
    );
    ...
    vBind({
      ...
      methods: {
        supplyMore(r) {
          let keyMutipler = keyMultiplier();
          if (
            keyMutipler + global.portal.transport.cargo.used >
            global.portal.transport.cargo.max
          ) {
            keyMutipler =
              global.portal.transport.cargo.max -
              global.portal.transport.cargo.used;
...
export function loadAlchemy(name, color, basic) {
  ...
  if (
    global.tech["alchemy"] &&
    (basic || global.tech.alchemy >= 2) &&
    name !== "Crystal"
  ) {
    let alchemy = $(
      `<div id="alchemy${name}" class="market-item" v-show="r.display"><h3 class="res has-text-${color}">${global.resource[name].name}</h3></div>`,
    );
    ...
    res.append(
      $(
        `<span tabindex="0" role="button" aria-label="transmute less ${global.resource[name].name}" class="sub has-text-danger" @click="subSpell('${name}')"><span>&laquo;</span></span>`,
      ),
    );
    res.append($(`<span class="current">{{ a.${name} }}</span>`));
    res.append(
      $(
        `<span tabindex="0" role="button" aria-label="transmute more ${global.resource[name].name}" class="add has-text-success" @click="addSpell('${name}')"><span>&raquo;</span></span>`,
      ),
    );
    ...
    popover(
      ...
      function () {
        ...
        return $(
          `<div>${loc("resource_alchemy", [1, loc(`resource_Mana_name`), 0.15, loc(`resource_Crystal_name`), +rate.toFixed(2), global.resource[name].name])}</div>`,
        );
      },
      {
        elm: `#alchemy${name} h3`,
...
export const spatialReasoning = (function () {
  ...
  return function (value, type, recalc) {
    ...
    if (!spatial[tkey][key] || recalc) {
      ...
      if (global.genes["store"]) {
        ...
        if (
          !type ||
          (type &&
            ((type === "plasmid" && global.race.universe !== "antimatter") ||
              (type === "anti" && global.race.universe === "antimatter")))
        ) {
          plasmids =
            global.race.universe === "antimatter"
              ? global.prestige.AntiPlasmid.count
          ...
          if (global.race["no_plasmid"]) {
            ...
            raw = Math.floor(
              plasmids / (global.race.universe === "antimatter" ? 2 : 5),
            );
          }
        ...
        if (!type || (type && type === "phage")) {
          if (global.genes["store"] >= 4) {
            plasmids += Math.round(
              global.prestige.Phage.count * (global.race["nerfed"] ? 1 / 3 : 1),
            );
          }
        }
        let divisor =
          global.genes.store >= 2
            ? global.genes.store >= 3
              ? 1250
              : 1666
            : 2500;
        ...
        if (global.genes["bleed"] && global.genes["bleed"] >= 3) {
          if (
            ...
            let raw =
              global.race.universe === "antimatter"
                ? global.prestige.Plasmid.count / 5
    ...
    return type
      ? spatial[tkey][key] * value
      : Math.round(spatial[tkey][key] * value);
  };
})();
...
export function faithBonus(num_temples = -1) {
  if (global.race["no_plasmid"] || global.race.universe === "antimatter") {
    ...
    if (num_temples > 0) {
      let temple_bonus =
        global.tech["anthropology"] && global.tech["anthropology"] >= 1
          ? 0.016
          : 0.01;
      if (global.tech["fanaticism"] && global.tech["fanaticism"] >= 2) {
        let indoc =
          ...
          highPopAdjust(
            global.race.universe === "antimatter" ? 0.0002 : 0.0004,
          );
        temple_bonus += indoc;
      }
      if (
        ...
        let priest_bonus =
          global.genes["ancients"] >= 5
            ...
            : global.genes["ancients"] >= 3
              ...
              : 0.0001;
        temple_bonus +=
          highPopAdjust(priest_bonus) *
          workerScale(global.civic.priest.workers, "priest");
      }
      if (global.race.universe === "antimatter") {
      ...
      if (global.race.universe !== "antimatter" || global.genes["bleed"]) {
        let active =
          ...
          (global.race["wish"] && global.race["wishStats"]
            ? global.race.wishStats.plas
            : 0);
        let plasmids = global.race["no_plasmid"]
          ? Math.min(active, global.prestige.Plasmid.count)
          : global.prestige.Plasmid.count;
        if (global.race.universe === "antimatter" && global.genes["bleed"]) {
          plasmids *= 0.025;
        }
        if (global.race["decayed"]) {
          plasmids -= Math.round(
            (global.stats.days - global.race.decayed) /
              (300 + global.race.gene_fortify * 6),
          );
        }
        let p_cap = 250 + global.prestige.Phage.count;
        if (plasmids > p_cap) {
          standard =
            +(Math.log(p_cap + 50) - 3.91202).toFixed(5) / 2.888 +
            Math.log(plasmids + 1 - p_cap) / Math.LN2 / 250;
        } else if (plasmids < 0) {
          standard = 0;
        } else {

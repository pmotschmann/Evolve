...
export const actions = {
    evolution: {
        ...
        organelles: {
            ...
            cost: {
                RNA(r={}){ return evolveCosts('organelles',12,8,r.offset); },
                DNA(r={}){ return evolveCosts('organelles',4,4,r.offset); }
            },
            effect(){
                let rna = global.race['rapid_mutation'] ? 2 : 1;
        ...
        nucleus: {
            ...
            cost: {
                RNA(r={}){ return evolveCosts('nucleus',38, global.tech['evo'] && global.tech.evo >= 4 ? 16 : 32, r.offset ); },
                DNA(r={}){ return evolveCosts('nucleus',18, global.tech['evo'] && global.tech.evo >= 4 ? 12 : 16, r.offset ); }
            },
            effect(){
        ...
        eukaryotic_cell: {
            ...
            cost: {
                RNA(r={}){ return evolveCosts('eukaryotic_cell',20,20,r.offset); },
                DNA(r={}){ return evolveCosts('eukaryotic_cell',40,12,r.offset); }
            },
            effect(){
        ...
        mitochondria: {
            ...
            cost: {
                RNA(r={}){ return evolveCosts('mitochondria',75,50,r.offset); },
                DNA(r={}){ return evolveCosts('mitochondria',65,35,r.offset); }
            },
        ...
        primordial: {
            id: 'evolution-primordial',
            title(){ return loc('evo_primordial_title'); },
            desc(){ return loc('evo_primordial_desc'); },
            reqs: { evo: 5, evo_primordial: 1 },
            grant: ['evo',7],
            condition(){
                let allowed = global.stats.achieve['living_extinction'] && global.stats.achieve['living_extinction'].l ? true : false;
                return allowed && genus_condition(5);
            },
            cost: {
                DNA(){ return 260; }
            },
            effect: loc('evo_primordial_effect'),
            action(args){
                if (payCosts(this)){
                    global.tech.evo_primordial = 2;
                    global.evolution['final'] = 100;
                    return true;
                }
                return false;
            ...
            queue_complete(){ return global.tech['evo'] && global.tech.evo === 6 ? 1 : 0; },
            emblem(){ return format_emblem('genus_primordial'); }
        },
        eggshell: {
    ...
    city: {
            ...
            action(args){
                if (!global.settings.pause){
                    if (global['resource']['Lumber'].amount < global['resource']['Lumber'].max){
                        modRes('Lumber',this.val(true),true);
                    }
        ...
        slave_market: buildTemplate(`slave_market`, 'city'),
        ...
        basic_housing: {
            ...
            cost: {
                Money(r={}){
                    r.offset = r.offset || 0;
                    if ((global.city['basic_housing'] ? global.city['basic_housing'].count : 0) + r.offset >= 5){
                        return costMultiplier('basic_housing', r.offset, 20, 1.17);
                    }
                    else {
                        return 0;
                    }
                },
                Lumber(r={}){ return global.race['kindling_kindred'] || global.race['smoldering'] ? 0 : costMultiplier('basic_housing', r.offset, 10, 1.23); },
                Stone(r={}){ return global.race['kindling_kindred'] ? costMultiplier('basic_housing', r.offset, 10, 1.23) : 0; },
                Chrysotile(r={}){ return global.race['smoldering'] ? costMultiplier('basic_housing', r.offset, 10, 1.23) : 0; },
        ...
        cottage: {
            ...
            desc(){
                return loc('city_cottage_desc',[this.citizens()]);
            },
            type: 'housing',
            category: 'residential',
            reqs: { housing: 2 },
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('cottage', r.offset, 900, 1.15); },
                Plywood(r={}){ return costMultiplier('cottage', r.offset, 25, 1.25); },
                Brick(r={}){ return costMultiplier('cottage', r.offset, 20, 1.25); },
                Wrought_Iron(r={}){ return costMultiplier('cottage', r.offset, 15, 1.25); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('cottage', r.offset, 5, 1.25) : 0; },
                Horseshoe(){ return global.race['hooved'] ? 2 : 0; }
            },
            effect(){
                let pop = this.citizens();
                let safe = spatialReasoning(this.storage.res('Money') * this.storage.multiplier());
        ...
        apartment: {
            ...
            desc(){
                return `<div>${loc('city_apartment_desc',[this.citizens()])}</div><div class="has-text-special">${loc('requires_power')}</div>`
            },
            type: 'housing',
            category: 'residential',
            reqs: { housing: 3 },
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('apartment', r.offset, 1750, 1.26) - 500; },
                Crystal(r={}){ return global.race.universe === 'magic' ? costMultiplier('apartment', r.offset, 25, 1.22) : 0; },
                Furs(r={}){ return costMultiplier('apartment', r.offset, 725, 1.32) - 500; },
                Copper(r={}){ return costMultiplier('apartment', r.offset, 650, 1.32) - 500; },
                Cement(r={}){ return costMultiplier('apartment', r.offset, 700, 1.32) - 500; },
                Steel(r={}){ return costMultiplier('apartment', r.offset, 800, 1.32) - 500; },
            ...
            effect(){
                ...
                if (global.tech['home_safe']){
                    let safe = spatialReasoning(this.storage.res('Money') * this.storage.multiplier());
                    return `<div>${loc('plus_max_citizens',[pop])}. <span class="has-text-caution">${loc('minus_power',[this.powered()])}</span></div><div>${loc('plus_max_resource',[`\$${safe.toLocaleString()}`,loc('resource_Money_name')])}</div>`;
                }
                else {
                    return `${loc('plus_max_citizens',[pop])}. <span class="has-text-caution">${loc('minus_power',[this.powered()])}</span>`;
                }
            },
            storage: {
                res(res){
        ...
        lodge: {
            ...
            cost: {
                Money(r={}){ return costMultiplier('lodge', r.offset, 50, 1.32); },
                Lumber(r={}){ return costMultiplier('lodge', r.offset, 20, 1.36); },
                Stone(r={}){ return costMultiplier('lodge', r.offset, 10, 1.36); },
                Horseshoe(){ return global.race['hooved'] ? 1 : 0; }
            },
            effect(){
                let pop = this.citizens();
                return global.race['carnivore'] && !global.race['artifical'] ? `<div>${loc('plus_max_resource',[pop,loc('citizen')])}</div><div>${loc('city_lodge_effect',[hugeEffect(5),global.resource.Food.name])}</div>` : loc('plus_max_resource',[pop,loc('citizen')]);
            },
            action(args){
                if (payCosts(this)){
        ...
        smokehouse: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('smokehouse', r.offset, 85, 1.32); },
                Lumber(r={}){ return costMultiplier('smokehouse', r.offset, 65, 1.36) },
                Stone(r={}){ return costMultiplier('smokehouse', r.offset, 50, 1.36); }
            },
            effect(){
                let food = spatialReasoning(this.storage.res('Food') * this.storage.multiplier());
                let reduce = (1 - (0.9 ** hugeAdjust(1))) * 100;
                return `<div>${loc('plus_max_resource',[food, global.resource.Food.name])}</div><div>${loc('city_smokehouse_effect',[+(reduce).toFixed(1)])}</div>`;
            },
            storage: {
                res(res){
                    let list = {
                ...
                multiplier(wiki){
                    return BHStorageMulti(1);
                }
            },
            action(args){
                if (payCosts(this)){
                    incrementStruct('smokehouse','city');
                    global['resource']['Food'].max += spatialReasoning(this.storage.res('Food') * this.storage.multiplier());
                    return true;
                }
                return false;
            },
        ...
        soul_well: {
            ...
            cost: {
                Money(r={}){
                    r.offset = r.offset || 0;
                    if ((global.city['soul_well'] ? global.city['soul_well'].count : 0) + r.offset >= 3){
                        return costMultiplier('soul_well', r.offset, 50, 1.32);
                    }
                    else {
                        return 0;
                    }
                },
                Lumber(r={}){ return costMultiplier('soul_well', r.offset, 20, 1.36); },
                Stone(r={}){ return costMultiplier('soul_well', r.offset, 10, 1.36); }
            },
            effect(){
                let souls = spatialReasoning(this.storage.res('Food') * this.storage.multiplier());
                let production = global.race['ghostly'] ? (2 + traits.ghostly.vars()[1]) : 2;
                return `<div>${loc('city_soul_well_effect',[hugeEffect(production)])}</div><div>${loc('plus_max_resource',[souls, loc('resource_Souls_name')])}</div>`;
            },
            storage: {
                res(res){
                    let list = {
        ...
        slave_pen: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('slave_pen', r.offset, 250, 1.32); },
                Lumber(r={}){ return costMultiplier('slave_pen', r.offset, 100, 1.36); },
                Stone(r={}){ return costMultiplier('slave_pen', r.offset, 75, 1.36); },
                Copper(r={}){ return costMultiplier('slave_pen', r.offset, 10, 1.36); },
                Nanite(r={}){ return global.race['deconstructor'] ? costMultiplier('slave_pen', r.offset, 4, 1.36) : 0; },
            },
            effect(){
                return `<div>${loc('plus_max_resource',[hugeEffect(4),global.resource.Slave.name])}</div>`;
            },
            action(args){
                if (payCosts(this)){
                    incrementStruct('slave_pen','city');
                    global.resource.Slave.display = true;
                    global.resource.Slave.max = Math.floor(hugeEffect(global.city.slave_pen.count * 4));
                    return true;
                }
                return false;
            },
        ...
        transmitter: {
            ...
            trait: ['artifical'],
            cost: {
                Money(r={}){ if (global.city['transmitter'] && global.city['transmitter'].count >= 3){ return costMultiplier('transmitter', r.offset, 50, 1.32);} else { return 0; } },
                Copper(r={}){ return costMultiplier('transmitter', r.offset, 20, 1.36); },
                Steel(r={}){ return costMultiplier('transmitter', r.offset, 10, 1.36); },
            },
            effect(){
                let signal = +(production('transmitter')).toFixed(2);
                let sig_cap = spatialReasoning(this.storage.res('Food') * this.storage.multiplier());
                return `<div>${loc('gain',[signal, global.resource.Food.name])}</div><div>${loc('city_transmitter_effect',[sig_cap])}</div><div class="has-text-caution">${loc('minus_power',[this.powered()])}</div>`;
            },
            storage: {
                res(res){
                    let list = {
        ...
        farm: {
            ...
            cost: {
                Money(r={}){
                    r.offset = r.offset || 0;
                    if ((global.city['farm'] ? global.city['farm'].count : 0) + r.offset >= 3){
                        return costMultiplier('farm', r.offset, 50, 1.32);
                    }
                    else {
                        return 0;
                    }
                },
                Lumber(r={}){ return costMultiplier('farm', r.offset, 20, 1.36); },
                Stone(r={}){ return costMultiplier('farm', r.offset, 10, 1.36); },
                Horseshoe(r={}){ return global.race['hooved'] && ((global.city['farm'] ? global.city['farm'].count : 0) + (r.offset || 0)) >= 2 ? 1 : 0; }
            },
            effect(){
                let pop = this.citizens();
        ...
        compost: {
            ...
            cost: {
                Money(r={}){
                    r.offset = r.offset || 0;
                    if ((global.city['compost'] ? global.city['compost'].count : 0) + r.offset >= 3){
                        return costMultiplier('compost', r.offset, 50, 1.32);
                    }
                    else {
                        return 0;
                    }
                },
                Lumber(r={}){ return costMultiplier('compost', r.offset, 12, 1.36); },
                Stone(r={}){ return costMultiplier('compost', r.offset, 12, 1.36); }
            },
            effect(){
                ...
                let store = spatialReasoning(this.storage.res('Food') * this.storage.multiplier());
                let wood = global.race['kindling_kindred'] || global.race['smoldering'] ? `` : `<div class="has-text-caution">${loc('city_compost_heap_effect2',[hugeEffect(0.5),global.resource.Lumber.name])}</div>`;
        ...
        mill: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('mill', r.offset, 1000, 1.31); },
                Lumber(r={}){ return costMultiplier('mill', r.offset, 600, 1.33); },
                Iron(r={}){ return costMultiplier('mill', r.offset, 150, 1.33); },
                Cement(r={}){ return costMultiplier('mill', r.offset, 125, 1.33); },
            },
            ...
            action(args){
                if (payCosts(this)){
                    ...
                    if (checkPowerRequirements(this)){
                        powerOnNewStruct(this);
                    }
                    return true;
                }
        ...
        windmill: {
            ...
            power_reqs: { false: 1 },
            cost: {
                Money(r={}){ return costMultiplier('windmill', r.offset, 1000, 1.31); },
                Lumber(r={}){ return costMultiplier('windmill', r.offset, 600, 1.33); },
                Iron(r={}){ return costMultiplier('windmill', r.offset, 150, 1.33); },
                Cement(r={}){ return costMultiplier('windmill', r.offset, 125, 1.33); },
            },
            effect(){
                let power = this.powered() * -1;
        ...
        silo: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('silo', r.offset, 85, 1.32); },
                Lumber(r={}){ return costMultiplier('silo', r.offset, 65, 1.36) },
                Stone(r={}){ return costMultiplier('silo', r.offset, 50, 1.36); },
                Iron(r={}){ return ((global.city.silo ? global.city.silo.count : 0) + (r.offset || 0)) >= 4 && global.city.ptrait.includes('unstable') ? costMultiplier('silo', r.offset, 10, 1.36) : 0; }
            },
            effect(){
                let food = spatialReasoning(this.storage.res('Food') * this.storage.multiplier());
        ...
        garrison: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('garrison', r.offset, 240, 1.5); },
                Stone(r={}){ return costMultiplier('garrison', r.offset, 260, 1.46); },
                Iron(r={}){ return ((global.city['garrison'] ? global.city.garrison.count : 0) + (r.offset || 0)) >= 4 && global.city.ptrait.includes('unstable') ? costMultiplier('garrison', r.offset, 50, 1.4) : 0; },
        ...
        hospital: {
            ...
            cost: {
                Money(r={}){ return costMultiplier('hospital', r.offset, 22000, 1.32); },
                Furs(r={}){ return costMultiplier('hospital', r.offset, 4000, 1.32); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('hospital', r.offset, 500, 1.32) : 0; },
                Aluminium(r={}){ return costMultiplier('hospital', r.offset, 10000, 1.32); },
            },
            effect(){
                ...
                let desc = `<div>${loc('city_hospital_effect',[hugeEffect(healing)])}</div>${clinic}`;
        ...
        boot_camp: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('boot_camp', r.offset, 50000, 1.32); },
                Lumber(r={}){ return costMultiplier('boot_camp', r.offset, 21500, 1.32); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('boot_camp', r.offset, 300, 1.32) : 0; },
                Aluminium(r={}){ return costMultiplier('boot_camp', r.offset, 12000, 1.32); },
                Brick(r={}){ return costMultiplier('boot_camp', r.offset, 1400, 1.32); },
            },
            effect(){
        ...
        shed: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('shed', r.offset, 75, 1.22); },
                Lumber(r={}){
                    if (global.tech['storage'] && global.tech['storage'] < 4){
                        return costMultiplier('shed', r.offset, 55, 1.32);
                    }
                    else {
                        return 0;
                    }
                ...
                Stone(r={}){
                    if (global.tech['storage'] && global.tech['storage'] < 3){
                        return costMultiplier('shed', r.offset, 45, 1.32);
                    }
                    else {
                        return 0;
                    }
                ...
                Iron(r={}){
                    if (global.tech['storage'] && global.tech['storage'] >= 4){
                        return costMultiplier('shed', r.offset, 22, 1.32);
                    }
                    else {
                        return 0;
                    }
                ...
                Cement(r={}){
                    if (global.tech['storage'] && global.tech['storage'] >= 3){
                        return costMultiplier('shed', r.offset, 18, 1.32);
                    }
                    else {
                        return 0;
                    }
        ...
        storage_yard: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('storage_yard', r.offset, 10, bananaPerk(1.36)); },
                Brick(r={}){ return costMultiplier('storage_yard', r.offset, 3, bananaPerk(1.35)); },
                Wrought_Iron(r={}){ return costMultiplier('storage_yard', r.offset, 5, bananaPerk(1.35)); }
            },
            effect(){
        ...
        warehouse: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('warehouse', r.offset, 400, bananaPerk(1.26)); },
                Cement(r={}){ return costMultiplier('warehouse', r.offset, 75, bananaPerk(1.26)); },
                Sheet_Metal(r={}){ return costMultiplier('warehouse', r.offset, 25, bananaPerk(1.25)); }
            },
            effect(){
        ...
        bank: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('bank', r.offset, traitCostMod('untrustworthy',250), 1.35); },
                Lumber(r={}){ return costMultiplier('bank', r.offset, traitCostMod('untrustworthy',75), 1.32); },
                Stone(r={}){ return costMultiplier('bank', r.offset, traitCostMod('untrustworthy',100), 1.35); },
                Iron(r={}){ return ((global.city['bank'] ? global.city.bank.count : 0) + (r.offset || 0)) >= 2 && global.city.ptrait.includes('unstable') ? costMultiplier('bank', r.offset, traitCostMod('untrustworthy',30), 1.3) : 0; }
            },
            effect(){
        ...
        pylon: {
            ...
            cost: {
                Money(r={}){
                    ...
                    if ((global.city['pylon'] ? global.city['pylon'].count : 0) + r.offset >= 2){
                        return costMultiplier('pylon', r.offset, 10, 1.48);
                    }
                    else {
                        return 0;
                    }
                },
                Stone(r={}){ return costMultiplier('pylon', r.offset, 12, 1.42); },
                Crystal(r={}){ return costMultiplier('pylon', r.offset, 8, 1.42) - 3; }
            },
            effect(){
                let max = spatialReasoning(this.storage.res('Mana') * this.storage.multiplier());
                let mana = 0.01 * darkEffect('magic');
                return `<div>${loc('gain',[hugeEffect(mana, 3),global.resource.Mana.name])}</div><div>${loc('plus_max_resource',[max,global.resource.Mana.name])}</div>`;
            },
            ...
            action(args){
                if (payCosts(this)){
                    ...
                    global.resource.Mana.max += this.storage.res('Mana') * this.storage.multiplier();
                    return true;
                }
                return false;
            },
        ...
        conceal_ward: {
            ...
            not_trait: ['cataclysm','orbit_decayed'],
            cost: {
                Money(r={}){return costMultiplier('conceal_ward', r.offset, 500, 1.25);  },
                Mana(r={}){ return costMultiplier('conceal_ward', r.offset, conceal_adjust(42), 1.25); },
                Crystal(r={}){ return costMultiplier('conceal_ward', r.offset, 5, 1.25); }
            },
            effect(){
                let ward = global.tech['roguemagic'] && global.tech.roguemagic >= 8 ? 1.25 : 1;
                return `<div>${loc('city_conceal_ward_effect',[hugeEffect(ward, 2)])}</div>`;
            },
            action(args){
                if (payCosts(this)){
        ...
        graveyard: {
            ...
            cost: {
                Money(r={}){
                    r.offset = r.offset || 0;
                    if ((global.city['graveyard'] ? global.city['graveyard'].count : 0) + r.offset >= 5){
                        return costMultiplier('graveyard', r.offset, 5, 1.85);
                    }
                    else {
                        return 0;
                    }
                },
                Lumber(r={}){ return costMultiplier('graveyard', r.offset, 2, 1.95); },
                Stone(r={}){ return costMultiplier('graveyard', r.offset, 6, 1.9); }
            },
            effect(){
                let lum = spatialReasoning(this.storage.res('Lumber') * this.storage.multiplier());
                return `<div>${loc('city_graveyard_effect',[hugeEffect(8)])}</div><div>${loc('plus_max_resource',[lum,global.resource.Lumber.name])}</div>`;
            },
            storage: {
                res(res){
                    let list = {
            ...
            action(args){
                if (payCosts(this)){
                    ...
                    global['resource']['Lumber'].max += spatialReasoning(this.storage.res('Lumber') * this.storage.multiplier());
                    return true;
                }
                return false;
            },
        ...
        lumber_yard: {
            ...
            cost: {
                Money(r={}){
                    r.offset = r.offset || 0;
                    if ((global.city['lumber_yard'] ? global.city['lumber_yard'].count : 0) + r.offset >= 5){
                        return costMultiplier('lumber_yard', r.offset, 5, 1.85);
                    }
                    else {
                        return 0;
                    }
                },
                Lumber(r={}){ return costMultiplier('lumber_yard', r.offset, 6, 1.9); },
                Stone(r={}){ return costMultiplier('lumber_yard', r.offset, 2, 1.95); }
            },
            effect(){
                let lum = spatialReasoning(this.storage.res('Lumber') * this.storage.multiplier());
                return `<div>${loc('production',[hugeEffect(2),global.resource.Lumber.name])}</div><div>${loc('plus_max_resource',[lum,global.resource.Lumber.name])}</div>`;
            },
            storage: {
                res(res){
                    let list = {
        ...
        sawmill: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('sawmill', r.offset, 3000, 1.26); },
                Iron(r={}){ return costMultiplier('sawmill', r.offset, 400, 1.26); },
                Cement(r={}){ return costMultiplier('sawmill', r.offset, 420, 1.26); }
            },
            effect(){
                let impact = global.tech['saw'] >= 2 ? 8 : 5;
                let lum = spatialReasoning(this.storage.res('Lumber') * this.storage.multiplier());
                let desc = `<div>${loc('plus_max_resource',[lum,global.resource.Lumber.name])}</div><div>${loc('production',[hugeEffect(impact),global.resource.Lumber.name])}</div>`;
                if (global.tech['foundry'] && global.tech['foundry'] >= 4){
                    desc = desc + `<div>${loc('crafting',[hugeEffect(2),global.resource.Plywood.name])}</div>`;
                }
                if (global.city.powered){
                    desc = desc + `<div class="has-text-caution">${loc('city_sawmill_effect3',[hugeEffect(4),+(this.powered()).toFixed(2)])}</div>`;
                }
                return desc;
            },
        ...
        rock_quarry: {
            ...
            cost: {
                Money(r={}){
                    r.offset = r.offset || 0;
                    if ((global.city['rock_quarry'] ? global.city['rock_quarry'].count : 0) + r.offset >= 2){
                        return costMultiplier('rock_quarry', r.offset, 20, 1.45);
                    }
                    else {
                        return 0;
                    }
                },
                Lumber(r={}){ return costMultiplier('rock_quarry', r.offset, 50, 1.36); },
                Stone(r={}){ return costMultiplier('rock_quarry', r.offset, 10, 1.36); }
            },
            effect(){
                let stone = spatialReasoning(this.storage.res('Stone') * this.storage.multiplier());
                let asbestos = global.race['smoldering'] ? `<div>${loc('plus_max_resource',[spatialReasoning(this.storage.res('Chrysotile') * this.storage.multiplier()),global.resource.Chrysotile.name])}</div>` : '';
                if (global.tech['mine_conveyor']){
                    return `<div>${loc('city_rock_quarry_effect1',[hugeEffect(2)])}</div><div>${loc('plus_max_resource',[stone,global.resource.Stone.name])}</div>${asbestos}<div class="has-text-caution">${loc('city_rock_quarry_effect2',[hugeEffect(4),this.powered()])}</div>`;
                }
                else {
                    return `<div>${loc('city_rock_quarry_effect1',[hugeEffect(2)])}</div><div>${loc('plus_max_resource',[stone,global.resource.Stone.name])}</div>${asbestos}`;
                }
            },
        ...
        cement_plant: {
            ...
            cost: {
                Money(r={}){ return costMultiplier('cement_plant', r.offset, 3000, 1.5); },
                Lumber(r={}){ return costMultiplier('cement_plant', r.offset, 1800, 1.36); },
                Stone(r={}){ return costMultiplier('cement_plant', r.offset, 2000, 1.32); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('cement_plant', r.offset, 275, 1.32) : 0; }
            },
            effect(){
                if (global.tech['cement'] >= 5){
                    let screws = global.tech['cement'] >= 6 ? 8 : 5;
                    return `<div>${loc('plus_max_resource',[jobScale(2),loc(`job_cement_worker`,[resName('Cement')])])}</div><div class="has-text-caution">${loc('city_cement_plant_effect2',[this.powered(),hugeEffect(screws),resName('Cement')])}</div>`;
                }
                else {
            ...
            action(args){
                if (payCosts(this)){
                    ...
                    global.civic.cement_worker.max += jobScale(2);
                    powerOnNewStruct(this);
                    return true;
                }
                return false;
        ...
        foundry: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('foundry', r.offset, 750, 1.36); },
                Stone(r={}){ return costMultiplier('foundry', r.offset, 100, 1.36); },
                Copper(r={}){ return costMultiplier('foundry', r.offset, 250, 1.36); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('foundry', r.offset, 40, 1.36) : 0; },
            },
            effect(){
                ...
                if (global.tech['foundry'] >= 2){
                    ...
                    desc = desc + `<div>${loc('city_crafted_mats',[hugeEffect(skill)])}</div>`;
                }
                if (global.tech['foundry'] >= 6){
                    desc = desc + `<div>${loc('city_foundry_effect2',[hugeEffect(2),global.resource.Brick.name])}</div>`;
                }
                return desc;
            },
            action(args){
        ...
        factory: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('factory', r.offset, 25000, dirt_adjust(1.32)); },
                Cement(r={}){ return costMultiplier('factory', r.offset, 1000, dirt_adjust(1.32)); },
                Steel(r={}){ return costMultiplier('factory', r.offset, 7500, dirt_adjust(1.32)); },
                Titanium(r={}){ return costMultiplier('factory', r.offset, 2500, dirt_adjust(1.32)); }
            },
            effect(){
                let desc = `<div>${loc('city_factory_effect')}</div><div class="has-text-caution">${loc('minus_power',[this.powered()])}</div>`;
                if (global.tech['foundry'] >= 7){
                    desc = desc + `<div>${loc('city_crafted_mats',[hugeEffect(5)])}</div>`;
                }
                return desc;
            },
        ...
        nanite_factory: {
            ...buildTemplate(`nanite_factory`,'city'),
            storage: {
                res(res){
                    let list = {
                        'Nanite': 2500
        ...
        smelter: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('smelter', r.offset, 1000, dirt_adjust(1.32)); },
                Iron(r={}){ return costMultiplier('smelter', r.offset, 500, dirt_adjust(1.33)); }
            },
            effect(){
        ...
        metal_refinery: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('metal_refinery', r.offset, 2500, 1.35); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('metal_refinery', r.offset, 125, 1.35) : 0; },
                Steel(r={}){ return costMultiplier('metal_refinery', r.offset, 350, 1.35); }
            },
            powered(){ return powerCostMod(2); },
            powerBalancer(){
        ...
        mine: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('mine', r.offset, 60, dirt_adjust(1.6)); },
                Lumber(r={}){ return costMultiplier('mine', r.offset, 175, dirt_adjust(1.38)); }
            },
            effect(){
                if (global.tech['mine_conveyor']){
                    return `<div>${loc('plus_max_resource',[jobScale(1),loc(`job_miner`)])}</div><div class="has-text-caution">${loc('city_mine_effect2',[this.powered(),hugeEffect(5)])}</div>`;
                }
                else {
        ...
        coal_mine: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('coal_mine', r.offset, 480, dirt_adjust(1.4)); },
                Lumber(r={}){ return costMultiplier('coal_mine', r.offset, 250, dirt_adjust(1.36)); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('coal_mine', r.offset, 28, dirt_adjust(1.36)) : 0; },
                Wrought_Iron(r={}){ return costMultiplier('coal_mine', r.offset, 18, dirt_adjust(1.36)); }
            },
            effect(){
                if (global.tech['mine_conveyor']){
                    return `<div>${loc('plus_max_resource',[jobScale(1),loc(`job_coal_miner`)])}</div><div class="has-text-caution">${loc('city_coal_mine_effect2',[this.powered(),hugeEffect(5)])}</div>`;
                }
                else {
            ...
            action(args){
                if (payCosts(this)){
                    ...
                    global.civic.coal_miner.max += jobScale(1);
                    powerOnNewStruct(this);
                    return true;
                }
                return false;
        ...
        oil_well: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('oil_well', r.offset, 5000, dirt_adjust(1.5)); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('oil_well', r.offset, 450, dirt_adjust(1.5)) : 0; },
                Cement(r={}){ return costMultiplier('oil_well', r.offset, 5250, dirt_adjust(1.5)); },
                Steel(r={}){ return costMultiplier('oil_well', r.offset, 6000, dirt_adjust(1.5)); }
            },
            effect(){
            ...
            action(args){
                if (payCosts(this)){
                    ...
                    global['resource']['Oil'].max += spatialReasoning(this.storage.res('Oil') * this.storage.multiplier());
        ...
        oil_depot: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('oil_depot', r.offset, 2500, dirt_adjust(1.46)); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('oil_depot', r.offset, 325, dirt_adjust(1.36)) : 0; },
                Cement(r={}){ return costMultiplier('oil_depot', r.offset, 3750, dirt_adjust(1.46)); },
                Sheet_Metal(r={}){ return costMultiplier('oil_depot', r.offset, 100, dirt_adjust(1.45)); }
            },
            effect() {
                let mult = this.storage.multiplier();
                let list = this.storage.res();
                let oil = spatialReasoning(list['Oil'] * mult);
                let effect = `<div>${loc('plus_max_resource',[oil,global.resource.Oil.name])}.</div>`;
                if (global.resource['Helium_3'].display){
                    let val = spatialReasoning(list['Helium_3'] * mult);
                    effect = effect + `<div>${loc('plus_max_resource',[val,global.resource.Helium_3.name])}.</div>`;
                }
                if (global.tech['uranium'] >= 2){
                    let val = spatialReasoning(list['Uranium'] * mult);
            ...
            storage: {
                ...
                multiplier(wiki){
                    return global.tech['world_control'] ? 1.5 : 1;
                }
            },
            action(args){
                if (payCosts(this)){
        ...
        trade: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('trade', r.offset, 500, 1.36); },
                Lumber(r={}){ return costMultiplier('trade', r.offset, 125, 1.36); },
                Stone(r={}){ return costMultiplier('trade', r.offset, 50, 1.36); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('trade', r.offset, 15, 1.36) : 0; },
                Furs(r={}){ return costMultiplier('trade', r.offset, 65, 1.36); }
            },
            effect(){
                return loc('city_trade_effect',[+(this.routes()).toFixed(1)]);
            },
            routes(){
        ...
        wharf: {
            ...
            cost: {
                Money(r={}){ return costMultiplier('wharf', r.offset, 62000, 1.32); },
                Lumber(r={}){ return costMultiplier('wharf', r.offset, 44000, 1.32); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('wharf', r.offset, 200, 1.32) : 0; },
                Cement(r={}){ return costMultiplier('wharf', r.offset, 3000, 1.32); },
                Oil(r={}){ return costMultiplier('wharf', r.offset, 750, 1.32); }
            },
            effect(){
            ...
            action(args){
                if (payCosts(this)){
                    ...
                    global.city.market.mtrade += hugeAdjust(2);
        ...
        tourist_center: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('tourist_center', r.offset, 100000, 1.36); },
                Stone(r={}){ return costMultiplier('tourist_center', r.offset, 25000, 1.36); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('tourist_center', r.offset, 1000, 1.36) : 0; },
                Furs(r={}){ return costMultiplier('tourist_center', r.offset, 7500, 1.36); },
                Plywood(r={}){ return costMultiplier('tourist_center', r.offset, 5000, 1.36); },
            },
            effect(wiki){
                ...
                let desc = `<div class="has-text-caution">${loc('city_tourist_center_effect1',[hugeEffect(50, 2), global.resource.Food.name])}</div>`;
                desc += `<div>${loc('city_tourist_center_effect2',[hugeEffect(amp,2,2),actions.city.amphitheatre.title()])}</div>`;
                desc += `<div>${loc('city_tourist_center_effect2',[hugeEffect(cas,2,2),structName('casino')])}</div>`;
                desc += `<div>${loc('city_tourist_center_effect2',[hugeEffect(mon,2),loc(`arpa_project_monument_title`)])}</div>`;
                if (global.stats.achieve['banana'] && global.stats.achieve.banana.l >= 4){
                    desc += `<div>${loc(`city_tourist_center_effect2`,[hugeEffect(trd,2,2), loc('city_trade')])}</div>`;
                }
                let piousVal = govActive('pious',1);
                if (piousVal){
                    piousVal *= modifier;
                    desc += `<div>${loc(`city_tourist_center_effect2`,[hugeEffect(piousVal,2,2), structName('temple')])}</div>`;
                }
        ...
        amphitheatre: {
            ...
            cost: {
                Money(r={}){ return costMultiplier('amphitheatre', r.offset, 500, 1.55); },
                Lumber(r={}){ return costMultiplier('amphitheatre', r.offset, 50, 1.75); },
                Stone(r={}){ return costMultiplier('amphitheatre', r.offset, 200, 1.75); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('amphitheatre', r.offset, 18, 1.36) : 0; },
            },
            effect(){
                let athVal1 = govActive('athleticism',0);
                let athVal2 = govActive('athleticism',1);
                return`<div>${loc('plus_max_resource',[jobScale(athVal2 ? athVal2 : 1),loc(`job_entertainer`)])}</div><div>${loc('city_max_morale',[hugeEffect(athVal1 ? athVal1 : 1)])}</div>`;
            },
            action(args){
                if (payCosts(this)){
        ...
        casino: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('casino', r.offset, traitCostMod('untrustworthy',350000), 1.35); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('casino', r.offset, traitCostMod('untrustworthy',2000), 1.35) : 0; },
                Furs(r={}){ return costMultiplier('casino', r.offset, traitCostMod('untrustworthy',60000), 1.35); },
                Plywood(r={}){ return costMultiplier('casino', r.offset, traitCostMod('untrustworthy',10000), 1.35); },
                Brick(r={}){ return costMultiplier('casino', r.offset, traitCostMod('untrustworthy',6000), 1.35); }
            },
            effect(){
            ...
            storage: {
                ...
                count(){
                    let casinos = global.city.casino?.count || 0;
                    casinos += global.space.spc_casino?.count || 0;
                    casinos += global.tauceti.tauceti_casino?.count || 0;
                    casinos += global.portal.hell_casino?.count || 0;
                    casinos += global.underground.under_casino?.count || 0;
                    return casinos;
                }
            },
            action(args){
        ...
        temple: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('temple', r.offset, 50, 1.36); },
                Lumber(r={}){ return costMultiplier('temple', r.offset, 25, 1.36); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('temple', r.offset, 6, 1.36) : 0; },
                Furs(r={}){ return costMultiplier('temple', r.offset, 15, 1.36); },
                Cement(r={}){ return costMultiplier('temple', r.offset, 10, 1.36); }
            },
            effect(){
        ...
        university: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('university', r.offset, 900, 1.5) - 500; },
                Lumber(r={}){ return costMultiplier('university', r.offset, 500, 1.36) - 200; },
                Stone(r={}){ return costMultiplier('university', r.offset, 750, 1.36) - 350; },
                Crystal(r={}){ return global.race.universe === 'magic' ? costMultiplier('university', r.offset, 5, 1.36) : 0; },
                Iron(r={}){ return ((global.city['university'] ? global.city.university.count : 0) + (r.offset || 0)) >= 3 && global.city.ptrait.includes('unstable') ? costMultiplier('university', r.offset, 25, 1.36) : 0; }
            },
            effect(wiki){
            ...
            knowVal(wiki){
                ...
                if (global.tech['science'] >= 4 && global.city.library){
                    multiplier += hugeAdjust(global.city.library.count) * 0.02;
                }
                if (global.space['observatory'] && global.space.observatory.count > 0){
                    multiplier += hugeAdjust(wiki ? global.space.observatory.on : support_on['observatory']) * 0.05;
                }
                if (global.portal['sensor_drone'] && global.tech['science'] >= 14){
                    multiplier += hugeAdjust(wiki ? global.portal.sensor_drone.on : p_on['sensor_drone']) * 0.02;
                }
                ...
                let num_tech_scavs_on = sg_on ? hugeAdjust(wiki ? (global.galaxy?.scavenger?.on ?? 0) : gal_on['scavenger']) : 0;
                ...
                if (decayPerks()){
                    if (global.space['satellite']){
                        gain *= 1 + hugeAdjust(global.space.satellite.count * 0.12);
                    }
        ...
        library: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('library', r.offset, 45, 1.2); },
                Crystal(r={}){ return global.race.universe === 'magic' ? costMultiplier('library', r.offset, 2, 1.2) : 0; },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('library', r.offset, 4, 1.2) : 0; },
                Furs(r={}){ return costMultiplier('library', r.offset, 22, 1.2); },
                Plywood(r={}){ return costMultiplier('library', r.offset, 20, 1.2); },
            ...
            effect(){
                ...
                return `<div>${loc('city_max_knowledge',[gain.toLocaleString()])}</div><div>${loc('city_library_effect',[hugeEffect(know)])}</div>`;
            },
            knowVal(){
                let gain = 125;
                ...
                if (global.tech['anthropology'] && global.tech['anthropology'] >= 2){
                    gain *= 1 + hugeAdjust(faithTempleCount() * 0.05);
                }
        ...
        wardenclyffe: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('wardenclyffe', r.offset, 5000, 1.22); },
                Knowledge(r={}){ return costMultiplier('wardenclyffe', r.offset, global.race['logical'] ? (1000 - traits.logical.vars()[0]) : 1000, 1.22); },
                Crystal(r={}){ return global.race.universe === 'magic' ? costMultiplier('wardenclyffe', r.offset, 100, 1.22) : 0; },
                Copper(r={}){ return costMultiplier('wardenclyffe', r.offset, 500, 1.22); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('wardenclyffe', r.offset, 75, 1.22) : 0; },
                Cement(r={}){ return costMultiplier('wardenclyffe', r.offset, 350, 1.22); },
                Sheet_Metal(r={}){ return costMultiplier('wardenclyffe', r.offset, 125, 1.2); },
            ...
            effect(){
                ...
                if (global.city.powered){
                    let pgain = +(this.knowVal(true)).toFixed(0);
                    if (global.tech.science >= 15){
                        desc = desc + `<div>${loc('city_wardenclyffe_effect4',[hugeEffect(2)])}</div>`;
                    }
                    if (global.race.universe === 'magic'){
                        let mana = spatialReasoning(this.storage.res('Mana') * this.storage.multiplier());
                    ...
                    if (global.tech['broadcast']){
                        ...
                        desc = desc + `<div class="has-text-caution">${loc('city_wardenclyffe_effect3',[this.powered(),pgain.toLocaleString(),hugeEffect(morale)])}</div>`
                    }
                    else {
                        desc = desc + `<div class="has-text-caution">${loc('city_wardenclyffe_effect2',[this.powered(),pgain.toLocaleString()])}</div>`;
                    }
                    if (global.race['artifical']){
                        let signal = spatialReasoning(this.storage.res('Food') * this.storage.multiplier());
                        desc = desc + `<div class="has-text-caution">${loc('city_transmitter_effect',[signal])}</div`;
                    }
                }
                return desc;
            },
            knowVal(powered){
                ...
                if (global.space['satellite']){
                    gain *= 1 + hugeAdjust(global.space.satellite.count * 0.04);
                }
                let athVal = govActive('athleticism',2);
                if (athVal){
                    gain *= 1 - (athVal / 100);
            ...
            storage: {
                ...
                multiplier(wiki){
                    return infiltratorFactor('city','wardenclyffe');
                },
                power_ratio(wiki){
                    return (p_on['wardenclyffe'] || 0) / (global.city.wardenclyffe?.count || 1) * this.multiplier(wiki);
                },
                mtype(res){
                    return res === 'Food' ? 'power_ratio' : 'multiplier';
                }
            },
            action(args){
                if (payCosts(this)){
        ...
        biolab: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('biolab', r.offset, 25000, 1.3); },
                Knowledge(r={}){ return costMultiplier('biolab', r.offset, 5000, 1.3); },
                Copper(r={}){ return costMultiplier('biolab', r.offset, 1250, 1.3); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('biolab', r.offset, 160, 1.3) : 0; },
            ...
            knowVal(wiki){
                let gain = 3000;
                if (global.portal['sensor_drone'] && global.tech['science'] >= 14){
                    gain *= 1 + hugeAdjust(wiki ? global.portal.sensor_drone.on : p_on['sensor_drone']) * 0.02;
                }
        ...
        coal_power: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('coal_power', r.offset, 10000, dirt_adjust(1.22)); },
                Crystal(r={}){ return global.race.universe === 'magic' ? costMultiplier('coal_power', r.offset, 125, dirt_adjust(1.22)) : 0; },
                Copper(r={}){ return costMultiplier('coal_power', r.offset, 1800, dirt_adjust(1.22)) - 1000; },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('coal_power', r.offset, 175, dirt_adjust(1.22)) : 0; },
                Cement(r={}){ return costMultiplier('coal_power', r.offset, 600, dirt_adjust(1.22)); },
                Steel(r={}){ return costMultiplier('coal_power', r.offset, 2000, dirt_adjust(1.22)) - 1000; }
            },
            effect(){
                let power = -(this.powered());
                return global.race['environmentalist'] ? `+${power}MW` : `<span>+${power}MW.</span> <span class="has-text-caution">${loc(global.race.universe === 'magic' ? 'city_mana_engine_effect' : 'city_coal_power_effect',[+(this.p_fuel().a).toFixed(2)])}</span>`;
            },
            powered(wiki){
            ...
            p_fuel(){
                if (global.race.universe === 'magic'){
                    return { r: 'Mana', a: global.race['environmentalist'] ? 0 : hugeAdjust(0.05) };
                }
                else {
                    return { r: 'Coal', a: global.race['environmentalist'] ? 0 : hugeAdjust(0.35) };
                }
            },
            action(args){
                if (payCosts(this)){
        ...
        oil_power: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('oil_power', r.offset, 50000, dirt_adjust(1.22)); },
                Copper(r={}){ return costMultiplier('oil_power', r.offset, 6500, dirt_adjust(1.22)) + 1000; },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('oil_power', r.offset, 180, dirt_adjust(1.22)) : 0; },
                Aluminium(r={}){ return costMultiplier('oil_power', r.offset, 12000, dirt_adjust(1.22)); },
                Cement(r={}){ return costMultiplier('oil_power', r.offset, 5600, dirt_adjust(1.22)) + 1000; }
            },
            effect(){
                let power = -(this.powered());
                return global.race['environmentalist'] ? `+${power}MW` : `<span>+${power}MW.</span> <span class="has-text-caution">${loc('city_oil_power_effect',[+(this.p_fuel().a).toFixed(2)])}</span>`;
            },
            powered(wiki){
                let power = 0;
            ...
            p_fuel(){ return { r: 'Oil', a: global.race['environmentalist'] ? 0 : hugeAdjust(0.65) }; },
            action(args){
                if (payCosts(this)){
        ...
        fission_power: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('fission_power', r.offset, 250000, 1.36); },
                Copper(r={}){ return costMultiplier('fission_power', r.offset, 13500, 1.36); },
                Iron(r={}){ return global.city.ptrait.includes('unstable') ? costMultiplier('fission_power', r.offset, 1750, 1.36) : 0; },
                Cement(r={}){ return costMultiplier('fission_power', r.offset, 10800, 1.36); },
                Titanium(r={}){ return costMultiplier('fission_power', r.offset, 7500, 1.36); }
            },
            effect(){
                return `<span>+${-(this.powered())}MW.</span> <span class="has-text-caution">${loc('city_fission_power_effect',[+(this.p_fuel().a).toFixed(2)])}</span>`;
            },
        ...
        mass_driver: {
            ...
            not_trait: ['cataclysm','lone_survivor'],
            cost: {
                Money(r={}){ return costMultiplier('mass_driver', r.offset, 375000, 1.32); },
                Copper(r={}){ return costMultiplier('mass_driver', r.offset, 33000, 1.32); },
                Iron(r={}){ return costMultiplier('mass_driver', r.offset, 42500, 1.32); },
                Iridium(r={}){ return costMultiplier('mass_driver', r.offset, 2200, 1.32); }
            },
            effect(){
                let exo = global.tech.mass >= 2 ? `<div>${loc('city_mass_driver_effect2',[hugeEffect(1, 1, 3),job_data.scientist.name()])}</div>` : '';
                let effect = (1 - (global.race['truepath'] ? 0.94 : 0.95) ** hugeAdjust(1)) * 100;
                return `${exo}<span>${loc('city_mass_driver_effect',[+(effect).toFixed(1),flib('name')])}</span> <span class="has-text-caution">${loc('minus_power',[this.powered()])}</span>`;
            },
            powered(){
        ...
        giant_thrusters: {
            ...
            cost: {
                Money(r={}){ return ((r.offset || 0) + (global.city.hasOwnProperty('giant_thrusters') ? global.city.giant_thrusters.count : 0)) < 100 ? 380000000 : 0; },
                Steel(r={}){ return ((r.offset || 0) + (global.city.hasOwnProperty('giant_thrusters') ? global.city.giant_thrusters.count : 0)) < 100 ? 7000000 : 0; },
                Mythril(r={}){ return ((r.offset || 0) + (global.city.hasOwnProperty('giant_thrusters') ? global.city.giant_thrusters.count : 0)) < 100 ? 400000 : 0; },
                Infernite(r={}){ return !global.race['truepath'] && ((r.offset || 0) + (global.city.hasOwnProperty('giant_thrusters') ? global.city.giant_thrusters.count : 0)) < 100 ? 1200000 : 0; },
                Unobtainium(r={}){ return global.race['truepath'] && ((r.offset || 0) + (global.city.hasOwnProperty('giant_thrusters') ? global.city.giant_thrusters.count : 0)) < 100 ? 20000 : 0; }
            },
            effect(wiki){
                let effectText = '';
        ...
        thruster_fuel: {
            ...
            cost: {
                Oil(r={}){ return ((r.offset || 0) + (global.city.hasOwnProperty('thruster_fuel') ? global.city.thruster_fuel.count : 0)) < 500 ? 6000000 : 0; },
                Helium_3(r={}){ return ((r.offset || 0) + (global.city.hasOwnProperty('thruster_fuel') ? global.city.thruster_fuel.count : 0)) < 500 ? 3000000 : 0; },
                Deuterium(r={}){ return !global.race['truepath'] && ((r.offset || 0) + (global.city.hasOwnProperty('thruster_fuel') ? global.city.thruster_fuel.count : 0)) < 500 ? 2000000 : 0; }
            },
            effect(wiki){
                let effectText = '';
    ...
    starDock: {
        probes: {
            ...
            reqs: { genesis: 4 },
            cost: {
                Money(r={}){ return costMultiplier('probes', r.offset, 350000, global.race['truepath'] ? 1.125 : 1.25,'starDock'); },
                Alloy(r={}){ return costMultiplier('probes', r.offset, 75000, global.race['truepath'] ? 1.125 : 1.25,'starDock'); },
                Polymer(r={}){ return costMultiplier('probes', r.offset, 85000, global.race['truepath'] ? 1.125 : 1.25,'starDock'); },
                Iridium(r={}){ return costMultiplier('probes', r.offset, 12000, global.race['truepath'] ? 1.125 : 1.25,'starDock'); },
                Mythril(r={}){ return costMultiplier('probes', r.offset, 3500, global.race['truepath'] ? 1.125 : 1.25,'starDock'); },
            },
            effect(){
        ...
        geck: {
            ...
            no_multi: true,
            cost: {
                Money(r={}){ return costMultiplier('geck', r.offset, 1000000, 1.25,'starDock'); },
                Elerium(r={}){ return costMultiplier('geck', r.offset, 1000, 1.25,'starDock'); },
                Plasmid(r={}){ return costMultiplier('geck', r.offset, 25, 1.4,'starDock'); },
            },
            effect(){
        ...
        seeder: {
            ...
            cost: {
                Money(r={}){ return ((r.offset || 0) + (global.starDock.hasOwnProperty('seeder') ? global.starDock.seeder.count : 0)) < 100 ? 100000 : 0; },
                Steel(r={}){ return ((r.offset || 0) + (global.starDock.hasOwnProperty('seeder') ? global.starDock.seeder.count : 0)) < 100 ? 25000 : 0; },
                Neutronium(r={}){ return ((r.offset || 0) + (global.starDock.hasOwnProperty('seeder') ? global.starDock.seeder.count : 0)) < 100 ? 240 : 0; },
                Elerium(r={}){ return ((r.offset || 0) + (global.starDock.hasOwnProperty('seeder') ? global.starDock.seeder.count : 0)) < 100 ? 10 : 0; },
                Nano_Tube(r={}){ return ((r.offset || 0) + (global.starDock.hasOwnProperty('seeder') ? global.starDock.seeder.count : 0)) < 100 ? 12000 : 0; },
            },
            effect(wiki){
            ...
            action(args){
                if (global.starDock.seeder.count < 100 && payCosts(this)){
        ...
        prep_ship: {
            ...
            queue_complete(){ return 0; },
            cost: {
                Helium_3(r={}){ return +fuel_adjust(global.race['gravity_well'] ? 150000 : 75000,false,r.wiki).toFixed(0); }
            },
            effect(){
...
export function setChallengeScreen(){
    ...
    if(global.stats.achieve['gone'] || global['sim']){
        global.evolution['iceage'] = { count:0 };
    }
    challengeGeneHeader();
    if (global.race['truepath'] || global.race['lone_survivor']){
    ...
    switch (key){
                ...
                action(args){
                    if (!global.settings.pause){
                        if (global['resource']['Stone'].amount < global['resource']['Stone'].max){
                            modRes('Stone',this.val(true),true);
                        }
                        global.stats.cstone++;
                        global.stats.tstone++;
                    }
        ...
        case 'food':
        {
            let action = {
                ...
                condition(){
                    ...
                    return global.race['soul_eater'] && !global.race['iceage'] ? false : true;
                },
                queue_complete(){ return 0; },
                cost: {
                ...
                action(args){
                    if (!global.settings.pause){
                        if(global['resource']['Food'].amount < global['resource']['Food'].max && !global.race['fasting']){
                            modRes('Food',this.val(true),true);
                        }
                        global.stats.cfood++;
                        global.stats.tfood++;
                    }
        ...
        case 'slave_market':
        {
            let action = {
                ...
                queue_complete(){ return global.city['slave_pen'] ? Math.floor(hugeAdjust(global.city.slave_pen.count * 4)) - global.resource.Slave.amount : 0; },
                action(args){
                    if (global.city['slave_pen'] && Math.floor(hugeAdjust(global.city.slave_pen.count * 4)) > global.resource.Slave.amount){
                        if (payCosts(this)){
        ...
        case 'firework':
        {
            let action = {
                ...
                action(args){
                    if (global[region].firework.count === 0 && payCosts(this)){
        ...
        case 'assembly':
        {
            ...
            let action = {
                ...
                cost: {
                    Money(r={}){ return global['resource'][global.race.species].amount ? costMultiplier('citizen', r.offset, assemblyCostAdjust(125), 1.01) : 0; },
                    Copper(r={}){ return global.race['deconstructor'] ? 0 : global['resource'][global.race.species].amount >= 5 ? costMultiplier('citizen', r.offset, assemblyCostAdjust(50), 1.01) : 0; },
                    Iron(r={}){ return global.race['deconstructor'] || !global.race['iceage'] ? 0 : global['resource'][global.race.species].amount >= 5 ? costMultiplier('citizen', r.offset, assemblyCostAdjust(50), 1.01) : 0},
                    Aluminium(r={}){ return global.race['deconstructor'] || global.race['iceage'] ? 0 : global['resource'][global.race.species].amount >= 5 ? costMultiplier('citizen', r.offset, assemblyCostAdjust(50), 1.01) : 0; },
                    Nanite(r={}){ return global.race['deconstructor'] ? (global['resource'][global.race.species].amount >= 3 ? costMultiplier('citizen', r.offset, assemblyCostAdjust(500), 1.01) : 0) : 0; },
                },
                effect(){
                    let warn = '';
                ...
                action(args){
                    ...
                    else if (global['resource'][global.race.species].max > global['resource'][global.race.species].amount && payCosts(this)){
        ...
        case 'nanite_factory':
        {
            let action = {
                ...
                region: 'city',
                cost: {
                    Money(r={}){ return costMultiplier('nanite_factory', r.offset, 25000, dirt_adjust(1.25)); },
                    Copper(r={}){ return costMultiplier('nanite_factory', r.offset, 1200, dirt_adjust(1.25)); },
        ...
        case 'captive_housing':
        {
            let action = {
                ...
                region: 'city',
                cost: {
                    Money(r={}){ return costMultiplier('captive_housing', r.offset, 40, 1.35); },
                    Lumber(r={}){ return costMultiplier('captive_housing', r.offset, 30, 1.35); },
                    Stone(r={}){ return costMultiplier('captive_housing', r.offset, 18, 1.35); },
                },
                effect(){
                    let desc = ``;
                    if (!global.race['artifical'] && !global.race['detritivore'] && !global.race['carnivore'] && !global.race['soul_eater']){
                        ...
                        desc += `<div>${loc(`city_captive_housing_cattle`,[Math.floor(cattle),Math.floor(cattleCap)])}</div>`;
                    }
                    ...
                    desc += `<div>${loc(`city_captive_housing_capacity`,[usedCap,Math.floor(raceCap)])}</div>`;
        ...
        case 'horseshoe':
        {
            let action = {
                ...
                cost: {
                    Lumber(r={}){
                        let shoes = (global.race['shoecnt'] || 0) + (r.offset || 0);
                        let active = !global.race['kindling_kindred'] && !global.race['smoldering'] && !global.race['iceage']
                    ...
                    Copper(r={}){
                        let shoes = (global.race['shoecnt'] || 0) + (r.offset || 0);
                        let lum = (global.race['kindling_kindred'] || global.race['smoldering'] || global.race['iceage']) ? false : true;
                ...
                action(args){
                    ...
                    if (global.resource.Horseshoe.display && payCosts(this)){
        ...
        case 's_alter':   
        {
            let action = {
                ...
                region: 'city',
                cost: {
                    Stone(r={}){ return ((r.offset || 0) + (global.city.hasOwnProperty('s_alter') ? global.city['s_alter'].count : 0)) >= 1 ? 0 : 100; }
                },
                effect(){
        ...
        case 'shrine':
        {
            let action = {
                ...
                region: 'city',
                cost: {
                    Money(r={}){ return costMultiplier('shrine', r.offset, 75, 1.32); },
                    Stone(r={}){ return costMultiplier('shrine', r.offset, 65, 1.32); },
                    Furs(r={}){ return costMultiplier('shrine', r.offset, 10, 1.32); },
                    Copper(r={}){ return costMultiplier('shrine', r.offset, 15, 1.32); }
                },
                effect(){
                    ...
                    if (global.city['shrine'] && know.active){
                        desc = desc + `<div>${loc('city_shrine_know',[(+(know.add).toFixed(0)).toLocaleString()])}</div>`;
        ...
        case 'meditation':
        {
            let action = {
                ...
                region: 'city',
                cost: {
                    Money(r={}){ return costMultiplier('meditation', r.offset, 50, 1.2); },
                    Stone(r={}){ return costMultiplier('meditation', r.offset, 25, 1.2); },
                    Furs(r={}){ return costMultiplier('meditation', r.offset, 8, 1.2); }
                },
                effect(){
                    let zen = global.resource.Zen.amount / (global.resource.Zen.amount + 5000);
                    return `<div>${loc(`city_meditation_effect`,[hugeEffect(traits.calm.vars()[0], 0)])}</div><div class="has-text-special">${loc(`city_meditation_effect2`,[2])}</div><div class="has-text-special">${loc(`city_meditation_effect3`,[1])}</div><div>${loc(`city_meditation_effect4`,[`${(zen * 100).toFixed(2)}%`])}</div>`;
                },
                action(args){
                    if (payCosts(this)){
                        incrementStruct('meditation','city');
                        global.resource.Zen.max += hugeAdjust(traits.calm.vars()[0]);
                        return true;
                    }
                    return false;
                },
        ...
        case 'banquet':
        {
            let action = {
                ...
                cost: {
                    Money(r={}){
                        const level = (r.offset ? r.offset : 0) + (global.city['banquet'] ? global.city['banquet'].level : 0);
                        switch (level){
                            case 0:
                                return 45000;
                            case 1:
                    ...
                    Aerogel(r={}){
                        const level = (r.offset ? r.offset : 0) + (global.city['banquet'] ? global.city['banquet'].level : 0);
                        if(r.wiki ? r.wiki.truepath : global.race['truepath']){
                            return 0;
                        }
                        switch (level){
                            case 3:
                ...
                powered(){ return 0; },
                action(args){
                    if (global.city['banquet'].level < global.stats.achieve['endless_hunger'].l && payCosts(this)){
...
export function hugeEffect(v, i=1, e=1){
    return +hugeAdjust(v, e).toFixed(i); //use hugeAdjust if you want an int instead of a string
}
    ...
    action(args){
        if (payCosts(this)){
            if (global.race[challengeList[challenge]]){
                ...
                $(`#${this.id}`).removeClass('hl');
                if (global.race['truepath'] || global.race['lone_survivor']){
                    delete global.race['nerfed'];
                    delete global.race['badgenes'];
                }
                ['junker','cataclysm','banana','truepath','lone_survivor','fasting','warlord','iceage'].forEach(function(s){
...
export function casinoEarn(){
    let cash = Math.log2(1 + hugeAdjust(global.resource[global.race.species].amount)) * 2.5;
    if (global.race['gambler']){
    ...
    cash = hugeAdjust(cash);
    return cash;
}

export function casinoEffect(){
    let money = spatialReasoning(casino_vault());
...
export function storageMultipler(scale = 1, wiki = false){
    ...
    if (global.tech['storage'] >= 7 && global.interstellar['cargo_yard']){
        multiplier *= 1 + hugeAdjust((global.interstellar['cargo_yard'].count * get_qlevel(wiki)) / 100);
    }
...
export function drawTech(){
    ...
    ['primitive','civilized','discovery','industrialized','globalized','glacial','early_space','deep_space','interstellar','intergalactic'].forEach(function (era){
        new_techs[era] = [];
    });
    ...
    if (c_action.cost && !old){
        ...
        Object.keys(costs).forEach(function (res){
            if (res === 'Custom'){
                ...
                let color = 'has-text-dark';
                if (res_cost > 0){
                    let label = loc(`resource_Fossil_name`);
                    if(global.resource.Spent_Fossil.amount + res_cost > fossilCount()){
                        color = 'has-text-danger';
                    }
                    empty = false;
                    //no res-${res}, this would otherwise cause midLoop to update the tooltip, changing resource color.
                    cost.append($(`<div class="${color} ${res}" data-${res}="${res_cost}">${label}: ${res_cost}</div>`));
                }
            }
            else if (res !== 'Morale' && res !== 'Army' && res !== 'Bool'){
                let res_cost = costs[res]();
    ...
    if (checkCosts(costs, pool)){
        Object.keys(costs).forEach(function (res){
            if (global.prestige.hasOwnProperty(res)){
                ...
                let cost = costs[res]();
                global.resource.Spent_Fossil.amount += cost;
            }
...
function checkMaxCosts(costs, pool){
    ...
    Object.keys(costs).forEach(function (res){
        if (res === 'Custom'){
            ...
            if(fossilCount() - global.resource.Spent_Fossil.amount < Number(costs[res]())){
                test = false;
                return;
            }
        }
            ...
            let f_res = res === 'Species' ? global.race.species : res;
            let cap = Number(poolCap(f_res, pool));
...
function drawModal(c_action,type){
    ...
    switch(type){
        ...
        case 'woodcutter':
            ...
            break;
        case 'thruster_fuel':
            loadIndustry('thruster_fuel', body);
            break;
        case 'titan_mine':
...
function mediumHousingLabel(){
    ...
    if (global.race['iceage']){
        return loc('underground_stone_house');
    }
...
// Lay out the strand after finalizing run traits.
    ...
    if (global.race['deconstructor']){
        ...
        initStruct(actions.city.nanite_factory); global.city.nanite_factory.count = 1;
        global.settings.showIndustry = true;
    }
...
function iceAgeStart(){
    if(global.race['iceage']){
        ...
        if(global.race['deconstructor']){
            initStruct(actions.underground.cave.nanite_factory); global.underground.nanite_factory.count = 1;
        }
        delete global.aberrants.trees.traits.empowered;
        delete global.aberrants.herbivores.traits.empowered;
        ...
        initStruct(actions.surface.thruster_site.giant_thrusters);
...
function fanaticTrait(trait,rank){
    if ((global.race['warlord'] || global.race['iceage']) && trait === 'kindling_kindred'){ trait = 'iron_wood'; }
...
export function bank_vault(){
    ...
    if (global.tech['ai_core'] && global.tech.ai_core >= 4){
        ...
        vault *= 1 + hugeAdjust(citadel / 100);
    }
...
export function thrusterProjection(){
    ...
    let desc = `<div class="has-text-advanced">${loc('interstellar_ascension_trigger_effect2',[gains.plasmid,plasmidType])}</div>
        ...
        <div class="has-text-advanced">${loc('interstellar_ascension_trigger_effect2',[gains.dark,loc('resource_Dark_name')])}</div>`;
    if(!global.race['iceage']){
        desc += `<div>${loc('city_giant_thrusters_effect1')}</div>`;
    }
    else{

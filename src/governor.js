...
export const gov_traits = {
  ...
  teacher: {
    ...
    effect(b) {
      return loc(`gov_trait_teacher_effect`, [
        $(this)[0].vars(b)[0],
        $(this)[0].vars(b)[1],
      ]);
    },
  ...
  runner: {
    ...
    effect(b) {
      return loc(`gov_trait_runner_effect`, [
        $(this)[0].vars(b)[0],
        $(this)[0].vars(b)[1],
      ]);
    },
...
export function drawnGovernOffice() {
  ...
  governorTitle.append(
    $(
      `<div class="has-text-caution" role="heading" aria-level="2">${loc(`governor_office`, [global.race.governor.g.n])}</div>`,
    ),
  );
  governorTitle.append(
    $(
      `<div><span class="has-text-warning">${loc(`governor_background`)}:</span> <span class="bg">${gmen[global.race.governor.g.bg].name}</span></div>`,
    ),
  );
  ...
  govHeader.append(
    $(
      `<div class="fire"><b-button v-on:click="fire" v-html="fireText()">${loc(`governor_fire`)}</b-button></div>`,
    ),
  );
  ...
  //Configs
  {
    ...
    let storeContain = $(
      `<div class="tConfig" v-show="showTask('storage')"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_storage`)}</div></div>`,
    );
    ...
    storage.append(
      $(
        `<b-field>${loc(`gov_task_storage_reserve`, [global.resource[crt_mat].name])}<b-numberinput min="0" :max="Number.MAX_SAFE_INTEGER" v-model="c.storage.crt" :controls="false"></b-numberinput></b-field>`,
      ),
    );
    storage.append(
      $(
        `<b-field>${loc(`gov_task_storage_reserve`, [global.resource[cnt_mat].name])}<b-numberinput min="0" :max="Number.MAX_SAFE_INTEGER" v-model="c.storage.cnt" :controls="false"></b-numberinput></b-field>`,
      ),
    );
  }

  {
    ...
    let storeContain = $(
      `<div class="tConfig" v-show="showTask('bal_storage')"><div class="hRow"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_bal_storage`)}</div><div class="chk"><b-checkbox v-model="c.bal_storage.adv">${loc(`advanced`)}</b-checkbox></div></div></div>`,
    );
    options.append(storeContain);
    ...
    Object.keys(global.resource).forEach(function (res) {
      if (global.resource[res].stackable) {
        ...
        storage.append(
          $(`<div class="ccmOption" :class="bStrEx()" v-show="showStrRes('${res}')"><span role="heading" aria-level="4">${global.resource[res].name}</span>
                <b-field>
...

  {
    ...
    let contain = $(
      `<div class="tConfig" v-show="showTask('merc')"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_merc`)}</div></div>`,
    );
    ...
    merc.append(
      $(
        `<b-field>${loc(`gov_task_merc_buffer`)}<b-numberinput min="0" :max="Number.MAX_SAFE_INTEGER" v-model="c.merc.buffer" :controls="false"></b-numberinput></b-field>`,
      ),
    );
    merc.append(
      $(
        `<b-field>${loc(`gov_task_merc_reserve`)}<b-numberinput min="0" :max="100" v-model="c.merc.reserve" :controls="false"></b-numberinput></b-field>`,
      ),
    );
  }

  {
    ...
    let contain = $(
      `<div class="tConfig" v-show="showTask('spy')"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_spy`)}</div></div>`,
    );
    options.append(contain);
    ...
    spy.append(
      $(
        `<b-field>${loc(`gov_task_merc_reserve`)}<b-numberinput min="0" :max="100" v-model="c.spy.reserve" :controls="false"></b-numberinput></b-field>`,
      ),
    );
  }

  {
    ...
    let contain = $(
      `<div class="tConfig" v-show="showTask('spyop')"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_spyop`)}</div></div>`,
    );
    options.append(contain);
    Object.keys(global.civic.foreign).forEach(function (gov) {
      if (
        ...
        spyop.append(`
                    <div role="heading" aria-level="4" class="has-text-caution">${loc("gov_task_spyop_priority", [govTitle(gov.substring(3))])}</div>
...

  {
    ...
    let contain = $(
      `<div class="tConfig" v-show="showTask('tax')"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_tax`)}</div></div>`,
    );
    options.append(contain);
    ...
    tax.append(
      $(
        `<b-field>${loc(`gov_task_tax_min`)}<b-numberinput min="0" :max="20" v-model="c.tax.min" :controls="false"></b-numberinput></b-field>`,
      ),
    );
  }

  {
    ...
    let contain = $(
      `<div class="tConfig" v-show="showTask('slave')"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_slave`, [global.resource.Slave.name])}</div></div>`,
    );
    options.append(contain);
    ...
    slave.append(
      $(
        `<b-field>${loc(`gov_task_merc_reserve`)}<b-numberinput min="0" :max="100" v-model="c.slave.reserve" :controls="false"></b-numberinput></b-field>`,
      ),
    );
  }

  {
    ...
    let advanced =
      global.genes.hasOwnProperty("governor") && global.genes.governor >= 3
        ? `<div class="chk"><b-checkbox v-model="c.trash.stab">${loc(`gov_task_auto_stabilize`)}</b-checkbox></div>`
        : ``;

    let contain = $(
      `<div class="tConfig" v-show="showTask('trash')"><div class="hRow"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_trash`)}</div>${advanced}</div></div>`,
    );
    options.append(contain);
    ...
    ["Infernite", "Elerium", "Copper", "Iron"].forEach(function (res) {
      trash.append(
        $(
          `<b-field class="trash"><div tabindex="0" class="trashButton" role="button" @click="trashStrat('${res}')" v-html="$options.methods.trashLabel('${res}')"></div><b-numberinput min="0" :max="1000000" v-model="c.trash.${res}.v" :controls="false"></b-numberinput></b-field>`,
        ),
      );
    });
  }

  {
    ...
    let contain = $(
      `<div class="tConfig" v-show="showTask('replicate')"><div class="has-text-warning" role="heading" aria-level="3">${loc(`gov_task_replicate`)}</div></div>`,
    );
    options.append(contain);
    ...
    replicate.append(
      $(
        `<div class="chk"><b-checkbox v-model="c.replicate.pow.on">${loc(`gov_task_replicate_auto`)}</b-checkbox></div>`,
      ),
    );
    replicate.append(
      $(
        `<b-field>${loc(`gov_task_replicate_pmax`)}<b-numberinput min="0" v-model="c.replicate.pow.cap" :controls="false"></b-numberinput></b-field>`,
      ),
    );
    replicate.append(
      $(
        `<b-field>${loc(`gov_task_replicate_buff`)}<b-numberinput min="0" v-model="c.replicate.pow.buffer" :controls="false"></b-numberinput></b-field>`,
      ),
    );
    ...
    res_bal.append(
      $(
        `<div class="chk"><b-checkbox v-model="c.replicate.res.que">${loc(`gov_task_replicate_que`)}</b-checkbox></div>`,
      ),
    );
    res_bal.append(
      $(
        `<div class="chk"><b-checkbox v-model="c.replicate.res.neg">${loc(`gov_task_replicate_neg`)}</b-checkbox></div>`,
      ),
    );
    res_bal.append(
      $(
        `<div class="chk"><b-checkbox v-model="c.replicate.res.cap">${loc(`gov_task_replicate_cap`)}</b-checkbox></div>`,
      ),
    );
  }
...
function appointGovernor() {
  ...
  govern.append(
    $(
      `<div class="appoint header"><span class="has-text-caution">${loc(`governor_candidate`)}</span><span class="has-text-caution">${loc(`governor_background`)}</span><span></span><div>`,
    ),
  );
  for (let i = 0; i < global.race.governor.candidates.length; i++) {
    let gov = global.race.governor.candidates[i];
    if (
      (global.race["warlord"] && gov.bg === "soldier") ||
      !global.race["warlord"]
    ) {
      govern.append(
        $(
          `<div class="appoint ${gov.bg}"><span class="has-text-warning" role="heading" aria-level="3">${gov.t} ${gov.n}</span><span class="bg">${gmen[gov.bg].name}</span><span><button class="button" v-on:click="appoint(${i})">${loc(`governor_appoint`)}</button></span><div>`,
        ),
      );
    }
  }
  ...
  bal_storage: {
    ...
    task() {
      if ($(this)[0].req()) {
        ...
        Object.keys(global.race.governor.config.bal_storage).forEach(
          function (res) {
      ...
      if ($(this)[0].req()) {
        ...
        range.forEach(function (gov) {
          if (
            ...
            global.race.governor.config.spyop[`gov${gov}`].every(
              function (mission) {
                switch (mission) {
                  case "influence":
                    if (
  ...
  mech: {
    ...
    task() {
      if ($(this)[0].req()) {
        ...
        let chassisTypes = global.race["warlord"]
          ? [
              "imp",
              "flying_imp",
              "hound",
          ...
          : ["hover", "spider", "wheel", "tread", "biped", "quad"];
        let weaponTypes = global.race["warlord"]
          ? [
              "laser",
              "kinetic",
              "shotgun",
        ...
        let equipTypes = global.race["warlord"]
          ? [
              "shields",
              "flare",
              "seals",
        ...
        if (global.race["warlord"]) {
          if (
            mechs.type.minion < 16 ||
            (mechs.type.minion < 32 &&
              mechs.type.fiend >= 19 &&
              mechs.type.cyberdemon >= 6 &&
              mechs.type.archfiend >= 5)
          ) {
            ctype = "minion";
            ...
            mechs.type.fiend < 14 ||
            (mechs.type.cyberdemon >= 4 && mechs.type.fiend < 19)
          ) {
            ctype = "fiend";
            mCosts = mechCost(ctype, false);
            cost = mCosts.c;
        ...
        if (
          ...
          if (global.race["warlord"]) {
            ...
            if (ctype === "minion") {
              ...
              if (
                counts.imp.c < 4 ||
                counts.flying_imp.c < 4 ||
                (mechs.type.minion >= 16 &&
                  (counts.imp.c < 8 || counts.flying_imp.c < 8))
              ) {
                type =
                  counts.imp.c > counts.flying_imp.c ? "flying_imp" : "imp";
              } else if (
                counts.hound.c < 4 ||
                (mechs.type.minion >= 16 && counts.hound.c < 8)
              ) {
                type = "hound";
              } else if (
                counts.harpy.c < 2 ||
                (mechs.type.minion >= 16 && counts.harpy.c < 8)
              ) {
                type = "harpy";
              ...
              let wTypes = validWeapons(ctype, type, 0).sort(
                () => Math.random() - 0.5,
              );
              let weapon = wTypes[0];
              wTypes.forEach(function (wep) {
                if (["imp", "flying_imp"].includes(type)) {
                  if (
                    counts.imp.w[wep] + counts.flying_imp.w[wep] <
                    counts.imp.w[weapon] + counts.flying_imp.w[weapon]
                  ) {
                    weapon = wep;
                  }
                } else {
              ...
              if (global.blood["prepared"]) {
                equipment =
                  mechs.minion.equip.scavenger < 16
                    ? ["scavenger"]
                    : ["scouter"];
              }
            } else if (ctype === "fiend") {
              ...
              if (counts.cambion.c < 4 || counts.rakshasa.c < 4) {
                type =
                  counts.cambion.c > counts.rakshasa.c ? "rakshasa" : "cambion";
              ...
              wTypes.forEach(function (wep) {
                if (["cambion", "rakshasa"].includes(type)) {
                  if (
                    counts.cambion.w[wep] + counts.rakshasa.w[wep] <
                    counts.cambion.w[weapon] + counts.rakshasa.w[weapon]
                  ) {
                    weapon = wep;
                  }
                } else {
              ...
              let eTypes = validEquipment(ctype, type, 0).sort(
                () => Math.random() - 0.5,
              );
              let equip = eTypes[0];
              let eTotals = {};
              eTypes.forEach(function (eq) {
                eTotals[eq] =
                  counts.cambion.e[eq] +
                  counts.minotaur.e[eq] +
                  counts.nightmare.e[eq] +
                  counts.rakshasa.e[eq] +
                  counts.golem.e[eq];
              });
              eTypes.forEach(function (eq) {
              ...
              for (let i = 0; i < 2; i++) {
                let weapon = weapons.includes(wTypes[i])
                  ? wTypes[i + 1]
                  : wTypes[i];
                wTypes.forEach(function (wep) {
                  if (
                    mechs[ctype].weapon[wep] < mechs[ctype].weapon[weapon] &&
                    !weapons.includes(wep)
                  ) {
                    weapon = wep;
                  }
                });
              ...
              if (type === "hydra") {
                ...
                if (["dragon", "snake"].includes(type)) {
                  weapons = [validWeapons(ctype, type, 0)[0]];
                } else {
                  weapons = [
                    counts.gorgon.w.hammer < counts.gorgon.w.axe
                      ? "hammer"
                      : "axe",
                  ];
                }

                let wTypes = validWeapons(ctype, type, 1).sort(
                  () => Math.random() - 0.5,
                );
                let weapon = wTypes[0];
                wTypes.forEach(function (wep) {
                  if (
                    counts.dragon.w[wep] +
                      counts.snake.w[wep] +
                      counts.gorgon.w[wep] <
                    counts.dragon.w[weapon] +
                      counts.snake.w[weapon] +
                      counts.gorgon.w[weapon]
                  ) {
                    weapon = wep;
                  }
                });
    ...
    task() {
      ...
      for (
        ...
        if (global.queue.queue[idx].action === "arpa") {
          ...
          let costs = mechCost(
            global.queue.queue[idx].type.size,
            global.queue.queue[idx].type.infernal,
            true,
          );
          let c_action = { cost: costs };
          tc = timeCheck(c_action, false, true);
        ...
        for (let i = 0; i < resSorted.length; i++) {
          if (
            ...
            !blacklist.includes(resSorted[i])
          ) {

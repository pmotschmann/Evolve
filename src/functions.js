...
export function buildQueue() {
  clearDragQueue();
  $("#buildQueue").attr("aria-busy", "true");
  clearElement($("#buildQueue"));
  $("#buildQueue").attr({
    role: "region",
    "aria-label": loc("building_queue"),
    "aria-live": "polite",
    "aria-atomic": "false",
  });
  $("#buildQueue").append(
    $(`
        ...
        <span tabindex="0" id="pausequeue" class="${global.queue.pause ? "pause" : "play"}" role="button" @click="pauseQueue()" :aria-label="pausedesc()" :aria-pressed="pause"></span>
    `),
  );
  ...
  queue.append(
    $(
      `<li v-for="(item, index) in queue"><a v-bind:id="setID(index)" class="has-text-warning queued" v-bind:class="{ 'qany': item.qa }" @click="remove(index)" role="button"><span v-bind:class="setData(index,'res')" v-bind="setData(index,'data')">{{ item.label }}{{ item.q | count }}</span> [<span v-bind:class="{ 'has-text-danger': item.cna, 'has-text-success': !item.cna }">{{ item.time | time }}{{ item.t_max | max_t(item.time) }}</span><span class="is-sr-only" v-if="item.cna"> (${loc("insufficient")})</span>]</a></li>`,
    ),
  );
  ...
  try {
      ...
      methods: {
        remove(index) {
          ...
          for (let i = 0; i < keyMult; i++) {
            let removed = global.queue.queue[index].label;
            ...
            if (global.queue.queue[index].q <= 0) {
              ...
              buildQueue();
              messageQueue(
                `${removed} removed from building queue.`,
                "info",
                true,
                ["queue"],
              );
        ...
        setData(index, prefix) {
          ...
          if (segments[0].substring(0, 4) === "arpa") {
            ...
            let costs = mechCost(
              global.queue.queue[index].type.size,
              global.queue.queue[index].type.infernal,
              true,
            );
            c_action = { cost: costs };
          } else if (
        ...
        pauseQueue() {
          ...
          if (global.queue.pause) {
            ...
            messageQueue("Building queue resumed.", "info", true, ["queue"]);
          } else {
            global.queue.pause = true;
            $(`#pausequeue`).addClass("pause");
            messageQueue("Building queue paused.", "info", true, ["queue"]);
          }
        },
        pausedesc() {
    ...
    dragQueue();
    $("#buildQueue").attr("aria-busy", "false");
  } catch {
    global.queue.queue = [];
    $("#buildQueue").attr("aria-busy", "false");
  }
    ...
    popover(
      ...
      function (obj) {
        ...
        if (struct.s[0].substring(0, 4) === "arpa") {
          ...
          struct.s[0].substring(0, 4) === "hell" &&
          struct.s[1].substring(0, 4) === "mech"
        ) {
...
export function modRes(res, val, notrack) {
  ...
  let max = notrack
    ...
    : tmp_vars.resource[res].temp_max;
  if (count > max && max >= 0) {
    count = max;
  } else if (count < 0) {
    success = false;
  ...
  if (!Number.isNaN(count)) {
    ...
    if (!notrack) {
      ...
      if (res === "Mana" && val > 0) {
        ...
        tmp_vars.resource[res].temp_max = Math.max(
          0,
          tmp_vars.resource[res].temp_max + val,
        );
      }
    }
  }
...
function kindlingAdjust(costs, offset, wiki) {
  if (
    (global.race["kindling_kindred"] || global.race["iron_wood"]) &&
...
export function drawPet() {
  ...
  if ($("#playerPet .flair").length === 0 && global.race["pet"]) {
    ...
    $("#playerPet").append(
      `<span class="flair" aria-label="${loc(`event_${global.race.pet.type}_name${global.race.pet.name}`)} (${global.race.pet.type})"><svg class="${color} ${pattern}" version="1.1" x="0px" y="0px" width="16px" height="16px" viewBox="${svgViewBox(global.race.pet.type)}" xml:space="preserve">${coat}${svgIcons(global.race.pet.type)}</svg></span>`,
    );
    ...
    popover(
      ...
      function (obj) {
        ...
        popper.append(
          `<div>${loc(`event_${global.race.pet.type}_name${global.race.pet.name}`)}</div>`,
        );
        return undefined;
      },
      {
...
export function calcGenomeScore(genome, wiki, tRanks) {
  ...
  for (let i = 0; i < genome.traitlist.length; i++) {
    ...
    let genus_origin =
      races[traits[genome.traitlist[i]].origin].type === "hybrid"
        ? races[traits[genome.traitlist[i]].origin].hybrid
        : [races[traits[genome.traitlist[i]].origin].type];
...
export function getTraitDesc(info, trait, opts) {
  ...
  if (tpage && ["genus", "major"].includes(traits[trait].type)) {
    rank = `<span><span tabindex="0" role="button" aria-label="${loc("wiki_trait_rank")} decrease" @click="down()">&laquo;</span><span class="has-text-warning">${loc(`wiki_trait_rank`)} {{ rank }}</span><span tabindex="0" role="button" aria-label="${loc("wiki_trait_rank")} increase" @click="up()">&raquo;</span></span>`;
  }
  if (tpage || rpage) {
    info.append(
    ...
    if (tpage && traits[trait].hasOwnProperty("val")) {
      info.append(
        `<div class="type has-text-caution">${loc(`wiki_trait_${traits[trait].type}`)}<span>${loc(`wiki_trait_value`, [traits[trait].val])}</span></div>`,
      );
    } else {
      info.append(
        `<div class="type has-text-caution">${loc(`wiki_trait_${traits[trait].type}`)}</div>`,
      );
    }
    if (fanatic) {
      info.append(
        `<div class="has-text-danger">${loc(`wiki_trait_fanaticism`, [fanatic])}</div>`,
      );
    }
  }
    ...
    if (
      ...
      if (trait === "elemental") {
        ...
        if (
          ...
          ["spiritual", "blasphemous"].includes(trait)
        ) {
          let alt_trait =
            trait === "spiritual" ? "manipulator" : "blasphemous_evil";
          trait_desc = loc(
            `wiki_trait_effect_${alt_trait}`,
            getTraitVals(trait, trank, species),
          );
        } else {
          let key =
            altTraitDesc[trait] &&
            global.race.hasOwnProperty(altTraitDesc[trait])
              ? altTraitDesc[trait]
              : "effect";
          trait_desc = loc(
            `wiki_trait_${key}_${trait}`,
            getTraitVals(trait, trank, species),
          );
        }
      }

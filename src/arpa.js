...
export const arpaProjects = {
    ...
    effect(nofool) {
      ...
      if (global.tech["storage"] >= 6) {
        if (global.race["warlord"]) {
          return loc("arpa_projects_lhc_warlord2", [
        ...
        if (global.race["warlord"]) {
          return loc("arpa_projects_lhc_warlord1", [
            loc("portal_twisted_lab_title"),
            sc,
          ]);
        } else {
  ...
  stock_exchange: {
    ...
    effect() {
      if (global.race["warlord"]) {
        return loc("arpa_projects_stock_exchange_warlord", [
          structName("casino"),
          5,
          1,
        ]);
...
function genetics() {
  ...
  if (global.tech["genetics"] > 1) {
    ...
    let sequence = $(
      `<div><span class="seqlbl has-text-warning">${label}</span> - ${loc("arpa_to_complete")} <span v-html="$options.filters.timer(time)"></span></div>`,
    );
    genome.append(sequence);
    let progress = $(
      `<progress class="progress" aria-label="${label}" :value="progress" max="${global.arpa.sequence.max}">{{ progress }}%</progress>`,
    );
    ...
    if (global.tech["genetics"] >= 5) {
      let boost = $(
        `<b-tooltip :label="boostLabel(false)" position="is-bottom" animated multilined><button class="button boost" @click="booster" :aria-label="boostLabel(true)">${loc("arpa_boost")}</button></b-tooltip>`,
      );
      genome.append(boost);
    }
    ...
    if (global.tech["genetics"] >= 6) {
      let boost = $(
        `<b-tooltip :label="novoLabel()" position="is-bottom" animated multilined><button class="button" @click="novo" :aria-label="novoLabel()">${loc(global.race["artifical"] ? "arpa_novo_artifical" : "arpa_novo")}</button></b-tooltip>`,
      );
      genome.append(boost);
    }
    ...
    if (global.tech["genetics"] >= 7) {
      let boost = $(
        `<b-tooltip :label="autoLabel(false)" position="is-bottom" animated multilined><button class="button auto" @click="auto_seq" :aria-label="autoLabel(true)">${loc(global.race["artifical"] ? "arpa_auto_compile" : "arpa_auto_sequence")}</button></b-tooltip>`,
      );
      genome.append(boost);
    }
  ...
  if (global.tech["genetics"] > 2) {
    ...
    breakdown.append(
      `<div class="trait major has-text-success" role="heading" aria-level="3">${loc("arpa_race_genetic_traids", [flib("name")])}</div>`,
    );
    ...
    Object.keys(trait_listing).forEach(function (trait) {
      if (
        ...
        let mimicTraits = [
          ...
          ...(global.race["iTraits"]
            ? Object.keys(global.race["iTraits"])
            : []),
        ];
        let readOnly = false;
        if (mimicTraits.includes(trait)) {
          ...
          (global.race.hasOwnProperty("absorbed") &&
            global.race.absorbed
              .map((r) => races[r].fanaticism)
              .includes(trait)) ||
          (global.race["warlord"] &&
            ["iron_wood", "unified", "apex_predator"].includes(trait))
        ) {
          readOnly = true;
        } else if (
          trait === "forager" &&
          mimicTraits.some((item) => ["herbivore", "carnivore"].includes(item))
        ) {
          readOnly = true;
        }
        if (
          ...
          let purge = $(
            `<span tabindex="0" class="remove${trait} basic-button has-text-danger" role="button" :aria-label="removeCost('${trait}')" @click="purge('${trait}')">${loc("arpa_remove_button")}</span>`,
          );
          ...
          major.append(
            $(
              `<span class="trait has-text-warning" id="raceTrait${trait}">${traitName[trait] ? traitName[trait] : traits[trait].name} (${loc(`arpa_genepool_rank`, [traitRank(trait)])})</span>`,
            ),
          );
          ...
          traitListing.append(
            `<div class="traitRow trait${trait}"><div class="trait has-text-warning${global.genes["mutation"] ? " indent" : ""}">${traitName[trait] ? traitName[trait] : traits[trait].name} (${loc(`arpa_genepool_rank`, [traitRank(trait)])})</div></div>`,
          );
        }
      }
    });
    ...
    if (global.genes["mutation"] && global.genes["mutation"] >= 3) {
      if (
        ...
        breakdown.append(
          `<div class="trait major has-text-success" role="heading" aria-level="3">${loc("arpa_race_genetic_gain")}</div>`,
        );
        ...
        for (let i = 0; i < trait_list.length; i++) {
          ...
          if (!["catnip", "anise"].includes(trait)) {
            let major = $(`<div class="traitRow"></div>`);
            let add = $(
              `<span tabindex="0" class="add${trait} basic-button has-text-success" role="button" :aria-label="addCost('${trait}')" @click="gain('${trait}')">${loc("arpa_gain_button")}</span>`,
            );

            major.append(add);
            major.append(
              $(
                `<span class="trait has-text-warning" id="raceTrait${trait}">${traitName[trait] ? traitName[trait] : traits[trait].name} (${loc(`arpa_genepool_rank`, [offspec_traits[trait] ? 0.5 : 1])})</span>`,
              ),
            );
    ...
    if (minor) {
      breakdown.prepend(
        `<div class="trait minor has-text-success" role="heading" aria-level="3">${loc("arpa_race_genetic_minor_traits", [flib("name")])}</div>`,
      );
    }
...
function bindTrait(breakdown, trait) {
  ...
  let gene = $(
    `<h4 class="is-sr-only">${trait}</h4><span tabindex="0" v-bind:class="['basic-button', 'gene', 'gbuy', genePurchasable('${trait}') ? '' : 'has-text-fade']" role="button" :aria-label="geneCost('${trait}')" @click="gene('${trait}')">${global.resource.Genes.name} (${global.race.minor[trait] || 0})</span>`,
  );
  m_trait.append(gene);
  if (global.prestige.Phage.count > 0) {
    let phage = $(
      `<span tabindex="0" v-bind:class="['basic-button', 'gene', 'pbuy', phagePurchasable('${trait}') ? '' : 'has-text-fade']" role="button" :aria-label="phageCost('${trait}')" @click="phage('${trait}')">${loc("resource_Phage_name")} (${global.genes.minor[trait] || 0})</span>`,
    );
  ...
  m_trait.append(
    `<span class="has-text-warning name">${total}${traitSkin("name", trait)}</span>`,
  );
  ...
  if (checkRequirements(project)) {
    ...
    let title =
      typeof arpaProjects[project].title === "string"
        ...
        : arpaProjects[project].title();
    let head = $(
      `<div class="head"><span class="desc has-text-warning" role="heading" aria-level="3">${title}</span><a v-on:click="srDescAndEffect" class="is-sr-only" role="button">{{ projectName() }} description</a><span aria-hidden="true" v-show="rank" class="rank">{{ rank | level }}</span><span class="is-sr-only">{{ rank | level }}</span></div>`,
    );
    current.append(head);

    let progress = $(
      `<div class="pbar"><progress class="progress" :aria-label="projectName() + ' progress'" :value="complete" max="100"></progress><span class="progress-value has-text-danger">{{ complete }}%</span></div>`,
    );
    ...
    buy.append(
      $(
        `<button :aria-label="loc('queue') + ' ' + projectName()" class="button" @click="queue('${project}')">${loc("queue")}</button>`,
      ),
    );
    buy.append(
      $(
    ...
    vBind({
      ...
      methods: {
        ...
        projectName() {
          return typeof arpaProjects[project].title === "string"
            ? arpaProjects[project].title
            : arpaProjects[project].title();
        },
...
export function arpaProjectCosts(id, project) {
  ...
  Object.keys(costs).forEach(function (res) {
    ...
    if (res_cost > 0) {
      ...
      cost.append(
        $(
          `<div class="${color}" data-${res}="${res_cost}">${label}${sizeApproximation(res_cost, 2)}</div>`,
        ),
      );
    }
  });
